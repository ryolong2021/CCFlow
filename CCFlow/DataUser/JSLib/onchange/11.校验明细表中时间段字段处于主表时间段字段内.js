...
function CheckDateRegion(s) {
    ...
    if (!frmDFromCtrl) {
        alert('メインテーブルでは確認されていませんTB_' + Form_DateFrom_Ctrl_ID + '列,管理者に連絡して、フローノードに対応するJSファイルを修正してくださいDataUser/JSLibData/' + getArgsFromHref('EnsName') + '.js中にForm_DateFrom_Ctrl_ID変数のマスタフォーム「終了日」フィールド！');
        return false;
    }

    if (!frmDToCtrl) {
        alert('メインテーブルでは確認されていませんTB_' + Form_DateTo_Ctrl_ID + 'テキストボックス,管理者に連絡して、フローノードに対応するJSファイルを修正してくださいDataUser/JSLibData/' + getArgsFromHref('EnsName') + '.js中にForm_DateTo_Ctrl_ID変数のマスタフォーム「終了日」フィールド！');
        return false;
    }
	...
	if(s.id.indexOf('TB_'+Dtl_DateFrom_Ctrl_ID) != -1){
		...
		if(!dtlDToCtrl){
			alert('未確認TB_' + Dtl_DateTo_Ctrl_ID + '列,管理者に連絡して、フローノードに対応するJSファイルを修正してくださいDataUser/JSLibData/' + getArgsFromHref('EnsName') + '.js中にDtl_DateTo_Ctrl_ID変数のマスタフォーム「終了日」フィールド！');
			return false;
		}
	}
	...
	if(s.id.indexOf('TB_'+Dtl_DateTo_Ctrl_ID) != -1){
		...
		if(!dtlDFromCtrl){
			alert('未查询到TB_' + Dtl_DateFrom_Ctrl_ID + '列,管理者に連絡して、フローノードに対応するJSファイルを修正してくださいDataUser/JSLibData/' + getArgsFromHref('EnsName') + '.js中にDtl_DateFrom_Ctrl_ID変数のマスタフォーム「終了日」フィールド！');
			return false;
		}
	}
