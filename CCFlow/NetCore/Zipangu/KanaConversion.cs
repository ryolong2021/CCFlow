...
namespace Zipangu
{
    ...
    public static class KanaConversion
    {
        ...
        static readonly IDictionary<char, char> KatakanaToHalfKatakanaMap = KatakanasByHalfKatakana.ZipToDictionary(HalfKatakanas);
        ...
        static readonly IDictionary<string, string> VoicedKatakanaMap =
            VoiceableHalfKatakanasForKatakana.Zip(VoicedKatakanas, (x, y) => new { k = $"{x}ﾞ", v = y.ToString() })
                ...
                .ToDictionary(_ => _.k, _ => _.v);
        static readonly IDictionary<string, string> VoicedHalfKatakanaMap =
            VoiceableHalfKatakanasForKatakana.Zip(VoicedKatakanas, (x, y) => new { k = $"{x}ﾞ", v = y.ToString() })
                .Concat(SemiVoiceableHalfKatakanas.Zip(SemiVoicedKatakanas, (x, y) => new { k = $"{x}ﾟ", v = y.ToString() }))
                .ToDictionary(_ => _.v, _ => _.k);
        ...
        /// <param name="value">変換対象の文字列。</param>
        /// <returns>変換後の文字列。</returns>
        public static string KatakanaToHalfKatakana(this string value)
        {
            if (value == null) return null;
            ...
            {
                current += c.ToString().ReplaceByMap(KatakanaToHalfKatakanaMap).ReplaceByMap(VoicedHalfKatakanaMap);
