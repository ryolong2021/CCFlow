    ...
    public class WF_Admin_AttrNode : BP.WF.HttpHandler.DirectoryPageBase
    {
        ...
        public string DocTemp_Upload()
        {
            if (HttpContextHelper.RequestFilesCount == 0)
                return "err@テンプレートをアップロードしてください。";

            int nodeId = int.Parse(this.GetRequestVal("pkval"));
            //上传附件
            var file = HttpContextHelper.RequestFiles(0);
            var fileName = file.FileName;
            string path = SystemConfig.PathOfDataUser + "FrmOfficeTemplate" + Path.DirectorySeparatorChar + "DocTemp" + Path.DirectorySeparatorChar + nodeId;
            string fileFullPath = path + Path.DirectorySeparatorChar + fileName;
        ...
        public string Bill_Save()
        {
            ...
            filepath = SystemConfig.PathOfDataUser + "CyclostyleFile" + Path.DirectorySeparatorChar + fileName;
        ...
        public void Bill_Download()
        {
            ...
            HttpContextHelper.ResponseWriteHeader("Content-Disposition", string.Format("attachment;filename={0}", en.TempFilePath.Substring(MyFilePath.LastIndexOf(Path.DirectorySeparatorChar) + 1)));
        ...
        public string NodeAttrs_Init()
        {
            ...
            foreach (Node node in nodes)
            {
                ...
                {
                    dr["HisDeliveryWayJsFnPara"] = "ByDept";
                    dr["HisDeliveryWayCountLabel"] = "人事";
        ...
        public string CheckFrm_Check()
        {
            if (BP.Web.WebUser.No != "admin")
                return "err@管理者だけがこの操作を実行する権限を持っています！";

            if (string.IsNullOrWhiteSpace(this.FK_MapData))
                return "err@パラメータFK_MapDataは空にできません！";
            ...
            foreach (MapExt me in mes)
            {
                ...
                {
                    me.DirectDelete();
                    msg += "拡張設定のMyPK=" + me.PKVal + "の設定項目を削除します。; <br />";
                }
            }
            ...
            foreach (FrmField ff in ffs)
            {
                ...
                {
                    ff.DirectDelete();
                    msg += "フィールド権限内のMyPK=" + ff.PKVal + "の設定項目を削除します。; <br />";
                }
            }
