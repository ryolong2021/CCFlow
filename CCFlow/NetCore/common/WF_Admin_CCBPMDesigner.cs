    ...
    public class WF_Admin_CCBPMDesigner : DirectoryPageBase
    {
        ...
        public string GetFlowTreeTable2019()
        {
            ...
            if (dt.Rows.Count == 0)
            {
                ...
                dt.Rows.Add("F99", "F0", "フローツリー", 0, 1, "FLOWTYPE", -1);
            }
            else
            {
                DataRow[] drs = dt.Select("NAME='フローツリー'");
        ...
        public string AdminerChang_LoginAs()
        {
            ...
            if (ae.RetrieveFromDBSources() == 0)
                return "err@あなたはこの組織の管理者ではありません。";
            ...
            if (ae1.RootOfDept.Equals(orgNo) == true)
                return "info@あなたはすでに組織の管理者であり、切り替える必要はありません。";
            ...
            //    return "err@用户名错误.";
            return "info@ログインは成功しました。システムが自動的に更新できない場合は、手動で更新してください。";
        }
        ...
        public string Logout()
        {
            ...
            return "安全に終了しました、ccbpmを使用してください。";
        }
        ...
        public string Default_Init()
        {
            ...
            {
                return "err@インターフェースの初期化中に次のエラーが発生しました:" + ex.Message;
            }
        }
        #endregion
        ...
        public string Login_Init()
        {
            ...
            if (!String.IsNullOrEmpty(sid) && !String.IsNullOrEmpty(userNo))
            {
                ...
                {
                    return "err@ログインに失敗しました" + ex.Message;
                }
            }
            ...
            {
                string msg = "err@アップグレードに失敗しました（ccbpmには自動修復機能があり、更新できます。システムは自動的にフィールドを作成します。複数回更新しても問題は解決しません。フィードバックをお寄せください）";
                msg += "@システムメッセージ:" + ex.Message;
                return msg;
            }
        }
        ...
        {
            if (WebUser.No == null)
                return "err@現在のユーザーはログインしていません、ログインしてもう一度お試しください。";
        ...
        public string GetFormTreeTable()
        {
            ...
            if (WebUser.No.Equals("admin") == false)
            {
                ...
                if (aemp.UserType != 1)
                    return "err@君は「" + WebUser.No + "」副管理者ではなくなりました。";
                if (aemp.RootOfForm == "")
                    return "err@第2レベルの管理者ではない「" + WebUser.No + "」フォームツリーの権限を設定します…";
        ...
        public string DelFlowSort()
        {
            ...
            if (DBAccess.RunSQLReturnValInt(ps) != 0)
                return "err@このディレクトリにはフローがあり、削除できません。";
            ...
            if (DBAccess.RunSQLReturnValInt(ps) != 0)
                return "err@このディレクトリの下にサブディレクトリがあり、削除できません。";
        ...
        public string CCForm_DelFormSort()
        {
            ...
            if (DBAccess.RunSQLReturnValInt(ps) != 0)
                return "err@このディレクトリにはサブカテゴリがあり、削除できません。";
            ...
            if (DBAccess.RunSQLReturnValInt(ps) != 0)
                return "err@このディレクトリにはフォームがあり、削除できません。";
