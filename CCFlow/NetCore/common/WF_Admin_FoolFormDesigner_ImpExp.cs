        ...
        public string Imp_LoadFrmTempleteFromLocalFile()
        {
            try
            {
                if (HttpContextHelper.RequestFilesCount == 0)
                    return "err@インポートしたテンプレートファイルをアップロードしてください。";

                //创建临时文件.
                string temp = SystemConfig.PathOfTemp + Path.DirectorySeparatorChar + Guid.NewGuid() + ".xml";
            ...
            catch (Exception ex)
            {
                ...
                {
                    return "err@インポートに失敗しました:" + newex.Message;
                }
            }
        }
        /// <summary>
