    ...
    public class WF_Admin_FoolFormDesigner_MapExt : DirectoryPageBase
    {
        ...
        public string TBFullCtrlDtl_Save()
        {
            ...
            foreach (MapDtl dtl in dtls)
            {
                ...
                if (sql.Contains("@Key")==false)
                    return "err@構成テーブルから:"+dtl.No+" sql入力が間違いました、 @Key列を含める必要があります、 @Keyは、現在のテキストボックスに入力された値です。 ";
        ...
        public string TBFullCtrlDDL_Save()
        {
            ...
            foreach (MapAttr attr in attrs)
            {
                ...
                if (sql.Contains("@Key") == false)
                    return "err@構成テーブルから:" + attr.KeyOfEn + " sql入力が間違いました、@Key列を含める必要があります。@Keyは、現在のテキストボックスに入力された値です。";
        ...
        public string ActiveDDL_Init()
        {
            ...
            if (dt.Rows.Count == 0)
                return "err@フォームにカスケードされるドロップダウンボックスはありません。";
