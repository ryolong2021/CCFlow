...
namespace BP.WF.HttpHandler
{
    ...
    public class WF_CCForm : DirectoryPageBase
    {
        ...
        public string Ath_Init()
        {
            try
            {
                ...
                if (dbs.Count == 0 && athDesc.IsWoEnableTemplete == true)
                {
                    ...
                    string templetePath = BP.Sys.SystemConfig.PathOfDataUser + "AthTemplete" + Path.DirectorySeparatorChar + athDesc.NoOfObj.Trim();
                    ...
                    if (fls.Length == 0)
                        throw new Exception("@フローデザインエラー、この複数の添付ファイル、テンプレートディレクトリでテンプレートコンポーネントが有効になっています:" + templetePath + "テンプレートファイルがありません。");
                    ...
                    foreach (FileInfo fl in fls)
                    {
                        ...
                        string saveTo = athDesc.SaveTo + Path.DirectorySeparatorChar + oid + "." + fl.Name.Substring(fl.Name.LastIndexOf('.') + 1);
                        if (saveTo.Contains("@") == true || saveTo.Contains("*") == true)
                        {
                            ...
                            if (saveTo.Contains("@") == true)
                                throw new Exception("@パスが正しく構成されておらず、変数が正しく置き換えられていない。" + saveTo);
                        }
                        fl.CopyTo(saveTo);
        ...
        public string HandlerMapExt()
        {
            ...
            switch (me.ExtType)
            {
                ...
                case BP.Sys.MapExtXmlList.FullData: // 填充其他控件.
                    ...
                    return "";
                default:
                    return "err@解析されたタグはありません" + me.ExtType;
            }
        ...
        {
            //找不不到标记就抛出异常.
            throw new Exception("@マーク[" + this.DoType + "]、未発見。@元のURL:" + HttpContextHelper.RequestRawUrl);
        }
        #endregion 执行父类的重写方法.
        ...
        public string Frm_Init()
        {
            ...
            if (md.HisFrmType == FrmType.Entity)
            {
                ...
                if (en.IsOIDEntity == true)
                {
                    ...
                    if (enOID == null)
                    {
                        return "err@システムエラー、転送できません" + md.PTable + "BP.En.EntityOIDに変換しました。";
                    }
                ...
                if (fn != null && fn.WhoIsPK != WhoIsPK.OID)
                {
                    ...
                    if (fn.WhoIsPK == WhoIsPK.PPPWorkID)
                    {
                        ...
                        if (DataType.IsNullOrEmpty(pppworkID) == true || pppworkID == "0")
                            throw new Exception("err@祖父と孫のフローの関係はありません。フロー設計が正しいかどうかを確認するには、管理者に連絡してください");
                    ...
                    if (fn.WhoIsPK == WhoIsPK.PPWorkID)
                    {
                        ...
                        {
                            throw new Exception("err@祖父母と孫の間に関係はありません。フロー設計が正しいかどうかを確認するには、管理者に連絡してください");
                        }
                    }
        ...
        public string FrmImgAthDB_Upload()
        {
            ...
            if (HttpContextHelper.RequestFilesCount > 0)
            {
                ...
                saveToPath = saveToPath + Path.DirectorySeparatorChar + myName + ".png";
                fileUPloadPath = fileUPloadPath + Path.DirectorySeparatorChar + myName + ".png";
        ...
        public string FrmGener_Init_ForBPClass()
        {
            ...
            catch (Exception ex)
            {
                ...
                return "err@フォームの読み込み中に次のエラーが発生しました、ccformには自動診断および修復機能があります。一度更新してください。それでも存在する場合は、管理者に連絡してください。@" + ex.Message;
            }
        }
        /// <summary>
        /// 执行数据初始化
                            ...
                            foreach (FrmField ff in ffs)
                            {
                                ...
                                switch (ff.DefVal)
                                {
                                    ...
                                    case "@RDT":
                                        ...
                                        continue;
                                    case "@yyyy年MM月dd日":
                                    case "@yyyy年MM月dd日HH時mm分":
                                    case "@yy年MM月dd日":
                                    case "@yy年MM月dd日HH時mm分":
                                        if (attr.UIIsReadonly == true)
                                        {
                ...
                if (this.FK_Node != 0 && DataType.IsNullOrEmpty(this.FK_Flow) == false)
                {
                    ...
                    switch (fn.WhoIsPK)
                    {
                        case WhoIsPK.FID:
                            pk = this.FID;
                            if (pk == 0)
                                throw new Exception("@パラメータFIDが受信されませんでした");
                            break;
                        case WhoIsPK.PWorkID: /*父流程ID*/
                            pk = this.PWorkID;
                            if (pk == 0)
                                throw new Exception("@パラメータPWorkIDが受信されませんでした");
                            break;
                        case WhoIsPK.OID:
                        default:
                            break;
        ...
        public string HandWriting_Save()
        {
            try
            {
                ...
                string tempPath = basePath + Path.DirectorySeparatorChar + ny + Path.DirectorySeparatorChar + this.FK_Node + Path.DirectorySeparatorChar;
            ...
            if (me.PopValWorkModel == PopValWorkModel.TablePage)
            {
                ...
                foreach (string cond in conds)
                {
                    ...
                    if (ds.Tables.Contains(para) == true)
                        throw new Exception("@設定されたクエリ、パラメータ名は競合しており、名前を付けることはできません:" + para);
        ...
        private void SingleAttach(string attachPk, Int64 workid, Int64 fid, int fk_node, string ensName)
        {
            ...
            saveTo = saveTo + Path.DirectorySeparatorChar + athDBPK + "." + HttpContextHelper.RequestFiles(0).FileName.Substring(HttpContextHelper.RequestFiles(0).FileName.LastIndexOf('.') + 1);
        ...
        public string MoreAttach()
        {
            ...
            for (int i = 0; i < HttpContextHelper.RequestFilesCount; i++)
            {
                ...
                if (athDesc.AthSaveWay == AthSaveWay.IISServer)
                {
                    ...
                    //替换关键的字串.
                    savePath = savePath.Replace("\\\\", Path.DirectorySeparatorChar.ToString());
                    try
                    {
                    ...
                    if (DataType.IsNullOrEmpty(exts))
                        return "err@アップロードされたファイル" + file.FileName + "拡張子名なし";

                    string guid = BP.DA.DBAccess.GenerGUID();
                    string fileName = file.FileName.Substring(0, file.FileName.LastIndexOf('.'));
                    string ext = System.IO.Path.GetExtension(file.FileName);
                    string realSaveTo = savePath + Path.DirectorySeparatorChar + guid + "." + fileName + ext;
                    ...
                    {
                        BP.Sys.Glo.WriteLineError("@AthUploadeBeforeイベント返却情報、ファイル：" + file.FileName + "，" + msg);
                    ...
                    if (!DataType.IsNullOrEmpty(msg))
                        BP.Sys.Glo.WriteLineError("@AthUploadeAfterイベント返却情報、ファイル：" + dbUpload.FileName + "，" + msg);
                }
                ...
                if (athDesc.AthSaveWay == AthSaveWay.DB || athDesc.AthSaveWay == AthSaveWay.FTPServer)
                {
                    ...
                    if (DataType.IsNullOrEmpty(msg) == false)
                    {
                        ...
                        throw new Exception("err@添付ファイルのアップロードエラー：" + msg);
                    }
                    ...
                    if (athDesc.AthSaveWay == AthSaveWay.FTPServer)
                    {
                        ...
                        {
                            throw new Exception("err@ポート番号が制限されているか、ファイアウォールが閉じられていません");
                        }
                        ftpconn.Close();
        ...
        public string FrmSingle_Init()
        {
            if (string.IsNullOrWhiteSpace(this.FK_MapData))
                throw new Exception("FK_MapDataパラメータは空にできません");

            MapData md = new MapData();
            md.No = this.FK_MapData;

            if (md.RetrieveFromDBSources() == 0)
                throw new Exception("未発見FK_MapData=" + this.FK_MapData + "フォーム、パラメータを確認してください");
        ...
        public string DtlOpt_Init()
        {
            ...
            if (dtl.ImpModel == 0)
                return "err@スレーブテーブルはインポートできません。";
            ...
            if (DataType.IsNullOrEmpty(dtl.ImpSQLInit))
                return "err@テーブルからのロードステートメントが空です、テーブルからロードされたSQLステートメントを設定してください。";
        ...
        public string DtlOpt_Add()
        {
            ...
            foreach (string str in strs)
            {
                ...
                if (dt.Rows.Count == 0)
                    return "err@データをインポートできませんでした:" + sql;
        ...
        public string DtlImpByExcel_Imp()
        {
            try
            {
                ...
                if (files.Count == 0)
                    return "err@アップロードするスレーブテンプレートを選択してください。";
                //求出扩展名.
                string fileName = files[0].FileName;
                if (fileName.Contains(".xls") == false)
                {
                    return "err@アップロードするファイルはExcelファイルである必要があります。";
                }
                ...
                //保存临时文件.
                string file = tempPath + Path.DirectorySeparatorChar + WebUser.No + ext;
                ...
                if (isHave == false)
                    throw new Exception("@インポートしたExcelファイルがシステムで必要な形式を満たしていません。テンプレートファイルをダウンロードして、もう一度入力してください。");
                #endregion
                    ...
                    foreach (BP.En.Attr attr in attrsExp)
                    {
                        ...
                        switch (attr.MyFieldType)
                        {
                            ...
                            case FieldType.PKEnum:
                                ...
                                if (isHavel == false)
                                {
                                    errMsg += "@最初のデータ形式は標準化されていません、第(" + i + ")行、列(" + attr.Desc + ")、データ(" + val + ")形式に準拠していません。変更された値は列挙リストにありません。";
                            ...
                            case FieldType.PKFK:
                                ...
                                if (isHavelIt == false)
                                    errMsg += "@最初のデータ形式は標準化されていません、第(" + i + ")行、列(" + attr.Desc + ")、データ(" + val + ")形式に準拠していません。変更された値は外部キーデータリストにありません。";
                                break;
                            default:
                                break;
                        }
                        ...
                        {
                            if (val.Trim() == "はい" || val.Trim().ToLower() == "yes")
                                val = "1";

                            if (val.Trim() == "いいえ" || val.Trim().ToLower() == "no")
                                val = "0";
                        }
                ...
                if (string.IsNullOrEmpty(errMsg) == true)
                    return "info@共有(" + i + ")データが正常にインポートされました。";
                else
                    return "共有(" + i + ")データは正常にインポートされましたが、次のエラーが発生しました:" + errMsg;
        ...
        private string BPDtlImpByExcel_Imp(DataTable dt, string fk_mapdtl)
        {
            try
            {
                ...
                if (isHave == false)
                    return "err@インポートしたExcelファイルがシステムで必要な形式を満たしていません。テンプレートファイルをダウンロードして、もう一度入力してください。";
                #endregion
        ...
        public string Print_Init()
        {
            ...
            string path = ApplicationPath + "DataUser" + Path.DirectorySeparatorChar + "CyclostyleFile" + Path.DirectorySeparatorChar + "FlowFrm" + Path.DirectorySeparatorChar + nd.FK_Flow + Path.DirectorySeparatorChar + nd.NodeID + Path.DirectorySeparatorChar;
            string[] fls = null;
            try
            {
        ...
        public string Print_Done()
        {
            ...
            BP.WF.Node nd = new BP.WF.Node(this.FK_Node);
            string path = ApplicationPath + Path.DirectorySeparatorChar + "DataUser" + Path.DirectorySeparatorChar + "CyclostyleFile" + Path.DirectorySeparatorChar + "FlowFrm" + Path.DirectorySeparatorChar + nd.FK_Flow + Path.DirectorySeparatorChar + nd.NodeID + Path.DirectorySeparatorChar;
            if (System.IO.Directory.Exists(path) == false)
            {
                return "err@テンプレートファイルが見つかりません。" + path;
            }

            string[] fls = System.IO.Directory.GetFiles(path);
            string file = fls[billIdx];
            file = file.Replace(ApplicationPath + "DataUser" + Path.DirectorySeparatorChar + "CyclostyleFile", "");

            FileInfo finfo = new FileInfo(file);
            string tempName = finfo.Name.Split('.')[0];
            string tempNameChinese = finfo.Name.Split('.')[1];

            string toPath = ApplicationPath + "DataUser" + Path.DirectorySeparatorChar + "Bill" + Path.DirectorySeparatorChar + "FlowFrm" + Path.DirectorySeparatorChar + DateTime.Now.ToString("yyyyMMdd") + Path.DirectorySeparatorChar;
            if (System.IO.Directory.Exists(toPath) == false)
                System.IO.Directory.CreateDirectory(toPath);

            // string billFile = toPath + Path.DirectorySeparatorChar + tempName + "." + this.FID + ".doc";
            string billFile = toPath + Path.DirectorySeparatorChar + tempNameChinese + "." + this.WorkID + ".doc";
        ...
        public BP.Sys.FrmAttachment GenerAthDesc()
        {
               ...
               && this.FK_FrmAttachment.Contains("AthMDtl"))
            {
                ...
                athDesc.Name = "私のスレーブテーブルアタッチメント";
                ...
                && this.FK_FrmAttachment.Contains("DocMultiAth"))
            {
                ...
                //多附件.
                athDesc.UploadType = AttachmentUploadType.Multi;
                athDesc.Name = "複数の添付ファイルがあるドキュメント（システムによって自動的に追加されます）";
        ...
        public string AttachmentUpload_DownZip()
        {
            ...
            if (dbs.Count == 0)
                return "err@ファイルが存在しないため、パッケージ化してダウンロードする必要はありません。";

            string basePath = SystemConfig.PathOfDataUser + "Temp";
            string tempUserPath = basePath + Path.DirectorySeparatorChar + WebUser.No;
            string tempFilePath = basePath + Path.DirectorySeparatorChar + WebUser.No + Path.DirectorySeparatorChar + this.OID;
            string zipPath = basePath + Path.DirectorySeparatorChar + WebUser.No;
            string zipFile = zipPath + Path.DirectorySeparatorChar + zipName + ".zip";

            string info = "";
            try
            {
                //删除临时文件，保证一个用户只能存一份，减少磁盘占用空间.
                info = "@ユーザーの一時ディレクトリを作成する:" + tempUserPath;
            ...
            {
                return "err@組織の一時ディレクトリでエラーが発生しました:" + ex.Message;
            }
            ...
            {
                return "err@ファイル編成中にエラーが発生しました:" + ex.Message;
            }
            ...
            {
                return "err@圧縮の実行エラー:" + ex.Message + "、パスtempPath:" + tempFilePath + ",zipFile=" + zipFile;
            }

            if (System.IO.File.Exists(zipFile) == false)
                return "err@圧縮ファイルは正常に生成されていません、一度クリックしてください。";
        ...
        public string Port_Init()
        {
            ...
            {
                return "err@必要なパラメーターが渡されていません、インターフェースルールを参照してください。";
            }
            ...
            foreach (string str in HttpContextHelper.RequestParamKeys)
            {
                ...
                if (val.IndexOf('@') != -1)
                    throw new Exception("パラメータがありません: [ " + str + " ," + val + " ] 指定された値では、URLは実行されません。");
                switch (str.ToLower())
                {
                    case "fk_mapdata":
                    case "workid":
