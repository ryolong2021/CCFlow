...
namespace BP.WF.HttpHandler
{
    ...
    public class WF_Comm : DirectoryPageBase
    {
        ...
        public string Tree_Init()
        {
            ...
            if (ens == null)
                return "err@エンティティ[" + this.EnsName + "]ツリーエンティティではありません。";
            //获取ParentNo
        ...
        public string Entity_Save()
        {
            try
            {
                Entity en = ClassFactory.GetEn(this.EnName);
                if (en == null)
                    return "err@エンティティクラス名エラー[" + this.EnName + "].";
        ...
        public string Entity_DoMethodReturnString()
        {
            ...
            if (mp == null)
                return "err@クラスが見つかりません[" + this.EnName + "]方法[" + methodName + "].";
        ...
        public string Entities_RetrieveAll()
        {
            ...
            {
                return "err@[Entities_RetrieveAll][" + this.EnsName + "]クラス名エラー、またはその他の例外:" + e.Message;
            }
        }
        /// <summary>
        /// 获得实体集合s
        ...
        public string Entities_DoMethodReturnString()
        {
            ...
            System.Reflection.MethodInfo mp = tp.GetMethod(methodName);
            if (mp == null)
                return "err@クラスが見つかりません[" + this.EnsName + "]方法[" + methodName + "].";
        ...
        public string Method_Init()
        {
            ...
            if (rm == null)
                return "err@メソッド名が間違っているか、メソッドが存在しません" + ensName;
        ...
        public string Method_Done()
        {
            ...
            foreach (Attr attr in rm.HisAttrs)
            {
                ...
                {
                    return "err@パラメータエラーの取得" + "attr=" + attr.Key + " attr = " + attr.Key + ex.Message;
                }
            }
            ...
            try
            {
                ...
                if (obj != null)
                    ...
                    return "err@実行後に情報が返されない。";
            }
            catch (Exception ex)
            {
                return "err@実行エラー:" + ex.Message;
            }
        }
        ...
        public string Search_MapBaseInfo()
        {
            ...
            Entities ens = ClassFactory.GetEns(this.EnsName);
            if (ens == null)
                return "err@クラス名:" + this.EnsName + "エラー";
        ...
        public string Search_SearchAttrs()
        {
            ...
            Entities ens = ClassFactory.GetEns(this.EnsName);
            if (ens == null)
                return "err@クラス名エラー:" + this.EnsName;
            ...
            private DataTable Search_Data(Entities ens, Entity en)
        {
            ...
            {
                if (map.IsShowSearchKey && DataType.IsNullOrEmpty(keyWord) == false && keyWord.Length >= 1)
                {
                    Attr attrPK = new Attr();
                    foreach (Attr attr in map.Attrs)
                    {
        ...
        public string Search_GenerPageIdx()
        {
            ...
            {
                myidx = PageIdx + 1;
                html += "<li><a href='" + url + "&PageIdx=" + myidx + "'><img style='vertical-align:middle' src='" + Glo.CCFlowAppPath + "WF/Img/Arr/Right.png' border=0/></a>&nbsp;<a href='" + url + "&PageIdx=" + pageNum + "'><img style='vertical-align:middle' src='" + Glo.CCFlowAppPath + "WF/Img/Arr/RightEnd.png' border=0/></a> &nbsp;&nbsp;ページ数:" + PageIdx + "/" + pageNum + "&nbsp;&nbsp;合計:" + recNum + "</li>";
            }
            else
            {
                html += "<li><img style='vertical-align:middle' src='" + Glo.CCFlowAppPath + "WF/Img/Arr/Right.png' border=0/></li>";
                html += "<li><img style='vertical-align:middle' src='" + Glo.CCFlowAppPath + "WF/Img/Arr/RightEnd.png' border=0/>&nbsp;&nbsp;ページ数:" + PageIdx + "/" + pageNum + "&nbsp;&nbsp;合計:" + recNum + "</li>";
            }
            html += "</ul>";
            return html;
        }
        ...
        public string Entities_Delete()
        {
            ...
            {
                if (this.Paras == null)
                    return "err@エンティティを削除します、パラメータは空にできません";
                string[] myparas = this.Paras.Split('@');
                ...
                if (paras.Count > 4)
                    return "err@エンティティクラスの削除、条件は4以下にする必要があります";
        ...
        public string DoOneEntity(Entity en, int rmIdx)
        {
            ...
            catch (Exception ex)
            {
                ...
                foreach (object obj in objs)
                    msg += "@" + obj.ToString();
                string err = "@実施した[" + this.EnsName + "][" + rm.ClassMethodName + "]中にエラーが発生しました:" + ex.Message + " InnerException= " + ex.InnerException + "[パラメータ:]" + msg;
        ...
        public string Exec(string clsName, string methodName, string paras = null)
        {
            ...
            if (clsName.Contains(".HttpHandler.") == true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    return "err@" + ex.Message + " パラメータ:" + parasStr;
                }
            }
            #endregion 处理 page 类.
            ...
            try
            {
                ...
                System.Reflection.MethodInfo mp = tp.GetMethod(methodName);
                if (mp == null)
                    return "err@クラスが見つかりません[" + clsName + "]方法[" + methodName + "].";
            ...
            {
                return "err@エンティティークラスの実行方法が間違っている:" + ex.Message;
            }
...
#warning zhoupeng把这个去掉了. 2018.10.24
        ...
        public string HttpHandler()
        {
            ...
            if (type == null)
            {
                ...
                if (obj == null)
                    return "err@ページ処理クラス名[" + httpHandlerName + "]、聞き取れませんでした。スペルミスを確認してください。";
        ...
        public string EntityAth_Upload()
        {
            ...
            if (files.Count == 0)
                return "err@アップロードするファイルを選択してください。";
            //获取保存文件信息的实体
            ...
            if (isBlank == true)
                return "err@ファイルをアップロードする前にエンティティ情報を保存してください";
            else
            ...
            int i = en.RetrieveFromDBSources();
            if (i == 0)
                return "err@データ[" + this.EnName + "]主キーは[" + en.PKVal + "]存在しないか、保存されていません。";
            ...
            else
            {
                ...
                filepath = fileSavePath + Path.DirectorySeparatorChar + this.PKVal + "." + ext;
        ...
        public string EntityMultiAth_Upload()
        {
            ...
            else
            {
                ...
                filepath = savePath + Path.DirectorySeparatorChar + fileName + ext;
                //存在文件则删除
        ...
        public string Group_Search()
        {
            ...
            if (ds == null)
                return "info@<img src='../Img/Warning.gif' /><b><font color=red> コンテンツ/分析項目を表示することを選択しなかった</font></b>";
        ...
        private DataSet GroupSearchSet(Entities ens, Entity en, Map map, UserRegedit ur, DataSet ds, ActiveAttrs aas)
        {
            ...
            foreach (string ct in statNumKeys)
            {
                ...
                {
                    AttrsOfNum.Add(new Attr("Group_Number", "Group_Number", 1, DataType.AppInt, false, "量"));
                }
                else
                {
        ...
        public string Group_Exp()
        {
            ...
            ds = GroupSearchSet(ens, en, map, ur, ds, aas);
            if (ds == null)
                return "info@<img src='../Img/Warning.gif' /><b><font color=red> 分析するデータを選択しませんでした</font></b>";
        ...
        private string readTxt()
        {
            try
            {
                string path = BP.Sys.SystemConfig.PathOfDataUser + "Fastenter" + Path.DirectorySeparatorChar + FK_MapData + Path.DirectorySeparatorChar + GetRequestVal("AttrKey"); ;
