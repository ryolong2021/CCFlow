    ...
    public class WF_Comm_RefFunc : DirectoryPageBase
    {
        ...
        public string Dot2DotTreeDeptEmpModel_SaveNodeEmps()
        {
            ...
            if (string.IsNullOrWhiteSpace(nodeid) || int.TryParse(nodeid, out nid) == false)
                throw new Exception("パラメータnodeidが正しくありません");
            ...
            else
            {
                ...
                if (parts.Length != 2)
                    throw new Exception("パラメータpartnoが正しくありません");
            ...
            else
            {
                ...
                if (lastpart)
                    ...
                    jr.Msg = string.Format("第{0}/{1}段は正常に保存されました", partidx, partcount);
            }
        ...
        public string Dot2DotStationModel_GetStructureTreeRoot()
        {
            ...
            if (string.IsNullOrWhiteSpace(parentrootid))
                throw new Exception("パラメータparentrootidは空にできません");
            ...
            if (isUnitModel)
            {
                ...
                if (dt.Rows.Count == 0)
                    dt.Rows.Add("-1", "単位のないデータ", parentrootid);
        ...
        public string Dot2DotStationModel_GetSubUnits()
        {
            ...
            if (string.IsNullOrWhiteSpace(parentid))
                throw new Exception("パラメータparentidは空にできません");
            if (string.IsNullOrWhiteSpace(nid))
                throw new Exception("パラメータnodeidは空にできません");
