...
namespace BP.WF.HttpHandler
{
    ...
    public class WF_MyFlow : DirectoryPageBase
    {
        ...
        public string MyFlow_Init()
        {
            ...
            if (this.WorkID != 0)
            {
                ...
                if (isCanDo == false)
                {
                    ...
                    return "err@君は[" + WebUser.No + "," + WebUser.Name + "]現在のジョブを実行できません。現在のジョブが実行されました[" + mygwf.NodeName + "]、ハンドラ[" + mygwf.TodoEmps + "]。";
                }

                gwf = new GenerWorkFlow();
                gwf.WorkID = this.WorkID;
                if (gwf.RetrieveFromDBSources() == 0)
                    return ("err@フローID{" + this.WorkID + "}存在しないか削除されました");
                IsExistGWF = true;
            ...
            if (this.currND.IsStartNode && this.IsCC == false && this.WorkID == 0)
            {
                try
                {
                    ...
                    {
                        /*是否可以发起流程？ @李国文. */
                        return "err@君は(" + BP.Web.WebUser.No + ")フローを開始または処理する権限がありません。";
                    }
                }
                catch (Exception ex)
                {
            ...
            if (this.WorkID == 0 && this.currND.IsStartNode && this.GetRequestVal("IsCheckGuide") == null)
            {
                ...
                switch (this.currFlow.StartGuideWay)
                {
                    ...
                    case StartGuideWay.ByHistoryUrl: // 历史数据.
                        ...
                        {
                            return "err@フロー構成が間違っています。フロントナビゲーションを同時に有効にすることはできません。最後のデータを自動的にロードする2つの機能。";
                        }
                        return "url@" + hostRun + "StartGuide.htm?FK_Flow=" + this.currFlow.No + "&WorkID=" + workid;
                    case StartGuideWay.BySystemUrlOneEntity:
            ...
            if (this.currND.HisFormType == NodeFormType.SDKForm)
            {
                ...
                {
                    return "err@読み取り状態フロー設計エラー状態エラーを設定します。フォームURLが設定されていません。";
                }
        ...
        public string MyFlow_StopFlow()
        {
            try
            {
                string str = BP.WF.Dev2Interface.Flow_DoFlowOver(this.FK_Flow, this.WorkID, "フローは正常に終了しました");
                if (str == "" || str == null)
                    return "フローは正常に終了しました";
                return str;
            }
            catch (Exception ex)
            {
        ...
        public string InitToolBar()
        {
            ...
            try
            {
                ...
                {
                    toolbar += "<input name='Send' type=button value='確定/完成' data-type='isAskFor' enable=true onclick=\" " + btnLab.SendJS + " if(SysCheckFrm()==false) return false;SaveDtlAll();Send(); \" />";
                ...
                if (this.IsCC)
                {
                    toolbar += "<input type=button  value='フローランニングトラック' enable=true onclick=\"WinOpen('" + appPath + "WF/WorkOpt/OneWork/OneWork.htm?CurrTab=Truck&WorkID=" + this.WorkID + "&FK_Flow=" + this.FK_Flow + "&FID=" + this.FID + "&FK_Node=" + this.FK_Node + "&s=" + tKey + "','ds'); \" />";
                    // 判断审核组件在当前的表单中是否启用，如果启用了.
                    FrmWorkCheck fwc = new FrmWorkCheck(this.FK_Node);
                    if (fwc.HisFrmWorkCheckSta != FrmWorkCheckSta.Enable)
                    {
                        /*如果不等于启用, */
                        toolbar += "<input type=button  value='レビューコメントを入力します' enable=true onclick=\"WinOpen('" + appPath + "WF/WorkOpt/CCCheckNote.htm?WorkID=" + this.WorkID + "&FK_Flow=" + this.FK_Flow + "&FID=" + this.FID + "&FK_Node=" + this.FK_Node + "&s=" + tKey + "','ds'); \" />";
                    }
                    return toolbar;
                }
                #endregion 是否是抄送.
                ...
                {
                    if (this.IsMobile == true)
                        toolbar += "<input name='SendHuiQian' type=button value='送信する副署' enable=true onclick=\" " + btnLab.SendJS + " if(SysCheckFrm()==false) return false;SaveDtlAll();SendIt(true); \" />";
                    else
                        toolbar += "<input name='SendHuiQian' type=button value='送信する副署' enable=true onclick=\" " + btnLab.SendJS + " if(SysCheckFrm()==false) return false;SaveDtlAll();Send(true); \" />";
                }
                ...
                {
                    /*移交*/
                    string url12 = "./WorkOpt/Forward.htm?FK_Node=" + this.FK_Node + "&WorkID=" + this.WorkID + "&FID=" + this.FID + "&FK_Flow=" + this.FK_Flow + "&Info=" + "引渡理由。";
                ...
                if (btnLab.WebOfficeWorkModel == WebOfficeWorkModel.Button)
                {
                    ...
                    toolbar += "<input type=button name='WebOffice'  value='" + btnLab.WebOfficeLab + "' enable=true onclick=\"WinOpen('" + urlr + "','公式文書本体'); \" />";
                }
                ...
                {
                    /* 启用了数据重置功能 */
                    toolbar += "<input type=button  value='データのリセット' enable=true  id='reset' name='reset' onclick=\"resetData(); \" />";
                }
                ...
                if (btnLab.FocusEnable == true)
                {
                    if (HisGenerWorkFlow.Paras_Focus == true)
                        toolbar += "<input type=button  value='フォロー解除' enable=true onclick=\"FocusBtn(this,'" + this.WorkID + "'); \" />";
                    else
                ...
                {
                    /*分配工作*/
                    string urlAllot = "./WorkOpt/AllotTask.htm?FK_Node=" + this.FK_Node + "&WorkID=" + this.WorkID + "&FID=" + this.FID + "&FK_Flow=" + this.FK_Flow + "&Info=" + "引渡理由。";
                ...
                if (btnLab.ConfirmEnable == true)
                {
                    if (HisGenerWorkFlow.Paras_Confirm == true)
                        toolbar += "<input type=button  value='確認のキャンセル' enable=true onclick=\"ConfirmBtn(this,'" + this.WorkID + "'); \" />";
                    else
                ...
                if (this.currND.IsStartNode == true)
                {
                    if (this.currFlow.IsDBTemplate == true)
                    {
                        ...
                        toolbar += "<input type=button name='DBTemplate'  value='テンプレート' enable=true/>";
                    }
                }
        ...
        public string InitToolBarForMobile()
        {
            ...
            try
            {
                ...
                if (this.IsCC)
                {
                    toolbar += "<a data-role='button'    value='フローランニングトラック' enable=true onclick=\"WinOpen('" + appPath + "WF/WorkOpt/OneWork/OneWork.htm?CurrTab=Truck&WorkID=" + this.WorkID + "&FK_Flow=" + this.FK_Flow + "&FID=" + this.FID + "&FK_Node=" + this.FK_Node + "&s=" + tKey + "','ds'); \" ></a>";
                    // 判断审核组件在当前的表单中是否启用，如果启用了.
                    FrmWorkCheck fwc = new FrmWorkCheck(this.FK_Node);
                    if (fwc.HisFrmWorkCheckSta != FrmWorkCheckSta.Enable)
                    {
                        /*如果不等于启用, */
                        toolbar += "<a data-role='button' type=button  value='レビューコメントを入力します' enable=true onclick=\"WinOpen('" + appPath + "WF/WorkOpt/CCCheckNote.htm?WorkID=" + this.WorkID + "&FK_Flow=" + this.FK_Flow + "&FID=" + this.FID + "&FK_Node=" + this.FK_Node + "&s=" + tKey + "','ds'); \" ></a>";
                    }
                    return toolbar;
                }
                #endregion 是否是抄送.
                ...
                if (btnLab.WebOfficeWorkModel == WebOfficeWorkModel.Button)
                {
                    /*公文正文 */
                    string urlr = appPath + "WF/WorkOpt/WebOffice.htm?FK_Node=" + this.FK_Node + "&FID=" + this.FID + "&WorkID=" + this.WorkID + "&FK_Flow=" + this.FK_Flow + "&s=" + tKey;
                    toolbar += "<a data-role='button' type=button name='WebOffice'  value='" + btnLab.WebOfficeLab + "' enable=true onclick=\"WinOpen('" + urlr + "','公式文書本体'); \" ></a>";
                }
                ...
                if (this.currFlow.IsResetData == true && this.currND.IsStartNode)
                {
                    ...
                    toolbar += "<a data-role='button' type=button  value='データのリセット' enable=true onclick=\"To('" + urlr3 + "','ds'); \" ></a>";
                }
                ...
                {
                    if (HisGenerWorkFlow.Paras_Focus == true)
                        toolbar += "<a data-role='button' type=button  value='フォロー解除' enable=true onclick=\"FocusBtn(this,'" + this.WorkID + "'); \" ></a>";
                    else
                ...
                {
                    if (HisGenerWorkFlow.Paras_Confirm == true)
                        toolbar += "<a data-role='button' type=button  value='確認のキャンセル' enable=true onclick=\"ConfirmBtn(this,'" + this.WorkID + "'); \" ></a>";
                    else
        ...
        public string Send()
        {
            try
            {
                ...
                if (i == 0)
                    return "このフローの作業は削除されました。管理者に連絡してください";
                ...
                switch (currNode.HisTurnToDeal)
                {
                    case TurnToDeal.SpecUrl:
                        ...
                        if (myurl.Contains("@"))
                        {
                            BP.WF.Dev2Interface.Port_SendMsg("admin", currFlow.Name + "に" + currND.Name + "ノードでエラーが発生しました", "フロー設計が間違っており、ノードリダイレクトURLのパラメーターは置き換えられません。Url:" + myurl, "Err" + currND.No + "_" + this.WorkID, SMSMsgType.Err, this.FK_Flow, this.FK_Node, this.WorkID, this.FID);
            ...
            catch (Exception ex)
            {
                ...
                if (ex.Message.Contains("送信する前に、次の必須フィールドが不完全であることを確認してください") == true || ex.Message.Contains("添付ファイルをアップロードしませんでした") == true || ex.Message.Contains("画像の添付ファイルをアップロードしていません") == true)
                {
        ...
        public string Save()
        {
            ...
            {
                return "err@保存できませんでした:" + ex.Message;
            }
        }
            ...
            foreach (FrmNode frmNode in frmNodes)
            {
                ...
                switch (frmNode.FrmEnableRole)
                {
                    ...
                    case FrmEnableRole.ByFrmFields:
                        throw new Exception("@このタイプの判断、ByFrmFieldsはまだ完了していません。");
                    ...
                    case FrmEnableRole.BySQL: // 按照SQL的方式.
                        ...
                        if (DataType.IsNullOrEmpty(mysql) == true)
                        {
                            ...
                            return "err@形" + frmNode.FK_Frm + ",[" + FrmMd.Name + "]ノードで[" + frmNode.FK_Node + "]有効化メソッドはSQLに従って有効化されていますが、SQL式を設定していません。";
                        }
                    ...
                    case FrmEnableRole.Disable: // 如果禁用了，就continue出去..
                        continue;
                    default:
                        throw new Exception("@判決のルールなし。" + frmNode.FrmEnableRole);
                }
                #endregion
        ...
        public void TansEntitiesToGenerTree(Entities ens, string rootNo, string checkIds)
        {
            ...
            if (root == null)
                throw new Exception("@未発見rootNo=" + rootNo + "のエンティティ。");
            appendMenus.Append("[{");
