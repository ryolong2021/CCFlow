...
namespace BP.WF.HttpHandler
{
    ...
    public class WF_RptDfine : DirectoryPageBase
    {
        ...
        public string Default_Init()
        {
            ...
            ht.Add("My", "フローを開始しました");
            ht.Add("MyJoin", "私の承認フロー");
        ...
        public string FlowSearch_Init()
        {
            if (string.IsNullOrWhiteSpace(this.FK_Flow))
                return "err@パラメータFK_Flowは空にできません";
            ...
            if (md.RetrieveFromDBSources() == 0)
            {
                ...
                if (this.SearchType == "My")
                    rd.DoReset(this.SearchType, "フローを開始しました");

                if (this.SearchType == "MyJoin")
                    rd.DoReset(this.SearchType, "私の承認フロー");

                if (this.SearchType == "MyDept")
                    rd.DoReset(this.SearchType, "この部門が開始したフロー");

                if (this.SearchType == "Adminer")
                    rd.DoReset(this.SearchType, "高度な検索");
            ...
            string tmpDir = BP.Sys.SystemConfig.PathOfDataUser + "TempleteExpEns" + Path.DirectorySeparatorChar + rptNo;
            ...
            switch (this.SearchType)
            {
                ...
                case "Adminer":
                    break;
                default:
                    return "err@" + this.SearchType + "エラーをマーク。";
            }
        ...
        public string FlowGroup_Init()
        {
            ...
            if (md.RetrieveFromDBSources() == 0)
            {
                ...
                if (this.GroupType == "My")
                    rd.DoReset(this.GroupType, "フローを開始しました");

                if (this.GroupType == "MyJoin")
                    rd.DoReset(this.GroupType, "私の承認フロー");

                if (this.GroupType == "MyDept")
                    rd.DoReset(this.GroupType, "この部門が開始したフロー");

                if (this.GroupType == "Adminer")
                    rd.DoReset(this.GroupType, "高度な検索");
        ...
        public string FlowGropu_Done()
        {
            ...
            if (!this.GroupType.Equals("My") && this.GroupType.Equals("MyJoin") && this.GroupType.Equals("MyDept") && this.GroupType.Equals("Adminer"))
                return "info@<img src='../Img/Pub/warning.gif' /><b><font color=red>" + this.GroupType + "マークアップが間違っています。</font></b>";
            DataSet ds = new DataSet();
            ds = FlowGroupDoneSet();
            if (ds == null)
                return "info@<img src='../Img/Pub/warning.gif' /><b><font color=red> 表示するものを選択しませんでした</font></b>";
