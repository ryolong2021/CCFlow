...
public partial class SDKFlowDemo_DemoEntity : System.Web.UI.Page
{
    ...
    public void ccbpmDemo()
    {
        ...
        if (1 == 2)
        {
            string returnMsg = BP.WF.Dev2Interface.Node_ReturnWork(flowNo, workid, 0, nextNodeID, 101, "リターンテスト", false);
        }
        ...
        if (1 == 2)
        {
            BP.WF.Dev2Interface.Node_Shift(flowNo, nextNodeID, workid, 0, "fuhui", "ハンドオーバーテスト.");
            return;
        }
    ...
    {
        // 写入一条消息.
        BP.Sys.Glo.WriteLineInfo("これはメッセージです。");

        // 写入一条警告.
        BP.Sys.Glo.WriteLineWarning("これは警告です。");

        // 写入一条异常或者错误.
        BP.Sys.Glo.WriteLineError("これはエラーです。");  // 以上这些日志写入了 \DataUser\Log\*.*

        //写入用户日志
        BP.Sys.Glo.WriteUserLog("Login", "stone", "システムログイン");
        BP.Sys.Glo.WriteUserLog("Login", "stone", "システムログイン", "192.168.1.100");  // 以上用户日志写入了 Sys_UserLog 表里.)
    }
    /// <summary>
    /// 全局的基本应用,获取当前操作员的信息.
    /// </summary>
    ...
    public void EntitiesDemo()
    {
        ...
        {
            strs += "<hr>";
            strs += "<br>ナンバリング:" + stu.No + " 名前:" + stu.Name + " 住所:" + stu.Addr + " 性別:" + stu.XB + " 性別:" + stu.XBText;
        }
        ...
        {
            strs += "<hr>";
            strs += "<br>ナンバリング:" + stu.No + " 名前:" + stu.Name + " 住所:" + stu.Addr;
        }
        this.Response.Write(strs);
        ...
        {
            strs += "<hr>";
            strs += "<br>ナンバリング:" + stu.No + " 名前:" + stu.Name + " 住所:" + stu.Addr + " クラス:" + stu.FK_BanJiText;
        }
        this.Response.Write(strs);
    ...
    public void EntityBaseApp()
    {
        ...
        {
            this.Response.Write("IDがzhangsanと等しい人員の記録はありません.");
            return;
        }
        else
        {
            msg = "";
            msg += "<BR>ナンバリング:" + myEmp.No;
            msg += "<BR>名前:" + myEmp.Name;
            msg += "<BR>パスワード:" + myEmp.Pass;
            msg += "<BR>部門番号:" + myEmp.FK_Dept;
            msg += "<BR>部門名:" + myEmp.FK_DeptText;
        ...
        if (delNum == 0)
            this.Response.Write("zhangsanを削除できませんでした。");

        if (delNum == 1)
            this.Response.Write("zhangsanを削除しました。");
        if (delNum > 1)
            this.Response.Write("例外の異常が発生してはならない。");
    ...
    public void EntitiesBaseApp()
    {
        ...
        foreach (Student stu in stus)
        {
            string str = "学生:" + stu.No + " 名前:" + stu.Name;
        ...
        this.Response.Write("RetrieveAll(" + num + ")件をクエリしました。");
        foreach (Emp emp in emps)
        {
            this.Response.Write("<hr>人員:" + emp.Name);
            this.Response.Write("<br>人員番号:" + emp.No);
            this.Response.Write("<br>部門番号:" + emp.FK_Dept);
            this.Response.Write("<br>部門名:" + emp.FK_DeptText);
        }
        ...
        this.Response.Write("RetrieveAllFromDBSource(" + num + ")件をクエリしました。");
        ...
        this.Response.Write("部門番号= 01の人を(" + num + ")件をクエリしました。");
        ...
        {
            this.Response.Write("<hr>人員:" + item.Name);
            this.Response.Write("<br>人員番号:" + item.No);
            this.Response.Write("<br>部門番号:" + item.FK_Dept);
            this.Response.Write("<br>部門名:" + item.FK_DeptText);
        }
        ...
        num = qo.DoQuery();  // 返回查询的个数.
        this.Response.Write("部門番号= 01および番号= guobaogenの人(" + num + ")件をクエリしました。");
        foreach (Emp emp in myEmps)
        {
            this.Response.Write("<hr>人員:" + emp.Name);
            this.Response.Write("<br>人員番号:" + emp.No);
        ...
        num = qo.DoQuery();  // 返回查询的个数.
        this.Response.Write("(" + num + ")件をクエリしました，WHERE IN (SELECT No FROM Port_Emp WHERE FK_Dept='02')人员。");
        foreach (Emp emp in myEmps)
        {
            this.Response.Write("<hr>人員:" + emp.Name);
            this.Response.Write("<br>人員番号:" + emp.No);
        ...
        num = qo.DoQuery();  // 返回查询的个数.
        this.Response.Write("人員はguoという人を(" + num + ")件をクエリしました。");
        foreach (Emp emp in myEmps)
        {
            this.Response.Write("<hr>人員:" + emp.Name);
            this.Response.Write("<br>人員番号:" + emp.No);
        ...
        // 判断是否包含是指定的主键值.
        bool isHave = myEmps.Contains("Name", "郭宝明"); //判断是否集合里面包含Name=郭宝明的实体.
        bool isHave1 = myEmps.Contains("guobaogeng"); //判断是否集合里面主键No=guobaogeng的实体.

        // 获取Name=郭宝庚的实体，如果没有就返回空。
        Emp empFind = myEmps.GetEntityByKey("Name", "郭宝明") as Emp;
        if (empFind == null)
            this.Response.Write("<br>エンティティが見つかりません：名前= Guo Baogeng.");
        else
            this.Response.Write("<br>もう見つけました。Name=郭宝庚の実体。彼の部門番号=" + empFind.FK_Dept + "，部門名=" + empFind.FK_DeptText);
        // 批量更新实体。
    ...
    public void EnttiyNo()
    {
        ...
        en.Addr = "山東省、済南省、ハイテクゾーン";
        en.Insert(); //这里会自动给该学生编号，从0001开始，编号规则打开该类的Map.

        string xuehao = en.No;
        this.Response.Write("情報が追加されました。学生IDは:" + xuehao);

        //查询出来该实体。
        BP.Demo.BPFramework.Student myen = new BP.Demo.BPFramework.Student(xuehao);

        this.Response.Write("学生の名前:" + myen.Name);
        this.Response.Write("住所:" + myen.Addr);
    }
    /// <summary>
    /// 实体OID
    /// </summary>
    public void EnttiyOID()
    {
        ...
        dtl.NianYue = "2014年4月";
        dtl.GongZuoDanWei = "済南ギャロッピング会社"; // 工作单位.
        dtl.ZhengMingRen = "Li Si";  //证明人,Li Si.
        dtl.Insert(); //这里会自动给该实体主键OID赋值， 他是一个自动增长的列.
        this.Response.Write("情報が追加されました,OID:" + dtl.OID);

        //初始化该实体，并把它显示出来.
        BP.Demo.Resume mydtl = new BP.Demo.Resume(dtl.OID);
        this.Response.Write("会社:" + dtl.GongZuoDanWei + "証人:" + dtl.ZhengMingRen);
    }
    /// <summary>
    ...
    public void EnttiyMyPK()
    {
        ...
        en.Cent = 100;
        en.Insert();  // 插入到数据库里.
        this.Response.Write("セントが情報を追加しました:" + en.Cent);

        BP.Demo.BPFramework.EmpCent myen = new BP.Demo.BPFramework.EmpCent(en.MyPK);
        this.Response.Write("人員:" + myen.FK_Emp + ",月:" + myen.FK_NY + ", スコア:" + myen.Cent);
    }
    /// <summary>
