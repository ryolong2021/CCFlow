...
function Frm() {
    ...
    if (flow.FlowFrmType == FlowFrmType.FoolFrm)
        url = "../../WF/Admin/FoolFormDesigner/Designer.htm?FrmID=" + frmID + "&FK_Flow=" + flowNo + "&FK_MapData=" + frmID + "&FK_Node=" + nodeID;

    if (flow.FlowFrmType == FlowFrmType.DeveloperFrm)
        url = "../../WF/Admin/DevelopDesigner/Designer.htm?FrmID=" + flowNo + "&FK_Flow=" + flowNo + "&FK_MapData=" + frmID + "&FK_Node=" + nodeID;
    ...
    var _canvas = $("#flowdesign_canvas").Flowdesign({
        ...
        canvasLabMenu: {
            ...
            "clmNewName": function (t) {
                ...
                var xiuNodename = '名前を編集:<input id="TB_LAB_' + activeId + '" type="text" value="' + windowtext + '">'
...
function FlowProperty() {
    url = "../../WF/Comm/En.htm?EnName=BP.WF.Template.FlowExt&PKVal=" + flowNo + "&Lang=CH";
...
function FlowRpt() {
    ...
    url = "../../WF/Admin/RptDfine/Default.htm?FK_Flow=" + flowNo + "&FK_MapData=ND" + flowId + "MyRpt";
...
function FlowRun() {
    ...
    var url = "../../WF/TestFlow.htm?FK_Flow=" + flowNo + "&Lang=CH";
    //WinOpen(url);
...
function FlowRun2020() {
    ...
    var url = "../../WF/TestingContainer/TestFlow2020.htm?FK_Flow=" + flowNo + "&Lang=CH";
...
function FlowRunAdmin() {
    ...
    var url = "../../WF/TestFlow.htm?DoType=TestFlow_ReturnToUser&DoWhat=StartClassic&UserNo=" + webUser.No + "&FK_Flow=" + flowNo;
...
function NodeFrmSln(nodeID) {
    //表单方案.
    var url = "../../WF/Admin/AttrNode/FrmSln/Default.htm?FK_Node=" + nodeID;
...
function CondDir(fromNodeID) {
    ...
    var url = "../../WF/Admin/Cond/ConditionLine.htm?FK_Flow=" + flowNo + "&FK_MainNode=" + fromNodeID + "&FK_Node=" + fromNodeID + "&ToNodeID=" + targetId + "&CondType=2&Lang=CH&t=" + new Date().getTime();
    $("#LineModal").hide();
    $(".modal-backdrop").hide();
...
function FrmPower(nodeID) {
    ...
    //傻瓜表单.
    var url = "../../WF/Comm/En.htm?EnName=BP.WF.Template.FrmNodeJiJian&MyPK=" + mypk + "&Lang=CH";
    //WinOpen(url);
...
function NodeFrmFool(nodeID) {
    //傻瓜表单.
    var url = "../../WF/Admin/FoolFormDesigner/Designer.htm?FK_MapData=ND" + nodeID + "&IsFirst=1&FK_Flow=" + flowNo + "&FK_Node=" + nodeID;
    //WinOpen(url);
    window.parent.addTab(nodeID + "_Fool", "デザインフォーム" + nodeID, url);
}
...
function NodeFrmFree(nodeID) {
    ...
    //自由表单.
    var url = "../../WF/Admin/CCFormDesigner/FormDesigner.htm?FK_MapData=ND" + nodeID + "&FK_Flow=" + flowNo + "&FK_Node=" + nodeID;
...
function NodeFrmDeveloper(nodeID) {
    //开发者表单.
    var url = "../../WF/Admin/DevelopDesigner/Designer.htm?FK_MapData=ND" + nodeID + "&FK_Flow=" + flowNo + "&FK_Node=" + nodeID;
...
function NodeAccepterRole(nodeID) {
    //接受人规则.
    var url = "../../WF/Admin/AttrNode/AccepterRole/Default.htm?FK_MapData=ND" + nodeID + "&FK_Flow=" + flowNo + "&FK_Node=" + nodeID;
    window.parent.addTab(nodeID + "_JSGZ", "受信者のタールール" + nodeID, url);
