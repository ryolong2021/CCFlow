...
//-----条件设置--strat----------------
    ...
    function fnAddLeftParenthesis(id){
        ...
        if(oObj.options.length>0){ //检查是否有条件
            ...
            alert("最初に条件を追加してから、括弧を選択してください");
            return;
        }
        ...
        if( (trim(sValue).substr(-3,3) == 'AND') || (trim(sValue).substr(-2,2) == 'OR') ){
            alert("既存の関係の条件を編集できません");
            return;
        }
        var sRelation = _id('relation_'+id).value;
        if( sValue.indexOf('(')>=0 ){
            if( !fnCheckExp(sValue) ){
                alert("条件式の記述が間違っています。括弧の一致を確認してください");
                return;
            } else {
...
//TAB
  ...
  $('#process_multiple').multiselect2side({
      ...
      labelDown: '下に移動',
      labelSort: 'ソート',
      labelsx: '<i class="icon-ok"></i> 次のステップ',
      labeldx: '<i class="icon-list"></i> 代替手順',
      autoSort: false,
