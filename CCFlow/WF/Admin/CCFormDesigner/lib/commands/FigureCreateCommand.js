...
FigureCreateCommand.prototype = {
    ...
    execute: function () {
        if (createFigureName == "FlowChart" || createFigureName == "ThreadDtl" || createFigureName == "SubFlowDtl" || createFigureName == "FrmCheck") {
            ...
            if (isExit == true) {
                $.messager.alert("エラー", "@既存のIDは(" + createFigureName + ")同じ名前の要素は許可されていません！", "error");
                return false;
            }
        }
        ...
        if (this.firstExecute) {
            ...
            switch (createFigureName) {
                ...
                case CCForm_Controls.HiddendField:
                    alert('まだありません{' + createFigureName + '}コントロールのサポートを提供します。');
                    return;
                ...
                case CCForm_Controls.AthImg: //图片附件.  以上控件都是用通用的 No,Name 数据框.
                    ...
                    break;
                case "CheckGroup":
                    alert('この機能は実装されていません' + createFigureName + ' 3つのフィールドを連続して作成する必要があります。');
                    break;
                ...
                default: //按照通用的接受编号，名称的方式来创建.
                    alert('コントロールタイプが判断されない{' + createFigureName + '}，または機能が実現されます。');
                    return;
            }
    ...
    DataImgCreate: function (createdFigure, x, y) {
        ...
        OpenEasyUiDialog(url, dgId, '新しい画像フィールド', 600, 394, 'icon-new', true, function () {
            var win = document.getElementById(dgId).contentWindow;
            var frmVal = win.GetFrmInfo();

            if (frmVal.Name == null || frmVal.Name.length == 0) {
                $.messager.alert('エラー', 'フィールド名を空にすることはできません。', 'error');
                return false;
            }
            if (frmVal.KeyOfEn == null || frmVal.KeyOfEn.length == 0) {
                $.messager.alert('エラー', '英語フィールドを空にすることはできません。', 'error');
                return false;
            }
            //判断主键是否存在
            var isExit = funIsExist(frmVal.KeyOfEn);
            if (isExit == true) {
                $.messager.alert("エラー", "既存のIDは(" + frmVal.KeyOfEn + ")同じ名前の要素は許可されていません！", "error");
                return false;
            }
...
//            //根据信息创建不同类型的数字控件
    ...
    DataFieldCreate: function (createdFigure, x, y) {
        ...
        OpenEasyUiDialog(url, dgId, '新しいテキストフィールド', 600, 394, 'icon-new', true, function (HidenFieldFun) {
            var win = document.getElementById(dgId).contentWindow;
            var frmVal = win.GetFrmInfo();
    ...
    PublicNoNameCtrlCreate: function (createdFigure, x, y, ctrlType) {
        ...
        switch (ctrlType) {
            ...
            case "iFrame":
                ...
                break;
            default:
                alert('判断のないコントロールタイプPublicNoNameCtrlCreate：' + ctrlType);
                return;
        }
        ...
        OpenEasyUiDialog(url, dgId, lab, 650, 394, 'icon-new', true, function () {
            ...
            if (frmVal.Name == null || frmVal.Name.length == 0) {
                $.messager.alert('エラー', '名前が必要です。', 'error');
                return false;
            }

            if (frmVal.No == null || frmVal.No.length == 0) {
                $.messager.alert('エラー', '番号を空にすることはできません。', 'error');
                return false;
            }
            ...
            if (isExit == true) {
                $.messager.alert("エラー", "@既存のIDは(" + frmVal.No + ")同じ名前の要素は許可されていません！", "error");
                return false;
            }
    ...
    FlowFieldCreate: function (createdFigure, x, y, createFigureName) {
        ...
        switch (createFigureName) {
            ...
            case "SubFlowDtl": //子流程.
                ...
                break;
            case "ThreadDtl": //子线程.
                frmVal.Name = "子スレッドコンポーネント";
                break;
            case "FrmTransferCustom": //流转自定义.
                frmVal.Name = "流通自定義";
                break;
            default:
                alert('未定義のタイプ：' + createFigureName);
                return;
        }
        //根据信息创建不同类型的数字控件.
        var transField = new TransFormDataField(createdFigure, frmVal, x, y);
    ...
    Transform: function () {
        ...
        switch (createdFigure.CCForm_Shape) {
            ...
            case "AthSingle":
                ...
                break;
            case "TextBoxStr":
                ctrlLab = 'コントロールのプロパティ-テキストボックス';
                break;
            case "FlowChart":
                ctrlLab = 'コントロールプロパティ-トラックグラフ';
            case "ThreadDtl":
                ctrlLab = 'コントロールプロパティ-子スレッド';
            case "SubFlowDtl":
                ctrlLab = '制御プロパティ-サブフロー';
            case "FrmCheck":
                ctrlLab = '制御プロパティ-監査コンポーネント';
            default:
                ctrlLab = 'コントロールプロパティ' + createdFigure.CCForm_Shape;
                break;
        }
