...
$(function () {
    ...
    if (fromWorkOpt == 2) {
        var PrintDocHtml = "<input type=button name='PrintDoc' value='領収書を印刷する' enable=true onclick='printDoc()' />";
...
function GenerFrm() {
    ...
    if (data.indexOf('err@') == 0) {
        alert('フォームの読み込み中にエラーが発生しました。コンソールを確認するか、管理者に報告してください。');
        alert(data);
        console.log(data);
        return;
    }
    ...
    try {
        ...
        alert(" frmDataデータをJSONに変換できませんでした：" + data);
        console.log(data);
        return;
    }
   ...
   //加载开发者表单的内容
    ...
    if (htmlContent == "" || htmlContent == null ) {
        alert("開発者が設計したフォームの内容が不足しています。管理者に連絡してください");
        return;
    }
