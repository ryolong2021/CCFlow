...
function InitMapAttrOfCtrlFool(mapAttr) {
    ...
    if (mapAttr.MyDataType == "1") {
        if (mapAttr.UIContralType == 6) {
            return "<div style='text-align:left;padding-left:10px' id='athModel_" + mapAttr.KeyOfEn + "'><label>クリックしてください[" + mapAttr.Name + "]アップロードを実行します</label></div>";
        }
        ...
        if (mapAttr.UIContralType == 4) {
            ...
            eleHtml += "<input type='button' name='select' value='選択' />";
        ...
        if (mapAttr.UIContralType == 101) {
            ...
            eleHtml += "&nbsp;&nbsp;<span class='score-tips' style='vertical-align: middle;color:#ff6600;font: 12px/1.5 tahoma,arial,\"Hiragino Sans GB\",ソンティ,sans-serif;'><strong>" + num + "  分</strong></span>";
            eleHtml += "</span></div>";
            return eleHtml;
        }
        //工作进度图
...
function GenerLabel(attr) {
    ...
    var tdUp = "<a href=\"javascript:Up('" + attr.MyPK + "','1');\" class='easyui-linkbutton l-btn l-btn-plain' data-options='iconCls:icon-left,plain:true'  alt='左のシーケンスに移動' ><span class='l-btn-left'><span class='l-btn-text'><span class='l-btn-empty icon-left'>&nbsp;</span></span></span></a>";
    var tdDown = "<a href=\"javascript:Down('" + attr.MyPK + "','1');\" class='easyui-linkbutton l-btn l-btn-plain' data-options='iconCls:icon-right,plain:true' alt='右のシーケンスに移動' ><span class='l-btn-left'><span class='l-btn-text'><span class='l-btn-empty icon-right'>&nbsp;</span></span></span></a>";
