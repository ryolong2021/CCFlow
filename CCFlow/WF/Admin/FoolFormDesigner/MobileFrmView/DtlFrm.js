...
function Dtl_InitPage(DtlEditType, dtlNo, OID) {
    ...
    if (DtlEditType == 2) {
        //增加新增按钮
        $('<button type="button" class="mui-btn mui-btn-primary mui-btn-block" style="width: 95%;margin: 15px 10px; height: 46px; padding: 0px;" onclick="SaveDtl(' + OID + ',\'' + dtlNo + '\',1,\'' + sys_MapDtl.MobileShowField+'\')">追加することを確認する</button>').appendTo('#btn');
    }
    viewApi.go("#frmDtl");
}
...
function SaveDtl(OID, dtlNo,type,field) {
    ...
    if (formCheckResult == false) {
        mui.toast("保存に失敗しました。必須フィールドに入力してください。");
        return false;
    }
