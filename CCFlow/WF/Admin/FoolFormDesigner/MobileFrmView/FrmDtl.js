...
function Load_DtlInit() {
    ...
    if(dbDtl.length == 0){
        ...
        _Html += " <div class='mui-indexed-list-empty-alert'>データなし</div>";
        _Html += "</div>";
    }
    ...
    for (var j = 0; j < dbDtl.length; j++) {
        ...
        _Html += "  <li class='mui-table-view-divider'>シリアルナンバー:" + dtl_Idx;
        if (sys_MapDtl.IsDelete == "1") {
            _Html += "   <div class='dtl_deleterow' id='" + dbDtl[j].OID + "'>削除</div>";
        }
        _Html += "	</li>";
        //循环分组
...
//                    continue;
    ...
    if (Form_ReadOnly == false && sys_MapDtl.IsInsert == "1") {
        ...
        _Html += "    <a><b class='dtl_addpanel_b'>+</b> 増加する" + sys_MapDtl.Name + "詳細</a>";
        _Html += " </div>";
        _Html += "</li>";
        _Html += "</ul>";
    }
...
function Load_DtlForm(dbDtl) {
    ...
    _Html += "<ul class='mui-table-view'>";
    _Html += "  <li class='mui-table-view-divider'>シリアルナンバー:" + index;
    _Html += "	</li>";
    //循环分组
    for (var g = 0; g < gfs.length; g++) {
        //收个分组标签不进行添加
...
function Dtl_DeleteByOID(oid) {
    var btnArray = ['いいえ', 'はい'];
    mui.confirm('選択したレコードを削除してもよろしいですか？', '提示', btnArray, function (e) {
        if (e.index == 1) {
            var dtl_No = $("#HD_CurDtl_No").val();
...
function Load_ChildDtlForm() {
    ...
    for (var j = 0; j < dbDtl.length; j++) {
        ...
        _Html += "  <li class='mui-table-view-divider'>シリアルナンバー:" + dtl_Idx;
        if (sys_MapDtl.IsDelete == "1") {
            _Html += "   <div class='dtlchild_deleterow' id='" + dbDtl[j].OID + "'>削除</div>";
        }
        _Html += "	</li>";
        //循环分组
        for (var g = 0; g < gfs.length; g++) {
...
function DtlChild_SaveData(CallBack) {
    ...
    $("#form_ChildDtl").ajaxSubmit({
        ...
        error: function (XmlHttpRequest, textStatus, errorThrown) {
            mui.toast("保存に失敗しました。フォームを確認してください。");
        }
    });
}
...
function Dtl_PopBranchesAndLeaf(mapExt, mapAttr, OID) {
    ...
    if (mapAttr.UIIsEnable != 0 && mapAttr.UIVisible != 0) {
        ...
        aLink.append($('<p style="margin-right:30px;margin-top:10px;min-width:160px;text-align:right;">選んでください</p>'));
