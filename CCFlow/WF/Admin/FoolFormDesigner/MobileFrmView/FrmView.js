...
function GenerFrm() {
    ...
    try {
        ...
        mui.alert("GenerFrmはJSONの変換に失敗しました：" + jsonStr);
        return;
    }
    //获取没有解析的外部数据源
    ...
    //设置标题.
    document.getElementById("title").innerHTML = "フォームプレビュー";
...
function getData(data, url, dataParam) {
    ...
    if (data.IsSuccess != true) {
        alert('パラメータを返すことができませんでした、ErrMsg：' + data.ErrMsg + ";Msg:" + data.Msg + ";url:" + url);
    }
    return data;
}
...
function figure_Template_Dtl(frmDtl) {
    ...
    if (!pageData.IsReadonly) {
        ...
        js += "   GenerPageKVs(); //kvsを生成するために呼び出す";
