...
function GenerFormElement() {
    ...
    AjaxMobileService({
        ...
        if (pushData.OfficeFile) {
            _html += "<li data-role=\"list-divider\">公式文書</li>";
            if (pushData.OfficeFile.FileExit == "false") {
                _html += "<li data-icon=\"false\">ファイルが存在しません</li>";
            } else {
                _html += "<li data-icon=\"false\"><a href='" + pushData.OfficeFile.FileWebPath + "' target=\"_blank\"><h2 style=\"color:#096BC1;font-size: 0.8em;\">公式文書をダウンロードする</h2></a></li>";
            }
        }
        //展显
        $(_html).appendTo('#Controls_view');
        ...
        for (var i = 0, j = groupFields.length; i < j; i++) {
            ...
            if (groupField.MapAths && groupField.MapAths.length > 0) {
                ...
                for (var iAth = 0; iAth < groupField.MapAths.length; iAth++) {
                    ...
                    _html += "        <span id='" + groupField.MapAths[iAth].MyPK + "_Count' class=\"athment_filecount\">（" + groupField.MapAths[iAth].AthMentDBs + "）個</span>";
                    _html += "     </h2>";
                    _html += "     <p>クリックして詳細を表示</p>";
                    _html += "   </a>";
                    _html += "</li>";
                }
            }
            ...
            if (groupField.MapDtls && groupField.MapDtls.length > 0) {
                ...
                for (var iDtl = 0; iDtl < groupField.MapDtls.length; iDtl++) {
                    ...
                    _html += "        <span id='" + groupField.MapDtls[iDtl].No + "_Count' class=\"athment_filecount\">（" + groupField.MapDtls[iDtl].Dtl_DBCount + "）記録</span>";
                    _html += "     </h2>";
                    _html += "     <p>クリックして詳細を表示</p>";
                    _html += "   </a>";
                    _html += "</li>";
        ...
        success: function (scorp) {
            ...
            if (pushData.Msg) {
                ...
                for (var i = 0; i < pushData.WorkCheck.tracks.length; i++) {
                    ...
                    _Html += "<p>署名：<span>" + signHtml + "</span></p>";
                    _Html += "<p>日付：<span>" + rdt + "</span></p>";
                    _Html += "</li>";
                }
                //是否添加审核意见框
                if (pushData.WorkCheck.WCState == "Enable") {
                    ...
                    _Html += "<li>";
                    _Html += "<label for=\"WorkCheck_Remark\" style=\"color:#69ca02;font-size: 0.95em;\">レビューコメントを記入してください:</label>";
            ...
            //如果没有信息则提示无信息
            if (_Html == "") {
                _Html = "<li><button class='ui-btn ui-state-disabled ui-icon-alert ui-btn-icon-top'>レコードが空です</button></li>";
            }
            return _Html;
        }
    });
...
function WorkCheck_Submit() {
    ...
    if (WorkCheck_Enable == true) {
        ...
        if (rsVal == undefined || rsVal == "") {
            MessageShow("レビューコメントを記入してください。", true);
            return false;
        }
        //执行发送
        var args = new RequestArgs();
        AjaxMobileService({ action: "saveworkcheck",
            ...
            if (scorp != "true") {
                MessageShow("レビューコメントの保存に失敗しました" + scorp, true);
                return false;
            }
        }, this);
    }
    ...
    if (typeof GetMapLocationCoords != "undefined") {
        ...
        GetMapLocationCoords(function (result) {
            if (result.latitude) {
                        ...
                        $.ajax({
                            ...
                            success: function (scope) {
                                if (scope == "true") {
                                    ...
                                    MessageShow("保存に失敗しました" + scope, true);
                                }
                            }
                        });
                ...
                MessageShow("場所の取得に失敗しました" + JSON.stringify(result), true);
            }
        });
    } else {
        MessageShow("Gaodeマップコントロールの呼び出しに失敗しました。DingTalkで開いてください。", true);
    }
}
...
function DeleteRecord(mediaId) {
    if (confirm("選択した音声を削除してもよろしいですか？")) {
        var args = new RequestArgs();
        $.ajax({
            type: "GET",
            url: "common/DingDingWebApi.ashx",
