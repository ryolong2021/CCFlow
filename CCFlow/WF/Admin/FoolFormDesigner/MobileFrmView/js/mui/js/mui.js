					...
					if (e.canBack) { //webview history back
						...
						if (wobj.id === plus.runtime.appid) { //首页
							...
							if (!$.__back__first) {
								...
								mui.toast('もう一度押してアプリケーションを終了します。');
								setTimeout(function() {
									$.__back__first = null;
								}, 2000);
							} else {
    ...
    var PullRefresh = {
        init: function(element, options) {
            this._super(element, $.extend(true, {
                ...
                down: {
                    height: 50,
                    contentinit: 'ドロップダウンはリフレッシュできます',
                    contentdown: 'ドロップダウンはリフレッシュできます',
                    contentover: 'リリース直後に更新',
                    contentrefresh: '更新中..'
                },
                up: {
                    height: 50,
...
(function($, window, document, name) {
	...
	var calPosition = function(popover, anchor, isActionSheet) {
		...
		setStyle(popover, 'block', pTop-44, pLeft);
	};
...
(function($, window, document) {
    ...
    var createAlert = function(message, title, btnValue, callback, type) {
        ...
        if (!$.os.plus || type === 'div') {
            return createPopup(createInner(message, title || 'ヒント') + createButtons([btnValue || '確認']), callback);
        }
        return plus.nativeUI.alert(message, callback, title || 'ヒント', btnValue || '確認');
    };
    var createConfirm = function(message, title, btnArray, callback, type) {
        ...
        if (!$.os.plus || type === 'div') {
            return createPopup(createInner(message, title || 'ヒント') + createButtons(btnArray || ['キャンセル', '確認']), callback);
        }
        return plus.nativeUI.confirm(message, callback, title, btnArray || ['キャンセル', '確認']);
    };
    var createPrompt = function(message, placeholder, title, btnArray, callback, type) {
        ...
        if (!$.os.plus || type === 'div') {
            return createPopup(createInner(message, title || 'ヒント', createInput(placeholder)) + createButtons(btnArray || ['キャンセル', '確認']), callback);
        }
        return plus.nativeUI.prompt(message, callback, title || 'ヒント', placeholder, btnArray || ['キャンセル', '確認']);
    };
...
(function($, window) {
    ...
    var Transparent = function(element, options) {
        ...
        if (!this.scrollByElem) {
            throw new Error("スクロールを待ち受けする要素は存在しません。");
        }
        ...
        if (color.length) {
            ...
            throw new Error("元素の背景色はRGBBAでなければなりません。");
        }
    };
...
(function($) {
    ...
    var Numbox = $.Numbox = $.Class.extend({
        ...
        init: function(holder, options) {
            var self = this;
            if (!holder) {
                throw "numboxを作成するときはコンテナの要素が足りません。";
            }
            self.holder = holder;
            options = options || {};
