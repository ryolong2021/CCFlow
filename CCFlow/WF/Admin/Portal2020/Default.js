...
function EventListener_TabCloseed() {
    ...
    var tabs = $('#tabs').tabs('tabs');
    if (tabs.length == 0) {
        addTab("welcome", "トップページ", "Welcome.htm", "");
    }
}
...
function LoginOut() {
    ...
    if (confirm("終了することを確定しますか？") == false)
        return;
...
function newFlow() {
    ...
    OpenEasyUiDialog(url, dgId, '新規フロー', 650, 350, 'icon-new', true, function () {
        ...
        if (newFlowInfo.FlowName == null || newFlowInfo.FlowName.length == 0
            ...
            alert('情報の記入が不完全です：' + newFlowInfo.FlowName + newFlowInfo.FlowSort);
        ...
        if (newFlowInfo.FlowMark != "") {
            ...
            if (flows.length > 0) {
                alert('該当のフローマーク[' + newFlowInfo.FlowMark + 'がシステムに既に存在します');
                return false;
            }
        }

        var html = $("#ShowMsg").html();
        $("#ShowMsg").html(html + " ccbpm フローを作成しています。しばらくお待ちください…");
...
function newFlowSort(isSub) {
    ...
    var propName = (isSub ? 'サブレベル' : '同じレベル') + 'フローの種類';
...
function editFlowSort() {
    ...
    var val = prompt("カテゴリ名を入力してください。", currSort.text);
    if (val == null || val == '')
        return;
...
function deleteFlowSort() {
    ...
    if (window.confirm("本当に名前は“" + currSort.text + "”のフロー種類を削除しますか？") == false)
        return;
    var handler = new HttpHandler("BP.WF.HttpHandler.WF_Admin_CCBPMDesigner");
...
function viewFlowSort() {
    ...
    OpenEasyUiDialog(url, dgId, 'フローツリーノードのプロパティ', 420, 300, 'icon-flow', false, function () {
...
function ImpFlowBySort() {
    ...
    addTab(fk_flowSort + "PO", "フローテンプレートをインポートする", url);
}
...
function ExpFlow() {
    ...
    addTab(fk_flow + "PO", "フローのテンプレートをエクスポートする", url);
}
...
function DeleteFlow() {
    ...
    if (window.confirm("本当に名前は“" + currFlow.text + "”のフローを削除しますか？") == false)
        return;
...
function FlowProperty() {
    ...
    addTab(currFlow + "PO", "フローのプロパティ" + fk_flow, url);
    //WinOpen(url);
}
...
function newCCFormSort(isSub) {
    ...
    var propName = (isSub ? 'サブレベル' : '同じレベル') + 'フォームの種類';
    var val = window.prompt(propName, "私のリストアップ");
...
function EditCCFormSort() {
    ...
    var val = prompt("フォームカテゴリ名を入力してください。", currCCFormSort.text);
    if (val == null || val == '')
        return;
...
function DeleteCCFormSort() {
    ...
    if (window.confirm("本当に名前は“" + currFormSort.text + "”のカテゴリを削除しますか？") == false)
        return;
...
function newFrm(frmType) {
    ...
    //如果右侧有打开该表单，则关闭
    var currTab = $('#tabs').tabs('getTab', "新規作成フォーム");
    if (currTab) {
        $('#tabs').tabs('close', "新規作成フォーム");
    }
    addTab("NewFrm", "新規作成フォーム", url);
}
...
function CCForm_Attr() {
    ...
    OpenEasyUiDialog(url, "CCForm_Attr", 'フォーム属性', 900, 560, "icon-window");
}
//单据属性
...
function Bill_Open() {
    ...
    if (en.EntityType == 0) {
        alert('スタンドアロンフォームは暫くリストを開くことをサポートしていません…');
        return;
    }
    ...
    OpenEasyUiDialog(url, "CCForm_Attr", '私のレシート', 1100, 560, "icon-window");
}
//设计自由表单
...
function CopyFrm() {
    ...
    var frmID = window.prompt('新しいフォームID', node.id);
    var frmName = window.prompt('新しいフォーム名称', node.text);
    if (frmID == null || frmID == "") {
        alert("フォームIDを空にすることはできません");
        return;
    }
    ...
    //设计表单.
    addTab("DesignerFrm" + frmID, "フォームデザイン-" + frmName, "../CCFormDesigner/GoToFrmDesigner.htm?FK_MapData=" + frmID);
}
