...
function editFlowSort() {
    ...
    var val = prompt("種別名を入力してください。", currSort.text);
    if (val == null || val == '')
        return;
...
function deleteFlowSort() {
    ...
    if (window.confirm("“" + currSort.text + "”のフロー種別を削除しますか？") == false)
        return;
    var handler = new HttpHandler("BP.WF.HttpHandler.WF_Admin_CCBPMDesigner");
...
function viewFlowSort() {
    ...
    OpenEasyUiDialog(url, dgId, 'フローツリーノード属性', 420, 300, 'icon-flow', false, function () {
...
function ImpFlow() {
    ...
    addTab(fk_flow + "PO", "フローテンプレートをインポート", url);
}
...
function ImpFlowBySort() {
    ...
    addTab(fk_flowSort + "PO", "フローテンプレートをインポート", url);
}
...
function ExpFlow() {
    ...
    addTab(fk_flow + "PO", "エクスポートフローのテンプレート", url);
}
...
function DeleteFlow() {
    ...
    if (window.confirm("本当に該当するファイル「" + currFlow.text + "」を削除しますか？") == false)
        return;
...
function DeleteCCFormSort() {
    ...
    if (window.confirm("本当に該当するカテゴリ「" + currFormSort.text + "」を削除しますか？") == false)
        return;
...
function CopyFrm() {
    ...
    var frmName = window.prompt('新しいフォーム名', node.text);
    if (frmID == null || frmID == "") {
        alert("フォームIDは空にできません");
        return;
    }
