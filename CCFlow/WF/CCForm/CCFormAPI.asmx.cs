...
namespace CCFlow.WF.CCForm
{
    ...
    public class CCFormAPI : System.Web.Services.WebService
    {
            ...
            ref DataSet ds, ref byte[] bytes)
        {
            ...
            if (b == false && 1 == 2)
                throw new Exception("err@[" + userNo + "]フローを表示する権限がありません、WorkID=" + workID);
        ...
        public string SaveBillDesingerTemplate_2019(string billNo, string frmID, byte[] bytes)
        {
            try
            {
                string filePath = SystemConfig.PathOfDataUser + "CyclostyleFile" + Path.DirectorySeparatorChar + "VSTO" + Path.DirectorySeparatorChar;
        ...
        {
            string filePath = SystemConfig.PathOfDataUser + "Siganture" + Path.DirectorySeparatorChar + userNo + ".jpg";
            if (System.IO.File.Exists(filePath) == false)
                filePath = SystemConfig.PathOfDataUser + "Siganture" + Path.DirectorySeparatorChar + "UnSiganture.jpg";
        ...
        public void SaveFrmAth(string userNo, string sid, string frmID, int nodeID, Int64 workID, byte[] byt, string guid)
        {
            ...
            if (aths.Count == 0)
            {
                BP.Sys.CCFormAPI.CreateOrSaveAthMulti(md.No, "Ath", "添付ファイル", 100, 100);
        ...
        public string WordDoc_GetWordFile(string flowNo, int nodeId, string userNo, long workId)
        {
            ...
            try
            {
                ...
                if (bytes == null)
                {
                    ...
                    strContext = "ccflow wordへようこそ";
                    doc.Paragraphs.Last.Range.Text = strContext;

                    string rootPath = BP.Sys.SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "worddoc" + Path.DirectorySeparatorChar;
