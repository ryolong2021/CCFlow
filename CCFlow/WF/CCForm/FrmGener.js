...
$(function () {
    ...
    if (fromWorkOpt == 2 && wfNode[0].PrintDocEnable == 1) {
        var PrintDocHtml = "<input type=button name='PrintDoc' value='レシートを印刷します' enable=true onclick='printDoc()' />";
...
function GenerFrm() {
    ...
    if (data.indexOf('err@') == 0) {
        alert('フォームの読み込み中にエラーが発生しました。コンソールのコンサートを確認したり、管理者にフィードバックしてください。');
        alert(data);
        console.log(data);
        return;
    }
    ...
    $.each(scoreDiv, function (idex, item) {
        ...
        $("#Star_" + KeyOfEn + " img").click(function () {
            ...
            $("#SP_" + KeyOfEn + " strong").html(index + "  ポイント");
...
function Save(scope) {
    ...
    if (CheckReg() == false) {
        alert("送信エラー：フィールドの枠が赤くなるのを確認してください。完全に記入されていますか？");
        return false;
    }
...
function figure_Template_HandWrite(HandWriteID, val) {
    ...
    OpenEasyUiDialogExt(url, 'サインボード', 400, 300, false);
}
//地图
