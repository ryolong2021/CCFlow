...
var uploadTools = {
    ...
    "initWithLayout":function(opt){
        ...
        btsStr += "<div>";
        btsStr += "<div class='selectFileBt'>ファイルを選択する</div>";
        btsStr += "</div>";
    ...
    "addFileList":function(fileList,opt){
        ...
        if(fileNumber+fileList.length>opt.maxFileNumber){
            alert("最大アップロード可能なファイル数："+opt.maxFileNumber+"個");
            return;
        }
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
                ...
                alert("このフォーマットファイルのアップロードはサポートされていません:"+fileList[i].name);
            }
        }
    ...
    "uploadFile":function(opt){
        ...
        var fileNumber = uploadTools.getFileNumber(opt);
        if(fileNumber<=0){
            alert("ファイルがありません。アップロードはサポートされていません。");
            return;
        }
