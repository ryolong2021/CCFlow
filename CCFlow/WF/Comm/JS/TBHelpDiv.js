...
function ShowHelpDiv(inputid, appPath, attrKey, enName, doType) {//显示层
    ...
    if (getobj(inputid + "mydiv").length <= 0) {//若尚未创建就建之
        ...
        //常用词汇
        if (doType == "wordhelp") {
            getobj(inputid + "li0")[0].innerHTML = "<a href='javascript:TBHelp(\"" + inputid + "\",\"" + appPath + "\",\"" + attrKey + "\",\"" + enName + "\")'>常用語彙</a>";
        }
        //弹出页面Pop返回值
        if (doType == "returnval") {
            getobj(inputid + "li0")[0].innerHTML = "<a href='javascript:HelpTBOpenPopWin(\"" + inputid + "\",\"" + appPath + "\",\"rev\")'>選択ページを開く</a>";
        }
        //内置POP返回值
        if (doType == "returnvalccformpopval") {
            getobj(inputid + "li0")[0].innerHTML = "<a href='javascript:HelpTBOpenPopWin(\"" + inputid + "\",\"" + attrKey + "\",\"" + enName + "\",\"ccform\")'>選択ページを開く</a>";
        }

        CreatHelpDiv(inputid + "myul", "li", inputid + "li1", "color:#f00;background:#fff"); //创建"clear"li
        getobj(inputid + "li1")[0].innerHTML = "<a style=\"color:Red;\" href='javascript:ClearHelpTBText(\"" + inputid + "\")'>クリアする</a>";
