...
function treeNodeManage(dowhat, nodeNo, callback, scope) {
    ...
    switch (dowhat) {
        case "sample": //新建同级节点
            ...
            var val = window.prompt('名前を入力してください', '新規ノード');
            if (val == null)
                return;
...
function CreateSampleNode() {
    ...
    if (node) {
        ...
        $.messager.alert('ヒント', 'ノードを選択してください！', 'info');
    }
}
//创建下级目录
...
function EditNode(type) {
    ...
    if (node) {
        ...
        if (enName == "" || enName == undefined) {
            $.messager.alert('ヒント', 'クラス名が見つかりませんでした！', 'info');
            return;
        }
        ...
        OpenEasyUiDialog(url, 'treeFrame', '編集', windowW, windowH, null, null, null, null, null, function () {
