...
function WinOpenAndBrowser(url) {
    ...
    if (Sys.firefox) {
        ...
        if (Sys.ie) {
            alert(encodeURI("このモードはIE 10以下のバージョンをサポートしていません。IEをアップグレードするか、またはchromeブラウザを使ってください。ありがとうございます。"));
        }
    }
}
...
function IsDigit(s) {
    ...
    if (!patrn.exec(s.value)) {
        alert("アルファベットまたは数字を入力してください。最初の文字はアルファベットでなければなりません。")
        s.value = "";
        return false;
    }
    return true;
...
function SigantureAct(ele, UserNo, FK_MapData, KeyOfEn, WorkID) {
    if (ele) {
        ...
        $.ajax({
            ...
            error: function () {
                //请求出错处理
                alert("デジタル署名でエラーが発生しました。フォームの属性設定を管理者に確認してください。");
            }
        });
    }
}
