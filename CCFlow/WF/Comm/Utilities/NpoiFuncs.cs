...
namespace CCFlow.WF.Comm.Utilities
{
    public static class NpoiFuncs
    {
            ...
            string creator = null, bool date = false, bool index = true, bool download = false)
        {
            ...
            //序
            if (index && dt.Columns.Contains("シリアルナンバー") == false)
            {
                dt.Columns.Add("シリアルナンバー", typeof(string)).ExtendedProperties.Add("width", 50);
                dt.Columns["シリアルナンバー"].SetOrdinal(0);

                for (int i = 0; i < dt.Rows.Count; i++)
                    dt.Rows[i]["シリアルナンバー"] = (i + 1).ToString();
            }
            //合计列
            ...
            try
            {
                using (FileStream fs = new FileStream(filename, FileMode.Create))
                {
                    ...
                    if (dateRow != null)
                    {
                        ...
                        cell.SetCellValue("日付：" + DateTime.Today.ToString("yyyy-MM-dd"));
                        cell.CellStyle = wb.CreateCellStyle();
                    ...
                    if (creatorRow != null)
                    {
                        ...
                        cell.SetCellValue("テーブル作成する人" + creator);
                        cell.CellStyle = wb.CreateCellStyle();
                        cell.CellStyle.Alignment = HorizontalAlignment.Right;
                        cell.CellStyle.VerticalAlignment = VerticalAlignment.Center;
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        foreach (DataColumn col in dt.Columns)
                        {
                            ...
                            {
                                case "Boolean":
                                    cell.SetCellValue(Equals(dr[col.ColumnName], true) ? "はい" : "いいえ");
                                    break;
                                case "DateTime":
        ...
        public static string FetchExcelDataIntoTable(string xlsfile, string fk_excelfile, string fk_frmAttachmentDB, Entity en)
        {
            ...
            if (ext != ".xls" && ext != ".xlsx")
                return "ファイルはMicrosoft Excelファイル（* .xlsまたは* .xlsx）である必要があります";
            ...
            using (FileStream fs = new FileStream(xlsfile, FileMode.Open, FileAccess.Read))
            {
                ...
                try
                {
                    ...
                    foreach (KeyValuePair<string, Dictionary<string, ExcelField>> def in dataCols)
                    {
                        ...
                        if (dataIsDtls[def.Key] == false)
                        {
                            ...
                            foreach (KeyValuePair<string, ExcelField> ef in def.Value)
                            {
                                ...
                                if (string.IsNullOrWhiteSpace(ef.Value.FK_ExcelSheet))
                                {
                                    ...
                                    {
                                        return string.Format("{0}[{1}] フィールド値の形式が正しくありません：" + ex.Message, ef.Value.Name,
                                                                    ef.Value.Field);
                                    }
                                    continue;
                                }
                                ...
                                if (esheet == null)
                                {
                                    return string.Format("ExcelSheet.No={0}のレコードは存在しません。", ef.Value.FK_ExcelSheet);
                                }

                                sheet = wb.GetSheet(esheet.Name);
                                if (sheet == null)
                                {
                                    return string.Format("{0}」という名前のシートはファイルに存在しません", esheet.Name);
                                }
                                ...
                                switch (ef.Value.ConfirmKind)
                                {
                                    case ConfirmKind.Cell:
                                        ...
                                        if (cell == null)
                                        {
                                            return string.Format("{0}[{1}] フィールドの対応するセル値が正しく設定されていません。このセルは取得されませんでした！",
                                                                        ef.Value.Name,
                                                                        ef.Value.Field);
                                        }
                                        break;
                                    default:
                                        if (string.IsNullOrWhiteSpace(ef.Value.ConfirmCellValue))
                                        {
                                            return string.Format("{0}[{1}] フィールドの対応するセル値は空にできません！", ef.Value.Name,
                                                                        ef.Value.Field);
                                        }
                                        ...
                                        if (cell == null)
                                        {
                                            return string.Format("{0}[{1}] フィールドのセル確認設定が正しくありません。このセルは取得されませんでした！",
                                                                        ef.Value.Name,
                                                                        ef.Value.Field);
                                        }
                                ...
                                {
                                    return string.Format("{0}[{1}] フィールドの値が正確ではありませんでした。確認してください！", ef.Value.Name,
                                                                ef.Value.Field);
                                }
                                ...
                                try
                                {
                                    switch (ef.Value.DataType)
                                    {
                                        ...
                                        case ExcelFieldDataType.ForeignKey:
                                            ...
                                            if (key == null)
                                                throw new Exception(string.Format("{1} ={2}のレコードが{0}外部キーテーブルに見つかりませんでした。",
                                                                                  ef.Value.UIBindKey,
                                        ...
                                        case ExcelFieldDataType.Enum:
                                            ...
                                            if (intKey == -1)
                                                throw new Exception(
                                                    string.Format("EnumKey = '{0}' AND Lab = '{1}'のレコードがSys_Enum列挙テーブルに見つかりません。",
                                ...
                                {
                                    return string.Format("{0}[{1}] フィールド値[{2}]の形式が正しくありません：" + ex.Message, ef.Value.Name,
                                                                ef.Value.Field,
                        ...
                        foreach (KeyValuePair<string, ExcelField> ef in def.Value)
                        {
                            ...
                            switch (ef.Value.ConfirmKind)
                            {
                                case ConfirmKind.Cell:
                                    ...
                                    if (cell == null)
                                    {
                                        return string.Format("{0}[{1}] フィールドの対応するセル値が正しく設定されていません。このセルは取得されませんでした！", ef.Value.Name,
                                                                    ef.Value.Field);
                                    }
                                    break;
                                default:
                                    ...
                                    if (cell == null)
                                    {
                                        return string.Format("{0}[{1}] フィールドのセル確認設定が正しくありません。このセルは取得されませんでした！", ef.Value.Name,
                                                                    ef.Value.Field);
                                    }
                        ...
                        {
                            return string.Format("スケジュール「{0}」のヘッダー行が誤って設定されており、同じ行または列ではありません。", esheet.Name);
                        }
                        ...
                        for (var i = begin; i <= (isHorizontal ? maxColumnIndex : sheet.LastRowNum); i++)
                        {
                            ...
                            foreach (KeyValuePair<string, KeyValuePair<int, int>> dh in dtlHeaders)
                            {
                                ...
                                if (value == "^ERROR$" || value == "^UNKNOWN$")
                                {
                                    return string.Format("「{0}」のセル{1}の値の形式が正しくありません。", esheet.Name,
                                ...
                                {
                                    return string.Format("{0}[{1}] フィールド値[{2}]の形式が正しくありません：" + ex.Message,
                ...
                {
                    return "データ抽出中に不明なエラーが発生しました：" + ex.Message;
                }
                //提取数据完成
            }
                    ...
                    foreach (DataRow dr in data.Value.Rows)
                    {
                        ...
                        try
                        {
                            if (1 != BP.DA.DBAccess.RunSQL(tsql))
                                throw new Exception(string.Format("向{0}テーブルにデータを挿入できませんでした：" + tsql));
                        }
                        catch (Exception ex)
                        {
                            return "データ抽出後のストレージエラー：" + ex.Message;
                        }
                    }
                }
            }
            ...
            {
                return "抽出されたデータをデータベースに書き込み中にエラーが発生しました：" + ex.Message;
            }
        ...
        public static string GetCellName(int columnIdx, int rowIdx)
        {
            ...
            if (col > maxs[2])
                throw new Exception("列番号が正しくなく、最大数を超えています");
