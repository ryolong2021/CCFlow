...
function InitMapAttrOfCtrlFool(flowData, mapAttr) {
    ...
    if (mapAttr.MyDataType == "1") {  //不是外键
        ...
        if (mapAttr.UIContralType == "6") {
            ...
            ath.MyPK = mypk;
            if (ath.RetrieveFromDBSources() == 0) {
                alert("添付ファイル属性が見つかりません、管理者に連絡してください");
                return;
            }
            ...
            if (dbs.length == 0) {
                if (athDesc.IsUpload == 1 || pageData.IsReadOnly == 0)
                    return "<div style='text-align:left;padding-left:10px' id='athModel_" + mapAttr.KeyOfEn + "'><label>クリックしてください[" + mapAttr.Name + "]アップロードを実行</label></div>";
                else
        ...
        if (mapAttr.UIContralType == "101") {
            ...
            eleHtml += "&nbsp;&nbsp;<span class='score-tips' id='SP_" + mapAttr.KeyOfEn + "' style='vertical-align: middle;color:#ff6600;font: 12px/1.5 tahoma,arial,\"Hiragino Sans GB\",ソンティ,sans-serif;'><strong>" + val + "  分</strong></span>";
    ...
    alert(mapAttr.Name + "タイプは判断されません。");
    return;
}
...
function OpenAth(url, title, keyOfEn, athMyPK, atPara, FK_MapData) {
    ...
    OpenBootStrapModal(url, "eudlgframe", title, flowData.Sys_MapData[0].FrmW, H, "icon-property", null, null, null, function () {
        ...
        var dbs = data["DBAths"];
        if (dbs.length == 0) {
            $("#athModel_" + keyOfEn).html("<label>クリックしてください[" + title + "]アップロードを実行</label>");
            return;
        }
...
function GetLab(flowData, attr) {
    ...
    if (contralType == 11) {
        ...
        if (count == 0) {
            alert("主な鍵は" + attr.MyPK + "名前は" + attr.Name + "画像制御情報がありません、管理者に連絡してください");
            return "";
        }
