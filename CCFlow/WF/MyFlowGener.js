...
function closeWindow() {
    if (window.opener) {
        ...
        } else if (window.opener.name && window.opener.name == "フローを実行する") {
            //测试运行流程，不进行刷新
        } else {
            //window.opener.location.href = window.opener.location.href;
        }
...
function DtlFrm(ensName, refPKVal, pkVal, frmType, InitPage, H) {
    ...
    if (typeof ((parent && parent.OpenBootStrapModal) || OpenBootStrapModal) === "function") {
        ((parent && parent.OpenBootStrapModal) || OpenBootStrapModal)(url, "editSubGrid", '編集', wWidth, wHeight, "icon-property", false, function () { }, null, function () {
            if (typeof InitPage === "function") {
                InitPage.call();
            } else {
                alert("フォームを手動で更新してください");
            }
        }, "editSubGridDiv");
    } else {
        window.open(url);
...
function DtlFrm(ensName, refPKVal, pkVal, frmType, InitPage, FK_MapData, FK_Node, FID, IsSave, H) {
    ...
    if (typeof ((parent && parent.OpenBootStrapModal) || OpenBootStrapModal) === "function") {
        OpenBootStrapModal(url, "editSubGrid", '編集', 1000, H, "icon-property", false, function () { }, null, function () {
            if (typeof InitPage === "function") {
                InitPage.call();
            } else {
                alert("フォームを手動で更新してください");
...
function figure_Template_HandWrite(HandWriteID, val) {
    ...
    OpenEasyUiDialogExt(url, '署名ボード', 400, 300, false);
}
//地图
function figure_Template_Map(MapID, UIIsEnable) {
    ...
    OpenBootStrapModal(url, "eudlgframe", "地図", 800, 500, null, false, function () { }, null, function () {
...
function ConfirmBtn(btn, workid) {
    ...
    btn.value = (btn.value == '確認' ? '取消確認' : '確認');
...
function CheckMinMaxLength() {
    ...
    if (editor) {
        ...
        if (wordslen > editor.MaxLen || wordslen < editor.MinLen) {
            msg += '@' + editor.BindFieldName + '、入力する値の長さは次の範囲内である必要があります:' + editor.MinLen + ', ' + editor.MaxLen + '間。現在入力は:' + wordslen;
        }
...
function Save(saveType) {
    ...
    if (data.indexOf('正常に保存されました') != 0 || data.indexOf('err@') == 0) {
...
function returnWorkWindowClose(data) {
    ...
    if (data.indexOf('err@') == 0 || data == "キャンセル") {//发送时发生错误
        $('#Message').html(data);
        $('#MessageDiv').modal().show();
        return;
    }
...
function Send(isHuiQian) {
    ...
    //必填项和正则表达式检查.
    if (checkBlanks() == false) {
        alert("必須フィールドにエラーがあり、境界線が赤になっているフィールドが完全に入力されていることを確認しますか？");
        return false;
    }

    if (checkReg() == false) {
        alert("送信エラー：境界線が赤くなったフィールドが完全に入力されているかどうかを確認してください。");
        return false;
    }
    ...
    if ($('[name=TransferCustom]').length > 0) {
        ...
        if (ens.length == 0) {
            alert("ノードでフローフローのカスタマイズが有効になっていますが、フローの方向が設定されていません。フローのカスタマイズボタンをクリックして設定してください");
            return false;
        }
    }
...
function OptSuc(msg) {
    ...
    msg = msg.replace("@検索<img src='/WF/Img/Btn/PrintWorkRpt.gif' >", '')

    $("#msgModalContent").html(msg.replace(/@/g, '<br/>').replace(/null/g, ''));
    var trackA = $('#msgModalContent a:contains("作業トラック")');
        ...
        $.each(ele, function (i, obj) {
            ...
            switch (eleM[0].tagName.toUpperCase()) {
                case "INPUT":
                    ...
                    break;
                case "SELECT":
                    if (eleM.val() == "" || eleM.children('option:checked').text() == "*選んでください") {
                        checkBlankResult = false;
                        eleM.addClass('errorInput');
                    } else {
                        eleM.removeClass('errorInput');
...
function GenerWorkNode() {
    ...
    try {
        ...
        //console.log(data);
        alert("GenerWorkNodeはJSONの変換に失敗しました。コンソールログを確認するか、管理者に連絡してください。");
        return;
    }
    ...
    if (flowData.AlertMsg.length != 0 && title.indexOf("承認をリクエストする") < 0) {
        $('#MessageDiv').modal().show();
    }
...
function initModal(modalType, toNode) {
    ...
    var html = '<div style="width:100%; height:auto;" class="modal fade" id="returnWorkModal" data-backdrop="static">' +
        ...
        + '<h4 class="modal-title" id="modalHeader">プロンプト情報</h4>'
        + '</div>'
    ...
    if (modalType != undefined) {
        switch (modalType) {
            ...
            case "HuiQian":
                ...
                if (toNode != null) {
                    $('#modalHeader').text("最初に署名され、次に送信されます。");
                    SetPageSize(80, 80);
                }
                else {
                    $('#modalHeader').text("カウンターサイン");
                    SetPageSize(80, 80);
                }
            ...
            case "sendAccepter": 
                ...
                break;
            case "DBTemplate":
                $('#modalHeader').text("履歴起動記録とテンプレート");
...
function DoStop(msg, flowNo, workid) {
    ...
    if (confirm('実行してもよろしいですか[' + msg + '] ?') == false)
        return;
...
function DoDelSubFlow(fk_flow, workid) {
    if (window.confirm('フローを終了してもよろしいですか？') == false)
        return;
...
function SaveSelfFrom() {
    ...
    if (frm == null) {
        alert('システムエラー、SelfFormIDが見つかりませんでした。');
    }
    //执行保存.
...
function ReqDDL(ddlID) {
    ...
    if (v == null) {
        alert('見つかりませんID=' + ddlID + 'ドロップダウンボックスコントロール。');
    }
    return v;
}
...
function ReqTB(tbID) {
    ...
    if (v == null) {
        alert('見つかりませんID=' + tbID + 'テキストボックスコントロール。');
    }
    return v;
}
// 获取CheckBox值
function ReqCB(cbID) {
    ...
    if (v == null) {
        alert('見つかりませんID=' + cbID + 'CheckBox（単一選択）コントロール。');
    }
    return v;
}
...
function ReqCBObj(cbID) {
    ...
    if (v == null) {
        alert('見つかりませんID=' + cbID + '無線制御（CheckBoxを取得）オブジェクト。');
    }
    return v;
}
// 设置值.
