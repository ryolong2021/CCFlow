...
function SaveSelfFrom() {
    ...
    if (frm == null) {
        alert('システムエラー、SelfFormのIDが見つかりませんでした。');
    }
   ...
   + '</div>';
    ...
    if (modalType != undefined) {
        switch (modalType) {
            ...
            //发送选择接收节点和接收人          
            case "sendAccepter":
                $('#modalHeader').text("ノードに送信：" + toNode.Name);
...
function SetAth(data) {
    ...
    if (atParamObj != undefined && atParamObj.IsViewWorkNode != 1 && divId != undefined && tbId != undefined) {
        if (atParamObj.AthShowModel == "1") {
            ...
            athDiv.html("添付ファイル<spanclass = 'badge'>" + data.length + "</span>個");
        }
    } else {
...
function ShowViewNodeAth(athLab, atParamObj, src) {
    ...
    athFormTitle.text("添付ファイルのアップロード：" + athLab);
    athModal.modal().show();
}
...
function GenerWorkNode() {
    ...
    try {
        ...
        alert("GenerWorkNodeはJSONの変換に失敗しました：" + jsonStr);
        return;
    }
