...
function EditEntityForm() {
    ...
    if (row) {
        ...
        $.messager.alert('ヒント', 'レコードを選んでから試してください！', 'info');
    }
}
//删除记录
function DeleteEntity() {
    var row = $('#ensGrid').datagrid('getSelected');
    if (row) {
        $.messager.confirm('確認', 'レコードを削除してもよろしいですか？', function (r) {
            if (r) {
                var fk_MapData = $("#EnNo").val();
                var params = {
                    method: 'deleteentity',
    ...
    queryData(params, function (js, scope) {
        ...
        if (js) {
            ...
            if (js.status && js.status == 500) {
                $("body").html("<b style='color:red;'>正しいパラメータ名を入力してください。<b>");
                return;
            }
            ...
            $.messager.confirm('確認ダイアログ', 'この表に表示列を設定していませんが、現在設定されていますか？', function (r) {
                if (r) {
                    SetEntity();
                }
            });
...
function queryData(param, callback, scope, method, showErrMsg) {
    ...
    $.ajax({
        ...
        error: function (XMLHttpRequest, errorThrown) {
            $("body").html("<b>ページへのアクセスエラー、パラメータエラー…<b>");
