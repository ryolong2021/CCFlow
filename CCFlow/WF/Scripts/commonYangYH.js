...
$(function () {
    ...
    var WeekCns = { 1: '月', 2: '火', 3: '水', 4: '木', 5: '金', 6: '土', 0: '日' };
...
Common.IsAdministrator = function () {
    if ($ != undefined && $.cookie != undefined) {//先判断是否引入了$.COOKIE
        return $.cookie()["MenuGroupNames"] != undefined && $.cookie()["MenuGroupNames"].indexOf('管理者') >= 0;
    } else {
        return false;
    }
}
...
Common.CustomPagePlug1 = function (operation) {
    ...
    PageData.ToPage = function (val) {
        ...
        switch (val.title) {
            case "次のページ":
                if (PageData.InitData.PageIndex == PageData.InitData.pageCount) {
                    alert("次のページがありません。もう一度選択してください。");
                    return;
                }
                else {
            ...
            case "前のページ":
                if (PageData.InitData.PageIndex == 1) {
                    alert("前のページがありません。もう一度選択してください。");
                    return;
                }
                else {
                    PageData.InitData.PageIndex--;
        ...
        if (PageData.InitData.PageIndex == 1) {
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=前のページ]").addClass("disabled");
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=前のページ]").attr("disabled", "disabled");
        } else if (PageData.InitData.PageIndex == PageData.InitData.PageCount) {
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=次のページ]").addClass("disabled");
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=次のページ]").attr("disabled", "disabled");
        }

        if (PageData.InitData.PageIndex != 1) {
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=前のページ]").removeClass("disabled");
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=前のページ]").removeAttr("disabled", "disabled");
        }
        if (PageData.InitData.PageIndex != PageData.InitData.PageCount) {
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=次のページ]").removeClass("disabled");
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=次のページ]").removeAttr("disabled", "disabled");
        }
    ...
    PageData.CountButton = function () {
        ...
        if (PageData.InitData.PageCount > 1) {
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=次のページ]").removeClass('disabled');
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=前のページ]").removeClass('disabled');
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=移動]").removeClass('disabled');
            $("#" + PageData.InitData.DivId + " .pagination-panel input").removeAttr("disabled", "disabled");
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=次のページ]").removeAttr("disabled", "disabled");
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=前のページ]").removeAttr("disabled", "disabled");
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=移動]").removeAttr("disabled", "disabled");
        }
        else {
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=次のページ]").addClass('disabled');
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=前のページ]").addClass('disabled');
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=移動]").addClass('disabled');
            $("#" + PageData.InitData.DivId + " .pagination-panel input").addClass('disabled');

            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=次のページ]").attr("disabled", "disabled");
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=前のページ]").attr("disabled", "disabled");
            $("#" + PageData.InitData.DivId + " .pagination-panel button[title=移動]").attr("disabled", "disabled");
                ...
                if (obje) {
                    ...
                    for (var i = 0; i < headers.length; i++) {
                        if ($(headers[i]).data != undefined && $(headers[i]).data().colname != undefined && obje[$(headers[i]).data().colname] != undefined) {
                            ...
                            html += ('<td><a style="text-decoration:underline;" href="#" onclick="updateReport(this)" class="btn btn-link btn_det">編集</a>' + '<a href="#" style="text-decoration:underline;" onclick="delReport(this)" class="btn btn-link btn_det">削除</a></td>');
                            html += "</tr>";
                        }
                            //自定义内容
    ...
    PageData.InitPlan = function (DivId) {
        $.ajax({
            ...
            success: function (Counts) {
                ...
                //上一页，下一页，转到按钮的事件绑定
                $("#" + PageData.InitData.DivId + " .pagination-panel button[title=前のページ]").bind('click', PageData.ToPage);
                $("#" + PageData.InitData.DivId + " .pagination-panel button[title=次のページ]").bind('click', PageData.ToPage);
                $("#" + PageData.InitData.DivId + " .pagination-panel button[title=移動]").bind('click', PageData.ToPage);
...
Common.CustomPagePlug = function (operation) {
    ...
    PageData.prototype.ToPage = function (val) {
        ...
        if (_this.InitData.PageIndex == 1) {
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=前のページ]").addClass("disabled");
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=前のページ]").attr("disabled", "disabled");
        } else if (_this.InitData.PageIndex == _this.InitData.PageCount) {
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=次のページ]").addClass("disabled");
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=次のページ]").attr("disabled", "disabled");
        }

        if (_this.InitData.PageIndex != 1) {
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=前のページ]").removeClass("disabled");
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=前のページ]").removeAttr("disabled", "disabled");
        }
        if (_this.InitData.PageIndex != _this.InitData.PageCount) {
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=次のページ]").removeClass("disabled");
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=次のページ]").removeAttr("disabled", "disabled");
        }
    ...
    PageData.prototype.CountButton = function () {
        ...
        if (_this.InitData.PageCount > 1) {
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=次のページ]").removeClass('disabled');
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=前のページ]").removeClass('disabled');
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=移動]").removeClass('disabled');
            $("#" + _this.InitData.DivId + " .pagination-panel input").removeAttr("disabled", "disabled");
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=次のページ]").removeAttr("disabled", "disabled");
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=前のページ]").removeAttr("disabled", "disabled");
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=移動]").removeAttr("disabled", "disabled");
        }
        else {
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=次のページ]").addClass('disabled');
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=前のページ]").addClass('disabled');
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=移動]").addClass('disabled');
            $("#" + _this.InitData.DivId + " .pagination-panel input").addClass('disabled');

            $("#" + _this.InitData.DivId + " .pagination-panel button[title=次のページ]").attr("disabled", "disabled");
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=前のページ]").attr("disabled", "disabled");
            $("#" + _this.InitData.DivId + " .pagination-panel button[title=移動]").attr("disabled", "disabled");
    ...
    PageData.prototype.InitPlan = function (DivId) {
        ...
        $.ajax({
            ...
            success: function (Counts) {
                ...
                //前のページ，次のページ，移動按钮的事件绑定
                $("#" + _this.InitData.DivId + " .pagination-panel button[title=前のページ]").unbind('click');
                $("#" + _this.InitData.DivId + " .pagination-panel button[title=前のページ]").bind('click', { thisPage: _this }, _this.ToPage);
                $("#" + _this.InitData.DivId + " .pagination-panel button[title=次のページ]").unbind('click');
                $("#" + _this.InitData.DivId + " .pagination-panel button[title=次のページ]").bind('click', { thisPage: _this }, _this.ToPage);
                $("#" + _this.InitData.DivId + " .pagination-panel button[title=移動]").unbind('click');
                $("#" + _this.InitData.DivId + " .pagination-panel button[title=移動]").bind('click', { thisPage: _this }, _this.ToPage);
                //绑定页数的选择
