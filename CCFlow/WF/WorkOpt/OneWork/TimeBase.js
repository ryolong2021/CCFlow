...
function InitPage() {
    ...
    for (var i = 0; i < tracks.length; i++) {
        ...
        if (at == ActionType.Return) {
            doc += "<p><span>返却ノード:</span><font color=green>" + track.NDToT + "</font><span>返却先:</span><font color=green>" + track.EmpToT + "</font></p>";
            doc += "<p><span>返却の意見は次の通りです</span>  </p>";
        }

        var isHaveCheck = false;
        if (at == ActionType.Forward || at == ActionType.FlowOver) {
            doc += "<p><span>到着ノード:</span><font color=green>" + track.NDToT + "</font><span>到着者:</span><font color=green>" + track.EmpToT + "</font> </p>";

            //判断是否隐藏
            if (Hide_IsOpenFrm == true) {
                doc += "<p><span><a href=\"javascript:OpenFrm('" + workid + "','" + track.NDFrom + "','" + fk_flow + "','" + fid+"','" + track.NDFrom + "')\">フォームを表示</a></span></p>";
            }
            ...
            for (var myIdx = 0; myIdx < tracks.length; myIdx++) {
                ...
                if (checkTrack.NDFrom == track.NDFrom && checkTrack.ActionType == ActionType.WorkCheck && checkTrack.EmpFrom == track.EmpFrom) {
                    ...
                    if (val.length == 2)
                        track.Msg = val[1];
                    doc += "<p><span>承認意見：</span><font color=green>" + track.Msg + "</font> </p>";
                }
            }
        }
        ...
        if (at == ActionType.TeampUp) {
            ...
            for (var myIdx = 0; myIdx < tracks.length; myIdx++) {
                ...
                if (checkTrack.NDFrom == track.NDFrom && checkTrack.ActionType == ActionType.WorkCheck && checkTrack.EmpFrom == track.EmpFrom) {
                    ...
                    if (val.length == 2)
                        track.Msg = val[1];
                    doc += "<p><span>連署意見：</span><font color=green>" + track.Msg.replace('null', '') + "</font> </p>";
                }
            }
        }
    ...
    if (gwls) {
        ...
        if (isHaveNoChecker == true) {
            ...
            var rowDay = "<tr>";
            rowDay += "<td colspan=3 class=TDDay ><span>承認待ち</span><b>" + gwf.NodeName + "</b></td>";
            rowDay += "</tr>";
            ...
            for (var i = 0; i < gwls.length; i++) {
                ...
                if (Hide_IsRead == true) {
                    ...
                    if (gwl.IsRead == "1")
                        doc += "<span><font color=green>閲覧済。</font></span>";
                    else
                        doc += "<span><font color=green>まだ閲覧していません。</font></span>";
                }
                doc += "<br>";
                doc += "<span>仕事到着日:</span>";
                doc += gwl.RDT;
