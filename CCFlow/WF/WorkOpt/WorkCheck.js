...
function InitPage() {
    ...
    $.each(tks, function () {
        ...
        nodeName = nodeName.replace('(連署)', '<br>(<font color=Gray>連署</font>)');
        html += nodeName;
        html += "</td>";
        //获取自定义常用短语
        ...
        if (this.IsDoc == "1" && isReadonly == false) {
            ...
            html += "<textarea id='WorkCheck_Doc' maxlength='2000' placeholder='内容を空にすることはできません。情報を入力してください、または常用語を使って選択してください。内容は2000文字を超えてはなりません。' rows='3' style='width:98%;border-style:solid;margin:5px; padding:5px;' onblur='SaveWorkCheck()' onkeydown='this.style.height=\"60px\";this.style.height=this.scrollHeight+\"px\";setIframeHeight();'>";
            html += msg;
            html += "</textarea>";
            ...
            html += "<option value='手続きを承諾する'>手続きを承諾する</option>";
            html += "<option value='同意します。上司の指示をお願いします。'>同意します。上司の指示をお願いします。</option>";
            html += "<option value='状況は事実で、上司の承認をお願いします。'>状況は事実で、上司の承認をお願いします。</option>";
            html += "<option value='同意しない'>同意しない</option>";
            html += "</select><font color=Gray>内容は2000文字を超えてはなりません。</font>";
            html += "</div>";
            html += "</td>";
            ...
            var returnMsg = this.ActionType == 2 ? "返却理由：" : "";
        ...
        if (SignType == null || SignType == undefined) {
            ...
            //签名，日期.
            html += "<tr>";
            html += "<td style='text-align:left;height:35px;line-height:35px;'><div style='float:left'><font color='Gray' >署名:</font>";
            ...
            html += "<font color='Gray'>日付:</font>" + rdt;
            html += "</div>";
            html += "</td>";
            ...
            for (var idx = 0; idx < SignType.length; idx++) {
                ...
                if (st.SignType == 0 || st.SignType == 2 || st.SignType == null) {
                    ...
                    html += "<td style='text-align:left;height:35px;line-height:35px;'><div style='float:left'><font color='Gray' >署名:</font>"
                                    ...
                                    + "<div style='float:right' ><font color='Gray' >日付:</font>" + (this.IsDoc ? "<span id='rdt'>" : "") + rdt + (this.IsDoc ? "</span>" : "") + "</div></td>";
                    html += "</tr>";
                    break;
                }
                ...
                if (st.SignType == 3) {
                    ...
                    html += "<a href='WorkCheck_CheckPass();'>署名してください</a>";

                    html += "</div>";

                    html += +" <div style='float:right' ><font color='Gray' >日付:</font>" + (this.IsDoc ? "<span id='rdt'>" : "") + rdt + (this.IsDoc ? "</span>" : "") + "</div></td>";
                    html += "</tr>";
                    break;
                }
            }
...
function DelWorkCheckAth(athPK) {
    ...
    if (confirm("選択したファイルを削除しますか？") == false)
        return;
...
function GetAthHtml(ath) {
    ...
    if (ath.CanDelete) {
        html += "<img alt='削除' align='middle' src='../Img/Btn/Delete.gif' onclick=\"DelWorkCheckAth('" + ath.MyPK + "')\" />&nbsp;&nbsp;";
    }
...
function AddUploadify(divid) {
    if ($("#file_upload").length == 0) {
        ...
        $('#file_upload').uploadify({
            ...
            'uploader':url,
            'auto': true,
            'fileTypeDesc': 'アップロードファイルを選択してください',
            'buttonText': '添付ファイルをアップロードする',
            //                    'hideButton': true,
            'width': 60,
            'fileTypeExts': '*.*',
            'height': 18,
...
function AddUploafFileHtm(divid) {
 if ($("#file_upload").length == 0) {
    var html ="<div id='s' style='float:right;margin-right:10px;margin-top:5px;' >"
        + "<label id='realBtn' class='btn btn-info' style=''><input type='file' name='file' id='file' style='display:inline;left:-9999px;position:absolute;' onchange='UploadChange();' ><span>ファイルアップロード</span></label>"
        + "</div>";
...
function WorkCheck_CheckPass() {
    ...
    var pass = window.prompt('署名パスワードを入力してください。初期化パスワードは123です。パスワードを変更できます。', "");
