...
function InitPage(data) {
    ...
    $("#deny_content").attr("maxlength", MAX_COMMENT_LEN);
}
...
function setDatePicker() {
    ...
    // 検索条件 -- 納品希望日の設定
    wfCommon.setdatepickerWithStartEnd(
        "#delivery_date_search_from",
        "#delivery_date_search_to",
        DATE_FORMAT_MOMENT_PATTERN_1,
        true,
        false,
        function () { },
...
function getSearchCondition() {
    ...
    // 手配区分の取得
    var arrangeClass = getSearchConditionForCheckbox("arrange_class_search");
    if (arrangeClass !== STRING_EMPTY) {
        arrCondition.push(arrangeClass);
    }

    // 処理区分の取得
    var processClass = getSearchConditionForCheckbox("process_class_search");
    if (processClass !== STRING_EMPTY) {
        arrCondition.push(processClass);
    }
...
function initConditions() {
    ...
    // 手配区分条件の初期化
    conditions.ObjArr["arrange_class_search"] = [];

    // 処理区分条件の初期化
    conditions.ObjArr["process_class_search"] = [];
}
...
function getDataWithServerAndCreatList() {
    ...
    // 手配業者依頼一覧検索条件
    handler.AddPara("ArrangeTraderReq", conditions.stringify());

    // 非同期データの取得
    handler.DoMethodSetString("GetArrangeTraderReqList", function (data) {
        //例外処理
    ...
    var columns = [
        {
            ...
            modify_type_class: 'type-data', // クラス種類  type-id / type-data / type-action
            formatter: function (res) {
                return dateFormatter(res.DeliveryDate);
            }
        }, {
            field: 'ArrangeKbn',
            title: '手配区分',
            ...
            formatter: function (res) {
                return pulldownFormatter(res.ProcessKbn, res.WorkID, res.AppCode);
            }
        }
    ];
 ...
 */
function pulldownFormatter(ProcessKbn, WorkID, AppCode) {

    // プルダウンリストタグの定義
    // const pulldownDiv = '<div  style="position:relative; width: 100px">';
    const pulldownDiv = '<div class="cp_ipselect cp_ipselect_arrange_trader cp_sl01">';
    ...
    // options
    var option_mitehai = STRING_EMPTY;
    var option_kakuninntyu = STRING_EMPTY;
    var option_tehaihunou = STRING_EMPTY;
    var option_kannryou = STRING_EMPTY;
    var option_kannryoutorikeshi = STRING_EMPTY;
    var option_cancel_paid = STRING_EMPTY;

    // プルダウンリストタグの作成
    switch (String(ProcessKbn)) {

        //未手配
        case STATE_MITEHAI:

            option_mitehai = getOptionStr(0, true);
            option_kakuninntyu = getOptionStr(3, false);
            option_tehaihunou = getOptionStr(2, false);
            option_kannryou = getOptionStr(1, false);
            option_kannryoutorikeshi = getOptionStr(4, false);

            break;

        //手配済み
        case STATE_TEHAIZIMI:
            
            option_kannryou = getOptionStr(1, true);
            option_cancel_paid = getOptionStr(5, false);

            break;

        //手配不能
        case STATE_TEHAIFUNO:

            isdisabled = 'disabled = "disabled"';
            option_tehaihunou = getOptionStr(2, true);
        ...
        case STATE_CONFIRMING:
            ...
            option_kakuninntyu = getOptionStr(3, true);
            option_tehaihunou = getOptionStr(2, false);
            option_kannryou = getOptionStr(1, false);
            option_kannryoutorikeshi = getOptionStr(4, false);
            option_cancel_paid = getOptionStr(5, false);
        ...
        case STATE_CANCEL:
            ...
            isdisabled = 'disabled = "disabled"';
            option_kannryoutorikeshi = getOptionStr(4, true);

            break;

        //手配済み後のキャンセル
        case STATE_CANCEL_PAID:

            isdisabled = 'disabled = "disabled"';
            option_cancel_paid = getOptionStr(5, true);
    ...
    return [
        pulldownDiv,
        '<select name="selectlist" id="selectlist' + WorkID + '" class="a-pulldown__select" ' + isdisabled + ' onchange="changeTehai(this, \'' + AppCode + '\', \'' + WorkID + '\', \'' + ProcessKbn +'\')">',
        option_mitehai,
        option_kakuninntyu,
        option_tehaihunou,
        option_kannryou,
        option_kannryoutorikeshi,
        option_cancel_paid,
        '</select>',
        // pulldownIconDiv,
        pulldownEndDiv
 ...
 */
function changeTehai(element, appCode, workId, befSelected) {

    // 手配状態の判断
    if (element.value === STATE_TEHAIZIMI || element.value === STATE_CONFIRMING ||
        element.value === STATE_CANCEL || element.value === STATE_CANCEL_PAID) {

        // 手配済み/確認中/キャンセル/キャンセル(有償)の場合、
        // 確認ダイアログを出てくる
        wfCommon.ShowDialog(
            DIALOG_CONFIRM,
            STRING_EMPTY,
            // wfCommon.MsgFormat(msg["W0001"], element.options[element.options.selectedIndex].label + "にします。," + appCode),
            wfCommon.MsgFormat(msg["W0003"], appCode),
            "はい",
            exeConfirm,
    ...
    } else if (element.value === STATE_TEHAIFUNO) {
    ...
    // $("#selectlist" + workId + " option[value='" + befSelected + "']").prop('selected', true);
...
function exeConfirm(selectVal, workId, appCode, chgBefSelectVal) {
    ...
    // 手配状態更新処理
    var ret = changeTehaiKbn(selectVal, workId, appCode, chgBefSelectVal);

    // 更新成功の場合、画面を再表示すること
    if (ret) {

        //メール送付を行う
        wfCommon.DoMailSend(MAIL_TYPE_CONDOLENCE, MAIL_STSTE_APPROVAL, workId, MAIL_TO_TEHAIGIYOSIYA);

        // 更新成功のダイアログを表示すること
        wfCommon.ShowDialog(
            DIALOG_ALERT,
            STRING_EMPTY,
            wfCommon.MsgFormat(msg["I0003"], appCode),
            "はい"
        );
 ...
 * @param {string} chgBefSelectVal 変更前の手配状態
 */
function changeTehaiKbn(selectVal, workId, appCode, chgBefSelectVal) {
    ...
    //手配状態を設定
    handler.AddPara("tehaiKbn", selectVal);
    //手配履歴(ユーザ)を設定
    handler.AddPara("tehaisyaNo", webUser.No);
    //手配不能コメントを設定
    handler.AddPara("tehaiFunouComment", $("#deny_content").val());
    //最初手配状態
    handler.AddPara("firsttehaiKbn", chgBefSelectVal);
    ...
    if (data === TEHAI_STATE) {
        ...
        wfCommon.ShowDialog(
            DIALOG_ALERT,
            STRING_EMPTY,
            wfCommon.MsgFormat(msg["E0001"], appCode),
            "はい"
        );
        ret = false;
    } else {
 ...
 * @param {object} data サーバーから取得のデータ
 **/
function getArrangeStsList(data) {

    //区分マスタ格納
    dtKbn = data['KBN'];

    // 手配状態の取得
    arrangeStsList = dtKbn["TEHAIKBN"];
}
...
function getOptionStr(optionKey, isSelected) {
    ...
    retVal += '>' + arrangeStsList.find(obj => obj.KBNVALUE == optionKey).KBNNAME + '</option >';
...
function checkRelation() {
    ...
    // 納品希望日From、Toの両方に入力されている場合
    // Toの日付が、Fromより過去、Fromの日付が、Toより未来であった場合
    if (checkEndTime($("#delivery_date_search_from").val(), $("#delivery_date_search_to").val())) {
        // Toの日付が、Fromより過去、Fromの日付が、Toより未来であった場合
        // 有効な日付を入力してください。
        wfCommon.ShowDialog(
            DIALOG_ALERT,
            STRING_EMPTY,
            wfCommon.MsgFormat(msg["E0003"], $("#delivery_date_search_title").text()),
            "はい"
        );
