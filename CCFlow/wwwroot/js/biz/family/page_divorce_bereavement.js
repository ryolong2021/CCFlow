...
function setSummry() {
    ...
    // 更新日	最終更新日時
    summry["content"].push({ "value": moment(wfCommon.getServerDate()).format(DATE_FORMAT_MOMENT_PATTERN_1), "name": "更新日" });
    // 社員番号	不幸従業員社員番号
    summry["content"].push({ "value": dtCondolence["UNFORTUNATE_SHAINBANGO"], "name": "社員番号" });
    // 続柄	亡くなられた方との続柄区分
    summry["content"].push({ "value": $("#select-died-relationship").text(), "name": "続柄" });
    // 同居 / 別居	亡くなられた方の同居 / 別居区分
    summry["content"].push({ "value": $("input[name=radio-died-living]:checked").next().find("div").html(), "name": "同居/別居" });
    // 逝去日	亡くなられた方の逝去日
    summry["content"].push({ "value": $("#text-died-death_date").val(), "name": "逝去日" });
