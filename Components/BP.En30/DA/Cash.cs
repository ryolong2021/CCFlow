...
{	 
    ...
    public class Cash
    {
        ...
        public static string GetBillStr(string cfile, bool isCheckCash)
        {
            ...
            if (val == null)
            {
                ...
                if (cfile.Contains(":"))
                    ...
                    file = SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "CyclostyleFile" + Path.DirectorySeparatorChar + cfile;
                ...
                {
                    throw new Exception("@伝票テンプレートの読み取り中にエラーが発生しました。 cfile=" + cfile + " @Ex=" + ex.Message);
                }
        ...
        public static string[] GetBillParas_Gener(string cfile, Attrs attrs)
        {
            ...
            foreach (char c in chars)
            {
                if (c == '>')
                {
                    ...
                    if (attrs != null && real.Contains("\\") && real.Contains("ND") == false)
                    {
                        ...
                        if (findKey == null)
                        {
                            msg += "@パラメータ:<font color=red><b>[" + real + "]</b></font>スペルミスの可能性がある。";
                            continue;
                        }
            ...
            if (msg != "")
            {
                string s = "@ヘルプ情報：テンプレートをメモ帳で開き、赤いフォントを見つけます。山かっこ内の無効な文字を削除します（例:《|f0|fs20RDT.NYR|lang1033|kerning2》、変更後の例：《RDT.NYR》。 @一重引用符の代わりに二重引用符、バックスラッシュの代わりに縦棒。";
        ...
        {
            if (fk_emp == null )
                throw new Exception("fk_emp.  csh パラメータの1つが空です。");
        ...
        public static void SetSQL(string clName, BP.En.SQLCash csh)
        {
            if (clName == null || csh == null)
                throw new Exception("clName.  csh パラメータの1つが空です。");
            SQL_Cash[clName] = csh;
        }
...
#warning 设置个数为
        ...
        public static void SetEnsDataExt(string clName, BP.En.Entities obj)
        {
            if (clName == null || obj == null)
                throw new Exception("clName.  obj パラメータの1つが空です。");
        ...
        public static object GetObj(string key, Depositary where)
        {
            ...
            if (where == Depositary.None)
                throw new Exception("[" + key + "]をsession or application に入れてないので、見つかりません。");
#endif
        ...
        {
            if (key == null)
                throw new Exception("obj=" + obj.ToString() + ",主キー値として設定します。key");

            if (obj == null)
                throw new Exception("obj = nullを主キーに設定する必要があります。key=" + key);

#if DEBUG
            if (where == Depositary.None)
                throw new Exception("[" + key + "]を session or application に入れて設定します。");
#endif
