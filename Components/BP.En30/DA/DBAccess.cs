...
namespace BP.DA
{
    ...
    public class DBAccess
    {
        ...
        public static void SaveBytesToDB(byte[] bytes, string tableName, string tablePK, object pkVal, string saveToFileField)
        {
            if (BP.Sys.SystemConfig.AppCenterDBType == DBType.MSSQL)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    throw new Exception("@このフィールドがありません[" + tableName + "," + saveToFileField + "],システムによって自動的に修復される場合があります." + ex.Message);
                }
                return;
            }
            ...
            if (BP.Sys.SystemConfig.AppCenterDBType == DBType.Oracle)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    throw new Exception("@このフィールドがない場合、システムによって自動的に修復される可能性があります." + ex.Message);
                }
                return;
            }
            ...
            if (BP.Sys.SystemConfig.AppCenterDBType == DBType.PostgreSQL)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    throw new Exception("@NpgsqlDbTypeこのフィールドがありません[" + tableName + "," + saveToFileField + "]、システムによって自動的に修復される場合があります." + ex.Message);
        ...
        public static byte[] GetByteFromDB(string tableName, string tablePK, string pkVal, string fileSaveField)
        {
            ...
            //最后仍然没有找到.
            throw new Exception("@ファイルを取得し、データベースでデータタイプを判断されません。");
        }
        ...
        public static IDbConnection GetAppCenterDBConn
        {
            get
            {
                ...
                switch (AppCenterDBType)
                {
                    ...
                    case DBType.Access:
                    default:
                        throw new Exception("err@GetAppCenterDBConn不明なデータベース接続タイプが見つかりました！");
                }
            }
        }
        ...
        public static IDbDataAdapter GetAppCenterDBAdapter
        {
            get
            {
                switch (AppCenterDBType)
                {
                    ...
                    case DBType.Access:
                    default:
                        throw new Exception("err@GetAppCenterDBAdapter不明なデータベース接続タイプが見つかりました！");
                }
            }
        }
        public static IDbCommand GetAppCenterDBCommand
        {
            get
            {
                switch (AppCenterDBType)
                {
                    ...
                    case DBType.Access:
                    default:
                        throw new Exception("err@GetAppCenterDBCommand不明なデータベース接続タイプが見つかりました！");
                }
            }
        }
        ...
        public static int RunSQL(string sql, SqlConnection conn, CommandType sqlType, string dsn)
        {
            ...
            catch (System.Exception ex)
            {
                step = "8";
                // lock_SQL_RunSQL = false;
                throw new Exception("RunSQL2 step=" + step + ex.Message + " 接続時間を設定する=" + conn.ConnectionTimeout);
            }
            finally
            {
                step = "9";
        ...
        public static void DropTablePK(string table)
        {
            ...
            switch (SystemConfig.AppCenterDBType)
            {
                ...
                case DBType.MySQL:
                    ...
                    break;
                default:
                    throw new Exception("err@DropTablePKサポートされていないデータベースタイプ." + SystemConfig.AppCenterDBType);
                    //break;
            }
            BP.DA.DBAccess.RunSQL(sql);
        }
        ...
        public static int RunSQL(string sql, Paras paras)
        {
            ...
            try
            {
                switch (AppCenterDBType)
                {
                    ...
                    //    break;
                    default:
                        throw new Exception("err@RunSQL不明なデータベース接続タイプが見つかりました！");
                }
                lockRunSQL = false;
                return result;
            }
            catch (Exception ex)
            {
                ...
                throw new Exception("SQLの実行エラー:" + ex.Message + " Paras(" + paras.Count + ")=" + msg + "<hr>" + mysql);
            }
        }
        ...
        private static int RunSQL_201902_PSQL(string sql, Paras paras)
        {
            ...
            catch (System.Exception ex)
            {
                ...
                string msg = "";
                if (paras.Count == 0)
                    msg = "SQL=" + sql + ",エラーメッセージ:" + ex.Message;
                else
                    msg = "SQL=" + paras.SQLNoPara + ",エラーメッセージ:" + ex.Message;
        ...
        private static int RunSQL_200705_Ora(string sql, Paras paras)
        {
            ...
            catch (System.Exception ex)
            {
                ...
                else
                {
                    ...
                    throw new Exception(ex.Message + "@実行可能なSQL:" + sql);
                }
            }
            finally
            {
        ...
        */
        ...
        private static DataTable RunSQLReturnTable_20191231_DM(string selectSQL, Paras paras)
        {
            ...
            {
                conn.Close();
                string msg = "@クエリを実行して、(RunSQLReturnTable_20191231_DM with paras)でエラーが出た。 sql=" + selectSQL + " @エラーメッセージ：" + ex.Message;
        ...
        private static DataTable RunSQLReturnTable_200705_Ora(string selectSQL, Paras paras)
        {
            ...
            {
                conn.Close();
                string msg = "@クエリを実行して、(RunSQLReturnTable_200705_Ora with paras)でエラーが出た。 sql=" + selectSQL + " @エラーメッセージ：" + ex.Message;
                msg += "@Para Num= " + paras.Count;
                foreach (Para pa in paras)
                {
                    msg += "@" + pa.ParaName + "=" + pa.val;
        ...
        private static DataTable RunSQLReturnTable_200705_SQL(string selectSQL)
        {
            ...
            catch (System.Exception ex)
            {
                ...
                string msg = "@クエリを実行して、(RunSQLReturnTable_200705_SQL)でエラーが出た。 sql=" + selectSQL + " @エラーメッセージ：" + msgErr;
                Log.DebugWriteError(msg);
                throw new Exception(msg);
            }
        }
        ...
        {
            if (DataType.IsNullOrEmpty(sql))
                throw new Exception("実行するSQL = null");
            ...
            try
            {
                ...
                switch (AppCenterDBType)
                {
                    ...
                    case DBType.MySQL:
                        ...
                        break;
                    default:
                        throw new Exception("err@RunSQLReturnTable不明なデータベース接続タイプが見つかりました！");
                }
                return dt;
            }
            catch (Exception ex)
        ...
        public static int RunSQLReturnValInt(string sql)
        {
            ...
            if (obj == null || obj == DBNull.Value)
                throw new Exception("@クエリするデータを取得できませんでした。SQLを確認してください:" + sql + " @問い合わせた詳細情報はログファイルに記録されていますので、お取り扱いください。");
        ...
        public static string RunSQLReturnString(string sql)
        {
            ...
            {
                throw new Exception("@RunSQLReturnStringの実行中にエラーが発生しました：" + ex.Message + sql);
            }
        }
        ...
        public static bool IsExits(string sql)
        {
            if (sql.ToUpper().Contains("SELECT") == false)
                throw new Exception("@不正なクエリステートメント" + sql);
        ...
        public static string GetTablePKName(string table)
        {
            ...
            switch (AppCenterDBType)
            {
                ...
                case DBType.PostgreSQL:
                    ...
                    break;
                default:
                    throw new Exception("@GetTablePKName未判断のデータベースタイプ.");
            }
            ...
            switch (dbType)
            {
                ...
                case DBType.Informix:
                    ...
                    if (dtaa.Rows.Count == 0)
                        throw new Exception("@テーブルが存在しません[" + tabelOrViewName + "]");
                ...
                case DBType.Access:
                    ...
                    if (dtw.Rows[0][0].ToString().Trim() == "5")
                        ...
                        return false;
                default:
                    throw new Exception("@判断が下されていない。");
            }
        ...
        public static bool IsExitsObject(DBUrl dburl, string obj)
        {
            ...
            switch (AppCenterDBType)
            {
                ...
                default:
                    throw new Exception("認識されないデータベース番号");
            }
        ...
        public static bool IsExitsTableCol(string table, string col)
        {
            ...
            switch (DBAccess.AppCenterDBType)
            {
                ...
                //    break;
                default:
                    throw new Exception("err@IsExitsTableCol未判断のデータベースタイプ.");
            }
        ...
        public static void LoadConfig(string cfgFile, string basePath)
        {
            if (!File.Exists(cfgFile))
                throw new Exception("設定ファイルが見つかりません==>[" + cfgFile + "]1");
            ...
            cfgFile = basePath + Path.DirectorySeparatorChar + "__$AppConfig.cfg";
