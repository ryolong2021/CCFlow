...
namespace BP.DA
{
    ...
    public class DBLoad
    {
        ...
        public static int ImportTableInto(DataTable impTb, string intoTb, string select, int clear)
        {
            ...
            {
                throw new Exception("ソーステーブルの形式が間違っています。確認してください！" + ex.Message + " :" + select);
            }
            ...
            else if (dbt == DBType.Oracle)
            {
                ...
                throw new Exception("データベース接続は取得されません！");
        ...
        private static int ImportTable(DataTable source, DataTable target, SqlDataAdapter sqlada)
        {
            ...
            try
            {
                ...
                source.Columns.Add("エラーメッセージ", typeof(string));
                source.Columns["エラーメッセージ"].MaxLength = 1000;
                ...
                while (i < source.Rows.Count)   //for( int i=0;i<;i++)
                {
                    ...
                    {
                        source.Rows[i]["エラーメッセージ"] = ex.Message;
                        i++;
                        continue;
                    }
                    count++; //已导入的记录数
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("データをインポートできませんでした！" + ex.Message);
            }
            return count;
        }
