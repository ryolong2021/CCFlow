...
namespace BP.DTS
{
    ...
    public class GenerPortalFromWS : DataIOEn
    {
        public GenerPortalFromWS()
        {
            ...
            this.Title = "WebサービスでPortalデータを更新する。";
        }
    ...
    public class AddEmpLeng : DataIOEn
    {
        ...
        {
            this.HisDoType = DoType.UnName;
            this.Title = "オペレーター番号の長さをアップグレードする";
	...
	abstract public class DataIOEn 
	{
        ...
        /// 标题
        /// </summary>
		public string Title="名前のないデータの同期";
		/// <summary>
		/// WHERE .
		/// </summary>
...
//		public string IncrementalDBSourceSQL;
		...
		public virtual void Do()
		{
			if ( this.HisDoType==DoType.UnName)
				throw new Exception("@同期の種類が指定されていません。基本情報（コンストラクタ）で同期の種類を設定してください。");
		...
		public void Inphase()
		{
				...
				foreach(FF ff in this.FFs)
				{
					switch(ff.DataType)
					{
						...
						case DataType.AppDouble:
							...
							break;
						default:
							throw new Exception("関係するタイプはありません。");
					}
				}
			...
			foreach(DataRow ToDR in ToDataTable.Rows)
			{
				...
				{
					throw new Exception("宛先データ異常エラー+テーブル名;キーワード"+this.ToTable+"キーワード"+ToDR[0].ToString());
				} 
			}
			#endregion			
