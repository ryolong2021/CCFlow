    ...
    public class RTFEngine
    {
        ...
        public string GetValueByKeyOfCheckNode(string[] strs)
        {
            foreach (Entity en in this.HisEns)
            {
                ...
                switch (strs.Length)
                {
                    case 1:
                    case 2:
                        throw new Exception("step1パラメータ設定エラー" + strs.ToString());
                    ...
                    case 4: // S.9001002.RDT.Year
                        switch (strs[3])
                        {
                            ...
                            default:
                                throw new Exception("step2パラメータ設定エラー" + strs);
                        }
                    default:
                        throw new Exception("step3パラメータ設定エラー" + strs);
                }
            }
            throw new Exception("step4パラメータ設定エラー" + strs);
        }
        ...
        public string GetValueByAtKey(string key)
        {
            foreach (Entity en in this.HisEns)
            {
                ...
                if (strs.Length == 2)
                {
                    ...
                    {
                        string path1 = BP.Sys.SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "ImgAth" + Path.DirectorySeparatorChar +"Data" + Path.DirectorySeparatorChar + strs[0].Trim() + "_" + en.PKVal + ".png";
                        //定义rtf中图片字符串.
                    ...
                    switch (strs[1].Trim())
                    {
                        ...
                        case "RMBDX":
                            return DA.DataType.ParseFloatToCash(float.Parse(val));
                        case "ImgAth":
                            string path1 = BP.Sys.SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "ImgAth" + Path.DirectorySeparatorChar + "Data" + Path.DirectorySeparatorChar + strs[0].Trim() + "_" + this.HisGEEntity.PKVal + ".png";
                            ...
                            return mypict.ToString();
                        case "Siganture":
                            string path = BP.Sys.SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "Siganture" + Path.DirectorySeparatorChar + val + ".jpg";
                            //定义rtf中图片字符串.
                            StringBuilder pict = new StringBuilder();
                            //获取要插入的图片
                            System.Drawing.Image img = System.Drawing.Image.FromFile(path);
                        ...
                        default:
                            throw new Exception("間違ったパラメータ設定、特別なメソッドの間違った値：" + key);
                    }
                }
            } // 实体循环。

            throw new Exception("パラメータ設定エラーGetValueByKey：" + key);
        }
        /// <summary>
        /// 获得所所有的审核人员信息.
        /// </summary>
        ...
        public string GetValueCheckWorks()
        {
           ...
           string sql = "SELECT FK_Emp, FK_Node FROM WF_GenerWorkerList WHERE IsPass!=1 AND WorkID=" + this.HisGEEntity.PKVal;
            ...
            foreach (DataRow dr in dtTrack.Rows)
            {
                ...
                msg += "審査員:" + empStrs + " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;日付:" + dr["RDT"].ToString();
        ...
        public string GetValueCheckWorkByKey(string key)
        {
            ...
            if (strs.Length == 3)
            {
                 ...
                 */
                if (dtTrack == null)
                    throw new Exception("@監査コンポーネントを取得するためのルールを設定したが、監査コンポーネントデータソースdtTrackに値を割り当てなかった。");
        ...
        public string GetValueByKey(string key)
        {
            ...
            if (this.HisGEEntity != null)
            {
                ...
                if (strs[1].Trim() == "Editor")
                {
                    ...
                    string htmlpath = BP.Sys.SystemConfig.PathOfDataUser + "Bill" + Path.DirectorySeparatorChar +"Temp" + Path.DirectorySeparatorChar +"EditorHtm.html";
                ...
                if (strs[1].Trim() == "ImgAth")
                {
                    string path1 = BP.Sys.SystemConfig.PathOfDataUser + "ImgAth" + Path.DirectorySeparatorChar +"Data" + Path.DirectorySeparatorChar + strs[0].Trim() + "_" + this.HisGEEntity.PKVal + ".png";
                    if (!File.Exists(path1))
                    {
                        ...
                        if (count == 1)
                        {
                            path1 = BP.Sys.SystemConfig.PathOfDataUser + "ImgAth" + Path.DirectorySeparatorChar +"Data" + Path.DirectorySeparatorChar + dbImgAth.FileName + ".png";
                            if (!File.Exists(path1))
                ...
                if (strs.Length == 2)
                {
                    ...
                    switch (strs[1].Trim())
                    {
                        ...
                        case "Siganture":
                            string path = BP.Sys.SystemConfig.PathOfDataUser + "Siganture" + Path.DirectorySeparatorChar + val + ".jpg";
                            //获取要插入的图片
            ...
            foreach (Entity en in this.HisEns)
            {
                ...
                /*说明就在这个字段内*/
                if (strs.Length == 1)
                    throw new Exception("パラメータ設定エラー、strs.length = 1。" + key);
                ...
                if (strs.Length == 3)
                {
                    ...
                    {
                        string path1 = SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "ImgAth" + Path.DirectorySeparatorChar + "Data" + Path.DirectorySeparatorChar + strs[1].Trim() + "_" + en.PKVal + ".png";
                        //定义rtf中图片字符串.
                        StringBuilder mypict = new StringBuilder();
                        //获取要插入的图片
                        System.Drawing.Image imgAth = System.Drawing.Image.FromFile(path1);
                    ...
                    switch (strs[2].Trim())
                    {
                        ...
                        case "RMBDX":
                            return DA.DataType.ParseFloatToCash(float.Parse(val));
                        case "ImgAth":
                            string path1 = SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "ImgAth" + Path.DirectorySeparatorChar + "Data" + Path.DirectorySeparatorChar + strs[0].Trim() + "_" + this.HisGEEntity.PKVal + ".png";
                            ...
                            return mypict.ToString();
                        case "Siganture":
                            string path = SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "Siganture" + Path.DirectorySeparatorChar + val + ".jpg";
                            //定义rtf中图片字符串.
                            StringBuilder pict = new StringBuilder();
                            //获取要插入的图片
                            System.Drawing.Image img = System.Drawing.Image.FromFile(path);
        ...
        public void MakeDoc(string templateRtfFile, string path, string file,bool isOpen, string billUrl=null)
        {
            ...
            if (this.HisEns.Count == 0)
                if (this.HisGEEntity == null)
                    throw new Exception("@レポートのデータソースを設定していません。");
            ...
            try
            {
                ...
                foreach (string para in paras)
                {
                    ...
                    { 
                        error += "@メインテーブルタグを置き換えてパラメータを取得する[" + para + "]エラーが発生しました。次のような状況でこのエラーが発生しました。1 Textで値を取ります。この属性は外キーではありません。2クラスにはこの属性がありません。3このフィールドは明細表フィールドですが、詳細表のマークがなくなっています。<br/>より詳細なメッセージロパティは外部キーではありません。<br>詳細情報：<br>" + ex.Message;
                                ...
                                switch (attr.MyDataType)
                                {
                                    ...
                                    case DataType.AppInt:
                                        ...
                                        if (attr.MyDataType == DataType.AppBoolean)
                                        {
                                            ...
                                            if (v == 1)
                                                rowData = rowData.Replace("<" + shortName + "." + attr.Key + "Text>", "はい");
                                            else
                                                rowData = rowData.Replace("<" + shortName + "." + attr.Key + "Text>", "いいえ");
                                        }
                                        else
                                        {
                                            if (attr.IsEnum)
            ...
            catch (Exception ex)
            {
                ...
                {  // 异常可能与单据的配置有关系。
                    try
                    {
                        this.CyclostyleFilePath = SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "CyclostyleFile" + Path.DirectorySeparatorChar + templateRtfFile;
                        str = Cash.GetBillStr(templateRtfFile, false);
                        msg = "@修理ラインRepairLineV2は正常に実行されましたが、修理ラインを再送するか、返品後に再度送ってください。この問題は解決できますか？";
                    }
                    catch (Exception ex1)
                    {
                        msg = "修復ラインの実行に失敗しました。RepairLineV2" + ex1.Message;
                    }
                }
                throw new Exception("ドキュメントの生成に失敗しました：帳票名[" + this.CyclostyleFilePath + "] 例外情報：" + ex.Message + " @請求書情報を自動的に修復する：" + msg);
            }
            if (isOpen)
