		...
		public string EnterDesc
		{
			get
			{
				if (this.UIContralType==UIContralType.TB)
				{
					...
					{
						return "このフィールドは読み取り専用です";
					}
					else
					{
						if (this.MyDataType==DataType.AppDate )
						{
							return "日付タイプを入力してください"+DataType.SysDataFormat;
						}
						else if (this.MyDataType==DataType.AppDateTime)
						{
							return "日付と時刻のタイプを入力してください"+DataType.SysDataTimeFormat;
						}
						else if (this.MyDataType==DataType.AppString)
						{ 
							return "必要な最小の長さを入力してください"+this.MinLength+"文字、最大長さ"+this.MaxLength+"キャラクター";
						}
			...
			get{
				switch (this.MyDataType)
				{
					...
					case   DataType.AppBoolean :
						...
						{
							throw new Exception("@セットアップ["+this.Key+"]デフォルト値は間違っています、["+this._defaultVal.ToString()+"]bookに変換できません、0/1を設定してください。");
						}					 
					...
					case 5: 					 
						...
						{
							throw new Exception("@セットアップ["+this.Key+"]デフォルト値は間違っています、["+_defaultVal.ToString()+"]doubleに変換できません。");
						}
					...
					case   DataType.AppDateTime :
						...
						break;
					default:
						throw new Exception("@bulider insert sql error: このデータ型がない、フィールド名:"+this.Desc+" 英語:"+this.Key);
				}
 ...
 @Key=MyFKField;Name=我的外键字段;Type=String;DefVal=01;AppType=FK;IsLine=false;BindKey=BP.Port.Depts
        ...
        public void Add(Attr attr)
        {
            if (attr.Field == null || attr.Field == "")
                throw new Exception("@プロパティ設定エラー：key ='" + attr.Key + "', "+attr.Desc+",の値を空に設定することはできません");
		...
		private void AddRefAttrText(Attr attr)
		{
				...
				||  attr.MyFieldType==FieldType.PKFK )
			{
               ...
               // myattr.UIBindKeyOfEn = attr.UIBindKeyOfEn;
				...
				string desc=myattr.Desc="名前";
				if (desc.IndexOf("ナンバリング") >=0 )
					myattr.Desc=attr.Desc.Replace("ナンバリング","名前");
				else
					myattr.Desc=attr.Desc+"名前";
