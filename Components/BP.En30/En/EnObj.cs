...
namespace BP.En
{
    ...
    abstract public class EnObj
    {
        ...
        {
            if (val == null)
                throw new Exception("@[" + attrDesc + "]の属性値をnullに設定できません。");
        ...
        public void SetDateValByKey(string attrKey, string val)
        {
            ...
            {
                throw new Exception("@不正な日付データ形式：key=[" + attrKey + "],value=" + val + " " + ex.Message);
            }
        }
        #endregion
        ...
        public DateTime GetValDateTime(string attrKey)
        {
            ...
            {
                throw new Exception("フィールド[" + this.EnMap.GetAttrByKey(attrKey).Desc + "],値[" + this.GetValStringByKey(attrKey) + "]、有効な時間形式ではありません.");
            }
        }
        /// <summary>
        ...
        public string GetValStringByKey(string attrKey)
        {
            switch (attrKey)
            {
                ...
                default:
                    ...
                    {
                        if (this.Row == null)
                            throw new Exception("@Rowを初期化されていません。");
                        return this.Row.GetValByKey(attrKey).ToString();
                    }
                    catch (Exception ex)
                    {
                        throw new Exception("@値の取得中に次の例外が発生しました：" + ex.Message + "  " + attrKey + " この属性をクラスに追加しませんでしたEnsName=" + this.ToString());
                    }
                    break;
            }
        }
        ...
        public string GetValRefTextByKey(string attrKey)
        {
            ...
            if (str == null)
            {
                ...
                if (this.Row.ContainsKey(attrKey + "Text") == false)
                    throw new Exception(attrKey + "非外部キーまたは列挙型はGetValRefTextByKeyからデータを取得することはできません。");
            }
            return str;
        }
        ...
        public int GetValIntByKey(string key)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("@エンティティークラス[" + this.ToString() + "]@[" + this.EnMap.GetAttrByKey(key).Desc + "]数字を入力してください、あなたが入力したものは[" + this.GetValStrByKey(key) + "],エラーメッセージ:"+ex.Message);
            }
        }
        /// <summary>
        /// 根据key 得到 bool val
        ...
        public decimal GetValDecimalByKey(string key)
        {
            ...
            {
                throw new Exception("@テーブル[" + this.EnDesc + "]プロパティを取得する[" + key + "]値、エラーが発生したため、[" + this.GetValStrByKey(key) + "]float型に変換できません。エラーメッセージ：" + ex.Message);
            }
        }
        ...
        public double GetValDoubleByKey(string key)
        {
            ...
            {
                throw new Exception("@テーブル[" + this.EnDesc + "]プロパティを取得する[" + key + "]値、エラーが発生したため、[" + this.GetValStringByKey(key) + "]double型に変換できません。エラーメッセージ：" + ex.Message);
            }
        }
        public string GetValAppDateByKey(string key)
        {
            ...
            {
                throw new Exception("@インスタンス：[" + this.EnMap.EnDesc + "]  の属性[" + key + "]値[" + this.GetValStringByKey(key).ToString() + "]日付形式変換エラー：" + ex.Message);
            }
            //return "2003-08-01";
        }
        #endregion
        ...
        public int PKCount
        {
            get
            {
                ...
                if (i == 0)
                    throw new Exception("@【" + this.EnDesc + "，" + this.EnMap.PhysicsTable + "】を主キーに定義されません。");
                else
                    return i;
            }
        }
        ...
        public virtual string PKField
        {
            get
            {
                ...
                throw new Exception("@【" + this.EnDesc + "】を主キーに定義されません。");
            }
        }
        /// <summary>
        /// 如果只有一个主键,就返回PK,如果有多个就返回第一个.PK
