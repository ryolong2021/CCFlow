    ...
    abstract public class Entity : EnObj
    {
            ...
            foreach (Attr attr in en.EnMap.Attrs)
            {
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppDateTime:
                        ...
                        break;
                    default:
                        throw new Exception("@bulider insert sql error: データ型なし");
                }
            }
            return dt;
        }
        ...
        public int RunSQL(Paras ps)
        {
            switch (this.EnMap.EnDBUrl.DBUrlType)
            {
                ...
                default:
                    throw new Exception("@タイプを設定してません");
            }
        }
        ...
        public Entities GetDtlEnsDa(string EnsName)
        {
            ...
            throw new Exception("@エンティティ["+this.EnDesc+"],"+EnsNameを含まない);	
            */
        }
        /// <summary>
        /// 取出他的数据实体
        ...
        public Entities GetDtlEnsDa(Entities ens)
        {
            ...
            throw new Exception("@[" + this.EnDesc + "]の詳細を取る時エラーが発生しました。[" + ens.GetNewEntity.EnDesc + "],コレクションにはありません。");
        }
        ...
        public Entities GetDtlEnsDa(EnDtl dtl, string pkval = null)
        {
            ...
            {
                throw new Exception("@取る[" + this.EnDesc + "]の詳細を取る時エラーが発生しました。[" + dtl.Desc + "],コレクションにはありません。");
            }
        }
        ...
        public string GenerNewNoByKey(int nolength, string attrKey, string attrGroupKey, string attrGroupVal)
        {
            if (attrGroupKey == null || attrGroupVal == null)
                throw new Exception("@グループ化フィールドattrGroupKey attrGroupValは空にできません");
        ...
        public virtual int Retrieve()
        {
            ...
            catch (Exception ex)
            {
                if (ex.Message.Contains("does not exist")
                    || ex.Message.Contains("存在しません")
                    || ex.Message.Contains("無効")
                ...
                throw new Exception(ex.Message + "@エンティティ内(" + this.ToString() + ")クエリ中にエラーが発生しました@" + ex.StackTrace);
            }
            ...
            {
                case "OID":
                    msg += "[ 主キー= OID値=" + this.GetValStrByKey("OID") + " ]";
                    break;
                case "No":
                    msg += "[ 主キー=No 値=" + this.GetValStrByKey("No") + " ]";
                    break;
                case "MyPK":
                    msg += "[ 主キー= MyPK値=" + this.GetValStrByKey("MyPK") + " ]";
                    break;
                case "NodeID":
                    msg += "[ 主キー= NodeID値=" + this.GetValStrByKey("NodeID") + " ]";
                    break;
                case "WorkID":
                    msg += "[ 主キー= WorkID値=" + this.GetValStrByKey("WorkID") + " ]";
                    break;
                default:
                    Hashtable ht = this.PKVals;
                    foreach (string key in ht.Keys)
                        msg += "[ 主キー=" + key + " 値=" + ht[key] + " ]";
                    break;
            }
            Log.DefaultLogWriteLine(LogType.Error, "@[" + this.EnMap.EnDesc + "  " + this.EnMap.PhysicsTable + "なし, クラス[" + this.ToString() + "], 物理的なテーブル[" + this.EnMap.PhysicsTable + "] インスタンス。 PK =" + this.GetValByKey(this.PK));
            throw new Exception("@レコードが見つかりません[" + this.EnMap.EnDesc + "  " + this.EnMap.PhysicsTable + ", " + msg + "レコードが存在しません。管理者に連絡するか、入力エラーを確認してください.");
        }
        /// <summary>
        /// 判断是不是存在的方法.
        /// </summary>
                ...
                try
                {
                    ...
                    switch (this.EnMap.EnDBUrl.DBType)
                    {
                        ...
                        case DBType.Access:
                            ...
                            break;
                        default:
                            throw new Exception("@設計されていません。" + this.EnMap.EnDBUrl.DBUrlType);
                    }
                    ...
                    switch (this.EnMap.EnDBUrl.DBUrlType)
                    {
                        ...
                        default:
                            throw new Exception("@DBUrlには関与しない。" + this.EnMap.EnDBUrl.DBUrlType);
                    }
        ...
        private bool CheckDB()
        {
            ...
            foreach (BP.En.EnDtl dtl in this.EnMap.Dtls)
            {
                ...
                if(dt.Rows.Count==0)
                    ...
                    throw new Exception("@["+this.EnDesc+"],削除中にエラーが発生しました。["+dt.Rows.Count+"]詳細が存在し、削除できません！");
                    */
            }
            #endregion
        ...
        public AtPara atPara
        {
            get
            {
                ...
                {
                    throw new Exception("@パラメータAtParaの取得中に例外が発生しました" + ex.Message + "、合意されたパラメータフィールドAtParaに参加しなかった可能性があります。" + ex.Message);
                }
            }
        }
        /// <summary>
...
#warning zhoupeng 打开如下注释代码？没有考虑到为什么要改变PK.
        ...
        public bool verifyData()
        {
            ...
            foreach (Attr attr in attrs)
            {
                ...
                if (attr.MyDataType == DataType.AppString && attr.MinLength > 0)
                {
                    ...
                    if (s.Length < attr.MinLength || s.Length > attr.MaxLength)
                    {
                        ...
                        {
                            if (this.GetValStringByKey(attr.Key).Trim().Length == 0 || this.GetValStringByKey(attr.Key) == "自動生成")
                                this.SetValByKey("No", this.GenerNewNoByKey("No"));
                        }
                        else
                        {
                            str += "@[" + attr.Key + "," + attr.Desc + "]入力エラー、" + attr.MinLength + " ～ " + attr.MaxLength + " 入力してください。文字範囲、現在は空。";
                        }
                    }
                }
            ...
            if (SystemConfig.IsDebug)
                throw new Exception("@[" + this.EnDesc + "]を保存する時,主キー[" + this.PK + "=" + this.PKVal + "]誤入力エラーは発生しました：" + str);
            else
                throw new Exception("@[" + this.EnDesc + "][" + this.EnMap.GetAttrByKey(this.PK).Desc + "=" + this.PKVal + "]を保存するエラー：" + str);
        }
        #endregion
        ...
        public int Update(string[] keys)
        {
            ...
            {
                if (ex.Message.Contains("列名") || ex.Message.Contains("文字列は切り捨てられます") || ex.Message.Contains("欠如") || ex.Message.Contains("値が大きすぎます"))
                {
                    /*说明字符串长度有问题.*/
                    this.CheckPhysicsTable();
                    ...
                    foreach (Attr attr in this.EnMap.Attrs)
                    {
                        ...
                        {
                            errs += "@マッピングで" + attr.Key + "," + attr.Desc + ", 入力データに対して：{" + this.GetValStrByKey(attr.Key) + "}, 長すぎる。";
                        }
                    }

                    if (errs != "")
                        throw new Exception("@[" + this.ToString() + "]更新を実行する時エラーが発生しました@errorフィールド:" + errs + " <br>もう一度提出してください。" + ex.Message);
                    else
                        throw ex;
                }
                ...
                if (SystemConfig.IsDebug)
                    throw new Exception("@[" + this.EnDesc + "]更新中にエラーが発生しました:" + str + ex.Message);
                else
                    throw ex;
            }
        }
        private int UpdateOfDebug(string[] keys)
        {
            ...
            try
            {
                ...
                str = "@beforeUpdateInsertActionでエラーが発生しました";
                if (this.beforeUpdateInsertAction() == false)
                {
                    return 0;
                }
                int i = EntityDBAccess.Update(this, keys);
                str = "@afterUpdateでエラーが発生しました";
                this.afterUpdate();
                str = "@afterInsertUpdateActionでエラーが発生しました";
                this.afterInsertUpdateAction();
                //this.UpdateMemory();
                return i;
            }
            ...
            {
                string msg = "@[" + this.EnDesc + "]UpdateOfDebug更新中にエラーが発生しました:" + str + ex.Message;
        ...
        public void SaveBigTxtToDB(string saveToField, string bigTxt)
        {
            ...
            try
            {
                string temp = BP.Sys.SystemConfig.PathOfTemp + Path.DirectorySeparatorChar + this.EnMap.PhysicsTable + this.PKVal + ".tmp";
            ...
            {
                string err = "err@大きなフィールドテキスト、クラスの保存中にエラーが発生しました:" + this.ToString() + " の属性:" + saveToField + ".";
                err += "@ディレクトリの権限が十分でない可能性があります。dataUserディレクトリのアクセス権限を設定してください。システムエラー@" + ex.Message;
        ...
        public void SaveFileToDB(string saveToField, byte[] bytesOfFile)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("@ファイルの保存中にエラーが発生しました。フィールドが自動的に作成されなかった可能性があります。データテーブルの作成と修復が実行されました。もう一度実行してください." + ex.Message);
            }
        }
            ...
            if (this._enMap.EnDBUrl.DBUrlType == DBUrlType.AppCenterDSN)
            {
                ...
                switch (DBAccess.AppCenterDBType)
                {
                    ...
                    case DBType.Access:
                        ...
                        break;
                    default:
                        throw new Exception("@未判断のデータベースタイプ。");
                }
        ...
        private void CheckPhysicsTable_SQL()
        {
            ...
            foreach (Attr attr in attrs)
            {
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppDateTime:
                        ...
                        {
                            string err = "err@フィールドタイプが一致しません、テーブル[" + this.EnMap.PhysicsTable + "]フィールド[" + attr.Key + "]名称[" + attr.Desc + "]マッピングタイプは[" + attr.MyDataTypeStr + "],データ型は[" + FType + "]";
            ...
            foreach (Attr attr in attrs)
            {
                ...
                {
                    throw new Exception("@列挙を自動的に追加中にエラーが発生しました。形式が正しいことを確認してください。" + ex.Message + "attr.UIBindKey=" + attr.UIBindKey);
                }
        ...
        public void CheckPhysicsTable()
        {
            ...
            switch (SystemConfig.AppCenterDBType)
            {
                ...
                case DBType.PostgreSQL:
                    ...
                    break;
                default:
                    throw new Exception("@関連しないデータベースの種類");
            }
...
#warning 没有处理好。
                ...
                if (val == null)
                    Log.DefaultLogWriteLineError("@フィールドが検出されませんでした:" + attr.Key);
                ...
                if (val.IndexOf("CHAR") != -1)
                {
                    ...
                    {
                        Log.DefaultLogWriteLineError("SQLの実行に失敗しました:alter table  " + this.EnMap.PhysicsTableExt + " modify " + attr.Field + " NUMBER " + ex.Message);
                    }
                }
            }
            #endregion
        ...
        private void CheckPhysicsTable_MySQL()
        {
            ...
            foreach (Attr attr in attrs)
            {
                ...
                string val = DBAccess.RunSQLReturnString(sql);
                if (val == null)
                    Log.DefaultLogWriteLineError("@フィールドeunmが検出されない" + attr.Key);
                ...
                if (val.IndexOf("CHAR") != -1)
                {
                    ...
                    {
                        Log.DefaultLogWriteLineError("SQLの実行に失敗しました:alter table  " + this._enMap.PhysicsTableExt + " modify " + attr.Field + " NUMBER " + ex.Message);
                    }
                }
            }
            #endregion
        ...
        public void AutoFull()
        {
            ...
            foreach (Attr attr in attrs)
            {
                ...
                switch (attr.AutoFullWay)
                {
                    ...
                    case AutoFullWay.Way2_SQL:
                        ...
                        {
                            throw new Exception("@フィールド(" + attr.Key + "," + attr.Desc + ")データの自動取得中にエラーが発生しました。（sql文の場合は、複数行のテーブルに戻ります。現在は一行のテーブルだけが埋められます。sqlを確認してください。):" + sql.Replace("'", "“") + " @Tech Info:" + ex.Message.Replace("'", "“") + "@SQLが実行されました:" + sql);
                        }
                    ...
                    case AutoFullWay.Way3_FK:
                                ...
                                if (ss[0] == "AttrKey")
                                {
                                    ...
                                    if (tempV == "" || tempV == null)
                                    {
                                        if (this.EnMap.Attrs.Contains(ss[1]) == false)
                                            throw new Exception("@自動的に取得された値情報は不完全であり、Key==" + ss[1] + "の属性はマップに含まれていません。");

                                        //throw new Exception("@自动获取值信息不完整,Map 中已经不包含Key=" + ss[1] + "的属性。");
                                        sqlfk = sqlfk.Replace('@' + ss[0], "'@xxx'");
                                        Log.DefaultLogWriteLineWarning("@自動評価中にエラーが発生しました:" + this.ToString() + " , " + this.PKVal + "情報は自動的に取得されません。");
                                    }
                                    else
                                    {
                        ...
                        {
                            throw new Exception("@処理中の自動補完：外部キー[" + attr.Key + ";" + attr.Desc + "],エラーが発生しました。エラーメッセージ：" + ex.Message);
                        }
                        break;
            ...
            foreach (Attr attr in al)
            {
                ...
                {
                    Log.DefaultLogWriteLineError("@(" + this.ToString() + ")処理中の自動計算{" + this.EnDesc + "}：" + this.PK + "=" + this.PKVal + "属性が[" + attr.Key + "]、計算内容[" + doc + "]、エラーが発生しました：" + ex.Message);
                    throw new Exception("@(" + this.ToString() + ")処理中の自動計算{" + this.EnDesc + "}：" + this.PK + "=" + this.PKVal + "属性が[" + attr.Key + "]、計算内容[" + doc + "]、エラーが発生しました：" + ex.Message);
                }
            }
    ...
    public abstract class Entities : CollectionBase
    {
        ...
        public void InitCollectionByTable(DataTable dt)
        {
            ...
            {
                throw new Exception("@このテーブルはコレクションに変換できません。詳細なエラー:" + ex.Message);
            }
        }
        /// <summary>
        /// 判断两个实体集合是不是相同.
        ...
        public int FlodInCash()
        {
            ...
            BP.DA.Log.DefaultLogWriteLineInfo("成功[" + en.ToString() + "-" + num + "]キャッシュに入れる。");
            return num;
        }
        /// <summary>
        /// 执行一次数据检查
        ...
        public DataTable ToDataTableDescField(string tableName="dt")
        {
            ...
            foreach (Entity myen in this)
            {
                ...
                foreach (Attr attr in en.EnMap.Attrs)
                {
                    ...
                    {
                        if (myen.GetValBooleanByKey(attr.Key))
                            dr[attr.Desc.Trim() + attr.Key] = "はい";
                        else
                            dr[attr.Desc.Trim() + attr.Key] = "いいえ";
                        continue;
                    }
