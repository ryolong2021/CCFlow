...
namespace BP.En
{
	...
	{		 
            ...
            get
            {
                ...
                try
                {
                    ...
                    switch (this.EnMap.EnDBUrl.DBUrlType)
                    {
                        ...
                        default:
                            throw new Exception("設計されていません。" + this.EnMap.EnDBUrl.DBType);
                    }
                }
                catch(Exception ex)
                {
		...
		protected override bool beforeDelete() 
		{
			...
			{				
				if (this.OID < 0 )
					throw new Exception("@エンティティ["+this.EnDesc+"]インスタンス化されていない、Delete()できない。");
				return true;
			} 
			catch (Exception ex) 
			{
        ...
        protected override bool beforeInsert()
        {
            ...
            if (this.OID > 0)
                throw new Exception("@[" + this.EnDesc + "], エンティティがインスタンス化されましたoid =[" + this.OID + "]、挿入できません.");
		...
		protected override bool beforeUpdate()
		{
			...
			/*
			if (this.OID <= 0 )
				throw new Exception("@エンティティ["+this.EnDesc+"]インスタンス化されていない、Update()できない。");
				*/
			return true;
		}
