...
namespace BP.En
{
    ...
    abstract public class EntityTree : Entity
    {
        ...
        public EntityTree(string no)
        {
            if (DataType.IsNullOrEmpty(no))
                throw new Exception(this.EnDesc + "@テーブル[" + this.EnDesc + "]をクェリする時、番号を指定しなければならない。");

            this.No = no;
            if (this.Retrieve() == 0)
                throw new Exception("@" + this._enMap.PhysicsTable + ", No = " + this.No + "の記録がありません。");
        }
        #endregion
            ...
            if (this.EnMap.IsAllowRepeatName == false)
            {
                if (this.PKCount == 1)
                {
                    if (this.ExitsValueNum("Name", this.Name) >= 1)
                        throw new Exception("@挿入できませんでした[" + this.EnMap.EnDesc + "] ナンバリング[" + this.No + "]名前[" + Name + "]繰り返し.");
                }
            }
            ...
            if (this.EnMap.IsAllowRepeatName == false)
            {
                if (this.PKCount == 1)
                {
                    if (this.ExitsValueNum("Name", this.Name) >= 2)
                        throw new Exception("@更新に失敗しました[" + this.EnMap.EnDesc + "] ナンバリング[" + this.No + "]名前[" + Name + "]繰り返し.");
                }
            }
    ...
    abstract public class EntitiesTree : Entities
    {
        ...
        public string ToJsonOfTree(string rootNo="0")
        {
            ...
            if (root == null)
               return "err@RootNo=" + rootNo + "のエンティティが見つかりません。";
