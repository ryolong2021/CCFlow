...
namespace BP.En
{
    ...
    public class GloEntity
    {
        ...
        public static string GetTextByValue(Entities ens, string no)
        {
            ...
            if (ens.Count == 0)
                throw new Exception("@エンティティコレクションにデータがありません。");
            else
                throw new Exception("@NO=" + no + "のエンティティが見つかりません。");
        }
        #endregion
        ...
        private static DataTable ToTable0(Entity en)
        {
            ...
            DataTable dt = new DataTable();
            dt.Columns.Add(new DataColumn("入力項目", typeof(string)));
            dt.Columns.Add(new DataColumn(nameOfEnterInfo, typeof(string)));
            dt.Columns.Add(new DataColumn("情報入力請求", typeof(string)));
