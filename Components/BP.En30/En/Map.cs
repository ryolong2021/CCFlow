...
namespace BP.En
{
    ...
    public class Map
    {
        ...
        public Attr GetAttrByKey(string key)
        {
            ...
            if (key == null)
                throw new Exception("@[" + this.EnDesc + "] 属性キー値の取得は空にできません.");


            if (this.ToString().Contains("."))
                throw new Exception("@[" + this.EnDesc + "," + this.PhysicsTable + "] key=[" + key + "]の属性は見つかりません。Mapファイルを確認してください。この問題の理由の1つは、設定システムのエンティティの属性がこのエンティティに関連付けられており、エンティティの情報を設定するときに、ルールに従ってreftextとrefvalueを記述しなかったためです。確認してください。");
            else
            {
                throw new Exception("@[" + this.EnDesc + "," + this.PhysicsTable + "] key=[" + key + "]の属性見つかりません。属性、Sys_MapAttrテーブルにデータがあるかどうかを確認して、SQLで実行してください： SELECT * FROM Sys_MapAttr WHERE FK_MapData='" + this.ToString() + "' AND KeyOfEn='" + key + "' データを照会できるかどうか、照会できない場合は、フィールド属性が失われます。");
            }
        }
        /// <summary>
        /// 获得属性.
        ...
        public Attr GetAttrByBindKey(string key)
        {
            ...
            if (key == null)
                throw new Exception("@[" + this.EnDesc + "] 属性descの値の取得は空にできません。");

            if (this.ToString().Contains("."))
                throw new Exception("@[" + this.EnDesc + "," + this.ToString() + "] key=[" + key + "]の属性は見つかりません。Mapファイルを確認してください。この問題の理由の1つは、設定システムのエンティティの属性がこのエンティティに関連付けられており、エンティティの情報を設定するときに、ルールに従ってreftextとrefvalueを記述しなかったためです。確認してください。");
            else
                throw new Exception("@[" + this.EnDesc + "," + this.ToString() + "] key=[" + key + "]の属性は見つかりません。Sys_MapAttrテーブルにデータがあるかどうかを確認して、SQLで実行してください： SELECT * FROM Sys_MapAttr WHERE FK_MapData='" + this.ToString() + "' AND KeyOfEn='" + key + "' データを照会できるかどうか、照会できない場合は、フィールド属性が失われます。");
        }
        /// <summary>
        /// 通过一个key 得到它的属性值。
        /// </summary>
        ...
        public Attr GetAttrByDesc(string desc)
        {
            ...
            throw new Exception("@[" + this.EnDesc + "] desc=[" + desc + "]の属性見つかりません。地図ファイルを確認してください。この問題の理由の1つは、設定システムのエンティティの属性がこのエンティティに関連付けられており、エンティティの情報を設定するときに、ルールに従ってreftextとrefvalueを記述しなかったためです。確認してください。");
        }
        #endregion
            ...
            get
            {
                if (this._HisCfgAttrs == null)
                {
                    ...
                    if (Web.WebUser.No == "admin")
                    {
                        ...
                        this._HisCfgAttrs.AddDDLSysEnum("UIRowStyleGlo", 2, "テーブルデータ行スタイル（グローバルに適用）", true, false, "UIRowStyleGlo",
                            "@0=スタイルなし@1 =交互スタイル@2 =マウス移動@3 =交互およびマウス移動");

                        this._HisCfgAttrs.AddBoolen("IsEnableDouclickGlo", true,
                             "ダブルクリックで開くかどうか（グローバルに適用）");

                        this._HisCfgAttrs.AddBoolen("IsEnableFocusField", true, "フォーカスフィールドを有効にするかどうか");
                        this._HisCfgAttrs.AddTBString("FocusField", null, "フォーカスフィールド（クリックして開いた列を表示するために使用）",
                            true, false, 0, 20, 20);
                        this._HisCfgAttrs.AddBoolen("IsEnableRefFunc", true, "関連関数列を有効にするかどうか");
                        this._HisCfgAttrs.AddBoolen("IsEnableOpenICON", true, "アイコン開くを有効にするかどうか");
                        this._HisCfgAttrs.AddDDLSysEnum("MoveToShowWay", 0, "表示モードに移動", true, false,
                            "MoveToShowWay", "@0=表示されない@1 =ドロップダウンリスト@2 =タイルを表示しない");
                        this._HisCfgAttrs.AddTBString("MoveTo", null, "フィールドに移動", true, false, 0, 20, 20);
                        this._HisCfgAttrs.AddTBInt("WinCardW", 820, "ポップアップウィンドウの幅", true, false);
                        this._HisCfgAttrs.AddTBInt("WinCardH", 480, "ポップアップウィンドウの高さ", true, false);
                        this._HisCfgAttrs.AddDDLSysEnum("EditerType", 0, "ラージブロックテキストエディター",
                            true, false, "EditerType", "@0=なし@1 = sina editor @2 = FKCEditer @3 = KindEditor @4 = UEditor");
                        this._HisCfgAttrs.AddTBString("ShowColumns", "", "列を選択", false, false, 0, 1000, 100);    //added by liuxc,2015-8-7,增加列を選択存储字段
        ...
        public string DTSearchLable = "デートから";
            ...
            foreach (DataTable dt in ds.Tables)
            {
                switch (dt.TableName)
                {
                    ...
                    case "Dot2Dot":
                        ...
                        break;
                    default:
                        throw new Exception("XML 設定情報が間違っています。合意されたマークがありません:" + dt.TableName);
                }
            }
            // 检查配置的完整性。
        ...
        {
            if (dt.Rows.Count != 1)
                throw new Exception("基本情報設定エラー、2行以上または1行未満のレコードできません。");
                    ...
                    case "EnType":
                        switch (val)
                        {
                            ...
                            case "XML":
                                ...
                                break;
                            default:
                                throw new Exception("合意されたマークなし：：EnType=  " + val);
                        }
                        break;
                    case "DepositaryOfEntity":
                        switch (val)
                        {
                            ...
                            case "Session":
                                ...
                                break;
                            default:
                                throw new Exception("合意されたマークなし：：DepositaryOfEntity=[" + val + "] アプリケーション、なし、セッションとして選択する必要があります");
                        }
                        break;
                    case "DepositaryOfMap":
                        switch (val)
                        {
                            ...
                            case "None":
                                ...
                                break;
                            default:
                                throw new Exception("合意されたマークなし：DepositaryOfMap=[" + val + "] Application、None、 Session選択する必要があります");
                        }
                        break;
                    case "PKs":
                        this.PKs = val;
                        break;
                    default:
                        throw new Exception("基本情報に合意されたマークなし：" + val);
                }
            }
        }
                ...
                foreach (DataColumn dc in dt.Columns)
                {
                    ...
                    switch (dc.ColumnName)
                    {
                        ...
                        case "TBLen":
                            ...
                            break;
                        default:
                            throw new Exception("合意されたマークなし：" + val);
                    }
                }
        ...
        public int GenerNoLength
        {
            get
            {
                if (this._GenerNoLength == 0)
                    throw new Exception("@文字列を生成するフィールドの長さが指定されていません。");
        ...
        public void AddAttrsFromMapData(string FK_MapData=null)
        {
            if (DataType.IsNullOrEmpty(this.FK_MapData) && DataType.IsNullOrEmpty(FK_MapData))
                throw new Exception("@MapのFK_MapDataに値を割り当てていません。");
        ...
        {
            this.AddTBInt(EntityNoMyFileAttr.MyFileNum, 0, "添付ファイル", false, false);
        ...
        public void AddTBDate(string key)
        {
            ...
            {
                case "RDT":
                    AddTBDate("RDT", "記録日", true, true);
                    break;
                case "UDT":
                    AddTBDate("UDT", "更新日", true, true);
                    break;
                default:
        ...
        {
            this.PKs = "MyPK";
            this.AddTBStringPK("MyPK", null, "主キーMyPK", uiVisable, true, 1, 100, 10);
            //Attr attr = new Attr();
            //attr.Key = "MyPK";
            //attr.Field = "MyPK";
        ...
        {
            this.AddTBInt("MyNum", "MyNum", 1, "数", true, true);
        }
        #endregion
