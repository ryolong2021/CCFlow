    ...
    public class SqlBuilder
    {
        ...
        public static String GetKeyConditionOfMS(Entity en)
        {
            ...
            if (en.EnMap.Attrs.Contains("OID"))
            {
                ...
                if (key == 0)
                    throw new Exception("@実行中[" + en.EnMap.EnDesc + " " + en.EnMap.PhysicsTable + "]、PK OIDが割り当てられていないため、クエリ操作を実行できません。");
                //   if (en.PKs.Length == 1)
                return " OID=@OID" + key;
            }
            ...
            foreach (Attr attr in en.EnMap.Attrs)
            {
                if (attr.MyFieldType == FieldType.PK || attr.MyFieldType == FieldType.PKFK || attr.MyFieldType == FieldType.PKEnum)
                {
                    if (attr.MyDataType == DataType.AppString)
                    {
                        ...
                        if (val == null || val == "")
                            throw new Exception("@実行中[" + en.EnMap.EnDesc + " " + en.EnMap.PhysicsTable + "]PK" + attr.Key + " を与割り当てえなかった、クエリ操作できません。");
        ...
        public static string Retrieve(Entity en)
        {
            ...
            switch (en.EnMap.EnDBUrl.DBType)
            {
                ...
                case DBType.Informix:
                    ...
                    break;
                case DBType.DB2:
                    throw new Exception("まだ実装されていません。");
                default:
                    throw new Exception("まだ実装されていません。");
            }
        ...
        public static string GenerCreateTableSQLOfMS(Entity en)
        {
            ...
            if (attrs.Count == 0)
                throw new Exception("@" + en.EnDesc + " , [" + en.EnMap.PhysicsTable + "]属性/フィールドセットはありません。attrs.Count = 0データテーブルの作成を実行できます。");
        ...
        public static string GenerCreateTableSQLOfOra(Entity en)
        {
            if (en.EnMap.PhysicsTable == null)
                throw new Exception("[" + en.EnDesc + "]を物理テーブルを設定されてません。");
            ...
            foreach (Attr attr in en.EnMap.Attrs)
            {
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppInt:
                        ...
                        if (attr.MyFieldType == FieldType.FK || attr.MyFieldType == FieldType.PKFK)
                        {
                            if (attr.HisFKEns == null)
                                throw new Exception("@SQLを生成するエラー Entity=" + en.ToString() + " 外部キーフィールド{" + attr.Key + "." + attr.Desc + ", UIBindKey=" + attr.UIBindKey + "｝すでに無効です。このクラスまたは外部キーフィールドが削除された可能性があります。解決するよう管理者に通知してください。");
                    ...
                    case DataType.AppDateTime:
                        ...
                        break;
                    default:
                        throw new Exception("@データ型が定義されていません！attr=" + attr.Key + " MyDataType =" + attr.MyDataType);
                }
            }
        ...
        public static string SelectSQLOfMS(Entity en, int topNum)
        {
            ...
            if (en.EnMap.Attrs.Count == 0)
            {
                ...
                throw new Exception("err@エラー:" + en.ToString() + "attrs属性がないと、SQLを生成できません。動的エンティティの場合は、閉じてからもう一度開いてください。");
            }
...
#warning 2011-12-03 不应出现异常。
                        ...
                        {
                            if (attr.HisFKEns == null)
                                throw new Exception("@QLを生成するエラー Entity=" + en.ToString() + " 外部キーフィールド{" + attr.Key + "." + attr.Desc + ", UIBindKey=" + attr.UIBindKey + "｝すでに無効です。このクラスまたは外部キーフィールドが削除された可能性があります。解決するよう管理者に通知してください。");
        ...
        public static string UpdateOfMSAccess(Entity en, string[] keys)
        {
            ...
            foreach (Attr attr in en.EnMap.Attrs)
            {
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppDateTime:
                        ...
                        break;
                    default:
                        throw new Exception("@SqlBulider.update, このデータ型はありません。");
                }
            }
            ...
            if (val == "")
            {
                ...
                throw new Exception(en.EnDesc + "更新実行のエラー：無効な属性[" + ms + "]このエンティティについて。");
            }
        ...
        public static string Update(Entity en, string[] keys)
        {
            ...
            if (map.Attrs.Count == 0)
                throw new Exception("@エンティティ：" + en.ToString() + " ,Attrs属性収集情報が失われ、SQLの生成に失敗する。");
                    ...
                    foreach (Attr attr in map.Attrs)
                    {
                        ...
                        switch (attr.MyDataType)
                        {
                            ...
                            case DataType.AppInt:
                                ...
                                break;
                            default:
                                throw new Exception("@SqlBulider.update, そのようなデータ型はありません。");
                        }
                    }
                    break;
            }
        ...
        public static Paras GenerParas(Entity en, string[] keys)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("パラメータ生成中のエラー:" + errKey + "@エラーメッセージ:" + ex.Message);
            }
            ...
            foreach (Attr attr in attrs)
            {
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppDateTime:
                        ...
                        break;
                    default:
                        throw new Exception("@bulider insert sql error: このデータ型はありません。");
                }
            }
            string sql = "INSERT INTO " + en.EnMap.PhysicsTable + " (" +
                field.Substring(1) + " ) VALUES ( " + val.Substring(1) + ")";
        ...
        public static string GetIsNullInSQL(string expression, string isNullBack)
        {
            switch (DBAccess.AppCenterDBType)
            {
                ...
                default:
                    throw new Exception("GetIsNullInSQL関連しないデータベースの種類");
            }
        }
    }
