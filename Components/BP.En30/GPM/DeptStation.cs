    ...
    public class DeptStation : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.EnDesc = "対応する部門の役職";
                map.Java_SetEnType(EnType.Dot2Dot); //实体类型，admin 系统管理员表，PowerAble 权限管理表,也是用户表,你要想把它加入权限管理里面请在这里设置。。

                map.AddTBStringPK(DeptStationAttr.FK_Dept, null, "部門", false, false, 1, 15, 1);
                map.AddDDLEntitiesPK(DeptStationAttr.FK_Station, null, "役職", new Stations(), true);
