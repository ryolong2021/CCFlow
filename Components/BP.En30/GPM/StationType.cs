	...
	public class StationType :EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("Port_StationType","職種");
                map.Java_SetCodeStruct("2");

                map.Java_SetDepositaryOfEntity(Depositary.None);
                map.Java_SetDepositaryOfMap( Depositary.Application);

                map.AddTBStringPK(StationTypeAttr.No, null, "ナンバリング", true, true, 2, 2, 2);
                map.AddTBString(StationTypeAttr.Name, null, "名前", true, false, 1, 50, 20);
                map.AddTBInt(StationTypeAttr.Idx, 0, "順序", true, false);
                map.AddTBString(StationTypeAttr.OrgNo, null, "組織番号", true, false, 0, 50, 20);
