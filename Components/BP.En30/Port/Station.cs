...
namespace BP.Port
{
    ...
    public class Station : EntityNoName
    {
        ...
        public Station(string no)
        {
            this.No = no.Trim();
            if (this.No.Length == 0)
                throw new Exception("@照会するポジション番号が空です。");
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBStringPK(EmpAttr.No, null, "ナンバリング", true, false, 4, 4, 4);
                map.AddTBString(EmpAttr.Name, null, "名前", true, false, 0, 100, 100);
                map.AddTBString(StationAttr.OrgNo, null, "所属機関番号", true, false, 0, 100, 100);
        ...
        public override int Retrieve()
        {
            if (BP.Sys.SystemConfig.OSDBSrc == OSDBSrc.WebServices)
            {
                ...
                if (dt.Rows.Count == 0)
                    throw new Exception("@番号(" + this.No + ")のポジションが存在しません。");
