        ...
        public string GetValueCheckWorks()
        {
           ...
           string sql = "SELECT FK_Emp, FK_Node FROM WF_GenerWorkerList WHERE IsPass!=1 AND WorkID=" + this.HisGEEntity.PKVal;
            ...
            foreach (DataRow dr in dtTrack.Rows)
            {
                ...
                msg += "監査人:" + empStrs + " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;日付:" + dr["RDT"].ToString();
        ...
        public void MakeDoc(string templateRtfFile, string path, string file,bool isOpen, string billUrl=null)
        {
            ...
            try
            {
                ...
                foreach (string para in paras)
                {
                    ...
                    { 
                        error += "@メインテーブルタグを置き換えてパラメータを取得する[" + para + "]エラー：このエラーの原因となる次の状況があります; 1 Text value timeを使用すると、このプロパティは外部キーではありません。 2.クラスにはこの属性はありません。 3.フィールドは詳細リストフィールドですが、リストマークがありません。<br>詳細情報：<br>" + ex.Message;
            ...
            catch (Exception ex)
            {
                ...
                {  // 异常可能与单据的配置有关系。
                    try
                    {
                        ...
                        msg = "@修理ラインRepairLineV2は正常に実行されましたが、修理ラインを再送するか、返品後に再度送ってください。この問題は解決できますか？ @" + s;
                    }
                    catch (Exception ex1)
                    {
                ...
                throw new Exception("ドキュメントの生成に失敗しました：伝票名[" + this.CyclostyleFilePath + "] 例外情報：" + ex.Message + " @請求書情報を自動的に修復する：" + msg);
            }
            if (isOpen)
