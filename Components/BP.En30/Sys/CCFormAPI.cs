...
namespace BP.Sys
{
    ...
    public class CCFormAPI
    {
            ...
            switch (ctrlType)
            {
                ...
                case "Fieldset": //分组.
                    ...
                    if (fe.RetrieveFromDBSources() != 0)
                        throw new Exception("@作成に失敗しました。同じ名前の要素[" + no + "]コントロールがすでに存在します。");
                ...
                case "HandSiganture"://签字版
                    ...
                    break;
                default:
                    throw new Exception("@判断しないストレージコントロール:" + ctrlType + ",コントロールを保存する前に、判断する必要があります。");
            }
        }
        /// <summary>
        /// 创建/修改-图片附件
        ...
        public static string NewCheckGroup(string frmID, string gKey, string gName)
        {
            ...
            if (i > 0)
                return "err@プレフィックスはすでに使用されています：" + gKey + " 、このレビューグループを追加したかどうかを確認するか、他の接頭辞を変更してください。";
            ...
            {
                //转化不成功就是不是节点表单字段.
                return "error:ノードフォームのみが監査グループ化コンポーネントを使用できます。";
            }
            return null;
        ...
        public static void CreateCheckGroup(string frmID, string groupName, string prx)
        {
            ...
            attr.Name = "審査意見"; // sta;  // this.ToE("CheckNote", "監査意見");
        ...
        public static void CreateFrm(string frmID, string frmName, string frmTreeID, FrmType frmType = FrmType.FreeFrm)
        {
            ...
            md.No = frmID;
            if (md.IsExits == true)
                throw new Exception("@フォームIDは:" + frmID + " もう存在している。");
        ...
        public static void SaveFrm(string fk_mapdata, string jsonStrOfH5Frm)
        {
            ...
            if (jd.IsObject == false)
                throw new Exception("err@フォームの形式が正しくなく、保存に失敗しました。");
        ...
        private static void SaveFrm(string fk_mapdata, LitJson.JsonData formData)
        {
            ...
            for (int idx = 0, jControl = form_Controls.Count; idx < jControl; idx++)
            {
                ...
                throw new Exception("@ccformの保存コントロールのタイプを判断されてません：shape =" + shape);
            }
        ...
        public static string CopyFrm(string srcFrmID, string copyFrmID, string copyFrmName, string fk_frmTree)
        {
            ...
            if (mymd.RetrieveFromDBSources() == 1)
                throw new Exception("@ターゲットフォームID:" + copyFrmID + "すでに存在します:" + mymd.FK_FormTreeText + "ディレクトリにあります");
            ...
            return "フォームが正常にコピーされました。確認するには、再度ログインするか、更新する必要があります。";
        }
        /// <summary>
        /// 导入表单API
        /// </summary>
        ...
        public static string SaveEnum(string enumKey, string enumLab, string cfg, bool isNew, string lang = "CH")
        {
            ...
            if (dataCount > 0 && isNew)
                return "err@列挙はすでに存在します" + enumKey + ",列挙名を変更してください";
