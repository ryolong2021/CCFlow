	...
	public class CField: EntityMyPK
	{
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null) 
                    return this._enMap;
                Map map = new Map("Sys_UserRegedit","列の選択");

                map.Java_SetEnType(EnType.Sys);
                map.Java_SetDepositaryOfEntity(Depositary.None);

                map.AddMyPK();
                map.AddTBString(CFieldAttr.EnsName, null, "エンティティークラス名", false, true, 0, 100, 10);
                map.AddTBString(CFieldAttr.FK_Emp, null, "スタッフ", false, true, 0, 100, 10);
                map.AddTBStringDoc(CFieldAttr.Attrs, null, "属性", true, false);
                this._enMap = map;
                return this._enMap;
            }
        }
