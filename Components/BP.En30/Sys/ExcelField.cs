    ...
    public class ExcelField : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.EnDesc = "Excelフィールド";

                map.AddTBStringPK(ExcelFieldAttr.No, null, "ナンバリング", true, true, 1, 36, 200);
                map.AddTBString(ExcelFieldAttr.Name, null, "名前", true, false, 1, 50, 100);
                map.AddTBString(ExcelFieldAttr.CellName, null, "セル名", true, false, 1, 50, 100);
                map.AddTBInt(ExcelFieldAttr.CellRow, 0, "行番号", true, false);
                map.AddTBInt(ExcelFieldAttr.CellColumn, 0, "列番号", true, false);
                map.AddDDLEntities(ExcelFieldAttr.FK_ExcelSheet, null, "所属ExcelSheet", new ExcelSheets(), false);
                map.AddTBString(ExcelFieldAttr.Field, null, "ストレージフィールド名", true, false, 1, 50, 100);
                map.AddDDLEntities(ExcelFieldAttr.FK_ExcelTable, null, "ストレージデータテーブル", new ExcelTables(), false);
                map.AddDDLSysEnum(ExcelFieldAttr.DataType, 0, "値のタイプ", true, true, ExcelFieldAttr.DataType, "@0=文字列@ 1 =整数@ 2 =浮動小数点@ 3 =日付@ 4 =日付時刻@ 5 =外部キー@ 6 =列挙");
                map.AddTBString(ExcelFieldAttr.UIBindKey, null, "外部キーテーブル/列挙型", true, false, 1, 100, 100);
                map.AddTBString(ExcelFieldAttr.UIRefKey, null, "外部キーテーブルNo", true, false, 1, 30, 100);
                map.AddTBString(ExcelFieldAttr.UIRefKeyText, null, "外部キーテーブル名", true, false, 1, 30, 100);
                map.AddTBString(ExcelFieldAttr.Validators, null, "検証者", true, false, 1, 4000, 100);
                map.AddDDLEntities(ExcelFieldAttr.FK_ExcelFile, null, "所属Excelテンプレート", new ExcelFiles(), false);
                map.AddTBStringDoc(ExcelFieldAttr.AtPara, null, "パラメータ", true, false, true);
                map.AddDDLSysEnum(ExcelFieldAttr.ConfirmKind, 0, "セルの確認方法", true, true, ExcelFieldAttr.ConfirmKind, "@0=現在のセル@ 1 =左のセル@ 2 =上のセル@ 3 =右のセル@ 4 =下のセル");
                map.AddTBInt(ExcelFieldAttr.ConfirmCellCount, 1, "セル確認方向シフト", true, false);
                map.AddTBString(ExcelFieldAttr.ConfirmCellValue, null, "対応するセルの値", true, false, 1, 200, 100);
                map.AddTBInt(ExcelFieldAttr.ConfirmRepeatIndex, 0, "対応するセル値に対して選択したシーケンスを繰り返します", true, false);
                map.AddBoolean(ExcelFieldAttr.SkipIsNull, false, "空でない", true, true);
                map.AddTBString(ExcelFieldAttr.SyncToField, null, "フィールドに同期", true, false, 1, 100, 100);
        ...
        protected override bool beforeInsert()
        {
            ...
            if (this.IsExit(ExcelFieldAttr.CellName, this.CellName, ExcelFieldAttr.FK_ExcelSheet, this.FK_ExcelSheet, ExcelFieldAttr.FK_ExcelTable, this.FK_ExcelTable))
                throw new Exception("セル" + this.CellName + " 使用済み、再利用できません！");
