...
namespace BP.Sys
{
    ...
    abstract public class FormEventBase
    {
        ...
        public DateTime GetValDateTime(string key)
        {
            ...
            {
                throw new Exception("@フォームイベントエンティティがパラメータを取得中にエラーが発生しました。フィールドを確認してください(" + key + ")スペルが正しいかどうか、エラーメッセージ:" + ex.Message);
            }
        }
        /// <summary>
        /// 获取字符串参数
        ...
        public string DoIt(string eventType, Entity en, string atPara)
        {
            ...
            switch (eventType)
            {
                ...
                default:
                    throw new Exception("@フォームイベントのタイプを判定されない。:" + eventType);
                    break;
            }
            #endregion 执行事件.
            return null;
