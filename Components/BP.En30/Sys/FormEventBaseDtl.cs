...
namespace BP.Sys
{
	...
	abstract public class FormEventBaseDtl
	{
		...
		public DateTime GetValDateTime(string key)
		{
			...
			{
				throw new Exception("@パラメータを取得するテーブルイベントエンティティからのフォーム中にエラーが発生しました。フィールドを確認してください(" + key + ")スペルが正しいかどうか、エラーメッセージ:" + ex.Message);
			}
		}
		/// <summary>
		/// 获取字符串参数
		...
		public string DoIt(string eventType, Entity en, Entity enDtl, string atPara)
		{
			...
			switch (eventType)
			{
				...
				default:
					throw new Exception("@判断されてないテーブルイベントタイプ:" + eventType);
					break;
			}
			#endregion 执行事件.
