    ...
    public class FrmAttachmentDB : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("Sys_FrmAttachmentDB", "添付データストレージ");
                ...
                map.AddTBString(FrmAttachmentDBAttr.FK_FrmAttachment, null, "添付ファイルの主キー", true, false, 1, 500, 20);
                map.AddTBString(FrmAttachmentDBAttr.NoOfObj, null, "添付ファイルの識別", true, false, 0, 50, 20);

                map.AddTBString(FrmAttachmentDBAttr.RefPKVal, null, "エンティティ主キー", true, false, 0, 50, 20);
                map.AddTBInt(FrmAttachmentDBAttr.FID, 0, "FID", true, false);
                map.AddTBInt(FrmAttachmentDBAttr.NodeID, 0, "ノードID", true, false);
                

                map.AddTBString(FrmAttachmentDBAttr.Sort, null, "カテゴリー", true, false, 0, 200, 20);
                map.AddTBString(FrmAttachmentDBAttr.FileFullName, null, "ファイルパス", true, false, 0, 700, 20);
                map.AddTBString(FrmAttachmentDBAttr.FileName, null, "名前", true, false, 0, 500, 20);
                map.AddTBString(FrmAttachmentDBAttr.FileExts, null, "拡張", true, false, 0, 50, 20);
                map.AddTBFloat(FrmAttachmentDBAttr.FileSize, 0, "ファイルサイズ", true, false);

                map.AddTBDateTime(FrmAttachmentDBAttr.RDT, null, "記録日", true, false);
                map.AddTBString(FrmAttachmentDBAttr.Rec, null, "記録者", true, false, 0, 50, 20);
                map.AddTBString(FrmAttachmentDBAttr.RecName, null, "レコーダーの名前", true, false, 0, 50, 20);
                map.AddTBString(FrmAttachmentDBAttr.FK_Dept, null, "部門", true, false, 0, 50, 20);
                map.AddTBString(FrmAttachmentDBAttr.FK_DeptName, null, "部署名", true, false, 0, 50, 20);
                map.AddTBStringDoc(FrmAttachmentDBAttr.MyNote, null, "備考", true, false);

                map.AddTBInt(FrmAttachmentDBAttr.IsRowLock, 0, "行をロックするかどうか", true, false);

                //顺序.
                map.AddTBInt(FrmAttachmentDBAttr.Idx, 0, "ソート", true, false);

                //这个值在上传时候产生.
                map.AddTBString(FrmAttachmentDBAttr.UploadGUID, null, "GUIDをアップロード", true, false, 0, 500, 20);
        ...
        public string GenerTempFile(AthSaveWay saveWay= Sys.AthSaveWay.IISServer)
        {
            ...
            if (saveWay == Sys.AthSaveWay.DB)
                throw new Exception("@データベースに保存されているファイルは処理されていません。");
