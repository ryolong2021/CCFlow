    ...
    public class FrmImg : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBString(MapAttrAttr.KeyOfEn, null, "対応するフィールド", true, false, 1, 100, 20);
                ...
                //如果是 seal 就是岗位集合。
                map.AddTBString(FrmImgAttr.Tag0, null, "パラメータ", true, false, 0, 500, 20);
                map.AddTBInt(FrmImgAttr.ImgSrcType, 0, "画像ソース0=ローカル、1=URL", true, false);
                map.AddTBInt(FrmImgAttr.IsEdit, 0, "編集できるかどうか", true, false);
                map.AddTBString(FrmImgAttr.Name, null, "日本語の名前", true, false, 0, 500, 20);
                map.AddTBString(FrmImgAttr.EnPK, null, "英語の名前", true, false, 0, 500, 20);
                map.AddTBInt(MapAttrAttr.ColSpan, 0, "セルの数", false, true);
                map.AddTBInt(MapAttrAttr.TextColSpan, 1, "テキストセルの数", false, true);
                map.AddTBInt(MapAttrAttr.RowSpan, 1, "行数", false, true);

                //显示的分组.
                map.AddDDLSQL(MapAttrAttr.GroupID, 0, "表示されたグループ",BP.Sys.FrmUI.MapAttrString.SQLOfGroupAttr, true);
