    ...
    public class FrmRB : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("Sys_FrmRB", "ラジオボタン");
          //      map.EnDBUrl = new DBUrl(DBUrlType.DBAccessOfMSSQL1);

                map.IndexField = FrmImgAthDBAttr.FK_MapData ; 

                map.AddMyPK();
                map.AddTBString(FrmRBAttr.FK_MapData, null, "フォームID", true, false, 0, 300, 20);
                map.AddTBString(FrmRBAttr.KeyOfEn, null, "フィールド", true, false, 0, 300, 20);
                map.AddTBString(FrmRBAttr.EnumKey, null, "列挙値", true, false, 0, 30, 20);
                map.AddTBString(FrmRBAttr.Lab, null, "ラベル", true, false, 0, 500, 20);
                map.AddTBInt(FrmRBAttr.IntKey, 0, "IntKey", true, false);

                map.AddTBInt(MapAttrAttr.UIIsEnable, 0, "有効にする要否", true, false);

                map.AddTBFloat(FrmRBAttr.X, 5, "X", true, false);
                map.AddTBFloat(FrmRBAttr.Y, 5, "Y", false, false);

                //要执行的脚本.
                map.AddTBString(FrmRBAttr.Script, null, "実行するスクリプト", true, false, 0, 4000, 20);
                map.AddTBString(FrmRBAttr.FieldsCfg, null, "構成情報@FieldName=Sta", true, false, 0, 4000, 20);
                map.AddTBString(FrmRBAttr.SetVal, null, "設定値", true, false, 0, 200, 20);

                map.AddTBString(FrmRBAttr.Tip, null, "選択後の提示", true, false, 0, 1000, 20);
