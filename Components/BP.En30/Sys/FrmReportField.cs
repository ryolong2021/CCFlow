    ...
    public class FrmReportField : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("Sys_FrmRePortField", "フォームレポート");
                map.Java_SetDepositaryOfEntity(Depositary.None);
                map.Java_SetDepositaryOfMap( Depositary.Application);
                map.Java_SetEnType(EnType.Sys);
                map.AddMyPK();
                map.AddTBString(FrmReportFieldAttr.FK_MapData, null, "フォーム番号", true, false, 1, 100, 80);
                map.AddTBString(FrmReportFieldAttr.KeyOfEn, null, "フィールド名", true, false,1,100,100);
                map.AddTBString(FrmReportFieldAttr.Name, null, "日本名を表示する", true, false, 1, 200, 200);
                map.AddTBString(FrmReportFieldAttr.UIWidth, "0", "幅", true, false, 1, 100, 100);
                map.AddBoolean(FrmReportFieldAttr.UIVisible, true, "表示要否", true, true);
                map.AddTBInt(FrmReportFieldAttr.Idx, 0, "表示順", true, false);
                this._enMap = map;
                return this._enMap;
            }
        }
