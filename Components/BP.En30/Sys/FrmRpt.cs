    ...
    public class FrmRpt : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("Sys_FrmRpt", "緯度レポート");
                map.Java_SetDepositaryOfEntity(Depositary.None);
                map.Java_SetDepositaryOfMap( Depositary.Application);
                ...
                map.AddTBStringPK(FrmRptAttr.No, null, "ナンバリング", true, false, 1, 20, 20);
                map.AddTBString(FrmRptAttr.Name, null, "記述", true, false, 1, 50, 20);
                map.AddTBString(FrmRptAttr.FK_MapData, null, "マスタ", true, false, 0, 100, 20);
                map.AddTBString(FrmRptAttr.PTable, null, "物理的なテーブル", true, false, 0, 30, 20);

                map.AddTBString(FrmRptAttr.SQLOfColumn, null, "列のデータソース", true, false, 0, 300, 20);
                map.AddTBString(FrmRptAttr.SQLOfRow, null, "行のデータソース", true, false, 0, 300, 20);
                ...
                map.AddBoolean(FrmRptAttr.IsHLDtl, false, "合流するかどうか", false, false);
                ...
                map.AddBoolean(FrmRptAttr.IsView, true, "見えるかどうか", false, false);
                ...
                map.AddBoolean(FrmRptAttr.IsEnablePass, false, "監査機能を有効かどうか", false, false);
                map.AddBoolean(FrmRptAttr.IsEnableAthM, false, "複数の添付ファイルを有効にするかどうか", false, false);

                map.AddBoolean(FrmRptAttr.IsEnableM2M, false, "M2Mを有効にするかどうか", false, false);
                map.AddBoolean(FrmRptAttr.IsEnableM2MM, false, "M2Mを有効にするかどうか", false, false);

                map.AddDDLSysEnum(FrmRptAttr.WhenOverSize, 0, "WhenOverSize", true, true,
                 FrmRptAttr.WhenOverSize, "@0=処理しない@1=下に行を増やす@2 =次のページを表示");

                map.AddDDLSysEnum(FrmRptAttr.DtlOpenType, 1, "データオープンタイプ", true, true,
                    FrmRptAttr.DtlOpenType, "@0=オペレーター @1 =ワークID @2 =フローID");

                map.AddDDLSysEnum(FrmRptAttr.EditModel, 0, "フォーマットを表示する", true, true,
               FrmRptAttr.EditModel, "@0=フォーム @1 =カード");
                ...
                //MTR 多表头列.
                map.AddTBString(FrmRptAttr.MTR, null, "複数のヘッダー列", true, false, 0, 3000, 20);
        ...
        protected override bool beforeUpdateInsertAction()
        {
            if (this.IsEnablePass)
            {
                ...
                if (attrs.Contains(MapAttrAttr.KeyOfEn, "IsPass") == false)
                    throw new Exception("フォームから有効にしました(" + this.Name + ")データレビューオプションですが、テーブルにIsPassフィールドがありません。ヘルプファイルを参照してください。");
            }
