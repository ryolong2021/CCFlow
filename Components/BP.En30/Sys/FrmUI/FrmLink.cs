    ...
    public class FrmLink : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("Sys_FrmLink", "ハイパーリンク");
                ...
                //连接目标.
                map.AddMyPK();
                map.AddTBString(FrmLinkAttr.Target, "_blank", "接続ターゲット(_blank,_parent,_self)", true, false, 0, 20, 20);
                map.AddTBString(FrmLinkAttr.Text, "New Link", "ラベル", true, false, 0, 500, 20, true);
                map.AddTBString(FrmLinkAttr.URL, null, "URL", true, false, 0, 500, 20, true);
                map.AddTBString(FrmLinkAttr.FK_MapData, null, "フォームID", false, false, 1, 100, 20);

                //显示的分组.
                map.AddDDLSQL(MapAttrAttr.GroupID, 0, "表示されたグループ",
