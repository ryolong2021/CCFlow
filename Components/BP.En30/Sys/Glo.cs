    ...
    public class Glo
    {
        ...
        public static BuessUnitBase GetBuessUnitEntityByEnName(string enName)
        {
            ...
            if (myen == null)
            {
                ...
                BP.DA.Log.DefaultLogWriteLineError("@クラス名に基づいてビジネスユニットインスタンスを取得するときにエラーが発生しました:" + enName + ",このタイプのエンティティは見つかりませんでした.");
                return null;
            }
            return myen;
        }
        ...
        public static void InitEntityAthInfo(BP.En.Entity en)
        {
            ...
            if (path == "" || path == null || path == string.Empty)
                path = BP.Sys.SystemConfig.PathOfDataUser + en.ToString() + Path.DirectorySeparatorChar;
            ...
            string reldir = path;
            string tmp = Path.DirectorySeparatorChar + "datauser" + Path.DirectorySeparatorChar;
            if (reldir.Length > SystemConfig.PathOfDataUser.Length)
                reldir =
                    reldir.Substring(reldir.ToLower().IndexOf(tmp) + tmp.Length).Replace(
                        @"\", "/");
            else
                reldir = "";
            ...
            string fullFile = path + Path.DirectorySeparatorChar + en.PKVal + "." + ext;
