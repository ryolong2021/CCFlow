	...
	public class GroupEnsTemplate: EntityOID
	{
		...
		public override Map EnMap
		{
			get
			{
				if (this._enMap!=null) 
					return this._enMap;
                Map map = new Map("Sys_GroupEnsTemplate", "レポートテンプレート");
				map.DepositaryOfEntity=Depositary.None;
				map.EnType=EnType.Sys;

				map.AddTBIntPKOID();
				map.AddTBString(GroupEnsTemplateAttr.EnName,null,"表現",false,false,0,500,20);
				map.AddTBString(GroupEnsTemplateAttr.Name,null,"レポート名",true,false,0,500,20);
				map.AddTBString(GroupEnsTemplateAttr.EnsName,null,"レポートクラス名",false,true,0,90,10);
				map.AddTBString(GroupEnsTemplateAttr.OperateCol,null,"操作属性",false,true,0,90,10);
				map.AddTBString(GroupEnsTemplateAttr.Attrs,null,"運用属性",false,true,0,90,10);
				map.AddTBString(GroupEnsTemplateAttr.Rec,null,"記録者",false,true,0,90,10);
				this._enMap=map;
				return this._enMap;
			}
		}
