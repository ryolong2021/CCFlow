    ...
    public class GroupField : EntityOID
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("Sys_GroupField", "簡易フォームグループ化");
                map.Java_SetDepositaryOfEntity(Depositary.None);
                map.Java_SetDepositaryOfMap( Depositary.Application);
                map.Java_SetEnType(EnType.Sys);

                map.AddTBIntPKOID();
                map.AddTBString(GroupFieldAttr.Lab, null, "ラベル", true, false, 0, 500, 20,true);
                map.AddTBString(GroupFieldAttr.FrmID, null, "フォームID", true, true, 0, 200, 20);

                map.AddTBString(GroupFieldAttr.CtrlType, null, "制御タイプ", true, true, 0, 50, 20);
                map.AddTBString(GroupFieldAttr.CtrlID, null, "コントロールID", true, true, 0, 500, 20);
                map.AddTBInt(GroupFieldAttr.Idx, 99, "シーケンス番号", true, false);
                ...
                rm = new RefMethod();
                rm.Title = "グループに属するフィールドを削除します";
                rm.Warning = "このグループのすべてのフィールドを削除してもよろしいですか？";
                rm.ClassMethodName = this.ToString() + ".DoDelAllField";
                rm.RefMethodType = RefMethodType.Func;
                map.AddRefMethod(rm);

                rm = new RefMethod();
                rm.Title = "フィールドの順序を調整する";
