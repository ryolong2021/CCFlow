            ...
            get
            {
                if (_dt == null)
                {
                    if (DataType.IsNullOrEmpty(this.UIBindKey))
                        throw new Exception("@属性：" + this.MyPK + " 属性UIBindKeyフィールドがありません。");
            ...
            get
            {
                ...
                switch (s.ToLower())
                {
                    ...
                    case "@yyyy年MM月dd日hh時mm分":
                        return DateTime.Now.ToString("yyyy年MM月dd日HH時mm分");
                    case "@yy年MM月dd日":
                        return DataType.CurrentDataCNOfShort;
                    case "@yy年MM月dd日hh時mm分":
                        return DateTime.Now.ToString("yy年MM月dd日HH時mm分");
                    default:
                        return s;
     ...
     /// <summary>
                ...
                switch (this.MyDataType)
                {
                    case DataType.AppString:
                        if (this.UIVisible == false)
                            return "長さ" + this.MinLen + "-" + this.MaxLen + "見えない";
                        else
                            return "長さ" + this.MinLen + "-" + this.MaxLen;
                    case DataType.AppDate:
                    case DataType.AppDateTime:
                    case DataType.AppInt:
                    case DataType.AppFloat:
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("Sys_MapAttr", "エンティティの属性");
                ...
                map.AddTBString(MapAttrAttr.FK_MapData, null, "エンティティID", true, true, 1, 100, 20);
                map.AddTBString(MapAttrAttr.KeyOfEn, null, "属性", true, true, 1, 200, 20);

                map.AddTBString(MapAttrAttr.Name, null, "記述", true, false, 0, 200, 20);
                map.AddTBString(MapAttrAttr.DefVal, null, "デフォルト", false, false, 0, 400, 20);
                map.AddTBInt(MapAttrAttr.DefValType, 0, "デフォルト値タイプ", true, false);

                map.AddTBInt(MapAttrAttr.UIContralType, 0, "コントロール", true, false);
                map.AddTBInt(MapAttrAttr.MyDataType, 1, "データのタイプ", true, false);

                map.AddDDLSysEnum(MapAttrAttr.LGType, 0, "論理型", true, false, MapAttrAttr.LGType, 
                    "@0=通常@1=列挙@2=外部キー@3=システムページを開く");
                ...
                map.AddTBInt(MapAttrAttr.MaxLen, 300, "最大長さ", true, false);

                map.AddTBString(MapAttrAttr.UIBindKey, null, "バインドされた情報", true, false, 0, 100, 20);
                map.AddTBString(MapAttrAttr.UIRefKey, null, "バインドされたキー", true, false, 0, 30, 20);
                map.AddTBString(MapAttrAttr.UIRefKeyText, null, "バインドされたテキスト", true, false, 0, 30, 20);


                map.AddTBInt(MapAttrAttr.ExtIsSum, 0, "合計を表示するかどうか（スレーブテーブルで有効）", true, true);
                map.AddTBInt(MapAttrAttr.UIVisible, 1, "見えるかどうか", true, true);
                map.AddTBInt(MapAttrAttr.UIIsEnable, 1, "有効にするかどうか", true, true);
                map.AddTBInt(MapAttrAttr.UIIsLine, 0, "別の列に表示するかどうか", true, true);
                map.AddTBInt(MapAttrAttr.UIIsInput, 0, "必須フィールドかどうか", true, true);

                map.AddTBInt(MapAttrAttr.IsRichText, 0, "リッチテキスト", true, true);
                map.AddTBInt(MapAttrAttr.IsSupperText, 0, "リッチテキスト", true, true);
                map.AddTBInt(MapAttrAttr.FontSize, 0, "リッチテキスト", true, true);

                // 是否是签字，操作员字段有效。2010-09-23 增加。 @0=无@1=图片签名@2=CA签名.
                map.AddTBInt(MapAttrAttr.IsSigan, 0, "署名？", true, false);
                ...
                map.AddTBInt(MapAttrAttr.EditType, 0, "編集タイプ", true, false);

                map.AddTBString(MapAttrAttr.Tag, null, "ロゴ", true, false, 0, 100, 20);
                map.AddTBString(MapAttrAttr.Tag1, null, "ロゴ1", true, false, 0, 100, 20);
                map.AddTBString(MapAttrAttr.Tag2, null, "ロゴ2", true, false, 0, 100, 20);
                map.AddTBString(MapAttrAttr.Tag3, null, "ロゴ3", true, false, 0, 100, 20);


                map.AddTBString(MapAttrAttr.Tip, null, "アクティベーション提示", false, true, 0, 200, 20);

                //单元格数量。2013-07-24 增加。
              //  map.AddTBString(MapAttrAttr.ColSpan, "1", "单元格数量", true, false, 0, 3, 3);
                map.AddTBInt(MapAttrAttr.ColSpan, 1, "セルの数", true, false);

                //文本占单元格数量
                map.AddTBInt(MapAttrAttr.TextColSpan, 1, "テキストセルの数", true,false);
                ...
                //显示的分组. @sly.
                map.AddTBInt(MapAttrAttr.GroupID, 1, "表示されたグループ", true, false);

                map.AddBoolean(MapAttrAttr.IsEnableInAPP, true, "携帯端末に表示するかどうか", true, true);
                map.AddTBInt(MapAttrAttr.Idx, 0, "シリアルナンバー", true, false);
                map.AddTBString(MapAttrAttr.CSS, "0", "カスタムスタイル", true, false, 1, 100, 20);
        ...
        {
            string file = SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "CCForm" + Path.DirectorySeparatorChar + "BigNoteHtmlText" + Path.DirectorySeparatorChar + this.FK_MapData + ".htm";
            //若文件夹不存在，则创建
        ...
        protected override bool beforeInsert()
        {
            if (DataType.IsNullOrEmpty(this.Name))
                throw new Exception("@フィールド名を入力してください。");
            ...
            if (this.KeyOfEn == null || this.KeyOfEn.Trim() == "")
            {
                try
                {
                    ...
                    if (this.KeyOfEn == null || this.KeyOfEn.Trim() == "")
                        throw new Exception("@フィールドの説明またはフィールド名を入力してください。");
                }
                catch (Exception ex)
                {
                    throw new Exception("@フィールドの説明またはフィールド名を入力してください、例外情報:" + ex.Message);
                }
            }
            else
            {
            ...
            if (PubClass.KeyFields.Contains("," + keyofenC + ",") == true)
                throw new Exception("@エラー：[" + this.KeyOfEn + "]フィールドキーであり、フィールドとして使用できません。");

            if (this.IsExit(MapAttrAttr.KeyOfEn, this.KeyOfEn,
                MapAttrAttr.FK_MapData, this.FK_MapData))
            {
                return false;
                throw new Exception("@[" + this.MyPK + "]にフィールド名はすでに存在します[" + this.Name + "]フィールド[" + this.KeyOfEn + "]");
            }
