...
namespace BP.Sys
{
    ...
    public class MapDtl : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBString(MapDtlAttr.Name, null, "記述", true, false, 1, 200, 20);
                map.AddTBString(MapDtlAttr.Alias, null, "アリアス", true, false, 1, 200, 20);
                map.AddTBString(MapDtlAttr.FK_MapData, null, "マスタ", true, false, 0, 100, 20);
                map.AddTBString(MapDtlAttr.PTable, null, "物理的なテーブル", true, false, 0, 200, 20);
               // map.AddTBInt(MapDtlAttr.PTableModel, 0, "物理表的保存方式", false, false);

                map.AddTBString(MapDtlAttr.GroupField, null, "グループ化フィールド", true, false, 0, 300, 20);
                map.AddTBString(MapDtlAttr.RefPK, null, "関連する主キー", true, false, 0, 100, 20);

                // 为明细表初始化事件类.
                map.AddTBString(MapDtlAttr.FEBD, null, "マップされたイベントエンティティクラス", true, false, 0, 100, 20);

                //map.AddTBInt(MapDtlAttr.Model, 0, "工作模式", false, false);
                map.AddDDLSysEnum(MapDtlAttr.Model, 0, "作業モード", true, true,
                 MapDtlAttr.Model, "@0=正常 @1=固定行");

                map.AddTBInt(MapDtlAttr.DtlVer, 0, "使用的なバージョン", false, false);
               // map.AddDDLSysEnum(MapDtlAttr.DtlVer, 0, "使用版本", true, true, MapDtlAttr.DtlVer, "@0=2017传统版@1=2019EasyUI版本");


                map.AddTBInt(MapDtlAttr.RowsOfList, 6, "初期化される行数", false, false);



                map.AddBoolean(MapDtlAttr.IsEnableGroupField, false, "グループ化フィールドを有効にするかどうか", false, false);

                map.AddBoolean(MapDtlAttr.IsShowSum, true, "合計を表示するかどうか", false, false);
                map.AddBoolean(MapDtlAttr.IsShowIdx, true, "シリアル番号は表示されてるかどうか", false, false);
                map.AddBoolean(MapDtlAttr.IsCopyNDData, true, "データのコピーを許可するかどうか", false, false);
                map.AddBoolean(MapDtlAttr.IsHLDtl, false, "合流かどうか", false, false);

                map.AddBoolean(MapDtlAttr.IsReadonly, false, "読み取り専用かどうか", false, false);
                map.AddBoolean(MapDtlAttr.IsShowTitle, true, "タイトルを表示するかどうか", false, false);
                map.AddBoolean(MapDtlAttr.IsView, true, "見えるかどうか", false, false);

                map.AddBoolean(MapDtlAttr.IsInsert, true, "行を挿入することは可能かどうか", false, false);
                map.AddBoolean(MapDtlAttr.IsDelete, true, "行を削除することは可能かどうか", false, false);
                map.AddBoolean(MapDtlAttr.IsUpdate, true, "更新できるかどうか", false, false);

                map.AddBoolean(MapDtlAttr.IsEnablePass, false, "監査機能は有効かどうか", false, false);
                map.AddBoolean(MapDtlAttr.IsEnableAthM, false, "複数の添付ファイルを有効にするかどうか", false, false);

                map.AddBoolean(MapDtlAttr.IsEnableM2M, false, "M2Mを有効にするかどうか", false, false);
                map.AddBoolean(MapDtlAttr.IsEnableM2MM, false, "M2Mを有効にするかどうか", false, false);


                // 超出行数
                map.AddTBInt(MapDtlAttr.WhenOverSize, 0, "リストデータ表示形式", false, false);

                //数据开放类型 .
                map.AddTBInt(MapDtlAttr.DtlOpenType, 1, "データオープンタイプ", false, false);

                map.AddTBInt(MapDtlAttr.ListShowModel, 0, "リストデータ表示形式", false, false);
                map.AddTBInt(MapDtlAttr.EditModel, 0, "行データ表示形式", false, false);

                map.AddTBInt(MapDtlAttr.MobileShowModel, 0, "モバイルデータ表示フォーマット", false, false);
                map.AddTBString(MapDtlAttr.MobileShowField, null, "モバイルリスト表示されるフィールド", true, false, 0, 100, 20);

                map.AddTBFloat(MapDtlAttr.X, 5, "左から", true, false);
                map.AddTBFloat(MapDtlAttr.Y, 5, "上から", false, false);

                map.AddTBFloat(MapDtlAttr.H, 150, "高さ", true, false);
                map.AddTBFloat(MapDtlAttr.W, 200, "幅", false, false);

                map.AddTBFloat(MapDtlAttr.FrmW, 900, "フォーム幅", true, true);
                map.AddTBFloat(MapDtlAttr.FrmH, 1200, "フォームの高さ", true, true);
                ...
                //SQL过滤表达式.
                map.AddTBString(MapDtlAttr.FilterSQLExp, null, "SQL式のフィルター", true, false, 0, 200, 20, true);
                ...
                map.AddTBInt(MapDtlAttr.ImpModel, 0, "インポートルール", false, false);
               ...
               // map.AddBoolean(MapDtlAttr.IsEnableSelectImp, false, "是否启用选择数据导入?", false, false);
                map.AddTBString(MapDtlAttr.ImpSQLSearch, null, "クエリSQL", true, false, 0, 500, 20);
                map.AddTBString(MapDtlAttr.ImpSQLInit, null, "SQLを初期化する", true, false, 0, 500, 20);
                map.AddTBString(MapDtlAttr.ImpSQLFullOneRow, null, "SQLを埋めるデータ", true, false, 0, 500, 20);
                map.AddTBString(MapDtlAttr.ImpSQLNames, null, "フィールドの日本語名", true, false, 0, 900, 20);
                map.AddBoolean(MapDtlAttr.IsImp, false, "IsImp", true, true);
                #endregion 导入导出填充.


                //列自动计算表达式.
                map.AddTBString(MapDtlAttr.ColAutoExp, null, "列自動計算式", true, false, 0, 200, 20, true);
        ...
        protected override bool beforeUpdateInsertAction()
        {
            ...
            if (this.IsRowLock == true)
            {
                ...
                if (attrs.Contains(MapAttrAttr.KeyOfEn, "IsRowLock") == false)
                    throw new Exception("フォーム(" + this.Name + ")から行データロックを有効にしましたが、テーブルにIsRowLockフィールドがありません。ヘルプファイルを参照してください。");
            }
            ...
            if (this.IsEnablePass)
            {
                ...
                if (attrs.Contains(MapAttrAttr.KeyOfEn, "IsPass") == false)
                    throw new Exception("フォーム(" + this.Name + ")からデータレビューオプションを有効にしましたが、テーブルにIsPassフィールドがありません。ヘルプファイルを参照してください。");
            }
    ...
    public class MapDtls : EntitiesNoName
    {
        ...
        {
            if (fk_mapdata == null)
                throw new Exception("fk_mapdata 渡された値は空であり、照会できません。");
