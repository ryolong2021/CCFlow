...
namespace BP.Sys
{
    ...
    public class MapExt : EntityMyPK
    {
        ...
        public string PopValSearchTip
        {
            get
            {
                return this.GetParaString("PopValSearchTip", "キーワードを入力してください");
            }
            set
            {
            ...
            get
            {
                ...
                switch (this.ExtType)
                {
                    ...
                    case MapExtXmlList.InputCheck:
                        dec += "フィールド：" + this.AttrOfOper + " 内容をチェック：" + this.Tag1;
                        break;
                    case MapExtXmlList.PopVal:
                        dec += "フィールド：" + this.AttrOfOper + " Url：" + this.Tag;
                        break;
                    default:
                        break;
                }
       ...
       /// <returns></returns>
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("Sys_MapExt","ビジネスのロジック");
                ...
                map.AddTBString(MapExtAttr.FK_MapData, null, "マスタ", true, false, 0, 100, 20);
                map.AddTBString(MapExtAttr.ExtType, null, "種類", true, false, 0, 30, 20);

                map.AddTBInt(MapExtAttr.DoWay, 0, "実行方法", true, false);

                map.AddTBString(MapExtAttr.AttrOfOper, null, "運用属性", true, false, 0, 30, 20);
                map.AddTBString(MapExtAttr.AttrsOfActive, null, "アクティブフィールド", true, false, 0, 900, 20);
                ...
                map.AddTBInt(MapExtAttr.H, 500, "高さ", false, false);
                map.AddTBInt(MapExtAttr.W, 400, "幅", false, false);

                // 数据类型 @0=SQL@1=URLJSON@2=FunctionJSON.
                map.AddTBInt(MapExtAttr.DBType, 0, "データのタイプ", true, false);
                map.AddTBString(MapExtAttr.FK_DBSrc, null, "データソース", true, false, 0, 100, 20);

                // add by stone 2013-12-21 计算的优先级,用于js的计算. 
                // 也可以用于 字段之间的计算 优先级.
                map.AddTBInt(MapExtAttr.PRI, 0, "PRI/シーケンス番号", false, false);
                map.AddTBString(MapExtAttr.AtPara, null, "パラメータ", true, false, 0, 3999, 20);
            ...
            switch (this.ExtType)
            {
                ...
                case MapExtXmlList.TBFullCtrl:
                    if (this.Doc.Contains("@Key") == false)
                        throw new Exception("@SQL式が間違っています。このキーワードである@Keyを含める必要があります。");
                    break;
                case MapExtXmlList.AutoFullDLL:
                    if (this.Doc.Length <= 13)
                        throw new Exception("@SQL式を入力する必要があります。");
                    break;
                case MapExtXmlList.AutoFull:
                    if (this.Doc.Length <= 3)
                        throw new Exception("式は入力する必要があります。たとえば、@ unit price; * @ quantity;");
                    break;
                case MapExtXmlList.PopVal:
                    break;
                default:
