    ...
    public class PubClass
    {
        ...
        public static void InitFrm(string fk_mapdata)
        {
            ...
            if (md.RetrieveFromDBSources() == 0)
            {
                ...
                {
                    throw new Exception("@これ:" + fk_mapdata + "にマッピング情報は存在しません.");
                }
                else
                {
                    md.Copy(mdtl);
        ...
        public static string KeyFields
        {
            ...
            {
                if (_KeyFields == null)
                    _KeyFields = BP.DA.DataType.ReadTextFile(SystemConfig.PathOfWebApp + SystemConfig.CCFlowWebPath + "WF" + Path.DirectorySeparatorChar + "Data" + Path.DirectorySeparatorChar + "Sys" + Path.DirectorySeparatorChar + "FieldKeys.txt");
                return _KeyFields;
            }
        }
        ...
        public static System.Data.DataTable GetDataTableByUIBineKey(string uiBindKey,Hashtable ht=null)
        {
            ...
            if (uiBindKey.Contains("."))
            {
                ...
                if (ens == null)
                    ens = BP.En.ClassFactory.GetEns(uiBindKey);
                if (ens == null)
                    throw new Exception("クラス名エラー:" + uiBindKey + ",ensに変換できません。");
        ...
        public static string DBRpt(DBCheckLevel level)
        {
            ...
            foreach (object obj in als)
            {
                ...
                {
                    msg += "<hr>" + ens.ToString() + "健康診断に失敗しました:" + ex.Message;
                }
            }
            ...
            foreach (MapData md in mds)
            {
                ...
                {
                    msg += "<hr>" + md.No + "健康診断に失敗しました:" + ex.Message;
                }
            }
            ...
            foreach (MapDtl dtl in dtls)
            {
                ...
                {
                    msg += "<hr>" + dtl.No + "健康診断に失敗しました:" + ex.Message;
                }
            }
        ...
        public static string AddComment(Entity en)
        {
            ...
            if (en == null)
                return "エンティティエラー en=null ";
            if (en.EnMap == null)
                return "エンティティエラーen.getEnMap=null ";
            if (en.EnMap.PhysicsTable == null)
                return "エンティティエラーen.getEnMap.getPhysicsTable=null";


            if (DBAccess.IsExitsObject(en.EnMap.PhysicsTable) == false)
                return "エンティティテーブルが存在しません。";
            ...
            {
                return "<hr>" + en.ToString() + "健康診断に失敗しました:" + ex.Message;
            }
        }
        private static void AddCommentForTable_Ora(Entity en)
        {
            ...
            foreach (Attr attr in en.EnMap.Attrs)
            {
                ...
                {
                    case FieldType.PK:
                        en.RunSQL("comment on column  " + en.EnMap.PhysicsTable + "." + attr.Field + " IS '" + attr.Desc + "-主キー'");
                        break;
                    ...
                    case FieldType.Enum:
                        ...
                        en.RunSQL("comment on column  " + en.EnMap.PhysicsTable + "." + attr.Field + " IS '" + attr.Desc + ",列挙型:" + ses.ToDesc() + "'");
                        break;
                    case FieldType.PKEnum:
                        ses = new SysEnums(attr.Key, attr.UITag);
                        en.RunSQL("comment on column  " + en.EnMap.PhysicsTable + "." + attr.Field + " IS '" + attr.Desc + ", 主キー：列挙型:" + ses.ToDesc() + "'");
                        break;
                    case FieldType.FK:
                        Entity myen = attr.HisFKEn; // ClassFactory.GetEns(attr.UIBindKey).GetNewEntity;
                        en.RunSQL("comment on column  " + en.EnMap.PhysicsTable + "." + attr.Field + " IS " + attr.Desc + ", 外部キー：物理テーブルに対応:" + myen.EnMap.PhysicsTable + ",テーブルの説明:" + myen.EnDesc);
                        break;
                    case FieldType.PKFK:
                        Entity myen1 = attr.HisFKEn; // ClassFactory.GetEns(attr.UIBindKey).GetNewEntity;
                        en.RunSQL("comment on column  " + en.EnMap.PhysicsTable + "." + attr.Field + " IS '" + attr.Desc + ", 主外部キー：対応する物理テーブル:" + myen1.EnMap.PhysicsTable + ",テーブルの説明:" + myen1.EnDesc + "'");
                        break;
                    default:
                        break;
                }
        ...
        private static void AddCommentForTable_MySql(Entity en)
        {
            ...
            foreach (Attr attr in en.EnMap.Attrs)
            {
                ...
                {
                    case FieldType.PK:
                        en.RunSQL(sql + string.Format("'{0} - 主キー'", attr.Desc));
                        break;
                    case FieldType.Normal:
                    ...
                    case FieldType.Enum:
                        ses = new SysEnums(attr.Key, attr.UITag);
                        en.RunSQL(sql + string.Format("'{0},列挙型：{1}'", attr.Desc, ses.ToDesc()));
                        break;
                    case FieldType.PKEnum:
                        ses = new SysEnums(attr.Key, attr.UITag);
                        en.RunSQL(sql + string.Format("'{0},主キー：列挙型：{1}'", attr.Desc, ses.ToDesc()));
                        break;
                    case FieldType.FK:
                        Entity myen = attr.HisFKEn; // ClassFactory.GetEns(attr.UIBindKey).GetNewEntity;
                        en.RunSQL(sql +
                                  string.Format("'{0},外部キー：対応する物理テーブル：{1}、テーブルの説明：{2} '", attr.Desc, myen.EnMap.PhysicsTable,
                                                myen.EnDesc));
                        break;
                    case FieldType.PKFK:
                        Entity myen1 = attr.HisFKEn; // ClassFactory.GetEns(attr.UIBindKey).GetNewEntity;
                        en.RunSQL(sql +
                                  string.Format("'{0},外部キー：対応する物理テーブル：{1}、テーブルの説明：{2} '", attr.Desc, myen1.EnMap.PhysicsTable,
                                                myen1.EnDesc));
                        break;
                    default:
                        break;
        ...
        public static void AddCommentForTable_MS(Entity en)
        {
            ...
            foreach (Attr attr in en.EnMap.Attrs)
            {
                ...
                {
                    case FieldType.PK:
                        AddColNote(en, en.EnMap.PhysicsTable, attr.Field, attr.Desc+"(主キー)");
                    ...
                    case FieldType.Enum:
                        ...
                        AddColNote(en, en.EnMap.PhysicsTable, attr.Field, attr.Desc + ",列挙型:" + ses.ToDesc());
                        break;
                    case FieldType.PKEnum:
                        ses = new SysEnums(attr.Key, attr.UITag);
                        AddColNote(en, en.EnMap.PhysicsTable, attr.Field, attr.Desc + ",主キー：列挙型:" + ses.ToDesc());
                        //en.RunSQL("comment on table "+ en.EnMap.PhysicsTable+"."+attr.Field +" IS '"+en.EnDesc+", 主キー：列挙型:"+ses.ToDesc()+"'" );
                        break;
                    case FieldType.FK:
                        Entity myen = attr.HisFKEn; // ClassFactory.GetEns(attr.UIBindKey).GetNewEntity;
                        AddColNote(en, en.EnMap.PhysicsTable, attr.Field, attr.Desc + ", 外部キー：物理テーブルに対応:" + myen.EnMap.PhysicsTable + ",テーブルの説明:" + myen.EnDesc);
                        //en.RunSQL("comment on table "+ en.EnMap.PhysicsTable+"."+attr.Field +" IS "+  );
                        break;
                    case FieldType.PKFK:
                        Entity myen1 = attr.HisFKEn; // ClassFactory.GetEns(attr.UIBindKey).GetNewEntity;
                        AddColNote(en, en.EnMap.PhysicsTable, attr.Field, attr.Desc + ", 主外部キー：対応する物理テーブル:" + myen1.EnMap.PhysicsTable + ",テーブルの説明:" + myen1.EnDesc);
                        //en.RunSQL("comment on table "+ en.EnMap.PhysicsTable+"."+attr.Field +" IS '"+  );
                        break;
                    default:
                        break;
        ...
        public static string DBRpt1(DBCheckLevel level, Entities ens)
        {
            ...
            foreach (Attr attr in fkAttrs)
            {
                ...
                try
                {
                    ...
                    {
                        //报告错误信息
                        enMsg += "<br>@エンティティをチェック：" + en.EnDesc + ",フィールド" + attr.Key + " , フィールドの説明:" + attr.Desc + " , 外部キー物理テーブル:" + reftable + "存在しません:" + sql;
                    }
                    else
                    {
                    ...
                    {
                        enMsg += "<br>@エンティティをチェック：" + en.EnDesc + ",物理的なテーブル:" + en.EnMap.PhysicsTable + "現れる" + attr.Key + "," + attr.Desc + "不正、共有[" + num + "]行レコードのデータなし。" + sql;
                    }
                    #endregion
                    ...
                    else
                    {
                        ...
                        enMsg += "<br>@" + en.EnDesc + ",物理的なテーブル:" + en.EnMap.PhysicsTable + "現れる" + attr.Key + "," + attr.Desc + "不正、共有[" + num + "]行の記録はデータに関連していません。物理的なテ-ブルと外キー表を確認してください。" + sql + "これらの対応しないデータを削除する場合は、次のSQLを実行してください。" + delsql + " 慎重に実行してください。";
                    }
                    #endregion
                ...
                if (enMsg != "")
                {
                    msg += "<BR><b>-- チェック[" + en.EnDesc + "," + en.EnMap.PhysicsTable + "]以下の問題が発生します、クラス名:" + en.ToString() + "</b>";
                    msg += enMsg;
                }
            }
            return msg;
