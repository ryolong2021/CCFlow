	...
	public class RptTemplate: Entity
	{
		...
		public override Map EnMap
		{
			get
			{
				if (this._enMap!=null) 
					return this._enMap;
                Map map = new Map("Sys_RptTemplate", "レポートテンプレート");
				map.DepositaryOfEntity=Depositary.Application;
				map.EnType=EnType.Sys;

                map.AddMyPK();
				map.AddTBString(RptTemplateAttr.EnsName,null,"クラス名",false,false,0,500,20);
                map.AddTBString(RptTemplateAttr.FK_Emp, null, "オペレーター", true, false, 0, 20, 20);
				...
				map.AddTBString(RptTemplateAttr.AlObjs,null,"分析対象",false,true,0,90,10);

				map.AddTBInt(RptTemplateAttr.Height,600,"Height",false,true);
				map.AddTBInt(RptTemplateAttr.Width,800,"Width",false,true);

				map.AddBoolean(RptTemplateAttr.IsSumBig,false,"合計を表示するかどうか",false,true);
				map.AddBoolean(RptTemplateAttr.IsSumLittle,false,"小計を表示するかどうか",false,true);
				map.AddBoolean(RptTemplateAttr.IsSumRight,false,"右合計を表示するかどうか",false,true);

				map.AddTBInt(RptTemplateAttr.PercentModel,0,"比率表示方法",false,true);
				this._enMap=map;
				return this._enMap;
			}
		}
