    ...
    public class SFDBSrc : EntityNoName
    {
        ...
        public DBType HisDBType
        {
            get
            {
                switch (this.DBSrcType)
                {
                    ...
                    default:
                        throw new Exception("err@HisDBType未判断のデータベースタイプ。");
                }
            }
        }
        #endregion
            ...
            switch (this.DBSrcType)
            {
                ...
                case Sys.DBSrcType.SQLServer:
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (cmd != null)
                            cmd.Dispose();
                        throw new Exception("RunSQL エラー、SQL=" + sql);
                    }
                case Sys.DBSrcType.Oracle:
                ...
                //    }
                default:
                    throw new Exception("@判断されないサポートされるデータベースタイプ。");
            }
            ...
            switch (this.DBSrcType)
            {
                case Sys.DBSrcType.Localhost:
                    if (DBAccess.AppCenterDBType != DBType.MSSQL)
                        throw new Exception("現在、メインのCCFlowデータベースがSqlServerであるモードのみをサポートし、他のデータベースタイプは現在、データソースの確立をサポートしていません。");
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("Sys_SFDBSrc", "データソース");
                map.Java_SetDepositaryOfEntity(Depositary.None);
                map.Java_SetDepositaryOfMap( Depositary.Application);
                map.Java_SetEnType(EnType.Sys);

                map.AddTBStringPK(SFDBSrcAttr.No, null, "データソース番号（英語である必要があります）", true, false, 1, 20, 20);
                map.AddTBString(SFDBSrcAttr.Name, null, "データソース名", true, false, 0, 30, 20);

                map.AddDDLSysEnum(SFDBSrcAttr.DBSrcType, 0, "データソースタイプ", true, true,
                  SFDBSrcAttr.DBSrcType, "@0=アプリケーションシステムのメインデータベース(デフォルト)@1=SQLServerデータベース@2=Oracleデータベース@3=MySQLデータベース@4=Informixデータベース@50=Dubboサビース@100=WebServiceデータソース");

                map.AddTBString(SFDBSrcAttr.UserID, null, "データベースログインユーザーID", true, false, 0, 30, 20);
                map.AddTBString(SFDBSrcAttr.Password, null, "データベースログインユーザーパスワード", true, false, 0, 30, 20);
                map.AddTBString(SFDBSrcAttr.IP, null, "IPアドレス/データベースインスタンス名", true, false, 0, 500, 20);
                map.AddTBString(SFDBSrcAttr.DBName, null, "データベース名/Oracleは空のままです", true, false, 0, 30, 20);
        ...
        public string DoConn()
        {
            ...
            if (this.DBSrcType == Sys.DBSrcType.SQLServer)
            {
                try
                {
                    ...
                    return "おめでとう、(" + this.Name + ")接続構成は成功しました。";
                }
                catch (Exception ex)
                {
                    return ex.Message;
            ...
            if (this.DBSrcType == Sys.DBSrcType.WebServices)
            {
                ...
                try
                {
                    ...
                    return myResponse.StatusCode == HttpStatusCode.OK ? "接続構成は成功しました。" : "接続構成に失敗しました。";//返回响应的状态
                }
                catch (Exception ex)
                {
                    return ex.Message;
        ...
        public DataTable GetAllTablesWithoutViews()
        {
            ...
            if (dbType == Sys.DBSrcType.Localhost)
            {
                switch (SystemConfig.AppCenterDBType)
                {
                    ...
                    case DBType.PostgreSQL:
                        ...
                        break;
                    default:
                        throw new Exception("関連されない接続テストタイプ。");
                }
            }
            ...
            else
            {
                ...
                {
                    throw new Exception("@失敗:" + ex.Message + " dns:" + dsn);
                }
            }
        ...
        public DataTable GetTables(bool isCutFlowTables=false)
        {
            ...
            switch (dbType)
            {
                case Sys.DBSrcType.SQLServer:
                    sql.AppendLine("SELECT NAME AS No,");
                    sql.AppendLine("       [Name] = '[' + (CASE xtype WHEN 'U' THEN 'テーブル' ELSE 'ビュー' END) + '] ' + ");
                    sql.AppendLine("       NAME,");
                ...
                case Sys.DBSrcType.MySQL:
                    ...
                    sql.AppendLine("                WHEN 'BASE TABLE' THEN 'テーブル'");
                    sql.AppendLine("                ELSE 'ビュー'");
                    sql.AppendLine("            END,");
                    sql.AppendLine("            '] ',");
            ...
            switch (dbType)
            {
                ...
                case Sys.DBSrcType.Oracle:
                    if (objType.ToLower() == "column")
                        ...
                        throw new Exception("@Oracleデータベースの名前変更ロジックは関与していません。");
                    break;
                case Sys.DBSrcType.MySQL:
                    if (objType.ToLower() == "column")
                    {
        ...
        {
            if (this.No == "local")
                throw new Exception("@デフォルトの接続（ローカル）では削除や更新はできません");
            ...
            if (mds.Count != 0)
            {
                str += "次のフォームはこのデータソースを使用しています。削除できません。";
            ...
            if (tabs.Count != 0)
            {
                str += "次の表はこのデータソースを使用しています。削除できません。";
            ...
            if (str != "")
                throw new Exception("@データソースを削除するときは、参照があり、エラーが発生していないか確認してください：" + str);
        ...
        protected override bool beforeUpdateInsertAction()
        {
            if (this.No != "local" && this.DBSrcType == Sys.DBSrcType.Localhost)
                throw new Exception("@システムに存在できるローカル接続は1つだけです。別のデータソースタイプを選択してください。");
    ...
    public class SFDBSrcs : EntitiesNoName
    {
        ...
        public override int RetrieveAll()
        {
            ...
            if (i == 0)
            {
                ...
                src.No = "local";
                src.Name = "アプリケーションシステムのメインデータベース（デフォルト）";
                src.Insert();
                this.AddEntity(src);
                return 1;
            }
