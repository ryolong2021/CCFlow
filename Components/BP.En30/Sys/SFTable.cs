    ...
    public class SFTable : EntityNoName
    {
        ...
        public System.Data.DataTable GenerHisDataTable(Hashtable ht = null)
        {
            ...
            if (this.SrcType == Sys.SrcType.WebServices)
            {
                ...
                switch (td[1])
                {
                    ...
                    case "Json":
                        ...
                        if (!jdata.IsArray)
                            throw new Exception("@返されるJSON形式の文字列“" + (result ?? string.Empty) + "”正しくない");
                    ...
                    case "Xml":
                        if (result == null || string.IsNullOrWhiteSpace(result.ToString()))
                            throw new Exception("@返されたXML形式の文字列が正しくありません。");
                        ...
                        return dt;
                    default:
                        throw new Exception("@サポートされていない戻りタイプ" + td[1]);
                }
            }
            #endregion
            ...
            if (this.SrcType == Sys.SrcType.SQL)
            {
                ...
                if (DataType.IsNullOrEmpty(runObj))
                    throw new Exception("@外部キータイプSQL構成エラー," + this.No + " " + this.Name + "（SQL）タイプ(" + this.GetValStrByKey("SrcType") + ")、しかしSQLは設定されていません。");
                ...
                if (runObj.Contains("@") == true && ht == null)
                    throw new Exception("@外部キータイプSQLエラー," + runObj + "一部のクエリ条件は置き換えられません。");
            ...
            get
            {
                switch (this.SrcType)
                {
                    case Sys.SrcType.TableOrView:
                        if (this.IsClass)
                            return "<img src='/WF/Img/Class.png' width='16px' broder='0' />エンティティークラス";
                        else
                            return "<img src='/WF/Img/Table.gif' width='16px' broder='0' />テーブル/ビュー";
                    case Sys.SrcType.SQL:
                        return "<img src='/WF/Img/SQL.png' width='16px' broder='0' />SQL表現式";
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddDDLSysEnum(SFTableAttr.SrcType, 0, "データテーブルタイプ", true, false, SFTableAttr.SrcType,
                    "@0=ローカルクラス@1=作成テーブル@2=テーブルあるいはビュー@3=SQLクエリテーブル@4=WebServices@5=マイクロサービスHandler外部データソース@6=JavaScript外部データソース@7=ダイナミックJson");

                map.AddDDLSysEnum(SFTableAttr.CodeStruct, 0, "辞書テーブルタイプ", true, false, SFTableAttr.CodeStruct);
                map.AddTBString(SFTableAttr.RootVal, null, "ルートノード値", false, false, 0, 200, 20);
                ...
                map.AddTBString(SFTableAttr.SelectStatement, null, "クェリ文", true, false, 0, 1000, 600, true);
        ...
        public string IsCanDelete()
        {
            ...
            if (attrs.Count != 0)
            {
                ...
                return "err@次のエンティティフィールドが参照されます:" + err + "。テーブルを削除できません。";
            }
            return null;
        }
