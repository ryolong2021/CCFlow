	...
	public class SysDocFile : EntityMyPK
	{
        ...
        public string DocText
        {
            ...
            set
            {
                ...
                while (true)
                {
                    ...
                    if (i > 20)
                        throw new Exception("データが大きすぎて保存できません。");
                }
            }
        }
        public string D1
		...
		public override Map EnMap
		{
            get
            {
                ...
                Map map = new Map("Sys_DocFile", "ノートフィールドファイルマネージャー");
                map.Java_SetDepositaryOfMap( Depositary.Application);
                map.AddMyPK();

                map.AddTBString(SysDocFileAttr.FileName, null, "名前", false, true, 0, 200, 30);
                map.AddTBInt(SysDocFileAttr.FileSize, 0, "サイズ", true, true);
                map.AddTBString(SysDocFileAttr.FileType, null, "ファイルタイプ", true, true, 0, 50, 20);
        ...
        public static string GetValHtmlV2(string enName, string pkVal)
        {
            try
            {
                return BP.DA.DataType.ReadTextFile(BP.Sys.SystemConfig.PathOfFDB + enName + Path.DirectorySeparatorChar + pkVal + ".fdb");
            }
            catch
            {
                return null;
        ...
        public static void SetValV2(string enName, string pkVal, string val)
        {
            try
            {
                string dir = BP.Sys.SystemConfig.PathOfFDB + enName + Path.DirectorySeparatorChar;
                if (System.IO.Directory.Exists(dir) == false)
                    System.IO.Directory.CreateDirectory(dir);

                BP.DA.DataType.SaveAsFile(dir + Path.DirectorySeparatorChar + pkVal + ".fdb", val);
            }
            catch (Exception ex)
            {
                throw ex;
