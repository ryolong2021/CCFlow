...
namespace BP.Sys
{
	...
	public class SysEnum : EntityMyPK
	{
		...
		public override Map EnMap
		{
            get
            {
                ...
                Map map = new Map("Sys_Enum", "列挙データ");
              ...
              */
                ...
                map.AddTBString(SysEnumAttr.Lang, "CH", "言語", true, false, 0, 10, 8);
	...
	public class SysEnums : Entities
	{
        ...
        public string GenerCaseWhenForOracle(string enName, string mTable, string key, string field, string enumKey, int def)
        {
            ...
            if (this.Count == 0)
                throw new Exception("@列挙値" + enumKey + "削除されました。");
        ...
        {
            if (this.Count == 0)
                throw new Exception("@列挙値（" + enumKey + "）削除されました。予期されたSQLを形成できません。");
        ...
        public void LoadIt(string enumKey)
        {
            if (this.Full(enumKey) == false)
            {
                ...
                {
                    throw new Exception("@[" + enumKey + "]列挙値をプレハブありません。列挙値の修復中のエラー:" + ex.Message);
                }
            }
        }
        /// <summary>
