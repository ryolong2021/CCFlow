    ...
    public class SystemConfig
    {
        ...
        public static void ReadConfigFile(string cfgFile)
        {
            ...
            if (File.Exists(cfgFile) == false)
                throw new Exception("ファイルが存在しません [" + cfgFile + "]");
        ...
        public static string PathOfTemp
        {
            get
            {
                return PathOfDataUser + "Temp" + Path.DirectorySeparatorChar;
            }
        }
        public static string PathOfWorkDir
        {
            get
            {
                ...
                {
                    string path1 = HttpContextHelper.PhysicalApplicationPath + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar;
                    System.IO.DirectoryInfo info1 = new DirectoryInfo(path1);
                    return info1.FullName;
                }
                else
                {
                    string path = AppDomain.CurrentDomain.BaseDirectory + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar;
        ...
        public static string PathOfFDB
        {
            get
            {
                ...
                if (s == "" || s == null)
                    return PathOfWebApp + Path.DirectorySeparatorChar + "DataUser" + Path.DirectorySeparatorChar + "FDB" + Path.DirectorySeparatorChar;
                return s;
            }
        }
        /// <summary>
        ...
        public static string PathOfData
        {
            get
            {
                return PathOfWebApp +  Path.DirectorySeparatorChar + "WF" + Path.DirectorySeparatorChar + "Data" + Path.DirectorySeparatorChar;
            }
        }
        public static string PathOfDataUser
        {
            get
            {
                ...
                {
                    tmp = PathOfWebApp + "DataUser" + Path.DirectorySeparatorChar;
                }
                else
                {
                    if (tmp.Contains(Path.DirectorySeparatorChar))
                    {
                        tmp.Replace(Path.DirectorySeparatorChar.ToString(), "");
                    }

                    tmp = PathOfWebApp + tmp + Path.DirectorySeparatorChar + "DataUser" + Path.DirectorySeparatorChar;
                }
                return tmp;
            }
        }
        ...
        public static string PathOfXML
        {
            get
            {
                return PathOfWebApp + Path.DirectorySeparatorChar + "WF" + Path.DirectorySeparatorChar + "Data" + Path.DirectorySeparatorChar + "XML" + Path.DirectorySeparatorChar;
            }
        }
        ...
        public static string PathOfCyclostyleFile
        {
            get
            {
                return PathOfWebApp + Path.DirectorySeparatorChar + "DataUser" + Path.DirectorySeparatorChar + "CyclostyleFile" + Path.DirectorySeparatorChar;
            }
        }
        /// <summary>
        /// 应用程序名称
        ...
        public static string PathOfWebApp
        {
            get
            {
                if (SystemConfig.IsBSsystem)
                {
                    // 09/29 TODO 検討した後で、再対応
                    return HttpContextHelper.PhysicalApplicationPath;
                   // return HttpContextHelper.Request.Scheme +"://"+ HttpContextHelper.Request.Host.Value+"/";
                }
                else
                {
                    if (SystemConfig.SysNo == "FTA")
                        ...
                        return AppDomain.CurrentDomain.BaseDirectory + ".." + Path.DirectorySeparatorChar + ".." + Path.DirectorySeparatorChar;
                }
            }
        }
        #endregion
        ...
        public static string SysName
        {
            get
            {
                ...
                if (s == null)
                    s = "web.configでSysName名を構成してください。";
                return s;
            }
        }
        ...
        {
            get { return PathOfWebApp + Path.DirectorySeparatorChar + "DataUser" + Path.DirectorySeparatorChar + "Log" + Path.DirectorySeparatorChar; }
        }
        ...
        public static string GetConfigXmlKeyVal(string key)
        {
            try
            {
                ...
                ds.ReadXml(BP.Sys.SystemConfig.PathOfXML + Path.DirectorySeparatorChar + "KeyVal.xml");
                ...
                throw new Exception("GetXmlConfigを使用して値を取得するとエラーが発生し、key =" + key + "が見つかりません。, /data/Xml/KeyVal.xmlを確認してください。");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static string GetConfigXmlNode(string fk_Breed, string enName, string key)
        {
            try
            {
                string file = BP.Sys.SystemConfig.PathOfXML + Path.DirectorySeparatorChar + "Node" + Path.DirectorySeparatorChar + fk_Breed + ".xml";
                DataSet ds = new DataSet("dss");
                try
                {
                    ds.ReadXml(file);
                ...
                if (dt.Columns.Contains(key) == false)
                    throw new Exception(file + "構成エラー、フォーマットに従って構成しなかった、マークが含まれていない" + key);
                foreach (DataRow dr in dt.Rows)
                {
        ...
        public static string GetConfigXmlEns(string key, string ensName)
        {
            try
            {
                ...
                {
                    DataSet ds = new DataSet("dss");
                    ds.ReadXml(BP.Sys.SystemConfig.PathOfXML + Path.DirectorySeparatorChar + "Ens" + Path.DirectorySeparatorChar + "ConfigEns.xml");
                    dt = ds.Tables[0];
        ...
        public static string GetConfigXmlSQL(string key)
        {
            ...
            {
                DataSet ds = new DataSet("dss");
                ds.ReadXml(BP.Sys.SystemConfig.PathOfXML + Path.DirectorySeparatorChar + "SQL" + Path.DirectorySeparatorChar + BP.Sys.SystemConfig.ThirdPartySoftWareKey + ".xml");
                DataTable dt = ds.Tables[0];
                foreach (DataRow dr in dt.Rows)
                {
                ...
                throw new Exception("GetXmlConfigを使用して値を取得するとエラーが発生し、key =" + key + ", が見つかりません。/Data/XML/" + SystemConfig.ThirdPartySoftWareKey + ".xml.を確認してください ");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static string GetConfigXmlSQLApp(string key)
        {
            ...
            {
                DataSet ds = new DataSet("dss");
                ds.ReadXml(BP.Sys.SystemConfig.PathOfXML + Path.DirectorySeparatorChar + "SQL" + Path.DirectorySeparatorChar + "App.xml");
                DataTable dt = ds.Tables[0];
                foreach (DataRow dr in dt.Rows)
                {
                    if (dr["No"].ToString() == key)
                ...
                throw new Exception("GetXmlConfigを使用して値を取得するとエラーが発生し、key =" + key + ", が見つかりません。/Data/XML/App.xmlを確認してください。");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static string AppCenterDSN
        {
            get
            {
                ...
                if (DataType.IsNullOrEmpty(str) == true)
                    throw new Exception("err@構成データベース接続文字列はありません。");
        ...
        public static string AppCenterDBDatabase
        {
            get
            {
                    ...
                    switch (BP.DA.DBAccess.AppCenterDBType)
                    {
                        ...
                        //    break;
                        default:
                            throw new Exception("@判定されないデータ型。");
                            break;
                    }
