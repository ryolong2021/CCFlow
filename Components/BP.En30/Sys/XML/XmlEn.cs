...
namespace BP.Sys.XML
{
    ...
    abstract public class XmlEn
    {
        ...
        public int GetValIntByKey(string key)
        {
            ...
            {
                throw new Exception("key=" + key + "int型に変換できません。val=" + this.GetValByKey(key));
            }
        }
        ...
        public int RetrieveByPK(string key, string val)
        {
            ...
            if (i > 1)
            {
               // BP.Sys.SystemConfig.DoClearCash();
                throw new Exception("@XML=[" + this.ToString() + "]中にPK=" + val + "一つだけではない。");
            }
            return 0;
        }
	...
	abstract public class XmlEnNoName:XmlEn
	{
        ...
        public XmlEnNoName(string no)
        {
           ...
           if (i == 0)
               throw new Exception("@クエリが見つかりません="+no+" XMLデータ。");
        }
	} 
	/// <summary>
	/// XmlEn 的摘要说明。
	...
	abstract public class XmlEns:System.Collections.CollectionBase
	{
        ...
        public string Tname
        {
            get
            {
                ...
                tname = tname.Substring(tname.LastIndexOf(Path.DirectorySeparatorChar) + 1) + this.TableName + "_X";
                return tname;
            }
        }
        ...
        private DataTable GetTableTxt(DataTable dt,FileInfo file)
        {
            ...
            while (true)
            {
                ...
                if (idx == -1)
                {
                    throw new Exception(this.File + "@ルールkey = valを満たしていません。");
                }
        ...
        public DataTable GetTable()
        {
            ...
            if (this.File.ToLower().IndexOf(".xml") > 0)
            {
                ...
                {
                    throw new Exception("ファイルを読む:" + this.File + "エラー。例外=" + ex.Message);
                }
            }
            ...
            if (di.Exists == false)
                throw new Exception("ファイルが存在しません:" + this.File);
            ...
            foreach (FileInfo fi in fis)
            {
                ...
                try
                {
                    ds.ReadXml(this.File + Path.DirectorySeparatorChar + fi.Name);
                }
                catch (Exception ex)
                {
                    throw new Exception("ファイルを読む:" + fi.Name + "エラー。例外=" + ex.Message);
                }
                try
                {
                    //ds.
                    if (dt.Columns.Count == 0)
                    {
                        ...
                        {
                            throw new Exception("ファイル" + fi.Name + "にテーブルが見つかりました:" + this.TableName + " exception=" + ex.Message);
                        }
                        tempDT = dt.Clone();
                        continue;
                    }
                    ...
                    if (mydt == null)
                        throw new Exception("そのようなテーブルはありません:" + this.TableName);
                    ...
                    foreach (DataRow mydr in mydt.Rows)
                    {
                        ...
                        foreach (DataColumn dc in tempDT.Columns)
                        {
                            ...
                            {
                                throw new Exception("xml 構成エラー、複数のファイルの非対称属性。" + ex.Message);
                            }
                        }
                ...
                {
                    throw new Exception("データの取得エラー：fileName=" + fi.Name + " clasName=" + this.ToString() + " MoreInfo=" + ex.Message);
                }
            }
            BP.DA.Cash.AddObj(this.Tname,
        ...
        public int Retrieve(string key, object val, string key1, string val1)
        {
            ...
            DataTable dt = this.GetTable();
            if (dt == null)
                throw new Exception("@エラー：クラス" + this.GetNewEntity.ToString() + " File= " + this.File + " Table=" + this.TableName + " 、 データは取得されませんでした。");
            ...
            {
                throw new Exception("@読み込みエラー：file="+this.File+" xml:"+this.ToString()+"Err:"+ex.Message);
            }
            return i;
        }
		/// <summary>
		...
		public XmlEn this[string key, string val]
		{
			get
			{
				...
				throw new Exception("["+this.TableName+","+this.File+","+this.ToString()+"]が見つかりませんkey="+key+", val="+val+"のインスタンス。");
			}
		}
		/// <summary>
		/// 增加一个xml en to Ens.
