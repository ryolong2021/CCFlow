    ...
    public class DingDing
    {
                    ...
                    foreach (DepartMentDetailInfo deptMentInfo in departMentList.department)
                    {
                        ...
                        if (userList != null)
                        {
                            foreach (DepartMentUserInfo userInfo in userList.userlist)
                            {
                                ...
                                {
                                    //大图标
                                    string UserIcon = savePath + Path.DirectorySeparatorChar + userInfo.userid + "Biger.png";
                                    File.Copy(savePath + Path.DirectorySeparatorChar + "DefaultBiger.png", UserIcon, true);

                                    //小图标
                                    UserIcon = savePath + Path.DirectorySeparatorChar + userInfo.userid + "Smaller.png";
                                    File.Copy(savePath + Path.DirectorySeparatorChar + "DefaultSmaller.png", UserIcon, true);

                                    //正常图标
                                    UserIcon = savePath + Path.DirectorySeparatorChar + userInfo.userid + ".png";
                                    File.Copy(savePath + Path.DirectorySeparatorChar + "Default.png", UserIcon, true);
                                }
                                else
                                {
                                    //大图标
            ...
            try
            {
                ...
                foreach (DepartMentDetailInfo deptMentInfo in departMentList.department)
                {
                    ...
                    if (deptMentInfo.id != "1")
                    {
                        ...
                        if (dept.IsExit(DeptAttr.No, deptMentInfo.id) == true)
                        {
                            ...
                            {
                                doSomeThing = true;
                                append.Append("\r\n部署名が変わりました：" + dept.Name + " --> " + deptMentInfo.name);
                            }
                            if (!dept.ParentNo.Equals(deptMentInfo.parentid))
                            {
                                doSomeThing = true;
                                append.Append("\r\n部門の親が変わりました：" + dept.ParentNo + " --> " + deptMentInfo.parentid);
                            }
                            //有变化，更新
                            if (doSomeThing == true)
                            {
                        ...
                        else
                        {
                            ...
                            dept.ParentNo = deptMentInfo.parentid;
                            dept.DirectInsert();
                            append.Append("\r\n新規部門：" + deptMentInfo.id + " - " + deptMentInfo.name);
                        }
                    }
                    //部门人员
                    DepartMentUser_List userList = GenerDeptUser_List(access_token, deptMentInfo.id);
                        ...
                        foreach (DepartMentUserInfo userInfo in userList.userlist)
                        {
                            ...
                            if (emp.RetrieveFromDBSources() > 0)
                            {
                                if (!emp.Name.Equals(userInfo.name))
                                {
                                    emp.Name = userInfo.name;
                                    emp.DirectUpdate();
                                    append.Append("\r\n人名が変わりました：" + emp.Name + " --> " + userInfo.name);
                                }
                                ...
                                deptEmp.FK_Emp = emp.No;
                                deptEmp.DirectInsert();
                                append.Append("\r\n人員帰属部門を追加：" + emp.Name + " - " + deptMentInfo.name);
                                continue;
                            }
                            ...
                            //emp.Idx = DataType.IsNullOrEmpty(userInfo.order) == true ? 0 : Int32.Parse(userInfo.order);
                            emp.DirectInsert();
                            append.Append("\r\n人員を追加：" + emp.Name + "帰属部門:" + deptMentInfo.name);
                ...
                foreach (Dept gpm_Dept in gpm_Depts)
                {
                    ...
                    if (isHave == false)
                    {
                        ...
                        dt.Delete();
                        append.Append("\r\n部門を削除：" + gpm_Dept.Name + " 部門のフルパス：" + gpm_Dept.NameOfPath);
                        append.Append("\r\n        部署" + iDeptStation + "件記録");
                        append.Append("\r\n        部門スタッフの役職" + iDeptEmpStation + "件記録");
                        append.Append("\r\n        部門スタッフ" + iDeptEmp + "件記録");
                    }
                    else
                    {
                        ...
                        {
                            append.Append("\r\n部署の担当者を削除、部署：" + gpm_Dept.Name + " 部門のフルパス：" + gpm_Dept.NameOfPath);
                        ...
                        foreach (DeptEmp deptEmp in deptEmps)
                        {
                            ...
                            if (isHave == false)
                            {
                                ...
                                ep.RetrieveFromDBSources();
                                append.Append("\r\n部署の担当者を削除、部署：" + gpm_Dept.Name + " 部門のフルパス：" + gpm_Dept.NameOfPath);
                                append.Append("\r\n        人員番号：" + deptEmp.FK_Emp + " 名前：" + ep.Name);
                            }
                        }
                    }
                }
            ...
            try
            {
                ...
                if (postVal != null)
                {
                    if (postVal.errcode != "0")
                        BP.DA.Log.DefaultLogWriteLineError("部門の追加できませんでした：" + postVal.errcode + "-" + postVal.errmsg);
        ...
        public Ding_Post_ReturnVal GPM_Ding_EditDept(Dept dept)
        {
            ...
            try
            {
                ...
                {
                    if (postVal.errcode != "0")
                        BP.DA.Log.DefaultLogWriteLineError("部門の変更に失敗しました：" + postVal.errcode + "-" + postVal.errmsg);
        ...
        public Ding_Post_ReturnVal GPM_Ding_DeleteDept(string deptId)
        {
            ...
            try
            {
                ...
                {
                    if (postVal.errcode != "0")
                        BP.DA.Log.DefaultLogWriteLineError("部門の削除できませんでした：" + postVal.errcode + "-" + postVal.errmsg);
            ...
            try
            {
                ...
                if (postVal != null)
                {
                    if (postVal.errcode != "0")
                    {
                        ...
                        BP.DA.Log.DefaultLogWriteLineError("新規要員の追加できませんでした：" + postVal.errcode + "-" + postVal.errmsg);
                    }
                    return postVal;
                }
            }
        ...
        public Ding_Post_ReturnVal GPM_Ding_EditEmp(Emp emp, List<string> deptIds = null)
        {
            ...
            try
            {
                ...
                if (postVal != null)
                {
                    ...
                    if (postVal.errcode != "0")
                    {
                        BP.DA.Log.DefaultLogWriteLineError("スタッフを更新できませんでした：" + postVal.errcode + "-" + postVal.errmsg);
                    }
                    return postVal;
                }
            }
        ...
        public Ding_Post_ReturnVal GPM_Ding_DeleteEmp(string userid)
        {
            ...
            try
            {
                ...
                {
                    if (postVal.errcode != "0")
                        BP.DA.Log.DefaultLogWriteLineError("人員の削除に失敗しました：" + postVal.errcode + "-" + postVal.errmsg);
