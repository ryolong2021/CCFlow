...
namespace BP.WF.DTS
{
    ...
    public class GenerCH : Method
    {
        ...
        public GenerCH()
        {
            this.Title = "評価データを生成します（すべてのフローについて、新しく構成されたノード評価情報に基づいて評価データを生成します）。";
            this.Help = "実行時に生成されたデータを削除してから、各フローの各ノードの実行データを自動的に生成する必要がある。";
            this.GroupName = "評価データ";
        ...
        public override object Do()
        {
                ...
                foreach (Node nd in nds)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        if (BP.Web.WebUser.No != fk_emp)
                        {
                            ...
                            {
                                err += "@人員ミス:" + fk_emp + "この人は削除されたかもしれません。" + ex.Message;
                            }
                        }
            ...
            {
                return "評価を生成できませんでした:" + ex.StackTrace;
            }

            //登录.
            BP.WF.Dev2Interface.Port_Login("admin");
            return "実行は成功しました、次の情報があります:" + err;
        }
    }
}
