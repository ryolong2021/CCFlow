...
namespace BP.WF.DTS
{
    ...
    public class AutoRunWF_Task : Method
    {
        ...
        {
            this.Title = "フローを自動的に開始し、WF_Taskテーブルをスキャンモードを使用します。";
            this.Help = "自動開始タスクのフロー, WF_Task";
        ...
        public override object Do()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (fl.HisFlowAppType == FlowAppType.PRJ)
                {
                    ...
                    {
                        info += "err@エンジニアリンフロー中に、PrjNo、PrjNameパラメータなし:"+fl.Name;
                ...
                try
                {
                    ...
                    {
                        info += "開始ノードフォーム:" + fTable + "デフォルトのフィールドMainPKは設定されていません。" + sql;
                        continue;
                    }
                    ...
                    if (fl.HisFlowAppType == FlowAppType.PRJ)
                    {
                        ...
                        {
                            info += "err@プロジェクト番号が見つかりません：MainPK" + mypk;
                            DBAccess.RunSQL("UPDATE WF_Task SET TaskSta=2,Msg='" + info + "' WHERE MyPK='" + mypk + "'");
                            continue;
                        }
                    }
