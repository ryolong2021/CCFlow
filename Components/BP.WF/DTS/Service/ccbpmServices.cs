...
namespace BP.WF.DTS
{
    ...
    public class ccbpmServices : Method
    {
        ...
        {
            this.Title = "ccbpmフローサービス";
            this.Help = "1,電子メールを自動的に送信します。2、フローを自動的に開始します。3。ノードタスクを自動的に実行します。.";
            this.Help += "すべての自動タスクの統合.";
        ...
        private void DoOverDueFlow()
        {
                    ...
                    switch (node.HisOutTimeDeal)
                    {
                        ...
                        case OutTimeDeal.AutoShiftToSpecUser: //走动移交给.
                            ...
                            try
                            {
                                ...
                                msg = "フロー '" + node.FlowName + "',タイトル： '" + title + "'で完了する時間は'" + compleateTime + "',現在のノード '" + node.Name +
                                      "'タイムアウトルールは「指定者に引き継がれ」、自動的に引き継がれている" + empShift.Name + ".";
                                BP.DA.Log.DefaultLogWriteLine(LogType.Info, msg);
                            }
                            catch (Exception ex)
                            {
                        ...
                        case OutTimeDeal.AutoTurntoNextStep:
                            ...
                            {
                                msg = "フロー  '" + node.FlowName + "',タイトル： '" + title + "'で完了する時間は'" + compleateTime + "',現在のノード '" + node.Name +
                                      "'タイムアウト処理ルールは「次のノードに自動的に送信」、例外を送信します:" + ex.Message;
                                BP.DA.Log.DefaultLogWriteLine(LogType.Error, msg);
                            }
                            break;
                        ...
                        case OutTimeDeal.RunSQL:
                            ...
                            {
                                msg = "フロー  '" + node.FlowName + "',タイトル： '" + title + "'で完了する時間は'" + compleateTime + "',現在のノード '" + node.Name +
                                      "'タイムアウト処理ルールは「SQL実行」です。SQLの実行中に例外が発生しました:" + ex.Message;
                                BP.DA.Log.DefaultLogWriteLine(LogType.Error, msg);
                            }
                            break;
            ...
            BP.DA.Log.DefaultLogWriteLine(LogType.Info, "期限切れのフローデータのスキャンを終了する.");
        }
        /// <summary>
        /// 特殊处理天津的流程
