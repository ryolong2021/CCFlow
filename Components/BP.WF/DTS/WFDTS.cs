...
namespace BP.WF.DTS
{
    ...
    public class UserPort : DataIOEn
    {
        ...
        public UserPort()
        {
            ...
            this.Title = "フロー部門の生成（システムが最初にインストールされたとき、または部門が変更されたときに実行されます）";
    ...
    public class DelWorkFlowData : DataIOEn
    {
        ...
        {
            this.HisDoType = DoType.UnName;
            this.Title = "<font color=red><b>フローデータを消去</ b> </ font>";
            //this.HisRunTimeType = RunTimeType.UnName;
            //this.FromDBUrl = DBUrlType.AppCenterDSN;
            //this.ToDBUrl = DBUrlType.AppCenterDSN;
        }
    ...
    public class InitBillDir : DataIOEn
    {
        ...
        {
            this.HisDoType = DoType.UnName;
            this.Title = "<font color=green><b>伝票カタログを作成します（伝票番号を変更するたびに、または1年に1日実行します)</b></font>";
            this.HisRunTimeType = RunTimeType.UnName;
            this.FromDBUrl = DBUrlType.AppCenterDSN;
            this.ToDBUrl = DBUrlType.AppCenterDSN;
        }
    ...
    public class OutputSQLs : DataIOEn
    {
        ...
        {
            Log.DefaultLogWriteLine(LogType.Info, BP.Web.WebUser.Name + "評価情報のスケジュールを開始する:" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
