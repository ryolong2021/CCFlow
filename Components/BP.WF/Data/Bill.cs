    ...
    public class Bill : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("WF_Bill", "伝票");

                map.AddMyPK(false);

                map.AddTBInt(BillAttr.WorkID, 0, "ジョブID", false, true);
                map.AddTBInt(BillAttr.FID, 0, "FID", false, true);
                map.AddTBString(BillAttr.FK_Flow, null, "フロー", false, false, 0, 4, 5);
                map.AddTBString(BillAttr.FK_BillType, null, "伝票タイプ", false, false, 0, 300, 5);
                map.AddTBString(BillAttr.Title, null, "タイトル", false, false, 0, 900, 5);
                map.AddTBString(BillAttr.FK_Starter, null, "起票者", true, true, 0, 50, 5);
                map.AddTBDateTime(BillAttr.StartDT, "開始時間", true, true);
                ...
                map.AddDDLEntities(BillAttr.FK_Emp, null, "印刷人", new Emps(), false);
                map.AddTBDateTime(BillAttr.RDT, "印刷時間", true, true);

                map.AddDDLEntities(BillAttr.FK_Dept, null, "所属部署", new BP.Port.Depts(), false);

                map.AddTBString(BillAttr.FK_NY, null, "所属年月", true, true, 0, 50, 5);


                map.AddTBString(BillAttr.Emps, null, "Emps", false, false, 0, 4000, 5);

                map.AddTBString(BillAttr.FK_Node, null, "ノード", false, false, 0, 30, 5);
