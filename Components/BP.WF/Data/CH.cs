    ...
    public class CH : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_CH", "時効考査");

                map.AddMyPK();
                    
                #region 基本属性.
                map.AddTBInt(CHAttr.WorkID, 0, "ジョブID", false, true);
                map.AddTBInt(CHAttr.FID, 0, "FID", false, true);
                map.AddTBString(CHAttr.Title, null, "タイトル", false, false, 0, 900, 5);

                map.AddTBString(CHAttr.FK_Flow, null, "フロー", false, false, 3, 3, 3);
                map.AddTBString(CHAttr.FK_FlowT, null, "フロー名", true, true, 0, 200, 5);

                map.AddTBInt(CHAttr.FK_Node, 0, "ノード", false, false);
                map.AddTBString(CHAttr.FK_NodeT, null, "ノード名", true, true, 0, 200, 5);


                map.AddTBString(CHAttr.Sender, null, "送信者", false, false, 0, 200, 3);
                map.AddTBString(CHAttr.SenderT, null, "送信者の名前", true, true, 0, 200, 5);


                map.AddTBString(CHAttr.FK_Emp, null, "当事者", true, true, 0, 30, 3);
                map.AddTBString(CHAttr.FK_EmpT, null, "当事者の名前", true, true, 0, 200, 5);

                //为邓州增加的属性. 解决多人处理一个节点的工作的问题.
                map.AddTBString(CHAttr.GroupEmps, null, "関連当事者", true, true, 0, 400, 3);
                map.AddTBString(CHAttr.GroupEmpsNames, null, "関連当事者の名前", true, true, 0, 900, 3);
                map.AddTBInt(CHAttr.GroupEmpsNum, 1, "関連当事者の数", false, false);


                map.AddTBString(CHAttr.DTFrom, null, "タスクのリリース時間", true, true, 0, 50, 5);
                map.AddTBString(CHAttr.DTTo, null, "タスク処理時間", true, true, 0, 50, 5);
                map.AddTBString(CHAttr.SDT, null, "予定完了日", true, true, 0, 50, 5);

                map.AddTBString(CHAttr.FK_Dept, null, "所属部署", true, true, 0, 50, 5);
                map.AddTBString(CHAttr.FK_DeptT, null, "部署名", true, true, 0, 500, 5);
                map.AddTBString(CHAttr.FK_NY, null, "入会月", true, true, 0, 10, 10);
                map.AddDDLSysEnum(CHAttr.DTSWay, 0, "評価方法", true, true, CHAttr.DTSWay, "@0=評価なし@ 1 =適時性による評価@ 2 =作業負荷による評価");
                #endregion 基本属性.

                #region 计算属性.
                map.AddTBString(CHAttr.TimeLimit, null, "期限を設定", true, true, 0, 50, 5);
                map.AddTBFloat(CHAttr.OverMinutes, 0, "期限を過ぎて", false, true);
                map.AddTBFloat(CHAttr.UseDays, 0, "実際の使用期間", false, true);
                map.AddTBFloat(CHAttr.OverDays, 0, "期限切れの日", false, true);
                map.AddTBInt(CHAttr.CHSta, 0, "状態", true, true);
                map.AddTBInt(CHAttr.WeekNum, 0, "何週目", false, true);
                map.AddTBFloat(CHAttr.Points, 0, "総減点", true, true);
                map.AddTBIntMyNum();
                #endregion 计算属性.
