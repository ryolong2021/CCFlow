	...
	public class Eval : EntityMyPK
	{
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("WF_CHEval", "仕事の品質評価");


                map.AddMyPK();
                map.AddTBString(EvalAttr.Title, null, "タイトル", false, true, 0, 500, 10);
                map.AddTBString(EvalAttr.FK_Flow, null, "フローの番号", false, true, 0, 7, 10);
                map.AddTBString(EvalAttr.FlowName, null, "フローの名前", false, true, 0, 100, 10);

                map.AddTBInt(EvalAttr.WorkID, 0, "ジョブID", false, true);
                map.AddTBInt(EvalAttr.FK_Node, 0, "評価ノード", false, true);
                map.AddTBString(EvalAttr.NodeName, null, "滞在ノード", false, true, 0, 100, 10);

                map.AddTBString(EvalAttr.Rec, null, "評価者", false, true, 0, 50, 10);
                map.AddTBString(EvalAttr.RecName, null, "評価者の名前", false, true, 0, 50, 10);

                map.AddTBDateTime(EvalAttr.RDT, "評価日", true, true);

                map.AddTBString(EvalAttr.EvalEmpNo, null, "評価される要員の番号", false, true, 0, 50, 10);
                map.AddTBString(EvalAttr.EvalEmpName, null, "評価される人の名前", false, true, 0, 50, 10);
                map.AddTBString(EvalAttr.EvalCent, null, "評価スコア", false, true, 0, 20, 10);
                map.AddTBString(EvalAttr.EvalNote, null, "評価内容", false, true, 0, 20, 10);

                map.AddTBString(EvalAttr.FK_Dept, null, "部門", false, true, 0, 50, 10);
                map.AddTBString(EvalAttr.DeptName, null, "部署名", false, true, 0, 100, 10);
