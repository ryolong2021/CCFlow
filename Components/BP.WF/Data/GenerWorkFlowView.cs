...
namespace BP.WF.Data
{
    ...
    public class GenerWorkFlowView : Entity
    {
        ...
        public GenerWorkFlowView(Int64 workId)
        {
            ...
            if (qo.DoQuery() == 0)
                throw new Exception("仕事 GenerWorkFlowView[" + workId + "]存在しません。");
        }
        /// <summary>
        /// 执行修复
        /// </summary>
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_GenerWorkFlow", "フロークエリ");

                map.AddTBIntPK(GenerWorkFlowViewAttr.WorkID, 0, "WorkID", true, true);
                map.AddTBString(GenerWorkFlowViewAttr.StarterName, null, "起票者", true, false, 0, 30, 10);
                map.AddTBString(GenerWorkFlowViewAttr.Title, null, "タイトル", true, false, 0, 100, 10, true);
                map.AddDDLSysEnum(GenerWorkFlowViewAttr.WFSta, 0, "フロー状態", true, false, GenerWorkFlowViewAttr.WFSta,
                    "@0=実行中@1=完了@2=その他");

                map.AddDDLSysEnum(GenerWorkFlowViewAttr.WFState, 0, "フローの状態", true, false, MyStartFlowAttr.WFState);
                map.AddTBString(GenerWorkFlowViewAttr.NodeName, null, "現在のノード名", true, false, 0, 100, 10);
                map.AddTBDateTime(GenerWorkFlowViewAttr.RDT, "記録日", true, true);
                map.AddTBString(GenerWorkFlowViewAttr.BillNo, null, "伝票番号", true, false, 0, 100, 10);
                map.AddTBStringDoc(GenerWorkFlowViewAttr.FlowNote, null, "備考", true, false, true);

                map.AddDDLEntities(GenerWorkFlowViewAttr.FK_FlowSort, null, "カテゴリー", new FlowSorts(), false);
                map.AddDDLEntities(GenerWorkFlowViewAttr.FK_Flow, null, "フロー", new Flows(), false);
                map.AddDDLEntities(GenerWorkFlowViewAttr.FK_Dept, null, "部門", new BP.Port.Depts(), false);

                map.AddTBInt(GenerWorkFlowViewAttr.FID, 0, "FID", false, false);
                map.AddTBInt(GenerWorkFlowViewAttr.FK_Node, 0, "FK_Node", false, false);

                map.AddDDLEntities(GenerWorkFlowViewAttr.FK_NY, null, "月", new GenerWorkFlowViewNYs(), false);
               ...
               // rm.HisAttrs.AddTBInt("NodeID", 0, "回滚到节点", true, false);
                ...
                rm.HisAttrs.AddTBString("EmpNo", null, "従業員番号にロールバックします",true,false,0,100,100);
                map.AddRefMethod(rm);
                ...
                rm = new RefMethod();
                rm.Title = "調整";
                rm.HisAttrs.AddTBString("wenben", null, "人員に調整する", true, false, 0, 100, 100);
                rm.HisAttrs.AddTBInt("shuzi", 0, "ノードに調整する", true, false);
        ...
        {
           return BP.WF.Dev2Interface.Flow_ReSend(this.WorkID, toNodeID, toEmpNo,"admin調整");
        }
        public string RepairDataIt()
        {
            ...
            {
                infos += "@ エラー:" + fl.No + " - Rec" + wk.Rec + " db=" + wk.OID + " - " + fl.Name;
            }
            ...
            {
                wk.DirectUpdate();
                return "データを更新する必要はありません.";
            }
            infos += "@WorkID=" + wk.OID + " =" + wk.Rec + "  dt=" + wk.RDT + "修理される.";
