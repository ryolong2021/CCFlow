    ...
    public class GenerWorkFlowViewNY : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("Pub_NY", "月");
                map.Java_SetEnType(EnType.View);

                map.AddTBStringPK(GenerWorkFlowViewNYAttr.No, null, "番号", true, false, 2, 30, 20);
                map.AddTBString(GenerWorkFlowViewNYAttr.Name, null, "名前", true, false, 0, 3900, 20);
                this._enMap = map;
                return this._enMap;
            }
        }
