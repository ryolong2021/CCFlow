...
namespace BP.WF.Data
{
	...
	{	
        ...
        public Monitor(Int64 workId)
        {
            ...
            if (qo.DoQuery() == 0)
                throw new Exception("作業 Monitor [" + workId + "]存在しません。");
        }
        /// <summary>
        /// 执行修复
        /// </summary>
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_EmpWorks", "フロー監視");
                map.Java_SetEnType(EnType.View);

                map.AddTBIntPK(MonitorAttr.WorkID, 0, "ジョブID", true, true);
                map.AddTBInt(MonitorAttr.FID, 0, "FID", false, false);
                map.AddTBString(MonitorAttr.Title, null, "フローのタイトル", true, false, 0, 300, 10,true);
                map.AddTBString(MonitorAttr.FK_Emp, null, "現在の処理者", true, false, 0, 30, 10);
                map.AddDDLEntities(MonitorAttr.FK_Flow, null, "フロー", new Flows(), false);
                map.AddDDLEntities(MonitorAttr.FK_Dept, null, "起票部門", new BP.Port.Depts(), false);
                map.AddTBString(MonitorAttr.Starter, null, "起票者番号", true, false, 0, 30, 10);
                map.AddTBString(MonitorAttr.StarterName, null, "名前", true, false, 0, 30, 10);
                map.AddTBString(MonitorAttr.NodeName, null, "滞在ノード", true, false, 0, 100, 10);
                map.AddTBString(MonitorAttr.TodoEmps, null, "処理者", true, false, 0, 100, 10);

                map.AddTBStringDoc(MonitorAttr.FlowNote, null, "備考", true, false,true);
              
                map.AddTBInt(MonitorAttr.FK_Node, 0, "FK_Node", false, false);
                map.AddTBString(MonitorAttr.WorkerDept, null, "スタッフ部門番号", 
                    false, false, 0, 30, 10);
                ...
                rm.HisAttrs.AddDDLEntities("ToEmp", null, "に転送:", new BP.WF.Data.MyDeptEmps(),true);
                rm.HisAttrs.AddTBString("Note", null, "転送理由", true, false, 0, 300, 100);
                map.AddRefMethod(rm);

                rm = new RefMethod();
                rm.Icon = "../../WF/Img/Btn/Delete.gif";
                rm.Title = "削除";
                rm.Warning = "このフローを削除してもよろしいですか？";
                ...
                rm.HisAttrs.AddTBInt("NodeID", 0, "ノードにロールバック", true, false);
                rm.HisAttrs.AddTBString("Note", null, "ロールバックの理由", true, false, 0, 300, 100);
                map.AddRefMethod(rm);
