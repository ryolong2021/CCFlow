...
namespace BP.WF.Data
{
	...
	{	
        ...
        public MyDeptFlow(Int64 workId)
        {
            ...
            if (qo.DoQuery() == 0)
                throw new Exception("仕事MyDeptFlow[" + workId + "]存在しません。");
        }
        /// <summary>
        /// 执行修复
        /// </summary>
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_GenerWorkFlow", "私の部門のフロー");
                map.Java_SetEnType(EnType.View);

                map.AddTBString(MyDeptFlowAttr.Title, null, "タイトル", true, false, 0, 100, 150, true);
                map.AddDDLEntities(MyDeptFlowAttr.FK_Flow, null, "フロー", new Flows(), false);
                map.AddTBString(MyDeptFlowAttr.BillNo, null, "伝票番号", true, false, 0, 100, 50);

                map.AddTBString(MyDeptFlowAttr.StarterName, null, "起票者", true, false, 0, 30, 40);
                map.AddTBDateTime(MyDeptFlowAttr.RDT, "起票日", true, true);

                map.AddTBString(MyDeptFlowAttr.NodeName, null, "現在のノード", true, false, 0, 100, 80);
                map.AddTBString(MyDeptFlowAttr.TodoEmps, null, "現在のプロセッサ", true, false, 0, 100, 80);

                map.AddDDLSysEnum(MyDeptFlowAttr.WFSta, 0, "状態", true, false, MyDeptFlowAttr.WFSta);
                map.AddDDLSysEnum(MyFlowAttr.TSpan, 0, "時間帯", true, false, MyFlowAttr.TSpan, "@0=今週@ 1 =先週@2 = 2週間前@3 = 3週間前@4 =以前");

                map.AddTBStringDoc(MyDeptFlowAttr.FlowNote, null, "備考", true, false,true);
               

                //工作ID
                map.AddTBIntPK(MyDeptFlowAttr.WorkID, 0, "ジョブID", true, true);

                //隐藏字段.
                map.AddTBInt(MyDeptFlowAttr.FID, 0, "FID", false, false);
                map.AddTBString(MyDeptFlowAttr.FK_Dept, null, "部門", false, false, 0, 30, 10);
                ...
                //增加隐藏的查询条件.
                AttrOfSearch search = new AttrOfSearch(MyDeptFlowAttr.FK_Dept, "部門",
