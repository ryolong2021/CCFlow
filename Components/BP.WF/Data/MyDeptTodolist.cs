...
namespace BP.WF.Data
{
	...
	{	
        ...
        public MyDeptTodolist(Int64 workId)
        {
            ...
            if (qo.DoQuery() == 0)
                throw new Exception("仕事MyDeptTodolist[" + workId + "]存在しません。");
        }
        /// <summary>
        /// 执行修复
        /// </summary>
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_EmpWorks", "私の部署の処理待ち");
                map.Java_SetEnType(EnType.View);

                map.AddTBInt(MyDeptTodolistAttr.FID, 0, "FID", false, false);
                map.AddTBString(MyDeptTodolistAttr.Title, null, "フローのタイトル", true, false, 0, 300, 10, true);
                map.AddDDLEntities(MyDeptTodolistAttr.FK_Flow, null, "フロー", new Flows(), false);
                map.AddTBString(MyDeptTodolistAttr.RDT, null, "開始時間", true, false, 0, 100, 10);

               
                map.AddTBString(MyDeptTodolistAttr.StarterName, null, "起票者の名称", true, false, 0, 30, 10);

                map.AddTBString(MyDeptTodolistAttr.NodeName, null, "滞在ノード", true, false, 0, 100, 10);
                //map.AddTBString(MyDeptTodolistAttr.TodoEmps, null, "当前处理人", true, false, 0, 100, 10);

                map.AddTBStringDoc(MyDeptTodolistAttr.FlowNote, null, "備考", true, false,true);

                //作为隐藏字段.
                map.AddTBString(MyDeptTodolistAttr.WorkerDept, null, "スタッフ部門番号", 
                    false, false, 0, 30, 10);
               
                map.AddDDLEntities(MyDeptTodolistAttr.FK_Emp, null, "現在の処理者", new BP.WF.Data.MyDeptEmps(), false);
                map.AddTBIntPK(MyDeptTodolistAttr.WorkID, 0, "ジョブID", true, true);
                ...
                //增加隐藏的查询条件.
                AttrOfSearch search = new AttrOfSearch(MyDeptTodolistAttr.WorkerDept, "部門",
                ...
                rm.HisAttrs.AddDDLEntities("ToEmp", null, "引き渡す:", new BP.WF.Flows(), true);
                rm.HisAttrs.AddTBString("Note", null, "引渡理由", true, false, 0, 300, 100);
                map.AddRefMethod(rm);
