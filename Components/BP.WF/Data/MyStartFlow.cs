...
namespace BP.WF.Data
{
    ...
    public class MyStartFlow : Entity
    {
        ...
        public MyStartFlow(Int64 workId)
        {
            ...
            if (qo.DoQuery() == 0)
                throw new Exception("作業MyStartFlow[" + workId + "]存在しません。");
        }
        /// <summary>
        /// 执行修复
        /// </summary>
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_GenerWorkFlow", "私からのフロー");

                map.Java_SetEnType(EnType.View);

                map.AddTBIntPK(MyStartFlowAttr.WorkID, 0, "WorkID", false, false);
                map.AddTBString(MyStartFlowAttr.Title, null, "タイトル", true, false, 0, 300, 200, true);

                map.AddDDLEntities(MyStartFlowAttr.FK_Flow, null, "フロー", new Flows(), false);

                map.AddTBString(MyStartFlowAttr.BillNo, null, "伝票番号", true, true, 0, 100, 50);
                map.AddTBInt(MyStartFlowAttr.FK_Node, 0, "ノード番号", false, false);

                map.AddDDLSysEnum(MyStartFlowAttr.WFSta, 0, "状態", true, true, MyStartFlowAttr.WFSta, "@0=実行中@1=完了@2=その他");
                map.AddTBString(MyStartFlowAttr.Starter, null, "起票者", false, false, 0, 100, 100);
                map.AddTBDateTime(MyStartFlowAttr.RDT, "起票日", true, true);

                map.AddTBString(MyStartFlowAttr.NodeName, null, "滞在ノード", true, true, 0, 100, 100, false);
                map.AddTBString(MyStartFlowAttr.TodoEmps, null, "現在の処理者", true, false, 0, 100, 100, false);
                map.AddTBStringDoc(MyFlowAttr.FlowNote, null, "備考", true, false, true);


                map.AddTBString(MyFlowAttr.Emps, null, "参加者", false, false, 0, 4000, 100, true);
                map.AddDDLSysEnum(MyFlowAttr.TSpan, 0, "時間帯", true, false, MyFlowAttr.TSpan, "@0=今週@1=先週@2=2週間前@3=3週間前@4=以前");


                //隐藏字段.
                map.AddTBInt(MyStartFlowAttr.WFState, 0, "状態", false, false);
                ...
                //我发起的流程.
                AttrOfSearch search = new AttrOfSearch(MyStartFlowAttr.Starter, "起票者",
                    MyStartFlowAttr.Starter, "=", BP.Web.WebUser.No, 0, true);

                map.AttrsOfSearch.Add(search);

                search = new AttrOfSearch(MyStartFlowAttr.WFState, "フローの状態",
