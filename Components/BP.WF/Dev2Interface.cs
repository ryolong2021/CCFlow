...
namespace BP.WF
{
    ...
    public class Dev2Interface
    {
            ...
            get
            {
                ...
                switch (emp.HisAuthorWay)
                {
                    ...
                    case Port.AuthorWay.SpecFlows:
                        ...
                        break;
                    case Port.AuthorWay.None:
                        throw new Exception("相手(" + WebUser.No + ")は認証をキャンセルしました。");
                    default:
                        throw new Exception("no such way...");
                }
                return BP.DA.DBAccess.RunSQLReturnValInt(ps);
        ...
        public static void DTS_AutoStarterFlow(Flow fl)
        {
            ...
            if (i == 0)
            {
                BP.DA.Log.DefaultLogWriteLineError("フローの開始ノード(" + fl.Name + ")の開始データは設定されていません。マニュアルを参照してください。");
                return;
            }
            ...
            {
                errMsg += "@割り当てのマスタにStarter列がありません。";
            }

            if (dtMain.Columns.Contains("MainPK") == false)
            {
                errMsg += "@割り当てのマスタにMainPK列がありません。";
            }

            if (errMsg.Length > 2)
            {
                BP.DA.Log.DefaultLogWriteLineError("フロー(" + fl.Name + ")の始ノードは、イニシャルデータを設定し、不完全である。" + errMsg);
                return;
            }
            #endregion 检查数据源是否正确.
            ...
            foreach (DataRow dr in dtMain.Rows)
            {
                ...
                if (BP.Web.WebUser.No != starter)
                {
                    ...
                    {
                        BP.DA.Log.DefaultLogWriteLineInfo("@データ駆動型の開始フロー(" + fl.Name + ")イニシエーターを設定する:" + emp.No + "存在しません。");
                        continue;
                    }
        ...
        public static string GetFlowStarters(string fk_flow)
        {
            ...
            switch (nd.HisDeliveryWay)
            {
                ...
                case DeliveryWay.FindSpecDeptEmpsInStationlist: /*按岗位*/
                    ...
                    break;
                default:
                    throw new Exception("@開始ノードの人員アクセスルールが間違っているため、このアクセスタイプは開始ノードで設定できません:" + nd.HisDeliveryWay);
                    break;
            }
            return sql;
        }
        ...
        public static DataTable DB_GenerEmpWorksOfDataTable(string userNo, int fk_node = 0, string showWhat = null)
        {
            if (DataType.IsNullOrEmpty(userNo) == true)
                throw new Exception("err@ログイン情報を紛失しました。");
            string wfStateSql = "";
            ...
            if (WebUser.IsAuthorize == false)
            {
                ...
                if (fk_node == 0)
                {
                    ...
                    else
                    {
                        if (BP.WF.Glo.IsEnableTaskPool == true)
                        {
                            ps.SQL = "SELECT F.PTable AS TblName,* FROM WF_EmpWorks A left join WF_Flow AS F on A.FK_Flow =F.No  WHERE FK_Emp=" + dbstr + "FK_Emp AND TaskSta=0 AND " + wfStateSql + " ORDER BY  ADT DESC ";
                        }
                        else
                        {
                            ps.SQL = "SELECT F.PTable AS TblName,* FROM WF_EmpWorks A left join WF_Flow AS F on A.FK_Flow =F.No  WHERE FK_Emp=" + dbstr + "FK_Emp  AND " + wfStateSql + " ORDER BY ADT DESC ";
                        }
        ...
        public static DataTable DB_Todolist(string userNo = null)
        {
            if (userNo == null)
            {
                ...
                if (WebUser.IsAuthorize == false)
                {
                    throw new Exception("@このインターフェイスは、許可されたログインモードでは呼び出すことができません.");
                }
            }
        ...
        public static DataTable DB_TaskPool()
        {
            if (BP.WF.Glo.IsEnableTaskPool == false)
            {
                throw new Exception("@この操作を実行するには、Web.configでIsEnableTaskPoolを有効にする必要があります。");
            }
        ...
        public static DataTable DB_GenerWillReturnNodes(int fk_node, Int64 workid, Int64 fid)
        {
            ...
            if (nd.IsStartNode == true)
            {
                ...
                if (gwf.PWorkID == 0)
                {
                    throw new Exception("@現在のノードは開始ノードであり、サブフローではないため、戻りを実行できません。");
                }
            ...
            if (nd.HisRunModel == RunModel.SubThread)
            {
                ...
                foreach (Node ndFrom in nds)
                {
                    ...
                    switch (ndFrom.HisRunModel)
                    {
                        ...
                        default:
                            throw new Exception("フロー設計が異常です。子スレッドの前のノードを通常のノードにすることはできません。");
                    }
                ...
                if (dt.Rows.Count == 0)
                {
                    throw new Exception("err@返されるノードのリストを取得できませんでした。");
                }
            ...
            switch (nd.HisReturnRole)
            {
                ...
                case ReturnRole.ReturnPreviousNode:
                    ...
                    else
                    {
                        ...
                        {
                            throw new Exception("前のステップのノードを取得します。関係しないデータベースタイプ");
                        }
                ...
                case ReturnRole.ReturnSpecifiedNodes: //退回指定的节点。
                    ...
                    if (rnds.Count == 0)
                    {
                        throw new Exception("@フロー設計エラー、指定されたノードに戻るようにノードを設定できますが、指定されたノードセットが空です。ノードのプロパティで指定されたノードを設定してください。");
                    }
                ...
                case ReturnRole.ByReturnLine: //按照流程图画的退回线执行退回.
                    ...
                    if (dirs.Count == 0)
                    {
                        throw new Exception("@フロー設計エラー：現在のノードは戻るための戻り線を描画しません。詳細については、戻りルールを参照してください.");
                    }
            ...
            if (dt.Rows.Count == 0)
            {
                throw new Exception("@返却するノードが計算されていません。ノード返却のルールが妥当かどうか管理者に確認してください。現在のノード名:" + nd.Name + ",戻るルール:" + nd.HisReturnRole.ToString());
            }
        ...
        public static DataTable DB_GenerRuning(string userNo, string fk_flow, bool isMyStarter = false, string domain = null, bool isContainFuture = false)
        {
            ...
            if (isContainFuture == true)
            {
                ...
                {
                    case DBType.MySQL:
                        futureSQL = " UNION SELECT A.WorkID,A.Starter,A.StarterName,A.Title,A.DeptName,D.Name AS NodeName,A.RDT,B.FK_Node,A.FK_Flow,A.FID,A.FlowName,C.EmpName AS TodoEmps," + currNode + " AS CurrNode ,1 AS RunType FROM WF_GenerWorkFlow A, WF_SelectAccper B,"
            ...
            if (WebUser.IsAuthorize == true)
            {
                ...
                if (DataType.IsNullOrEmpty(fk_flow))
                {
                    if (isMyStarter == true)
                    {
                        sql = "SELECT DISTINCT a.WorkID,a.Starter,a.StarterName,a.Title,a.DeptName,a.NodeName,a.RDT,a.FK_Node,a.FK_Flow,a.FID,a.FlowName,a.TodoEmps," + currNode + " AS CurrNode ,0 AS RunType  FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND A.Starter=" + dbStr + "Starter  AND B.FK_Emp=" + dbStr + "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < -1) AND A.FK_Flow IN " + emp.AuthorFlows;
                        if (isContainFuture == true)
                        {
                    ...
                    {
                        sql = "SELECT DISTINCT a.WorkID,a.Starter,a.StarterName,a.Title,a.DeptName,a.NodeName,a.RDT,a.FK_Node,a.FK_Flow,a.FID ,a.FlowName,a.TodoEmps," + currNode + " AS CurrNode ,0 AS RunType FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr + "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < -1) AND A.FK_Flow IN " + emp.AuthorFlows;
                        if (isContainFuture == true)
                        {
                            sql += futureSQL;
                        }
                ...
                else
                {
                    if (isMyStarter == true)
                    {
                        sql = "SELECT DISTINCT a.WorkID,0 AS RunType FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE  A.FK_Flow=" + dbStr + "FK_Flow  AND A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr + "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < -1) AND  A.Starter=" + dbStr + "Starter AND A.FK_Flow IN " + emp.AuthorFlows;
                        if (isContainFuture == true)
                        {
                            sql += futureSQL;
                        }
                    ...
                    {
                        sql = "SELECT DISTINCT a.WorkID ,0 AS RunType FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.FK_Flow=" + dbStr + "FK_Flow  AND A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr + "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < -1) AND A.FK_Flow IN " + emp.AuthorFlows;
                        if (isContainFuture == true)
                        {
                            sql += futureSQL;
                        }
            ...
            if (WebUser.IsAuthorize == false)
            {
                ...
                if (DataType.IsNullOrEmpty(fk_flow))
                {
                    if (isMyStarter == true)
                    {
                        sql = "SELECT DISTINCT F.PTable AS TblName,a.WorkID,a.Starter,a.StarterName,a.Title,a.DeptName,a.NodeName,a.RDT,a.FK_Node,a.FK_Flow,a.FID ,a.FlowName,a.sendDT ,a.TodoEmps," + currNode + " AS CurrNode,0 AS RunType FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B left join WF_Flow AS F on B.FK_Flow =F.No WHERE A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr + "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < -1) AND  A.Starter=" + dbStr + "Starter ";
                        if (isContainFuture == true)
                        {
                            sql += futureSQL;
                        }
                ...
                else
                {
                    if (isMyStarter == true)
                    {
                        sql = "SELECT DISTINCT F.PTable AS TblName, a.WorkID,a.Starter,a.StarterName,a.Title,a.DeptName,a.NodeName,a.RDT,a.FK_Node,a.FK_Flow,a.FID ,a.FlowName,a.sendDT ,a.TodoEmps ," + currNode + " AS CurrNode,0 AS RunType FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B left join WF_Flow AS F on B.FK_Flow =F.No WHERE A.FK_Flow=" + dbStr + "FK_Flow  AND A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr + "FK_Emp AND B.IsEnable=1 AND (B.IsPass=1 or B.IsPass < -1 ) AND  A.Starter=" + dbStr + "Starter ";
                        if (isContainFuture == true)
                        {
                            sql += futureSQL;
                        }
        ...
        public static bool Port_CheckUserLogin(string userNo, string SID)
        {
            ...
            if (mysid == null)
            {
                throw new Exception("@ユーザー(" + userNo + ")のSIDを獲得していません。");
            }
        ...
        public static bool Port_SetSID(string userNo, string sid)
        {
            ...
            catch (Exception ex)
            {
                if (BP.DA.DBAccess.IsView("Port_Emp", SystemConfig.AppCenterDBType) == true)
                {
                    throw new Exception("@SIDの更新に失敗しました。たとえば、組織構造を統合するときにSIDを更新するようにSQLを構成する必要があります。例：update MyUserTable SET SID = @ SID WHERE BH = '@ No'");
                }
            ...
            string cFlowInfo, string optionMsg = null, string empNoTo = null, string empNameTo = null, string empNoFrom = null, string empNameFrom = null, string rdt = null, string fwcView = null)
        {
            if (at == ActionType.CallChildenFlow)
            {
                ...
                {
                    throw new Exception("@CallChildenFlowモードで、情報cFlowInfo情報を入力する必要があります。");
                }
            }
        ...
        public static string Flow_DoDeleteFlowByReal(string flowNo, Int64 workID, bool isDelSubFlow = false)
        {
            ...
            {
                throw new Exception("@削除前のエラー，" + ex.StackTrace);
            }
            return "正常に削除されました";
        }
        public static string Flow_DoDeleteDraft(string flowNo, Int64 workID, bool isDelSubFlow)
        {
            ...
            {
                return "err@フローを開始しなかったか、管理者ではないため、下書きを削除できません。";
            }
            ...
            BP.DA.Log.DefaultLogWriteLineInfo(WebUser.Name + "FlowNo：'" + flowNo + "',workID：'" + workID + "'のデータを削除しました。");
        ...
        public static string Flow_DoDeleteFlowByFlag(string flowNo, Int64 workID, string msg, bool isDelSubFlow)
        {
            ...
            if (isDelSubFlow)
            {
                ...
                {
                    Flow_DoDeleteFlowByFlag(item.FK_Flow, item.WorkID, "サブフローを削除:" + msg, false);
                }
                //删除子流程
        ...
        public static DataTable Flow_GetPreviousNodeTrack(Int64 WorkID, int FK_Node)
        {
            ...
            if (gwf.RetrieveFromDBSources() == 0)
                throw new Exception("関連するビジネスインスタンスは見つかりませんでした");
        ...
        public static string FlowOverAutoSendParentOrSameLevelFlow(GenerWorkFlow gwf, Flow flow)
        {
            ...
            if (gwf.PWorkID != 0)
            {
                ...
                if (slWorkID == 0)//启动该流程的是父子流程
                {
                    ...
                    if (count == 0)
                        throw new Exception("親子フロー関係の構成情報がありません。管理者に連絡してください");
                    ...
                    if (flow.IsToParentNextNode == true || subFlow.IsAutoSendSubFlowOver == 1)
                    {
                        ...
                        return "親フローは自動的に次のノードまで実行されます," + returnObjs.ToMsgOfHtml();
                    }
                    //结束父流程
                    if (subFlow.IsAutoSendSubFlowOver == 2)
                    {
                        ...
                        string flowOver = BP.WF.Dev2Interface.Flow_DoFlowOver(gwf.PFlowNo, gwf.PWorkID, "親フロー[" + fl.Name + "],WorkIDが[" + gwf.PWorkID + "]正常に終了しました");
                        return flowOver;
                    }
                ...
                else // 启动的是同级子流程
                {
                    ...
                    if (count == 0)
                        throw new Exception("同じレベルのサブフロー関係の構成情報が失われています。管理者に連絡してください");
                    SubFlow subFlow = subFlows[0] as SubFlow;
                    ...
                    if (subFlow.IsAutoSendSLSubFlowOver == 2)
                    {
                        if (subgwf.WFState == WFState.Complete)
                            return "";
                        return BP.WF.Dev2Interface.Flow_DoFlowOver(slFlowNo, slWorkID, "同じレベルのサブフローフロー[" + fl.Name + "],WorkIDが[" + slWorkID + "]正常に終了しました");
                    }
        ...
        public static bool Flow_IsCanStartThisFlow(string flowNo, string userNo, string pFlowNo = null, int pNodeID = 0, Int64 pworkID = 0)
        {
            ...
            if (nd.IsGuestNode == true)
            {
                if (BP.Web.WebUser.No != "Guest")
                    throw new Exception("@現在のノードはゲスト処理ノードですが、現在は{" + BP.Web.WebUser.No + "}ゲストアカウントではありません。");
                return true;
            }
            ...
            if (SystemConfig.OSDBSrc == OSDBSrc.Database)
            {
                switch (nd.HisDeliveryWay)
                {
                    ...
                    case DeliveryWay.BySelected:
                        ...
                        break;
                    default:
                        throw new Exception("@開始ノードはこのアクセスルールを設定できません：" + nd.HisDeliveryWay);
                }
            }
            else
            {
                switch (nd.HisDeliveryWay)
                {
                    ...
                    case DeliveryWay.ByDept:
                        ...
                        //ps.Add("FK_Node", nd.NodeID);
                        //num = DBAccess.RunSQLReturnValInt(ps);
                        throw new Exception("@現在サポートされていません.");
                        break;
                    case DeliveryWay.ByBindEmp:
                        ps.SQL = "SELECT COUNT(*) AS Num FROM WF_NodeEmp WHERE FK_Emp=" + dbstr + "FK_Emp AND FK_Node=" + dbstr + "FK_Node";
                        ps.Add("FK_Emp", userNo);
            ...
            foreach (SubFlowHand item in subflows)
            {
                ...
                if (item.StartOnceOnly == true)
                {
                    ...
                    {
                        throw new Exception("フローは一度だけ開始できます.");
                    }
                }
                ...
                if (item.CompleteReStart == true)
                {
                    ...
                    if (dt.Rows.Count != 0)
                    {
                        ...
                        {
                            throw new Exception("フローは開始されましたが、実行が終了しておらず、再度開始することはできません.");
                        }
                ...
                if (item.IsEnableSpecFlowStart == true)
                {
                    ...
                    foreach (string flStr in fls)
                    {
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            ...
                            throw new Exception("フロー：[" + myflow.Name + "]起動せずに開始することはできません[" + item.SubFlowName + "]。");
                        }
                    }
                }
                ...
                if (item.IsEnableSpecFlowOver == true)
                {
                    ...
                    foreach (string flStr in fls)
                    {
                        ...
                        {
                            BP.WF.Flow myflow = new Flow(flStr);
                            throw new Exception("フロー：[" + myflow.Name + "]終わらないと始められない[" + item.SubFlowName + "]。");
                        }
                    }
                }
            }
            ...
            if (fl.StartLimitRole == StartLimitRole.OnlyOneSubFlow)
            {
                ...
                throw new Exception("このフローでは、1つのサブフローのみを開始できます.");
            }
            #endregion 判断流程属性的规则.
        ...
        public static bool Flow_IsCanViewTruck(string flowNo, Int64 workid, string userNo = null)
        {
            ...
            if (fk_dept == null)
            {
                ...
                if (fk_dept == null)
                {
                    throw new Exception("@フローエンジンデータが削除されました。");
                }
            }
        ...
        public static string Flow_DeleteSubThread(string flowNo, Int64 workid, string info)
        {
            ...
            if (gwf.RetrieveFromDBSources() > 0)
            {
                ...
                BP.WF.Dev2Interface.WriteTrackInfo(flowNo, gwf.FK_Node, gwf.NodeName, gwf.FID, 0, info, "子スレッドを削除");
                return msg;
            }
            return null;
        }
        ...
        public static string Flow_DoPress(Int64 workID, string msg, bool isPressSubFlow = false)
        {
            ...
            string mailTitle = "催促:" + gwf.Title + ", 送信者:" + WebUser.Name;
            ...
            if (isPressSubFlow)
            {
                ...
                foreach (GenerWorkFlow item in gwfs)
                {
                    subMsg += "@子スレッド:" + item.Title + "リマインダーは以下の通りです:";
                    subMsg += Flow_DoPress(item.WorkID, msg, false);
                }
                return "システムはあなたの情報をに通知しました:" + toEmpName + "" + subMsg;
            }
            else
            {
                return "システムはあなたの情報をに通知しました:" + toEmpName;
            }
        }
        /// <summary>
        /// 重新设置流程标题
        ...
        public static bool Flow_SetFlowParas(string flowNo, Int64 workid, string paras)
        {
            ...
            if (gwf.RetrieveFromDBSources() == 0)
            {
                throw new Exception("作成フローIDが存在しません。");
            }
        ...
        public static string Flow_Schedule(Int64 workid, int toNodeID, string toEmper)
        {
            ...
            return "フロー(" + gwf.Title + ")、(" + nd.Name + ")にスケジューリングされています,(" + emp.Name + ")に割り当てられた";
        }
        /// <summary>
        /// 设置流程运行模式
        ...
        public static string Flow_BBSDelete(string flowNo, string mypk, string username)
        {
            ...
            {
                return "削除できませんでした。自分のコメントのみ削除できます。";
            }
        }
        ...
        public static void Flow_Focus(Int64 workid)
        {
            ...
            if (i == 0)
            {
                throw new Exception("@設定ミス：WorkID=" + workid + " のインスタンスが見つかりませんでした。");
            }
        ...
        public static string Flow_ReSend(Int64 workid, int toNodeID, string toEmpIDs, string note)
        {
            ...
            {
                return "err@フローが実行され、調整を実行できません。ロールバックを実行できます.";
            }
            ...
            if (i == 0)
            {
                return "err@現在のTo Do担当者が見つかりません.";
            }
            ...
            return "調整が成功しました。" + gwf.NodeName + " から調整:, 調整までに:" + todoEmps;
        }
        /// <summary>
        /// 取消、确认.
        /// </summary>
        ...
        {
            if (WebUser.No == null)
                throw new Exception("@不正なユーザーです。ログインして、もう一度お試しください。");
        ...
        public static bool Flow_AuthorCancel()
        {
            ...
            BP.DA.Log.DefaultLogWriteLineInfo("承認をキャンセル:" + WebUser.No + "(" + myau.Author + ")の授権をキャンセルしました。");
            myau.Author = "";
            myau.AuthorWay = 0;
            myau.AuthorDate = "";
            myau.AuthorToDate = "";
...
#warning 增加是防止手动启动子流程或者平级子流程时关闭子流程页面找不到待办 保存到待办
        ...
        public static void Node_AddTodolist(Int64 workid, string todoEmps)
        {
            GenerWorkFlow gwf = new GenerWorkFlow(workid);
            if (gwf.WFState == WFState.Complete)
                throw new Exception("フロー：" + gwf.Title + "既に完了しているため、受信者を追加できません.");
        ...
        public static void Node_InsertOrderEmp(string flowNo, int nodeID, Int64 workid, Int64 fid, string empNo, string empName)
        {
            ...
            if (i == 0)
            {
                throw new Exception("@現在のスタッフのタスクを見つけることができません。ノードでフローが実行されているかどうかを確認してください。");
            }
            ...
            {
                throw new Exception("@この人物はすでに処理キューに存在しているため、追加できません。");
            }
            ...
            string msgTitle, string msgDoc)
        {
            ...
            return toEmpNo + "," + toEmpName + " にCCしました。";
        }
        /// <summary>
        /// 执行抄送
        /// </summary>
        ...
        public static string Node_CC(string fk_flow, int fk_node, Int64 workID, string toEmpNo, string toEmpName, string msgTitle, string msgDoc, string pFlowNo, Int64 pWorkID)
        {
            ...
            //发送邮件.
            BP.WF.Dev2Interface.Port_SendMsg(toEmpNo, WebUser.Name + "仕事:" + gwf.Title, "CC:" + msgTitle, "CC" + nd.NodeID + "_" + workID + "_", BP.WF.SMSMsgType.CC,
        ...
        public static void Node_DeleteDraft(string fk_flow, Int64 workID)
        {
            ...
            if (gwf.RetrieveFromDBSources() == 1)
            {
                ...
                {
                    throw new Exception("@下書きフローではないので、は削除できません:" + gwf.Title);
                }

                if (gwf.WFState != WFState.Draft)
                {
                    throw new Exception("@下書きではないと削除できません");
                }
        ...
        public static void Node_SetDraft2Todolist(string fk_flow, Int64 workID)
        {
            ...
            if (gwf.RetrieveFromDBSources() == 1 && (gwf.WFState == WFState.Draft || gwf.WFState == WFState.Blank || gwf.WFState == WFState.Runing))
            {
                if (gwf.FK_Node != int.Parse(fk_flow + "01"))
                {
                    throw new Exception("@to-doの設定がエラーです。ノードの起動時にのみto-doを設定できます。現在のノードは:" + gwf.NodeName);
                }
        ...
        public static void Node_SetDraft(string fk_flow, Int64 workID)
        {
            ...
            gwf.WorkID = workID;
            if (gwf.RetrieveFromDBSources() == 0)
                throw new Exception("@仕事は失われます。");

            if (gwf.WFState == WFState.Blank)
            {
                if (gwf.FK_Node != int.Parse(fk_flow + "01"))
                {
                    throw new Exception("@正常に下書きを設定できませんでした。ドラフトを設定できるのは、ノードを起動したときのみです。現在のノードは:" + gwf.Title);
                }
        ...
        {
            if (htWork == null)
                throw new Exception("パラメータエラー、htWorkは空にできません。保存に失敗しました。");
            ...
            try
            {
                ...
                {
                    return "err@保存の実行後、イベント中にエラーが発生しました:" + ex.Message;
                }
                #endregion
            ...
            {
                throw new Exception("err@Node_SaveWork保存エラー:" + ex.Message + ", 技術情報：" + ex.StackTrace);
            }
        }
        /// <summary>
        /// 保存独立表单
        ...
        public static bool Node_TaskPoolTakebackOne(Int64 workid)
        {
            ...
            {
                throw new Exception("@構成が共有タスクプールの状態に設定されていません。");
            }

            GenerWorkFlow gwf = new GenerWorkFlow(workid);
            if (gwf.TaskSta == TaskSta.None)
            {
                throw new Exception("@このタスクは共有タスクではありません。");
            }

            if (gwf.TaskSta == TaskSta.Takeback)
            {
                throw new Exception("@タスクは他の誰かに奪われました。");
            }
            ...
            BP.WF.Dev2Interface.WriteTrackInfo(gwf.FK_Flow, gwf.FK_Node, gwf.NodeName, workid, 0, "タスクは" + WebUser.Name + "タスクプールから奪う.", "取得");
            if (i > 0)
            {
                Paras ps1 = new Paras();
        ...
        public static void Node_TaskPoolPutOne(Int64 workid)
        {
            ...
            {
                throw new Exception("@タスクはすでに共有されています。");
            }
            ...
            if (i < 0)//有可能是只有一个人
            {
                throw new Exception("@フローデータが間違っているため、データを更新できないわけない。");
            }
            ...
            if (i > 0)
            {
                ...
                {
                    throw new Exception("@フローデータエラー、更新する保留中の人物が見つかりませんでした。");
                }
            ...
            BP.WF.Dev2Interface.WriteTrackInfo(gwf.FK_Flow, gwf.FK_Node, gwf.NodeName, workid, 0, "タスクは" + WebUser.Name + "タスクセット.", "に入れる");
        }
        /// <summary>
        ...
        public static string Node_Askfor(Int64 workid, AskforHelpSta askforSta, string askForEmp, string askForNote)
        {
            ...
            if (emp.RetrieveFromDBSources() == 0)
            {
                throw new Exception("@署名する個人番号が間違っています:" + askForEmp);
            }
            ...
            {
                throw new Exception("@現在のジョブは他のユーザーによって処理されたか、ジョブを処理する権限がありません.");
            }
            ...
            {
                throw new Exception("@承認が失敗しました。選択した承認者は現在の作業を処理できます。");
            }
            ...
            if (gwls.Count == 0)
            {
                ...
                {
                    Flow_SetFlowTitle(gwf.FK_Flow, workid, "から" + WebUser.Name + "仕事の承認。");
                }
            }
            // endWarning.
            ...
            string msg = "あなたの仕事がの作品は承認のために(" + askForEmp + " " + emp.Name + ")に提出されました。";
        ...
        public static string Node_AskforReply(Int64 workid, string replyNote)
        {
            ...
            {
                if (ex.Message.Contains("仕事の次のステップを選択してください") == true || ex.Message.Contains("ユーザーは送信先のノードを選択しませんでした") == true)
                {
                    ...
                    return "err@次のノードの受信者ルールでは、現在のノードが選択対象として選択されています。現在のノードのプロパティで、受信者ボタンをアクティブにしないと、システムが自動的に起動を開始します。ウィンドウを閉じて、もう一度開いてください。。" + ex.Message;
                }
                return ex.Message;
            }
        ...
        public static string Node_Shift(Int64 workID, string toEmp, string msg)
        {
                ...
                || nd.TodolistModel == TodolistModel.TeamupGroupLeader)
            {
                ...
                {
                    return "@転送に失敗しました、転送した人(" + emp.No + " " + emp.Name + ")すでにエージェントリストにあります.";
                }
                ...
                string info = "@仕事の移管は成功しました。 @あなたは作品をに転送しました：" + emp.No + " , " + emp.Name;

                //移交后事件 @yuanlina
                string atPara1 = "@SendToEmpIDs=" + emp.No;
                info += "@" + nd.HisFlow.DoFlowEventEntity(EventListOfNode.ShitAfter, nd, work, atPara1);

                info += "@<a href='" + Glo.CCFlowAppPath + "WF/MyFlowInfo.htm?DoType=UnShift&FK_Flow=" + nd.FK_Flow + "&WorkID=" + workID + "&FK_Node=" + gwf.FK_Node + "&FID=" + gwf.FID + "' ><img src='./Img/Action/UnSend.png' border=0 />仕事の転送を元に戻す</a>.";
            ...
            {
                throw new Exception("@処理が完了しました、引き継ぎはできません。");
            }
            ...
            string inf1o = "@仕事の移管は成功しました。 @あなたは作品を" + emp.No + " に転送しました：, " + emp.Name;
            //移交后事件 @yuanlina
        ...
        public static string Node_ReturnWork(string fk_flow, Int64 workID, Int64 fid, int currentNodeID, int returnToNodeID,
            string returnToEmp, string msg = "無し", bool isBackToThisNode = false, string pageData = null)
        {
        ...
        public static void Node_SetWorkRead(int nodeID, Int64 workid, string empNo)
        {
            ...
            if (nd.ReadReceipts == ReadReceipts.BySysField)
            {
                ...
                if (fromNodeID == 0)
                {
                    throw new Exception("@前の仕事が見つかりませんでした。");
                }
                ...
                {
                    throw new Exception("@フロー設計エラー:" + ex.Message + " 現在のノードで、前のステップのフォームフィールドを設定して、領収書を返すかどうかを決定しましたが、前のノードのフォームに同意されたフィールドがありません。");
                }
            }
            ...
            if (nd.ReadReceipts == ReadReceipts.BySDKPara)
            {
                ...
                {
                    throw new Exception("@フロー設計エラー：現在のノードで、開発者パラメーターを設定して、領収書を返すかどうかを決定しましたが、パラメーターが見つかりません。");
                }
            ...
            if (isSend == true)
            {
                ...
                //发送已读回执。
                BP.WF.Dev2Interface.Port_SendMsg(sender, "領収書を読む:" + title,
                    "送信したジョブは" + WebUser.Name + "に" + DataType.CurrentDataTimeCNOfShort + " 開く",
        ...
        public static string CCForm_Seal(Int64 workid, int nodeid, string deptno, string stationno)
        {
            try
            {
                ...
                {
                    return @"署名ファイル：" + sealimg + "存在しません、管理者に連絡してください！";
                }
        ...
        public static string CCForm_AddAth(int nodeid, string flowid, Int64 workid, String athNo, string frmID, string filePath, string fileName, string sort = null, Int32 fid = 0, Int32 pworkid = 0)
        {
            ...
            if (athDesc.AthSaveWay == AthSaveWay.IISServer)
            {
                ...
                if (savePath.Contains("@") == true || savePath.Contains("*") == true)
                {
                    ...
                    {
                        throw new Exception("@パスが正しく構成されておらず、変数が正しく置き換えられていない。" + savePath);
                    }
                }
                else
                {
                    savePath = athDesc.SaveTo + Path.DirectorySeparatorChar + pkVal;
                }
                ...
                {
                    savePath = SystemConfig.PathOfDataUser + "UploadFile" + Path.DirectorySeparatorChar + mapData.No + Path.DirectorySeparatorChar;
                ...
                {
                    throw new Exception("err@パスの作成中にエラーが発生しました。許可がないか、パス構成に問題がある可能性があります:" + savePath + "@例外情報:" + ex.Message);
                }

                string guid = BP.DA.DBAccess.GenerGUID();
                string ext = fileName.Substring(fileName.LastIndexOf("."));
                string realSaveTo = savePath + Path.DirectorySeparatorChar + guid + "." + fileName;
        ...
        public static int WorkOpt_ToNodes_GetLasterSelectNodeID(string flowNo, int nodeID)
        {
            ...
            switch (SystemConfig.AppCenterDBType)
            {
                ...
                case DBType.PostgreSQL:
                    ...
                    break;
                default:
                    throw new Exception("@このタイプのデータベースサポートは実装していません。");
            }
        ...
        private static DataTable WorkOpt_Accepter_ByStation(int nodeID)
        {
            if (nodeID == 0)
            {
                throw new Exception("@フロー設計が間違っており、ターニングポイントはありません。例：現在のノードはAです。ノードAのプロパティで[受信者]ボタンを有効にすると、それは一連の回転ノード（つまり、Aが回転できるノードのセット）に含まれます。例：AからB、AからC、次にB、Cノードが回転ノードコレクション）、ノードの属性に[アクセスルール]が[前のステップで送信者によって選択された]に設定されているノードが必要です");
            }
            ...
            if (stas.Count == 0)
            {
                ...
                throw new Exception("@フロー設計エラー：設計者がノード[" + toNd.Name + "]を受信者の仕事の範囲に設計していません。");
            }
            // 优先解决本部门的问题。
            string sql = "";
        ...
        public static string SaveImageAsFile(byte[] img, string pkval, string fk_Frm_Ele)
        {
            ...
            using (MemoryStream ms = new MemoryStream(img, 0, img.Length))
            {
                ...
                if (System.IO.Directory.Exists(fe.HandSigantureSavePath + Path.DirectorySeparatorChar + fe.FK_MapData + Path.DirectorySeparatorChar) == false)
                {
                    System.IO.Directory.CreateDirectory(fe.HandSigantureSavePath + Path.DirectorySeparatorChar + fe.FK_MapData + Path.DirectorySeparatorChar);
                }

                string saveTo = fe.HandSigantureSavePath + Path.DirectorySeparatorChar + fe.FK_MapData + Path.DirectorySeparatorChar + pkval + ".jpg";
        ...
        {
            string path = HttpContextHelper.RequestApplicationPath + Path.DirectorySeparatorChar + "DataUser" + Path.DirectorySeparatorChar + "UploadFile";
        ...
        public static void DTS_GenerWorkFlowTodoSta()
        {
            ...
            if (DateTime.Now.Hour >= 9 && DateTime.Now.Hour < 12)
            {
                ...
                if (time == null)
                {
                    ...
                    var.No = timeKey;
                    var.Name = "時間依存のスケジューリングWFTodoSta PMスケジューリング";
                    var.GroupKey = "WF";
                    var.Val = timeKey;  //更新时间点.
                    var.Note = "時間に敏感なスケジューリングPM" + timeKey;
                    var.Insert();
                    time = var.Val;
                }
                else
            ...
            if (DateTime.Now.Hour >= 13 && DateTime.Now.Hour < 18)
            {
                ...
                if (time == null)
                {
                    GloVar var = new GloVar();
                    var.No = timeKey;
                    var.Name = "時間依存のスケジューリングWFTodoSta AMスケジューリング";
                    var.GroupKey = "WF";
                    var.Val = timeKey;  //更新时间点.
                    var.Note = "時間依存のスケジューリングAM" + timeKey;
                    var.Insert();
                    time = var.Val;
                }
                else
        ...
        public static void DTS_GenerWorkFlowTimeSpan()
        {
            if (DateTime.Now.Hour >= 8 && DateTime.Now.Hour < 10 && DateTime.Today.DayOfWeek == DayOfWeek.Monday)
            {
                ...
                if (time == null)
                {
                    GloVar var = new GloVar();
                    var.No = timeKey;
                    var.Name = "期間を設定" + timeKey + "週に一度.";
                    var.GroupKey = "WF";
                    var.Val = timeKey;  //更新时间点.
                    var.Note = "期間PMを設定" + timeKey;
                    var.Insert();
                }
                else
                {
