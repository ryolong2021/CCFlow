    ...
    public class GenerBill : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("Frm_GenerBill", "帳票制御テーブル");
                map.Java_SetEnType(EnType.Sys);

                map.AddTBIntPK(GenerBillAttr.WorkID, 0, "WorkID", true, true);

                map.AddTBString(GenerBillAttr.FK_FrmTree, null, "帳票制御テーブルタイプ", true, false, 0, 10, 10);
                map.AddTBString(GenerBillAttr.FrmID, null, "帳票ID", true, false, 0, 100, 10);
                map.AddTBString(GenerBillAttr.FrmName, null, "帳票名", true, false, 0, 200, 10);

                map.AddTBString(GenerBillAttr.BillNo, null, "帳票番号", true, false, 0, 100, 10);
                map.AddTBString(GenerBillAttr.Title, null, "タイトル", true, false, 0, 1000, 10);
                map.AddDDLSysEnum(GenerBillAttr.BillSta, 0, "ステータス（簡略）", true, false, GenerBillAttr.BillSta, "@0=実行中@ 1 =完了@ 2 =その他");
                map.AddDDLSysEnum(GenerBillAttr.BillState, 0, "帳票のステータス", true, false, GenerBillAttr.BillState, "@0=空白@ 1 =ドラフト@ 2 =編集@ 100 =アーカイブ");

                map.AddTBString(GenerBillAttr.Starter, null, "作成者", true, false, 0, 200, 10);
                map.AddTBString(GenerBillAttr.StarterName, null, "作成者名前", true, false, 0, 200, 10);
                map.AddTBString(GenerBillAttr.Sender, null, "発送者", true, false, 0, 200, 10);

                map.AddTBDateTime(GenerBillAttr.RDT, "記録日", true, true);
                map.AddTBDateTime(GenerBillAttr.SendDT, "帳票の活動時間", true, true);
                map.AddTBInt(GenerBillAttr.NDStep, 0, "ステップ", true, false);
                map.AddTBString(GenerBillAttr.NDStepName, null, "ステップ名", true, false, 0, 100, 10);

                map.AddTBString(GenerBillAttr.FK_Dept, null, "部門", true, false, 0, 100, 10);
                map.AddTBString(GenerBillAttr.DeptName, null, "部門名", true, false, 0, 100, 10);
                map.AddTBInt(GenerBillAttr.PRI, 1, "優先", true, true);

                map.AddTBDateTime(GenerBillAttr.SDTOfNode, "ノード完了時間", true, true);
                map.AddTBDateTime(GenerBillAttr.SDTOfFlow, "帳票完了時間", true, true);

                //父子单据信息.
                map.AddTBString(GenerBillAttr.PFrmID, null, "親帳票番号", true, false, 0, 3, 10);
                map.AddTBInt(GenerBillAttr.PWorkID, 0, "親帳票ID", true, true);
                map.AddDDLSysEnum(GenerBillAttr.TSpan, 0, "時間帯", true, false, GenerBillAttr.TSpan, "@0=今週@ 1 =先週@ 2 =先々週@ 3 =もっと前");
                 
                //参数.
                map.AddTBString(GenerBillAttr.AtPara, null, "パラメーター（親帳操作設定に一時的に保存されるパラメーター）", true, false, 0, 2000, 10);
                map.AddTBString(GenerBillAttr.Emps, null, "参加者", true, false, 0, 4000, 10);
