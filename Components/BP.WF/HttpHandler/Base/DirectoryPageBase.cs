...
namespace BP.WF.HttpHandler
{
    abstract public class DirectoryPageBase
    {
        ...
        public string DoMethod(DirectoryPageBase myEn, string methodName)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (ex.InnerException != null)
                    ...
                    return "err@呼び出しクラス:[" + myEn + "]方法:[" + methodName + "]エラーが発生しました:" + ex.InnerException;
                else
                    if (ex.Message.IndexOf("err@") == 0)
                        return ex.Message;
                    else
                        return "err@呼び出しクラス:[" + myEn + "]方法:[" + methodName + "]エラーが発生しました:" + ex.Message;
            }
        }
        /// <summary>
        /// 执行默认的方法名称
        ...
        protected virtual string DoDefaultMethod()
        {
            ...
            return "err@サブクラス[" + this.ToString() + "]書き直さなかった[" + this.DoType + "]メソッド、メソッドが欠落しているか非公開かを確認してください。";
        }
        #endregion 执行方法.
        ...
        public int GetValIntFromFrmByKey(string key)
        {
            ...
            if (str == null || str == "")
                throw new Exception("@パラメータ:" + key + "値なし。");
            return int.Parse(str);
        }
        ...
        protected string ExportGroupExcel(System.Data.DataSet ds, string title,string paras)
        {
            ...
            try
            {
                ...
                string strLine = "シリアルナンバー" + Convert.ToChar(9);
                //生成文件标题
            ...
            {   
                ...
                foreach (Attr attr in selectedAttrs)
                {
                    ...
                    if(attr.MyFieldType == FieldType.RefText)
                        strLine = strLine + attr.Desc.Replace("名前","") + Convert.ToChar(9);
                    else
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    foreach (Attr attr in selectedAttrs)
                    {
                        ...
                        {
                            strLine = strLine + (dr[attr.Key].Equals(1) ? "はい" : "いいえ") + Convert.ToChar(9);
                        }
                        else
                        {
                ...
                objStreamWriter.WriteLine();
                objStreamWriter.WriteLine(Convert.ToChar(9) + " 時計職人：" + Convert.ToChar(9) + WebUser.Name + Convert.ToChar(9) + "日付：" + Convert.ToChar(9) + DateTime.Now.ToShortDateString());
            ...
            string creator = null, bool date = false, bool index = true, bool download = false)
        {
            ...
            //序
            if (index && dt.Columns.Contains("シーケンス") == false)
            {
                dt.Columns.Add("シーケンス", typeof(int)).ExtendedProperties.Add("width", 50);
                dt.Columns["シーケンス"].SetOrdinal(0);
            ...
            using (FileStream fs = new FileStream(file, FileMode.Create))
            {
                ...
                foreach (DataColumn col in dt.Columns)
                {
                    ...
                    cell.CellStyle = col.ColumnName == "シーケンス" ? idxTitleStyle : titleStyle;
                ...
                if (creatorRow != null)
                {
                    ...
                    cell.SetCellValue("時計職人：" + creator);
                    cell.CellStyle = userStyle;
                    ...
                    foreach (DataColumn col in dt.Columns)
                    {
                        ...
                        switch (col.DataType.Name)
                        {
                            ...
                            case "Int64":
                                ...
                                cell.CellStyle = col.ColumnName == "シーケンス"
                                                     ? idxCellStyle
