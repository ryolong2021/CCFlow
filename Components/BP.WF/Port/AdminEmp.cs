    ...
    public class AdminEmp : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddDDLSysEnum(AdminEmpAttr.UserType, 3, "ユーザーステータス", true, true, AdminEmpAttr.UserType, "@0=通常のユーザー@1=管理者ユーザー");

                map.AddDDLEntities(AdminEmpAttr.RootOfFlow, null, "フロー権限ノード", new BP.WF.Template.FlowSorts(), true);
                map.AddDDLEntities(AdminEmpAttr.RootOfForm, null, "フォーム許可ノード", new BP.WF.Template.SysFormTrees(), true);
                map.AddDDLEntities(AdminEmpAttr.RootOfDept, null, "組織構造権限ノード", new BP.WF.Port.Incs(), true);
        ...
        public string DoAdd(string empNo, string orgNo)
        {
            ...
            if (emp.RetrieveFromDBSources() == 0)
                return "err@管理者の追加が失敗しました、ID=" + empNo + "ユーザーテーブルはありません。追加する管理者はPort_Empユーザーテーブルに存在している必要があります";
            ...
            {
                if (ae.IsAdmin == true)
                    return "err@管理者は既に存在します。該当管理者を削除して、もう一度追加してくださいdelete from wf_emp where no='" + empNo + "'";
                ae.Delete();
            }
