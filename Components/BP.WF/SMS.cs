    ...
    public class SMS : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("Sys_SMS", "メッセージ");

                map.AddMyPK();

                map.AddTBString(SMSAttr.Sender, null, "送信者（空にすることができます）", false, true, 0, 200, 20);
                map.AddTBString(SMSAttr.SendTo, null, "送信先（空にすることができます）", false, true, 0, 200, 20);
                map.AddTBDateTime(SMSAttr.RDT, "書き込み時間", true, false);

                map.AddTBString(SMSAttr.Mobile, null, "電話番号（空欄でもかまいません）", false, true, 0, 30, 20);
                map.AddTBInt(SMSAttr.MobileSta, (int)MsgSta.UnRun, "メッセージのステータス", true, true);
                map.AddTBString(SMSAttr.MobileInfo, null, "SMSメッセージ", false, true, 0, 1000, 20);

                map.AddTBString(SMSAttr.Email, null, "Email(空にすることができます)", false, true, 0, 200, 20);
                map.AddTBInt(SMSAttr.EmailSta, (int)MsgSta.UnRun, "EmaiStaメッセージのステータス", true, true);
                map.AddTBString(SMSAttr.EmailTitle, null, "タイトル", false, true, 0, 3000, 20);
                map.AddTBStringDoc(SMSAttr.EmailDoc, null, "内容", false, true);
                map.AddTBDateTime(SMSAttr.SendDT, null, "発送時間", false, false);

                map.AddTBInt(SMSAttr.IsRead, 0, "読み込みますか？", true, true);
                map.AddTBInt(SMSAttr.IsAlert, 0, "ヒントを与えますか？", true, true);

                map.AddTBString(SMSAttr.MsgFlag, null, "メッセージタグ（繰り返し送信を防ぐために使用されます）", false, true, 0, 200, 20);
                map.AddTBString(SMSAttr.MsgType, null, "メッセージの種類（CC CC、実行するTodolist、Returnなどのその他のメッセージ...）", false, true, 0, 200, 20);
        ...
        public static async Task<bool> SendEmailNowAsync(string mail, string mailTitle, string mailDoc)
        {
            return await Task.Run(() =>
            {
                ...
                string displayName = SystemConfig.GetValByKey("SendEmailDisplayName", "ギャロッピングBPM");
        ...
        protected override void afterInsert()
        {
            ...
            {
                BP.DA.Log.DebugWriteError("@メッセージメカニズムが正常に構成されていません。" + ex.Message);
            }
            base.afterInsert();
        }
    }
