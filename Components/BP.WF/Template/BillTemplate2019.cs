...
namespace BP.WF.Template
{
	...
	public class BillTemplate2019 : EntityNoName
    {
        ...
        public byte[] GenerTemplateFile()
        {
            ...
            //如果没有找到，就看看默认的文件是否有.
            string tempExcel = BP.Sys.SystemConfig.PathOfDataUser + "CyclostyleFile" + Path.DirectorySeparatorChar + this.No + ".rtf";
            if (System.IO.File.Exists(tempExcel) == false)
                tempExcel = BP.Sys.SystemConfig.PathOfDataUser + "CyclostyleFile" + Path.DirectorySeparatorChar + "Wordドキュメントテンプレート定義demo.docx";
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("WF_BillTemplate", "ドキュメントテンプレート");
                ...
                map.AddTBString(BillTemplate2019Attr.Name, null, "名前", true, false, 0, 200, 20);
                map.AddTBString(BillTemplate2019Attr.TempFilePath, null, "テンプレートのパス", true, false, 0, 200, 20);
                map.AddTBInt(BillTemplate2019Attr.NodeID, 0, "NodeID", true, false);
                map.AddTBString(BillTemplate2019Attr.MyFrmID, null, "フォーム番号", true, true, 0,300,300);

                map.AddDDLSysEnum(BillTemplate2019Attr.BillFileType, 0, "生成されるファイルの種類", true, false,
                    "BillFileType","@0=Word@1=PDF@2=Excel(不完全)@3=Html(不完全)");

                map.AddDDLSysEnum(BillTemplate2019Attr.BillOpenModel, 0, "生成されたファイルを開く", true, true,
                    "BillOpenModel", "@0=ローカル@1=WebOfficeをオンラインでダウンロード");

                map.AddDDLSysEnum(BillTemplate2019Attr.QRModel, 0, "QRコード生成方法", true, true,
                   BillTemplate2019Attr.QRModel, "@0=作成しない@1=QRコード作成");

                map.AddDDLSysEnum(BillTemplate2019Attr.TemplateFileModel, 1, "テンプレートモード", true, false,
                 BillTemplate2019Attr.TemplateFileModel, "@0=rtfテンプレート@1=vstoモードのWordテンプレート@2=vstoモードのExcelテンプレート");
