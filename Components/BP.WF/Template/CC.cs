    ...
    public class CC : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_Node", "Ccルール");

                map.Java_SetEnType(EnType.Admin);

                map.AddTBIntPK(NodeAttr.NodeID, 0, "ノードID", true, true);
                map.AddTBString(NodeAttr.Name, null, "ノード名", true, true, 0, 100, 10, false);
                map.AddTBString(NodeAttr.FK_Flow, null, "FK_Flow", false, false, 0, 3, 10);
                map.AddDDLSysEnum(NodeAttr.CCWriteTo, 0, "Ccデータ書き込みルール", true, true, NodeAttr.CCWriteTo,"@0=Ccリストに書き込み@1=やることを書く@2=やることとccリストを書く");
                map.AddBoolean(CCAttr.CCIsAttr, false, "フォームフィールドによる案内", true, true, true);
                map.AddTBString(CCAttr.CCFormAttr, null, "Cc人員フィールド", true, false, 0, 100, 10, true);
                
                map.AddBoolean(CCAttr.CCIsStations, false, "有効にするかどうか？-郵便によるCc", true, true, false);
                map.AddDDLSysEnum(CCAttr.CCStaWay, 0, "Cｃポストの計算方法", true, true, CCAttr.CCStaWay,
                    "@0=位置のみによる計算@1=位置によるインテリジェント計算（現在のノード）@2=位置によるインテリジェント計算（ノードに送信）@3=位置と部門の交差点@4=定位置の上位部門（現在ノード）@5=ラインの上位部門（ノードを受け入れる）に従って、そのポジションの下の担当者を検索します。");

                map.AddBoolean(CCAttr.CCIsDepts, false, "有効にするかどうか？-部門別のCｃ", true, true, false);
                map.AddBoolean(CCAttr.CCIsEmps, false, "有効にするかどうか？-スタッフによるCｃ", true, true, false);
                map.AddBoolean(CCAttr.CCIsSQLs, false, "有効にするかどうか？-SQLによるCｃ", true, true, true);
                map.AddTBString(CCAttr.CCSQL, null, "SQL表現式", true, false, 0, 200, 10, true);

                map.AddTBString(CCAttr.CCTitle, null, "Cｃタイトル", true, false, 0, 100, 10, true);
                map.AddTBStringDoc(CCAttr.CCDoc, null, "Ccコンテンツ（タイトルとコンテンツサポート変数）", true, false, true);
                ...
                map.AttrsOfOneVSM.AddGroupPanelModel(new BP.WF.Template.CCStations(), new BP.WF.Port.Stations(),
                    ...
                    BP.WF.Template.NodeStationAttr.FK_Station, "CC投稿（AddGroupPanelModel）", StationAttr.FK_StationType);

                map.AttrsOfOneVSM.AddGroupListModel(new BP.WF.Template.CCStations(), new BP.WF.Port.Stations(),
                  BP.WF.Template.NodeStationAttr.FK_Node,
                  BP.WF.Template.NodeStationAttr.FK_Station, "Cc位置（AddGroupListModel）", StationAttr.FK_StationType);


                //节点绑定人员. 使用树杆与叶子的模式绑定.
                map.AttrsOfOneVSM.AddBranches(new BP.WF.Template.CCDepts(), new BP.Port.Depts(),
                   BP.WF.Template.NodeDeptAttr.FK_Node,
                   BP.WF.Template.NodeDeptAttr.FK_Dept, "Cc部門AddBranches", EmpAttr.Name, EmpAttr.No, "@WebUser.FK_Dept");


                //节点绑定人员. 使用树杆与叶子的模式绑定.
                map.AttrsOfOneVSM.AddBranchesAndLeaf(new BP.WF.Template.CCEmps(), new BP.Port.Emps(),
                   BP.WF.Template.NodeEmpAttr.FK_Node,
                   BP.WF.Template.NodeEmpAttr.FK_Emp, "受信者のCｃ（AddBranchesAndLeaf）", EmpAttr.FK_Dept, EmpAttr.Name, EmpAttr.No, "@WebUser.FK_Dept");
