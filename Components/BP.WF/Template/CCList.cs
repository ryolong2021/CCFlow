    ...
    public class CCList : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("WF_CCList", "Ccリスト");



                map.AddMyPK();

                map.AddTBString(CCListAttr.Title, null, "題名", true, true, 0, 500, 10, true);
                map.AddTBInt(CCListAttr.Sta, 0, "状態", true, true);

                map.AddTBString(CCListAttr.FK_Flow, null, "フロー番号", true, true, 0, 3, 10, true);
                map.AddTBString(CCListAttr.FlowName, null, "フロー名", true, true, 0, 200, 10, true);
                map.AddTBInt(CCListAttr.FK_Node, 0, "ノード", true, true);
                map.AddTBString(CCListAttr.NodeName, null, "ノード名", true, true, 0, 500, 10, true);

                map.AddTBInt(CCListAttr.WorkID, 0, "ジョブID", true, true);
                map.AddTBInt(CCListAttr.FID, 0, "FID", true, true);
                
                map.AddTBStringDoc();

                map.AddTBString(CCListAttr.Rec, null, "Ccスタッフ", true, true, 0, 50, 10, true);
                map.AddTBDateTime(CCListAttr.RDT, null, "記録日", true, false);


                map.AddTBString(CCListAttr.CCTo, null, "Ccする", true, false, 0, 50, 10, true);
                map.AddTBString(CCListAttr.CCToName, null, "（人の名前）にCc", true, false, 0, 50, 10, true);

                map.AddTBString(CCListAttr.CCToDept, null, "部署にCc", true, false, 0, 50, 10, true);
                map.AddTBString(CCListAttr.CCToDeptName, null, "部署名にCc", true, false, 0, 600, 10, true);

                map.AddTBDateTime(CCListAttr.CDT, null, "オープンタイム", true, false);

                map.AddTBString(CCListAttr.PFlowNo, null, "親フロー番号", true, true, 0, 100, 10, true);
                map.AddTBInt(CCListAttr.PWorkID, 0, "親フローWorkID", true, true);
                //added by liuxc,2015.7.6，标识是否在待办列表里显示
                map.AddBoolean(CCListAttr.InEmpWorks, false, "やることリストに追加するかどうか", true, true);
