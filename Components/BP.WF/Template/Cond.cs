...
namespace BP.WF.Template
{
    ...
    public class Cond : EntityMyPK
    {
        ...
        public string SpecOper
        {
            get
            {
                ...
                if (way == Template.SpecOperWay.SpecNodeOper)
                {
                    ...
                    if (fk_emp == null)
                        throw new Exception("@方向条件の設定を間違えた場合、指定された人に尋ねる場合は、指定されたノードをたどってください[" + this.SpecOperPara + "]、プロセッサとして。ただし、ノードに人がいない。クエリSQL:" + sql);
                    return fk_emp;
                }
                ...
                if (way == Template.SpecOperWay.SpecSheetField)
                {
                    if (this.en.Row.ContainsKey(this.SpecOperPara.Replace("@", "")) == false)
                        throw new Exception("@方向条件の設定中にエラーが発生しました。指定された担当者に尋ねるときは、指定されたフィールドに従ってください[" + this.SpecOperPara + "]プロセッサとして、しかしフィールドは存在しません。");
                ...
                if (way == Template.SpecOperWay.SpenEmpNo)
                {
                    if (DataType.IsNullOrEmpty(this.SpecOperPara) == false)
                        throw new Exception("@方向条件の設定を間違えた場合、担当者に依頼する際は担当者の指示に従ってください[" + this.SpecOperPara + "]プロセッサーとして、ただし人事パラメーターが設定されていない。");
                    return this.SpecOperPara;
                }

                throw new Exception("@構成例外、条件タイプは判断されません。");
            }
        }
        #endregion 参数属性.
        ...
        public string FK_Attr
        {
            ...
            {
                if (value == null)
                    throw new Exception("FK_Attrがnullに設定することはできません");
            ...
            get
            {
                ...
                if (this.HisDataFrom == ConnDataFrom.Stas)
                {
                    ...
                    if (BP.WF.Glo.OSModel == BP.Sys.OSModel.OneMore)
                    {
                        ...
                        foreach (BP.GPM.DeptEmpStation st in sts)
                        {
                            ...
                            {
                                this.MsgOfCond = "@投稿で方向を判断し、条件がtrue：投稿の収集" + strs + "、オペレーター(" + BP.Web.WebUser.No + ")役職:" + st.FK_Station + st.FK_StationT;
                                return true;
                            }
                    ...
                    this.MsgOfCond = "@位置に基づいて方向を判断し、条件がfalse：位置の収集" + strs + "、オペレーター(" + BP.Web.WebUser.No + ")役職:" + strs1;
                    return false;
                    #endregion
                }
                ...
                if (this.HisDataFrom == ConnDataFrom.Depts)
                {
                    ...
                    foreach (BP.GPM.DeptEmp st in sts)
                    {
                        ...
                        {
                            this.MsgOfCond = "@位置によって方向を判断し、条件がtrue：部署の収集" + strs + "、オペレーター(" + BP.Web.WebUser.No + ")部門:" + st.FK_Dept;
                            return true;
                        }
                        strs1 += st.FK_Dept;
                    }

                    this.MsgOfCond = "@部門ごとに方向を判断し、条件がfalse：部門を設定" + strs + "、オペレーター(" + BP.Web.WebUser.No + ")部門:" + strs1;
                    return false;
                ...
                if (this.HisDataFrom == ConnDataFrom.SQLTemplate)
                {
                    ...
                    if (sqltemplate.RetrieveFromDBSources() == 0)
                        throw new Exception("@構成されたSQLTemplate番号は[" + sqltemplate + "]削除されました、判定条件は失われます。");
                    ...
                    throw new Exception("@設定するSQL戻り値はccflowの要件を満たしていないため、0または1以上でなければなりません。");
                    #endregion
                }
                ...
                if (this.HisDataFrom == ConnDataFrom.Url)
                {
                    ...
                    {
                        throw new Exception("@URLの方向が間違っていると判断します:" + ex.Message + "、URL実行エラー:" + url);
                    }
                    #endregion 对url进行处理.
                ...
                if (this.HisDataFrom == ConnDataFrom.NodeForm)
                {
                    if (en.EnMap.Attrs.Contains(this.AttrKey) == false)
                        throw new Exception("err@判定条件の方向が間違っています：エンティティ：" + nd.EnDesc + " 属性" + this.AttrKey + "削除された方向条件の判定に失敗しました。");

                    this.MsgOfCond = "@フォームの値、値で方向を判断します" + en.EnDesc + "." + this.AttrKey + " (" + en.GetValStringByKey(this.AttrKey) + ") オペレーター：(" + this.FK_Operator + ") 判定値：(" + this.OperatorValue.ToString() + ")";
                ...
                switch (this.FK_Operator.Trim().ToLower())
                {
                    ...
                    case "baohan":
                        if (en.GetValStringByKey(this.AttrKey).IndexOf(this.OperatorValue.ToString()) == -1)
                            ...
                            return true;
                    default:
                        throw new Exception("@操作記号が見つかりません(" + this.FK_Operator.Trim().ToLower() + ").");
                }
            }
            catch (Exception ex)
            {
                Node nd23 = new Node(this.NodeID);
                throw new Exception("@判定条件：ノード=[" + this.NodeID + "," + nd23.EnDesc + "]、エラーが発生しました。@" + ex.Message + "。不正な条件判断方法を設定した可能性があります");
            }
        }
        /// <summary>
        /// 属性
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBInt(CondAttr.CondType, 0, "条件タイプ", true, true);
                //map.AddDDLSysEnum(CondAttr.CondType, 0, "条件类型", true, false, CondAttr.CondType,"@0=节点完成条件@1=流程完成条件@2=方向条件");
                map.AddTBInt(CondAttr.DataFrom, 0, "条件付きデータソース0フォーム、1ポジション（方向条件に有効）", true, true);
                map.AddTBString(CondAttr.FK_Flow, null, "処理する", true, true, 0, 60, 20);
                map.AddTBInt(CondAttr.NodeID, 0, "MainNodeで発生したイベント", true, true);
                map.AddTBInt(CondAttr.FK_Node, 0, "ノードID", true, true);

                map.AddTBString(CondAttr.FK_Attr, null, "の属性", true, true, 0, 80, 20);
                map.AddTBString(CondAttr.AttrKey, null, "属性キー", true, true, 0, 60, 20);
                map.AddTBString(CondAttr.AttrName, null, "中国語の名前", true, true, 0, 500, 20);
                map.AddTBString(CondAttr.FK_Operator, "=", "標識の計算", true, true, 0, 60, 20);
                map.AddTBString(CondAttr.OperatorValue, "", "計算する値", true, true, 0, 4000, 20);
                map.AddTBString(CondAttr.OperatorValueT, "", "計算される値T", true, true, 0, 4000, 20);
                map.AddTBInt(CondAttr.ToNodeID, 0, "ToNodeID（方向条件に有効）", true, true);
                map.AddDDLSysEnum(CondAttr.ConnJudgeWay, 0, "条件付き関係", true, false,
                    CondAttr.ConnJudgeWay, "@0=or@1=and");

                map.AddTBInt(CondAttr.MyPOID, 0, "MyPOID", true, true);
                map.AddTBInt(CondAttr.PRI, 0, "コンピューティングの優先順位", true, true);
                map.AddTBInt(CondAttr.CondOrAnd, 0, "方向条件タイプ", true, true);

                map.AddTBString(CondAttr.Note, null, "備考", true, true, 0, 500, 20);
    ...
    public class Conds : Entities
    {
        ...
        public bool IsAllPassed
        {
            ...
            {
                if (this.Count == 0)
                    throw new Exception("@判断するセットはありません。");
