	...
	public class Direction :EntityMyPK
	{
        ...
        public override Map EnMap
		{
			get
			{
                ...
                Map map = new Map("WF_Direction", "ノード方向情報");
                 ...
                 */
                map.AddMyPK();
                map.AddTBString(DirectionAttr.FK_Flow, null, "フロー", true, true, 0, 10, 0, false);
                map.AddTBInt(DirectionAttr.Node, 0, "スレーブノード", false, true);
				map.AddTBInt( DirectionAttr.ToNode,0,"ノードへ",false,true);
                map.AddTBInt(DirectionAttr.IsCanBack, 0, "同じように返すことができるかどうか（バックラインで有効）", false, true);
                /*
                 * Dots 存储格式为: @x1,y1@x2,y2
                 */
                map.AddTBString(NodeReturnAttr.Dots, null, "追跡情報", true, true, 0, 300, 0, false);
                map.AddTBInt(DirectionAttr.Idx, 0, "順番", true, true);
                ...
                map.AttrsOfOneVSM.Add(new BP.WF.Template.DirectionStations(), new BP.WF.Port.Stations(),
                    ...
                    StationAttr.Name, StationAttr.No, "オリエンテーションの条件と位置");
