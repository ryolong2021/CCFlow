    ...
    public class FindWorkerRole : EntityOIDName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_FindWorkerRole", "ルールを見つける");
                ...
                map.AddTBInt(FindWorkerRoleAttr.FK_Node, 0, "ノードID", false, false);
                ...
                map.AddTBInt(FindWorkerRoleAttr.IsEnable, 1, "使えるかどうか", false, false);
                ...
                case "SpecEmps":
                    ...
                    switch (this.HisFindLeaderType)
                    {
                        ...
                        case FindLeaderType.SpecNodeSubmiter: // 指定节点提交人的直线领导.
                            ...
                            if (dt.Rows.Count == 0)
                                throw new Exception("@指定されたノードデータが見つかりません。システム管理者、技術情報にフィードバックしてください:" + sql);
                        ...
                        case FindLeaderType.BySpecField: //指定节点字段人员的直接领导..
                            ...
                            if (DataType.IsNullOrEmpty(empNo))
                                throw new Exception("@指定されたノードフィールド(" + this.TagVal1 + ")空です。");
                            //指定它
                            Emp emp = new Emp();
                            emp.No = empNo;
                            if (emp.RetrieveFromDBSources() == 0)
                                throw new Exception("@指定されたノードフィールド(" + this.TagVal1 + ")の価値(" + empNo + ")不法職員番号です…");
                            empDept = emp.FK_Dept;
                            break;
                        default:
                            throw new Exception("@未処理のケース" + this.HisFindLeaderType);
                            break;
                    }
                    if (DataType.IsNullOrEmpty(empNo))
                        throw new Exception("@判断ステップが欠落しているため、指定されたスタッフメンバーは見つかりませんでした。");
                    #endregion
            ...
            switch (this.HisFindColleague)
            {
                case FindColleague.All: // 所有该部门性质下的人员.
                    ...
                    if (DataType.IsNullOrEmpty(leader))
                        throw new Exception("@システム管理者は与えませんでした(" + empNo + ")部門で(" + empDept + ")直接のリーダーシップ。");
