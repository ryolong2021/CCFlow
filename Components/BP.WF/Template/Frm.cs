    ...
    public class Frm : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("Sys_MapData", "フォームライブラリ");
                map.Java_SetCodeStruct("4");

                map.AddTBStringPK(FrmAttr.No, null, null, true, true, 1, 200, 4);
                map.AddTBString(FrmAttr.Name, null, null, true, false, 0, 50, 10);
                map.AddTBString(FrmAttr.FK_Flow, null, "独立したフォーム属性：FK_Flow", true, false, 0, 50, 10);
             //   map.AddDDLSysEnum(FrmAttr.FrmType, 0, "独立表单属性:运行类型", true, false, FrmAttr.FrmType);

                //表单的运行类型.
                map.AddDDLSysEnum(FrmAttr.FrmType, (int)BP.Sys.FrmType.FreeFrm, "フォームタイプ",true, false, FrmAttr.FrmType);

                map.AddTBString(FrmAttr.PTable, null, "物理的なテーブル", true, false, 0, 50, 10);
                ...
                //表单类别.
                map.AddTBString(MapDataAttr.FK_FrmSort, "01", "フォームのカテゴリ", true, false, 0, 500, 20);

                map.AddTBInt(BP.Sys.MapDataAttr.FrmW, 900, "フォーム幅", true, false);
                map.AddTBInt(BP.Sys.MapDataAttr.FrmH, 1200, "フォームの高さ", true, false);
