...
namespace BP.WF.Template
{
    ...
    public class FrmThreadAttr : EntityNoAttr
    {
        ...
        /// 显示标签
        /// </summary>
        public const string FrmThreadLab = "FrmThreadLab";
        /// <summary>
        /// 状态
        /// </summary>
    ...
    public class FrmThread : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_Node", "子スレッドコンポーネント");

                map.AddTBIntPK(NodeAttr.NodeID, 0, "ノードID", true, true);
                map.AddTBString(NodeAttr.Name, null, "ノード名", true, true, 0, 100, 10);

                map.AddTBString(FrmThreadAttr.FrmThreadLab, "子スレッド", "ラベルを表示", true, false, 0, 200, 10, true);


                #region 此处变更了 NodeSheet类中的，map 描述该部分也要变更.

                map.AddDDLSysEnum(FrmThreadAttr.FrmThreadSta, (int)FrmThreadSta.Disable, "コンポーネントのステータス",
                   true, true, FrmThreadAttr.FrmThreadSta, "@0=無効@1=有効");

                map.AddTBFloat(FrmThreadAttr.FrmThread_X, 5, "位置X", true, false);
                map.AddTBFloat(FrmThreadAttr.FrmThread_Y, 5, "位置Y", true, false);

                map.AddTBFloat(FrmThreadAttr.FrmThread_H, 300, "高さ", true, false);
                map.AddTBFloat(FrmThreadAttr.FrmThread_W, 400, "幅", true, false);
