    ...
    public class FrmTrack : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_Node", "トラックアイコンコンポーネント");

                map.AddTBIntPK(NodeAttr.NodeID, 0, "ノードID", true, true);
                map.AddTBString(NodeAttr.Name, null, "ノード名", true, true, 0, 100, 10);
                map.AddTBString(FrmTrackAttr.FrmTrackLab, "追跡", "ラベルを表示", true, false, 0, 200, 10, false);

                #region 此处变更了 NodeSheet类中的，map 描述该部分也要变更.

                map.AddDDLSysEnum(FrmTrackAttr.FrmTrackSta, (int)FrmTrackSta.Disable, "コンポーネントのステータス",
                   true, true, FrmTrackAttr.FrmTrackSta, "@0=無効@1=トラックグラフを表示する@2=トラックテーブルを表示する");

                map.AddTBFloat(FrmTrackAttr.FrmTrack_X, 5, "位置X", false, false);
                map.AddTBFloat(FrmTrackAttr.FrmTrack_Y, 5, "位置Y", false, false);

                map.AddTBFloat(FrmTrackAttr.FrmTrack_H, 300, "高さ", true, false);
                map.AddTBFloat(FrmTrackAttr.FrmTrack_W, 400, "幅", true, false);
