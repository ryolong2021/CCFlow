    ...
    public class FrmTransferCustom : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_Node", "カスタムコンポーネントを循環させる");

                map.AddTBIntPK(NodeAttr.NodeID, 0, "ノードID", true, true);
                map.AddTBString(NodeAttr.Name, null, "ノード名", true, true, 0, 100, 10);
                map.AddTBString(FTCAttr.FTCLab, "循環カスタム", "ラベルを表示", true, false, 0, 50, 10, true);


                #region 此处变更了 NodeSheet类中的，map 描述该部分也要变更.

                map.AddDDLSysEnum(FTCAttr.FTCSta, (int)FTCSta.Disable, "コンポーネントのステータス",
                   true, true, FTCAttr.FTCSta, "@0=無効@1=読み取り専用@2=担当者を設定できます");

                map.AddDDLSysEnum(FTCAttr.FTCWorkModel,0, "動作モード",
                  true, true, FTCAttr.FTCWorkModel, "@0=簡潔モード@1=詳細モード");

                map.AddTBFloat(FTCAttr.FTC_X, 5, "位置X", false, false);
                map.AddTBFloat(FTCAttr.FTC_Y, 5, "位置Y", false, false);

                map.AddTBFloat(FTCAttr.FTC_H, 300, "高さ", true, false);
                map.AddTBFloat(FTCAttr.FTC_W, 400, "幅", true, false);
