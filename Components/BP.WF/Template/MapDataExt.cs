    ...
    public class MapDataExt : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("Sys_MapData", "フォーム属性");
                map.Java_SetEnType(EnType.Sys);
                map.Java_SetCodeStruct("4");

                #region 基本属性.
                map.AddTBStringPK(MapDataAttr.No, null, "フォーム番号", true, false, 1, 190, 20);
                map.AddTBString(MapDataAttr.Name, null, "フォーム名", true, false, 0, 500, 20);
                map.AddTBString(MapDataAttr.PTable, null, "収納テーブル", true, false, 0, 500, 20);

                //表单的运行类型.
                map.AddDDLSysEnum(MapDataAttr.FrmType, (int)BP.Sys.FrmType.FreeFrm, "フォームタイプ",true, true, MapDataAttr.FrmType);

                map.AddTBString(MapDataAttr.Url, null, "URL接続（埋め込みフォームで有効）", true, false, 0, 500, 20, true);
                //数据源.
