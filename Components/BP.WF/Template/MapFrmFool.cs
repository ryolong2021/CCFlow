    ...
    public class MapFrmFool : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("Sys_MapData", "フールフォーム属性");
                ...
                map.AddTBString(MapDataAttr.PTable, null, "収納テーブル", true, false, 0, 100, 20);
                map.AddTBString(MapDataAttr.Name, null, "フォーム名", true, false, 0, 500, 20, true);

                map.AddDDLSysEnum(MapDataAttr.TableCol, 0, "フォームに表示される列の数", true, true, "表示方法",
                    "@0=カラム@1=6カラム@2=3カラム（上下モード）");

              //  map.AddTBInt(MapDataAttr.TableWidth, 900, "傻瓜表单宽度", true, false);
               // map.AddTBInt(MapDataAttr.TableHeight, 900, "傻瓜表单高度", true, false);

                map.AddTBInt(MapDataAttr.FrmW, 900, "フォーム幅", true, false);
                map.AddTBInt(MapDataAttr.FrmH, 900, "フォームの高さ", true, false);

                //数据源.
                map.AddDDLEntities(MapDataAttr.DBSrc, "local", "情報源", new BP.Sys.SFDBSrcs(), true);
                map.AddDDLEntities(MapDataAttr.FK_FormTree, "01", "フォームのカテゴリ", new SysFormTrees(), true);
                //表单的运行类型.
                map.AddDDLSysEnum(MapDataAttr.FrmType, (int)BP.Sys.FrmType.FreeFrm, "フォームタイプ",
                    true, false, MapDataAttr.FrmType);
                //表单解析 0 普通 1 页签展示
                map.AddDDLSysEnum(MapDataAttr.FrmShowType, 0, "フォーム表示方法", true, true, "フォーム表示方法",
                    "@0=普通方式@1=ページ方式");
                ...
                map.AddTBString(MapDataAttr.DesignerUnit, null, "単位", true, false, 0, 500, 20, true);
               ...
               // map.AddTBString(MapFrmFreeAttr.DesignerTool, null, "表单设计器", true, true, 0, 30, 20);
                ...
                map.AddTBString(MapDataAttr.Note, null, "備考", true, false,0,400,100, true);
                //增加参数字段.
        ...
        public string DoCheckFixFrmForUpdateVer()
        {
            ...
            foreach (MapDtl dtl in dtls)
            {
                ...
                gf.Save();
                str += "@スレーブテーブル" + dtl.Name + " グループ化を追加";
            }
            ...
            foreach (FrmAttachment ath in aths)
            {
                ...
                str += "@添付ファイルとして" + ath.Name + " グループ化を追加";
            }
