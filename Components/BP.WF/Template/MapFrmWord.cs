	...
	public class MapFrmWord : EntityNoName
	{
		...
		public override Map EnMap
		{
			get
			{
				if (this._enMap != null)
					return this._enMap;
				Map map = new Map("Sys_MapData", "Wordフォーム属性");
				map.Java_SetEnType(EnType.Sys);



				#region 基本属性.
                map.AddTBStringPK(MapFrmWordAttr.No, null, "フォーム番号", true, true, 1, 190, 20);
				map.AddTBString(MapFrmWordAttr.PTable, null, "収納テーブル", true, false, 0, 100, 20);
				map.AddTBString(MapFrmWordAttr.Name, null, "フォーム名", true, false, 0, 500, 20, true);

				//数据源.
				map.AddDDLEntities(MapFrmWordAttr.DBSrc, "local", "情報源", new BP.Sys.SFDBSrcs(), true);
				map.AddDDLEntities(MapFrmWordAttr.FK_FormTree, "01", "フォームのカテゴリ", new SysFormTrees(), true);

				//表单的运行类型.
				map.AddDDLSysEnum(MapFrmWordAttr.FrmType, (int)BP.Sys.FrmType.FreeFrm, "フォームタイプ", true, false, MapFrmWordAttr.FrmType);
				#endregion 基本属性.

				#region 模版属性。
				map.AddTBString(MapFrmWordAttr.TemplaterVer, null, "テンプレート番号", true, false, 0, 30, 20);
                map.AddTBString(MapFrmWordAttr.DBSave, null, "Wordデータファイルストレージ", true, false, 0, 50, 20);
                map.SetHelperAlert(MapFrmWordAttr.DBSave,
                    "テーブルのどのフィールドがバイナリWordファイルに保存されていますか？デフォルトはDBFileです。このテーブルが複数のWordファイルに対応している場合、バイナリのWordファイルストレージが上書きされます。");
				#endregion 模版属性。

				#region 设计者信息.
				map.AddTBString(MapFrmWordAttr.Designer, null, "デザイナー", true, false, 0, 500, 20);
				map.AddTBString(MapFrmWordAttr.DesignerContact, null, "連絡先", true, false, 0, 500, 20);
				map.AddTBString(MapFrmWordAttr.DesignerUnit, null, "単位", true, false, 0, 500, 20, true);
				map.AddTBString(MapFrmWordAttr.GUID, null, "GUID", true, true, 0, 128, 20, false);
				map.AddTBString(MapFrmWordAttr.Ver, null, "バージョンナンバー", true, true, 0, 30, 20);
			//	map.AddTBString(MapFrmFreeAttr.DesignerTool, null, "表单设计器", true, true, 0, 30, 20);

				map.AddTBStringDoc(MapFrmWordAttr.Note, null, "備考", true, false, true);

				//增加参数字段.
				map.AddTBAtParas(4000);
				map.AddTBInt(MapFrmWordAttr.Idx, 100, "シーケンス番号", false, false);
				#endregion 设计者信息.

				map.AddMyFile("フォームテンプレート", null, SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "FrmOfficeTemplate" + Path.DirectorySeparatorChar);
        ...
        {
            return SystemConfig.CCFlowWebPath + "WF/Comm/En.htm?EnName=BP.WF.Template.MapFrmWord&No=" + this.No;
        }
