    ...
    public class PartParentSubGuide : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_Part", "フロントナビゲーション-親子フロー");

                map.AddMyPK();

                map.AddTBString(PartAttr.FK_Flow, null, "フロー番号", true, true, 0, 100, 10);
                map.AddTBString(PartAttr.PartType, null, "種類", false, true, 0, 100, 10);

                map.AddTBString(PartAttr.Tag0, null, "親フロー番号", true, true, 0, 2000, 10);
                map.AddTBString(PartAttr.Tag1, null, "フロー名", true, true, 0, 2000, 10);

                map.AddTBString(PartAttr.Tag2, null, "クエリ条件を非表示にする", true, false, 0, 2000, 10,true);
                map.SetHelperAlert(PartAttr.Tag2, "形式は次のとおりです。WFState=3 AND FlowStarter=＠WebUser.Noこれらの列は、SELECT * FROM Sys_MapAttr WHERE FK_MapData = NDxxxRptによってNDxxRptテーブルにあります。");

                map.AddTBString(PartAttr.Tag3, null, "表示される列", true, false, 0, 2000, 10, true);
                map.SetHelperAlert(PartAttr.Tag3, "形式は次のとおりです。＠Title、見出＠Tel、携帯電話＠Email、Mailこれらの列は、NDxxRptテーブルのSELECT * FROM Sys_MapAttr WHERE FK_MapData = NDxxxRptで見つけることができます。");
