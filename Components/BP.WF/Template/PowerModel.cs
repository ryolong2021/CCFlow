    ...
    public class PowerModel : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_PowerModel", "許可モデル");

                map.AddMyPK();

                //比如： FlowData , FrmData
                map.AddTBString(PowerModelAttr.Model, null, "モジュール", true, false, 0, 100, 10);

                //权限标记: FlowDataDelete
                map.AddTBString(PowerModelAttr.PowerFlag, null, "機関ID", true, false, 0, 100, 10);
                //权限名称: 流程删除
                map.AddTBString(PowerModelAttr.PowerFlagName, null, "許可タグ名", true, false, 0, 100, 10);

                map.AddDDLSysEnum(PowerModelAttr.PowerCtrlType, 0, "制御タイプ", true, false, PowerModelAttr.PowerCtrlType,
                    "@0=役職@1=人員");

                map.AddTBString(PowerModelAttr.EmpNo, null, "人員", true, false, 0, 100, 10);
                map.AddTBString(PowerModelAttr.EmpName, null, "人名", true, false, 0, 100, 10);

                map.AddTBString(PowerModelAttr.StaNo, null, "投稿番号", true, false, 0, 100, 10);
                map.AddTBString(PowerModelAttr.StaName, null, "ポジションタイトル", true, false, 0, 100, 10);

                //Model标记.
                map.AddTBString(PowerModelAttr.FlowNo, null, "フロー番号", true, false, 0, 100, 10);
              //  map.AddTBInt(PowerModelAttr.NodeID, 0, "节点", true, false);
                map.AddTBString(PowerModelAttr.FrmID, null, "フォームID", true, false, 0, 100, 10);
