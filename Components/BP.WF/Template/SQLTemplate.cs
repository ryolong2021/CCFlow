	...
	public class SQLTemplate : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("WF_SQLTemplate", "SQLテンプレート");
                
                map.Java_SetCodeStruct("3");

                map.AddTBStringPK(SQLTemplateAttr.No, null, "ナンバリング", true, true, 3, 3, 3);
                map.AddDDLSysEnum(SQLTemplateAttr.SQLType, 0, "テンプレートSQLタイプ", true, true, SQLTemplateAttr.SQLType,
                    "@0=方向条件@1=受信者ルール@2=ドロップダウンボックスデータフィルタリング@3=カスケードドロップダウンボックス@4=PopVal開いているウィンドウの戻り値@5=個人セレクター人材選択範囲");

                map.AddTBString(SQLTemplateAttr.Name, null, "SQL説明", true, false, 0, 200, 20,true);

                map.AddTBStringDoc(SQLTemplateAttr.Docs, null, "SQLテンプレート", true, false,true);
