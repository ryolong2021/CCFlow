    ...
    public class SelectAccper : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_SelectAccper", "個人情報を受け入れる/選択する");
                map.AddMyPK();

                map.AddTBInt(SelectAccperAttr.FK_Node, 0, "受信者ノード", true, false);
                ...
                map.AddTBString(SelectAccperAttr.DeptName, null, "部署名", true, false, 0, 400, 10);
                map.AddTBInt(SelectAccperAttr.AccType, 0, "タイプ（@0=受取人@1=Cc人）", true, false);
                map.AddTBString(SelectAccperAttr.Rec, null, "レコーダー", true, false, 0, 20, 10);
                map.AddTBString(SelectAccperAttr.Info, null, "意見情報の取り扱い", true, false, 0, 200, 10);

                map.AddTBInt(SelectAccperAttr.IsRemember, 0, "この計算に従って将来的に送信するかどうか", true, false);
                map.AddTBInt(SelectAccperAttr.Idx, 0, "シーケンス番号（フローキューレビューモードで使用可能）", true, false);
                /*
                 *  add 2015-1-12.
                 * 为了解决多维度的人员问题.
                 ...
                 * 这个维度，有可能是一个类别，批次。
                 */
                map.AddTBString(SelectAccperAttr.Tag, null, "寸法情報タグ", true, false, 0, 200, 10);

                map.AddTBInt(SelectAccperAttr.TimeLimit, 0, "制限日数", true, false);
                map.AddTBFloat(SelectAccperAttr.TSpanHour, 0, "制限時間", true, false);

                //应该完成日期，为了自动计算未来的日期.
                map.AddTBDateTime(SelectAccperAttr.PlanADT, null, "到着日（予定）", true, false);
                map.AddTBDateTime(SelectAccperAttr.PlanSDT, null, "納期（予定）", true, false);
        ...
        protected override bool beforeUpdateInsertAction()
        {
            if (this.DeptName.Length == 0)
            {
                ...
                {
                    this.DeptName = DBAccess.RunSQLReturnStringIsNull("select a.NameOfPath from port_dept a,port_emp b where a.No=b.fk_dept and b.no='" + this.FK_Emp + "'", "なし");
                    if (this.DeptName == "なし")
                        this.DeptName = DBAccess.RunSQLReturnStringIsNull("select a.name from port_dept a,port_emp b where a.No=b.fk_dept and b.no='" + this.FK_Emp + "'", "なし");
                }
                else
                    this.DeptName = DBAccess.RunSQLReturnStringIsNull("select a.name from port_dept a,port_emp b where a.No=b.fk_dept and b.no='" + this.FK_Emp + "'", "なし");
            }
