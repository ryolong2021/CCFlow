    ...
    public class Selector : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                #region 字段.
                Map map = new Map("WF_Node", "セレクタ");

                map.Java_SetDepositaryOfEntity(Depositary.Application);

                map.AddTBIntPK(SelectorAttr.NodeID, 0, "NodeID", true, true);
                map.AddTBString(SelectorAttr.Name, null, "ノード名", true, true, 0, 100, 100);

                map.AddDDLSysEnum(SelectorAttr.SelectorModel, 5, "表示方法", true, true, SelectorAttr.SelectorModel,
                    "@0=位置別@1=部門別@2=人員別@3=SQL@4=SQLテンプレートで計算@5=一般人事セレクターを使用@6=部署と位置の交点@7=カスタムURL@8=使用一般部門の職務担当者セレクター@9=職階ごとのインテリジェントな計算（オペレーターの部門）");

                map.AddDDLSQL(SelectorAttr.FK_SQLTemplate, null, "SQLテンプレート", "SELECT No,Name FROM WF_SQLTemplate WHERE SQLType=5", true);

                map.AddBoolean(SelectorAttr.IsAutoLoadEmps, true, "最後に選択した人物を自動的に読み込むかどうか？", true, true);
                map.AddBoolean(SelectorAttr.IsSimpleSelector, false, "選択肢は1つですか（選択できるのは1人だけです）？", true, true);
                map.AddBoolean(SelectorAttr.IsEnableDeptRange, false, "部署検索範囲限定を有効にするか（一般人事セレクター有効）？", true, true, true);
                map.AddBoolean(SelectorAttr.IsEnableStaRange, false, "求人検索の範囲制限を有効にするかどうか（一般人事セレクタの使用に有効）？", true, true, true);
                ...
                map.AddTBStringDoc(SelectorAttr.SelectorP1, null, "グループ化パラメーター：空にすることができます。例：SELECT No、Name、ParentNo FROM Port_Dept", true,false,0,300,100,3);
                map.AddTBStringDoc(SelectorAttr.SelectorP2, null, "オペレーターデータソース：例：SELECT No、Name、FK_Dept FROM Port_Emp", true, false, 0, 300, 100, 3);

                map.AddTBStringDoc(SelectorAttr.SelectorP3, null, "デフォルトで選択されているデータソース：例：SELECT FK_Emp FROM WF_GenerWorkerList WHERE FK_Node = 102 AND WorkID = @WorkID", true, false, 0, 300, 100, 3);
                map.AddTBStringDoc(SelectorAttr.SelectorP4, null, "データソースの必須の選択：例：SELECT FK_Emp FROM WF_GenerWorkerList WHERE FK_Node = 102 AND WorkID = @WorkID", true, false, 0, 300, 100, 3);
                ...
                map.AttrsOfOneVSM.AddGroupPanelModel(new BP.WF.Template.NodeStations(), new BP.WF.Port.Stations(),
                    ...
                    BP.WF.Template.NodeStationAttr.FK_Station, "バインドポスト（タイル）", StationAttr.FK_StationType, "Name", "No");

                map.AttrsOfOneVSM.AddGroupListModel(new BP.WF.Template.NodeStations(), new BP.WF.Port.Stations(),
                  BP.WF.Template.NodeStationAttr.FK_Node,
                  BP.WF.Template.NodeStationAttr.FK_Station, "バインディングポスト（ツリー）", StationAttr.FK_StationType, "Name", "No");

                //节点绑定部门. 节点绑定部门.
                map.AttrsOfOneVSM.AddBranches(new BP.WF.Template.NodeDepts(), new BP.Port.Depts(),
                   BP.WF.Template.NodeDeptAttr.FK_Node,
                   BP.WF.Template.NodeDeptAttr.FK_Dept, "バウンド部門", EmpAttr.Name, EmpAttr.No, "@WebUser.FK_Dept");

                //节点绑定人员. 使用树杆与叶子的模式绑定.
                map.AttrsOfOneVSM.AddBranchesAndLeaf(new BP.WF.Template.NodeEmps(), new BP.Port.Emps(),
                   BP.WF.Template.NodeEmpAttr.FK_Node,
                   BP.WF.Template.NodeEmpAttr.FK_Emp, "バインド受信者", EmpAttr.FK_Dept, EmpAttr.Name, EmpAttr.No, "@WebUser.FK_Dept");
                #endregion
        ...
        public System.Data.DataSet GenerDataSet(int nodeid, Entity en)
        {
            ...
            switch (this.SelectorModel)
            {
                ...
                case SelectorModel.AccepterOfDeptStationOfCurrentOper:
                    ...
                    break;
                default:
                    throw new Exception("@エラー：未選択の選択タイプ:" + this.SelectorModel);
                    break;
            }
