    ...
    public class FrmSubFlow : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_Node", "父子フロー");

                map.AddTBIntPK(NodeAttr.NodeID, 0, "ノードID", true, true);
                map.AddTBString(NodeAttr.Name, null, "ノード名", true, true, 0, 100, 10);
                map.AddTBString(FrmSubFlowAttr.SFLab, "サブフロー", "ラベルを表示", true, false, 0, 200, 10, true);

                #region 此处变更了 NodeSheet类中的，map 描述该部分也要变更.

                map.AddDDLSysEnum(FrmSubFlowAttr.SFSta, (int)FrmSubFlowSta.Disable, "コンポーネントのステータス",
                   true, true, FrmSubFlowAttr.SFSta, "@0=無効@1=有効@2=読み取り専用");

                map.AddDDLSysEnum(FrmSubFlowAttr.SFShowModel, (int)FrmWorkShowModel.Free, "表示方法",
                    true, true, FrmSubFlowAttr.SFShowModel, "@0=フォームメソッド@1=フリーモード"); //此属性暂时没有用.

                map.AddTBString(FrmSubFlowAttr.SFCaption, "プロモーターフロー", "リンクのタイトル", true, false, 0, 100, 10, true);
                map.AddTBString(FrmSubFlowAttr.SFDefInfo, null, "開始できるサブフローの数（コンマで区切られた複数）", false, false, 0, 50, 10, true);
                map.AddTBString(FrmSubFlowAttr.SFActiveFlows, null, "トリガーできるサブフローの数（カンマで区切られた複数）", false, false, 0, 50, 10, true);

                map.AddTBFloat(FrmSubFlowAttr.SF_X, 5, "X位置", true, false);
                map.AddTBFloat(FrmSubFlowAttr.SF_Y, 5, "Y位置", true, false);

                map.AddTBFloat(FrmSubFlowAttr.SF_H, 300, "高", true, false);
                map.AddTBFloat(FrmSubFlowAttr.SF_W, 400, "幅", true, false);

                map.AddTBString(FrmSubFlowAttr.SFFields, null, "承認形式フィールド", true, false, 0, 50, 10, true);

                map.AddDDLSysEnum(FrmSubFlowAttr.SFShowCtrl, (int)SFShowCtrl.All, "表示制御方法",
                  true, true, FrmSubFlowAttr.SFShowCtrl, "@0=すべてのサブフローを表示できます@1=自分が開始したサブフローのみを表示できます"); //此属性暂时没有用.

                map.AddDDLSysEnum(FrmSubFlowAttr.SFOpenType, 0, "サブフロー表示を開く",
                 true, true, FrmSubFlowAttr.SFOpenType, "@0=ジョブビューア@1=ばかフォームトラックビューア"); //此属性暂时没有用.


                #endregion 此处变更了 NodeSheet类中的，map 描述该部分也要变更.

                RefMethod rm = new RefMethod();
                rm.Title = "サブフローを手動で開始する"; 
                rm.ClassMethodName = this.ToString() + ".DoSubFlowHand";
                rm.RefMethodType = RefMethodType.RightFrameOpen;
                map.AddRefMethod(rm);

                rm = new RefMethod();
                rm.Title = "サブフローを自動的にトリガーする"; 
