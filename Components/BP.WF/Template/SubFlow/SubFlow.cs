    ...
    public class SubFlow : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_NodeSubFlow", "サブフロー（すべてのタイプのサブフロー属性）");
                map.IndexField = SubFlowAttr.FK_Node;

                map.AddMyPK();

                map.AddTBString(SubFlowAttr.FK_Flow, null, "メインフロー番号", true, true, 0, 10, 150);
                map.AddTBInt(SubFlowAttr.FK_Node, 0, "メインフローノード", false, true);

                map.AddTBInt(SubFlowAttr.SubFlowType, 0, "サブフロータイプ", false, true);
                map.AddTBInt(SubFlowAttr.SubFlowModel, 0, "サブフローパターン", false, true);

                map.AddTBInt(FlowAttr.IsAutoSendSubFlowOver, 0, "親子フロー終了ルール", false, true);
                map.AddTBInt(FlowAttr.IsAutoSendSLSubFlowOver, 0, "同じレベルのサブフローの終了ルール", false, true);


                map.AddTBString(SubFlowAttr.SubFlowNo, null, "サブフロー番号", true, true, 0, 10, 150, false);
                map.AddTBString(SubFlowAttr.SubFlowName, null, "サブフロー名", true, true, 0, 200, 150, false);

                //启动限制规则0.
                map.AddTBInt(SubFlowAttr.StartOnceOnly, 0, "一度しか呼び出せない", false, true);

                //启动限制规则1.
                map.AddTBInt(SubFlowAttr.IsEnableSpecFlowStart, 0, "指定されたフローが開始された後、サブフローを開始できます(テキストボックスでサブフローを構成してください)。", false, true);
                ...
                //启动限制规则2.
                map.AddTBInt(SubFlowHandAttr.IsEnableSpecFlowOver, 0, "指定したフローが終了すると、サブフローを開始できます(テキストボックスでサブフローを構成してください)。", true, true);
                ...
                map.AddTBInt(SubFlowAttr.ExpType, 0, "式のタイプ", false, true);
                map.AddTBString(SubFlowAttr.CondExp, null, "条件式", true, false, 0, 500, 150, true);

                map.AddTBInt(SubFlowAttr.YBFlowReturnRole, 0, "返品方法", false, true);

                map.AddTBString(SubFlowAttr.ReturnToNode, null, "返されるノード", true, true, 0, 200, 150, false);

                map.AddTBInt(SubFlowAttr.SendModel, 0, "自動的にトリガーされるサブフロー送信方法", false, true);

                map.AddTBInt(SubFlowAttr.Idx, 0, "順番", true, false);
                this._enMap = map;
                return this._enMap;
            }
        }
