    ...
    public class SysForm : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("Sys_MapData", "システム形態");
                ...
                //表单类型.
                map.AddTBInt(Sys.MapDataAttr.FrmType, 1, "フォームタイプ", true, false);
               

                //该表单对应的物理表
                map.AddTBString(SysFormAttr.PTable, null, "物理的なテーブル", true, false, 0, 50, 10);

                // FrmType=嵌入式表单时, 该字段有效. 
                map.AddTBString(SysFormAttr.URL, null, "Url", true, false, 0, 50, 10);

                //系统表单类别.
                map.AddTBString(SysFormAttr.FK_FormTree, null, "フォームツリー", true, false, 0, 10, 20);

                map.AddTBInt(Sys.MapDataAttr.FrmW, 900, "システムフォーム幅", true, false);
                map.AddTBInt(Sys.MapDataAttr.FrmH, 1200, "システムフォームの高さ", true, false);
