    ...
    public class Task : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("WF_Task", "タスク");
                map.Java_SetEnType(EnType.Admin);

                map.AddMyPK(); //唯一的主键.
                map.AddTBString(TaskAttr.FK_Flow, null, "フロー番号", true, false, 0, 200, 10);
                map.AddTBString(TaskAttr.Starter, null, "スポンサー", true, false, 0, 200, 10);

                //为上海同事科技增加两个字段. 可以为空.
                map.AddTBInt(TaskAttr.ToNode, 0, "ノードに到達しました", true, false);
                map.AddTBString(TaskAttr.ToEmps, null, "到着人員", true, false, 0, 200, 10);

                map.AddTBString(TaskAttr.Paras, null, "パラメータ", true, false, 0, 4000, 10);

                // TaskSta 0=未发起，1=成功发起，2=发起失败.
                map.AddTBInt(TaskAttr.TaskSta, 0, "タスクのステータス", true, false);

                map.AddTBString(TaskAttr.Msg, null, "メッセージ", true, false, 0, 4000, 10);
                map.AddTBString(TaskAttr.StartDT, null, "開始時間", true, false, 0, 20, 10);
                map.AddTBString(TaskAttr.RDT, null, "データ時間を挿入", true, false, 0, 20, 10);
