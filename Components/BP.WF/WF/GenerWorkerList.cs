...
namespace BP.WF
{
    ...
    public class GenerWorkerList : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_GenerWorkerlist", "ワーカー");

                map.IndexField = GenerWorkerListAttr.WorkID;

                map.AddTBIntPK(GenerWorkerListAttr.WorkID, 0, "ジョブID", true, true);
                map.AddTBStringPK(GenerWorkerListAttr.FK_Emp, null, "人事", true, false, 0, 20, 100);
                map.AddTBIntPK(GenerWorkerListAttr.FK_Node, 0, "ノードID", true, false);

                map.AddTBInt(GenerWorkerListAttr.FID, 0, "フローID", true, false);

                map.AddTBString(GenerWorkerListAttr.FK_EmpText, null, "人名", true, false, 0, 30, 100);
                map.AddTBString(GenerWorkerListAttr.FK_NodeText, null, "ノード名", true, false, 0, 100, 100);

                map.AddTBString(GenerWorkerListAttr.FK_Flow, null, "フロー", true, false, 0, 3, 100);
                map.AddTBString(GenerWorkerListAttr.FK_Dept, null, "使用部門", true, false, 0, 100, 100);

                //如果是流程属性来控制的就按流程属性来计算。
                map.AddTBDateTime(GenerWorkerListAttr.SDT, "期日", false, false);
                map.AddTBDateTime(GenerWorkerListAttr.DTOfWarning, "警告日", false, false);
               // map.AddTBFloat(GenerWorkerListAttr.WarningHour, 0, "预警天", true, false);
                map.AddTBDateTime(GenerWorkerListAttr.RDT, "記録タイム", false, false);
                map.AddTBDateTime(GenerWorkerListAttr.CDT, GenerWorkerListAttr.CDT, DateTime.Now.ToString(DataType.SysDataTimeFormat+":ss"), "完全な時間", false, false);
                map.AddTBInt(GenerWorkerListAttr.IsEnable, 1, "使えるかどうか", true, true);

                //  add for 上海 2012-11-30
                map.AddTBInt(GenerWorkerListAttr.IsRead, 0, "読むかどうか", true, true);

                //对会签节点有效
                map.AddTBInt(GenerWorkerListAttr.IsPass, 0, "合格かどうか（合流ノードで有効）", false, false);

                // 谁执行它？
                map.AddTBInt(GenerWorkerListAttr.WhoExeIt, 0, "誰が実行するか", false, false);

                //发送人. 2011-11-12 为天津用户增加。
                map.AddTBString(GenerWorkerListAttr.Sender, null, "発信人", true, false, 0, 200, 100);

                //优先级，2012-06-15 为青岛用户增加。
                map.AddTBInt(GenerWorkFlowAttr.PRI, 1, "優先順", true, true);

                //催办次数. 为亿阳信通加.
                map.AddTBInt(GenerWorkerListAttr.PressTimes, 0, "リマインダー", true, false);

                // 挂起
                map.AddTBDateTime(GenerWorkerListAttr.DTOfHungUp,null, "ハングタイム", false, false);
                map.AddTBDateTime(GenerWorkerListAttr.DTOfUnHungUp,null, "停止解除までの推定時間", false, false);
                map.AddTBInt(GenerWorkerListAttr.HungUpTimes, 0, "ハングの数", true, false);

                //外部用户. 203-08-30
                map.AddTBString(GenerWorkerListAttr.GuestNo, null, "外部ユーザー番号", true, false, 0, 30, 100);
                map.AddTBString(GenerWorkerListAttr.GuestName, null, "外部ユーザー名", true, false, 0, 100, 100);
    ...
    public class GenerWorkerLists : Entities
    {
        ...
        public GenerWorkerLists(Int64 workId, int nodeId, bool isWithEmpExts)
        {
            ...
            if (i == 0)
                throw new Exception("@システムエラー、スタッフが不足しています。管理者に連絡してください。 NodeID=" + nodeId + " WorkID=" + workId);
