    ...
    public class GetTask : BP.En.Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_Node", "タスクを取得する");

                #region 字段
                map.AddTBIntPK(NodeAttr.NodeID, 0,"NodeID", true, true);
                map.AddTBString(NodeAttr.Name, null,"ノード名", true, false, 0, 100, 10);
                map.AddTBInt(NodeAttr.Step,0, "手順", true, false);
                map.AddTBString(NodeAttr.FK_Flow, null, "フロー番号", true, false, 0, 10, 10);
                map.AddTBString(GetTaskAttr.CheckNodes, null, "作業ノードs", true, false, 0, 50, 100);
