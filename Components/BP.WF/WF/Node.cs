...
namespace BP.WF
{
    ...
    public class Node : Entity
    {
        ...
        public static string CheckFlow(Flow fl)
        {
            ...
            if (nds.Count == 0)
                return "フロー[" + fl.No + fl.Name + "]ノードデータがありません。このフローに登録する必要があります。";
        ...
        public string No
        {
            get
            {
                ...
                catch (Exception ex)
                {
                    ...
                    throw new Exception("@取得できませんでしたNodeID =" + this.NodeID);
                }
            }
        }
        /// <summary>
        ...
        public string NodeFrmID
        {
            get
            {
                    ...
                    (this.HisFormType == NodeFormType.FoolForm || this.HisFormType == NodeFormType.FreeForm))
                {
                    ...
                    if (nodeID == 0)
                        throw new Exception("err@指定されたノードが見つかりませんでした。");
...
//#warning 2012-01-24修订,没有自动计算出来属性。
                switch (this.HisRunModel)
                {
                    ...
                    default:
                        throw new Exception("@判定タイプなしNodeWorkType。");
                }
            }
            set
            {
            ...
            get
            {
                ...
                if (DataType.IsNullOrEmpty(str))
                {
                    if (this.BlockModel == WF.BlockModel.CurrNodeAll)
                        return "送信できないサブフローがまだあります。送信する前に、すべてのサブフローが完了するまで待つ必要があります。";
        ...
        public Node(string ndName)
        {
            ...
            {
                if (this.RetrieveFromDBSources() <= 0)
                    throw new Exception("Node Retrieve エラーIDなし=" + ndName);
            }
            else
            {
                if (this.Retrieve() <= 0)
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBString(NodeAttr.Name, null, "名前", true, false, 0, 150, 10);
                map.AddTBString(NodeAttr.Tip, null, "操作のヒント", true, true, 0, 100, 10, false);

                map.AddTBInt(NodeAttr.Step, (int)NodeWorkType.Work, "フローステップ", true, false);

                map.AddTBString(NodeAttr.ICON, null, "ノードICONイメージパス", true, false, 0, 70, 10);

                map.AddTBInt(NodeAttr.NodeWorkType, 0, "ノードタイプ", false, false);
                map.AddTBInt(NodeAttr.SubThreadType, 0, "子スレッドID", false, false);

                map.AddTBString(NodeAttr.FK_Flow, null, "FK_Flow", false, false, 0, 3, 10);
                map.AddTBInt(NodeAttr.IsGuestNode, 0, "クライアント実行ノードかどうか", false, false);

                map.AddTBString(NodeAttr.FlowName, null, "フロー名", false, true, 0, 200, 10);
                map.AddTBInt(NodeAttr.IsToParentNextNode, 0, "子フローがこのノードで実行されると、親フローが次のステップまで自動的に実行されるようにします", false, false);
                ...
                map.AddTBInt(NodeAttr.IsBUnit, 0, "ノードテンプレートかどうか（ビジネスユニット）", true, false);
                #endregion 基本属性.

                #region 审核组件.
                map.AddTBInt(NodeAttr.FWCSta, 0, "監査コンポーネント", false, false);
                map.AddTBFloat(NodeAttr.FWC_H, 0, "監査コンポーネントの高さ", false, true);
                map.AddTBInt(FrmWorkCheckAttr.FWCOrderModel, 0, "コラボレーションモードでのオペレーターの表示シーケンス", false, false);
                map.AddTBInt(FrmWorkCheckAttr.FWCVer, 0, "コンポーネントのバージョンを確認する", false, false);
                map.AddTBInt("FWCAth", 0, "添付ファイルの監査が有効になっています", false, false);
                #endregion 审核组件.

                #region 考核属性.
                map.AddTBString(BtnAttr.CHLab, "ノードの時間制限", "ノードの時間制限", true, false, 0, 50, 10);
                map.AddTBInt(BtnAttr.CHRole, 0, "制限時間",true,false);

                map.AddTBFloat(NodeAttr.TimeLimit, 2, "締め切り（日）", true, false); //"締め切り（日）".
              //  map.AddTBFloat(NodeAttr.TSpanHour, 0, "小时", true, false); //"限期(分钟)".
                map.AddTBInt(NodeAttr.TWay, 0, "時間の計算方法", true, false); //0=不计算节假日,1=计算节假日.
                ...
                map.AddTBFloat(NodeAttr.WarningDay, 1, "作業警告（日）", true, false);    // "警告期限(0不警告)"
                ...
                //考核相关.
                map.AddTBInt(NodeAttr.IsEval, 0, "仕事の質の評価かどうか", true, true);
                map.AddTBInt(NodeAttr.OutTimeDeal, 0, "残業対応方式", false, false);
                map.AddTBString(NodeAttr.DoOutTime, null, "残業コンテンツ内容", true, false, 0, 300, 10, true);

                #endregion 考核属性.

                map.AddTBString(FrmWorkCheckAttr.FWCNodeName, null, "ノード意見名", true, false, 0, 100, 10);
                map.AddTBString(NodeAttr.Doc, null, "解説", true, false, 0, 100, 10);
                map.AddBoolean(NodeAttr.IsTask, true, "割り当ては許可されていますか？", true, true);

                //退回相关.
                map.AddTBInt(NodeAttr.ReturnRole, 2, "返品ルール", true, true);
                map.AddTBString(NodeAttr.ReturnReasonsItems, null, "返却理由", true, false, 0, 50, 10, true);
                map.AddTBString(NodeAttr.ReturnAlert, null, "返却後の情報プロンプト", true, false, 0, 50, 10, true);
                map.AddBoolean(NodeAttr.ReturnCHEnable, false, "返品評価ルールを有効にするかどうか", true, true);

                map.AddTBInt(NodeAttr.ReturnOneNodeRole, 0, "単一ノードの返品ルール", true, true);
                map.AddTBString(BtnAttr.ReturnField, null, "フィールドに入力する情報を返す", true, false, 0, 50, 10, true); 

                map.AddTBInt(NodeAttr.DeliveryWay, 0, "アクセスルール", true, true);
                map.AddTBInt(NodeAttr.IsExpSender, 1, "このノードの受信者は、前のステップの送信者を含めることはできません", true, true);

                map.AddTBInt(NodeAttr.CancelRole, 0, "取消規則", true, true);
                map.AddTBInt(NodeAttr.CancelDisWhenRead, 0, "相手が読んだため、元に戻すことはできません", true, true);
         
                map.AddTBInt(NodeAttr.WhenNoWorker, 0, "ハンドラーが見つからない場合", true, true);
                map.AddTBString(NodeAttr.DeliveryParas, null, "アクセスルール設定", true, false, 0, 300, 10);
                map.AddTBString(NodeAttr.NodeFrmID, null, "ノードフォームID", true, false, 0, 50, 10);

                map.AddTBInt(NodeAttr.CCRole, 0, "CCルール", true, true);
                map.AddTBInt(NodeAttr.CCWriteTo, 0, "Ccデータ書き込みルール", true, true);

                map.AddTBInt(BtnAttr.DelEnable, 0, "削除規則", true, true);
                map.AddTBInt(NodeAttr.SaveModel, 0, "保存モード", true, true);

                map.AddTBInt(NodeAttr.IsCanDelFlow, 0, "フローを削除できますか", true, true);

                map.AddTBInt(NodeAttr.ThreadKillRole, 0, "子スレッドの削除方法", true, true);

                map.AddTBInt(NodeAttr.TodolistModel, 0, "マルチプレイヤールール", true, true);

                //add.
                map.AddTBInt(NodeAttr.TeamLeaderConfirmRole, 0, "チームリーダーがルールを確認する", true, true);
                map.AddTBString(NodeAttr.TeamLeaderConfirmDoc, null, "チームリーダーが設定を確認する", true, false, 0, 100, 10);
                map.AddTBInt(BtnAttr.HuiQianLeaderRole, 0, "グループリーダーの副署名規則", true, true);

                map.AddTBInt(NodeAttr.IsAllowRepeatEmps, 0, "子スレッドは譜表の繰り返しを受け入れることを許可されていますか（子スレッドポイントに有効）？", true, true);
                map.AddTBInt(NodeAttr.IsBackTracking, 1, "帰国後も同じように帰ることはできますか？(リターン機能が有効になっている場合にのみ機能します)", true, true);
                map.AddTBInt(NodeAttr.IsRM, 1, "配信経路の自動メモリー機能は有効になっていますか？", true, true);
                map.AddTBInt(NodeAttr.IsOpenOver, 0, "開いて承認するかどうか。", true, true);
                map.AddBoolean(NodeAttr.IsHandOver, false, "お譲りできますか", true, true);
                map.AddTBDecimal(NodeAttr.PassRate, 100, "合格率", true, true);
                map.AddTBInt(NodeAttr.RunModel, 0, "動作モード（通常のノードで有効）", true, true);
                map.AddTBInt(NodeAttr.BlockModel, 0, "ブロックモード", true, true);
                map.AddTBString(NodeAttr.BlockExp, null, "ブロッキング式", true, false, 0, 200, 10);
                map.AddTBString(NodeAttr.BlockAlert, null, "ブロックされたプロンプトメッセージ", true, false, 0, 100, 10);

                map.AddTBInt(NodeAttr.WhoExeIt, 0, "誰が実行するか", true, true);
                map.AddTBInt(NodeAttr.ReadReceipts, 0, "領収書を読む", true, true);
                map.AddTBInt(NodeAttr.CondModel, 0, "方向性条件制御ルール", true, true);

                // 自动跳转.
                map.AddTBInt(NodeAttr.AutoJumpRole0, 0, "プロセッサは著者です0", false, false);
                map.AddTBInt(NodeAttr.AutoJumpRole1, 0, "ハンドラーはすでに表示されています1", false, false);
                map.AddTBInt(NodeAttr.AutoJumpRole2, 0, "プロセッサーは前のステップ2と同じです2", false, false);

                //父子流程.
                map.AddTBString(NodeAttr.SFActiveFlows, null, "サブフロー", true, false, 0, 100, 10);

                // 批处理.
                map.AddTBInt(NodeAttr.BatchRole, 0, "バッチ処理", true, true);
                map.AddTBInt(NodeAttr.BatchListCount, 12, "バッチ数量", true, true);
                map.AddTBString(NodeAttr.BatchParas, null, "パラメータ", true, false, 0, 500, 10);
                map.AddTBInt(NodeAttr.PrintDocEnable, 0, "印刷方法", true, true);
                //打印PDF的处理
                map.AddTBInt(BtnAttr.PrintPDFModle, 0, "PDF印刷規則", true, true);
                map.AddTBString(BtnAttr.ShuiYinModle, null, "印刷水印規則", true, false, 20, 100, 100, true);
                ...
                //表单相关.
                map.AddTBInt(NodeAttr.FormType, 1, "フォームタイプ", false, false);
                map.AddTBString(NodeAttr.FormUrl, "http://", "フォームURL", true, false, 0, 300, 10);
                map.AddTBInt(NodeAttr.TurnToDeal, 0, "操舵", false, false);
                map.AddTBString(NodeAttr.TurnToDealDoc, null, "送信後にメッセージを表示", true, false, 0, 200, 10, true);
                map.AddTBInt(NodeAttr.NodePosType, 0, "ポジション", false, false);
                map.AddTBInt(NodeAttr.IsCCFlow, 0, "処理完了条件はありますか", false, false);
                map.AddTBString(NodeAttr.HisStas, null, "役職", false, false, 0, 300, 10);
                map.AddTBString(NodeAttr.HisDeptStrs, null, "部門", false, false, 0, 300, 10);
                map.AddTBString(NodeAttr.HisToNDs, null, "ノードに移動", false, false, 0, 50, 10);
                map.AddTBString(NodeAttr.HisBillIDs, null, "ドキュメントIDs", false, false, 0, 50, 10);
                ...
                map.AddTBString(NodeAttr.PTable, null, "物理的なテーブル", false, false, 0, 100, 10);

                map.AddTBString(NodeAttr.GroupStaNDs, null, "ポストグループノード", false, false, 0, 200, 10);
                map.AddTBInt(NodeAttr.X, 0, "X座標", false, false);
                map.AddTBInt(NodeAttr.Y, 0, "Y座標", false, false);

                map.AddTBString(NodeAttr.FocusField, null, "ポイントフィールド", false, false, 0, 30, 10);
                map.AddTBString(NodeAttr.JumpToNodes, null, "遷移可能なノード", true, false, 0, 100, 10, true);

                map.AddTBString(NodeAttr.RefOneFrmTreeType, "", "独立フォームタイプ", false, false, 0, 100, 10);//RefOneFrmTree
                ...
                // 启动子线程参数 2013-01-04
                map.AddTBInt(NodeAttr.SubFlowStartWay, 0, "子スレッド開始モード", true, false);
                map.AddTBString(NodeAttr.SubFlowStartParas, null, "起動パラメーター", true, false, 0, 100, 10);

                // 启动自动运行. 2013-01-04
                map.AddTBInt(NodeAttr.AutoRunEnable, 0, "自動運転を開始するか？", true, false);
                map.AddTBString(NodeAttr.AutoRunParas, null, "自動運転パラメータ", true, false, 0, 100, 10);

                map.AddTBString(NodeAttr.SelfParas, null, "カスタムパラメータ（小さすぎる場合は、手動で拡張できます）", true, false, 0, 1000, 10);

                #region 与参数有关系的属性。

                #region 子流程相关的参数
                map.AddTBFloat(FrmSubFlowAttr.SF_H, 300, "高", true, false);
                map.AddTBFloat(FrmSubFlowAttr.SF_W, 400, "幅", true, false);
                #endregion 子流程相关的参数
        ...
        protected override bool beforeDelete()
        {
            ...
            if (this.NodePosType == NodePosType.End)
            {
                ...
                {
                    BP.WF.Dev2Interface.Flow_DoFlowOver(gwf.FK_Flow, gwf.WorkID, "フローは正常に終了しました");
                }
            }
            //判断是否可以被删除. 
             num = DBAccess.RunSQLReturnValInt("SELECT COUNT(*) FROM WF_GenerWorkerlist WHERE FK_Node=" + this.NodeID);
            if (num != 0)
                throw new Exception("@ノード[" + this.NodeID + "," + this.Name + "]実行するジョブがあります。削除できません。");
        ...
        public string RepareMap(Flow fl)
        {
            ...
            if (attrs.Contains(MapAttrAttr.KeyOfEn, WorkAttr.Rec, MapAttrAttr.FK_MapData, md.No) == false)
            {
                ...
                if (this.IsStartNode == false)
                    attr.Name = "レコーダー"; // "レコーダー";
                else
                    attr.Name = "スポンサー"; //"スポンサー";
            ...
            if (this.FocusField != "")
            {
                if (attr.IsExit(MapAttrAttr.KeyOfEn, this.FocusField, MapAttrAttr.FK_MapData, md.No) == false)
                    msg += "@ポイントフィールド" + this.FocusField + " 違法に削除された";
            }
            return msg;
        }
        /// <summary>
