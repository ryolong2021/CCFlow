    ...
    public class RememberMe : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("WF_RememberMe", "私を覚えてますか");

                map.AddMyPK();

                map.AddTBInt(RememberMeAttr.FK_Node, 0, "ノード", false, false);
                map.AddTBString(RememberMeAttr.FK_Emp, "", "現在のオペレーター", true, false, 1, 30, 10);

                map.AddTBString(RememberMeAttr.Objs, "", "担当者を割り当てる", true, false, 0, 4000, 10);
                map.AddTBString(RememberMeAttr.ObjsExt, "", "担当者内線Ext", true, false, 0, 4000, 10);

                map.AddTBString(RememberMeAttr.Emps, "", "すべてのスタッフ", true, false, 0, 4000, 10);
                map.AddTBString(RememberMeAttr.EmpsExt, "", "スタッフ内線Ext", true, false, 0, 4000, 10);
                this._enMap = map;
                return this._enMap;
            }
        }
