    ...
    public class ReturnWork : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_ReturnWork", "バックトラック");

                map.AddMyPK();

                map.AddTBInt(ReturnWorkAttr.WorkID, 0, "WorkID", true, true);

                map.AddTBInt(ReturnWorkAttr.ReturnNode, 0, "戻りノード", true, true);
                map.AddTBString(ReturnWorkAttr.ReturnNodeName, null, "ノード名を返す", true, true, 0, 100, 10);

                map.AddTBString(ReturnWorkAttr.Returner, null, "帰国者", true, true, 0, 50, 10);
                map.AddTBString(ReturnWorkAttr.ReturnerName, null, "帰国者の名前", true, true, 0, 100, 10);

                map.AddTBInt(ReturnWorkAttr.ReturnToNode, 0, "ReturnToNode", true, true);
                map.AddTBString(ReturnWorkAttr.ReturnToEmp, null, "戻る", true, true, 0, 4000, 10);

                //如果是frm类型，就是格式化的json数据.
                map.AddTBString(ReturnWorkAttr.BeiZhu, null, "返却理由", true, true, 0, 4000, 10);

                map.AddTBDateTime(ReturnWorkAttr.RDT, null, "返却日", true, true);
                map.AddTBInt(ReturnWorkAttr.IsBackTracking, 0, "同じように戻りますか？", true, true);
