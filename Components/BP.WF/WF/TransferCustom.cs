    ...
    public class TransferCustom : EntityMyPK
    {
        ...
        public override Map EnMap
        {
            get
            {
                if (this._enMap != null)
                    return this._enMap;
                Map map = new Map("WF_TransferCustom", "カスタム実行パス");
                ...
                map.AddTBInt(TransferCustomAttr.FK_Node, 0, "ノードID", true, false);
                map.AddTBString(TransferCustomAttr.NodeName, null, "ノード名", true, false, 0, 200, 10);

                map.AddTBString(TransferCustomAttr.Worker, null, "プロセッサー（複数の人はコンマで区切られます）", true, false, 0, 200, 10);
                map.AddTBString(TransferCustomAttr.WorkerName, null, "プロセッサー（複数の人はコンマで区切られます）", true, false, 0, 200, 10);

                map.AddTBString(TransferCustomAttr.SubFlowNo, null, "通過するサブフローの数", true, false, 0, 3, 10);
                map.AddTBDateTime(TransferCustomAttr.PlanDT, null, "完成予定日", true, false);
                map.AddTBInt(TransferCustomAttr.TodolistModel, 0, "複数人作業処理モード", true, false);
                map.AddTBInt(TransferCustomAttr.IsEnable, 0, "有効にするかどうか", true, false);

                map.AddTBInt(TransferCustomAttr.Idx, 0, "シーケンス番号", true, false);
