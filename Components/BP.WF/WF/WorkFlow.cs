    ...
    public class WorkFlow
    {
        ...
        public bool IsCanDoCurrentWork(string empId)
        {
            ...
            if (gwls.Count == 0)
            {
                ...
                throw new Exception("@ワークフロー定義が間違っており、この作業を実行する人が見つかりません。関連情報：WorkID=" + this.WorkID + ",NodeID=" + wn.HisNode.NodeID);
            }
        ...
        public string DoReject(Int64 fid, int fk_node, string msg)
        {
            ...
            return "作業は拒否されました(" + wl.FK_Emp + " , " + wl.FK_EmpText + ")";
            // wl.HisNode
        }
        /// <summary>
        /// 逻辑删除流程
        ...
        public void DoDeleteWorkFlowByFlag(string msg)
        {
            ...
            {
                Log.DefaultLogWriteLine(LogType.Error, "@トゥームストーンエラー:" + ex.Message);
                throw new Exception("@トゥームストーンエラー:" + ex.Message);
            }
        }
        /// <summary>
        /// 恢复逻辑删除流程
        ...
        {
            Log.DebugWriteInfo("フローの削除を開始：フロー番号:" + flowNo + "-WorkID:" + workID + "-" + ". サブフローを削除しますか:" + isDelSubFlow + ";削除の理由:" + note);
            ...
            {
                string err = "@ワークフローエラーの削除 Err " + ex.TargetSite;
                Log.DefaultLogWriteLine(LogType.Error, err);
                throw new Exception(err);
            }
            info = "@削除フローが正常に削除されました";
            #endregion 正常的删除信息.
            ...
            BP.DA.Log.DefaultLogWriteLineInfo("@[" + fl.Name + "]フローは[" + BP.Web.WebUser.No + BP.Web.WebUser.Name + "]削除、WorkID[" + workID + "]。");
            return "完了したフローはあなたによって正常に削除されました。";
        }
        /// <summary>
        /// 删除子线程
        /// </summary>
        ...
        private string DoDeleteSubThread()
        {
            ...
            {
                throw new Exception("フローIDとフロー番号の取得中にエラーが発生しました。" + ex.Message);
            }
            ...
            {
                string err = "@ワークフローを削除[" + this.HisStartWork.OID + "," + this.HisStartWork.Title + "] Err " + ex.Message;
                Log.DefaultLogWriteLine(LogType.Error, err);
                throw new Exception(err);
            }
            string info = "@削除フローが正常に削除されました";
            #endregion 正常的删除信息.
            ...
            if (1 == 2)
            {
                ...
                if (fk_node != 0)
                {
                    ...
                    if (nextNode.PassRate > 0)
                    {
                        ...
                        if (priNodes.Count != 1)
                            throw new Exception("@サブフローの異なるスレッドに対する要件はありません。");
                        ...
                        if (all == 0)
                        {
                            ...
                            info += "@すべての子スレッドが終了しました。";
                            info += "@メインフロー情報を終了します。";
                            info += "@" + wf.DoFlowOver(ActionType.FlowOver, "合流点フローは終了します", null, null);
                        }
            ...
            wn.AddToTrack(ActionType.DeleteSubThread, empOfWorker.No, empOfWorker.Name,
             wn.HisNode.NodeID,
             wn.HisNode.Name, "子スレッドは:" + BP.Web.WebUser.Name + "削除。");
            #endregion 写入删除日志.

            return "子スレッドが正常に削除されました";
        }
        /// <summary>
        /// 删除已经完成的流程
        /// </summary>
        ...
        {
            if (this.FID == 0)
                throw new Exception("@フローはサブスレッドフローインスタンスではないため、メソッドを実行できません。");
...
#warning 应该删除一个子线程后，就需要计算完成率的问题。但是目前应用到该场景极少,因为。能够看到河流点信息，说明已经到达了完成率了。
            ...
            if (num == 0)
            {
                ...
                if (i == 0)
                {
                    ...
                    if (ndMain.IsHL == true)
                    {
                        ...
                        if (num == 0)
                        {
                            ...
                            if (nd.IsFL == false)
                                throw new Exception("@プログラムエラー、最も近いブランチノードが見つかりませんでした。");
                ...
                else
                {
                    ...
                    gwl.Update();
                    return "子スレッドが正常に削除されました。これが最後に削除された子スレッドです。" + gwfMain.NodeName + "}to-doが生成されます、<a href='/WF/MyFlow.htm?WorkID=" + gwfMain.WorkID + "&FK_Flow=" + gwfMain.FK_Flow + "'>クリックして作業を処理</a>";
        ...
        public string DoDeleteWorkFlowByReal(bool isDelSubFlow)
        {
            ...
            #region 正常的删除信息.
            BP.DA.Log.DefaultLogWriteLineInfo("@[" + this.HisFlow.Name + "]フローは[" + BP.Web.WebUser.No + BP.Web.WebUser.Name + "]削除、WorkID[" + this.WorkID + "]。");
            string msg = "";
            try
            {
                Int64 workId = this.WorkID;
                    ...
                    switch (fND.HisNodeWorkType)
                    {
                        ...
                        default:
                            ...
                            if (num == 0)
                            {
                                ...
                                info += "@すべての子スレッドが終了しました。";
                                info += "@メインフロー情報を終了します。";
                                info += "@" + wf.DoFlowOver(ActionType.FlowOver, "メインフローの終了", null, null);
                            }
                            break;
                    }
                }
        ...
        public void DoComeBackWorkFlow(string msg)
        {
            try
            {
                ...
                if (wls.Count == 0)
                    throw new Exception("@リカバリフローでエラーが発生したため、ワーカーのリストが表示されました");

                foreach (GenerWorkerList item in wls)
                    BP.WF.Dev2Interface.Port_SendMsg(item.FK_Emp, "フロー再開通知" + gwf.Title, "フロー[" + gwf.Title + "]、開いてください。", "rback");
            }
            catch (Exception ex)
            {
                Log.DefaultLogWriteLine(LogType.Error, "@回復フローのエラー。" + ex.Message);
                throw new Exception("@回復フローのエラー。" + ex.Message);
            }
        }
        #endregion
        ...
        public WorkNode GetCurrentWorkNode()
        {
            ...
            if (gwf.RetrieveFromDBSources() == 0)
            {
                this.DoFlowOver(ActionType.FlowOver, "異常終了、現在のフローレコードが見つかりません。", null, null);
                throw new Exception("@" + string.Format("ワークフロー{0}が完了しました。", this.HisStartWork.Title));
            }
            ...
            {
                Log.DefaultLogWriteLineError("@WorkID=" + this.WorkID + ",FK_Node=" + gwf.FK_Node + ".クエリが機能しないように見えないはずです。"); // 没有找到当前的工作节点的数据，流程出现未知的异常。
                ...
                {
                    Log.DefaultLogWriteLineError("@現在の作業ノードのデータが見つからず、フローに不明な例外があります" + ex.Message + "、表示されるべきではない"); // 没有找到当前的工作节点的数据
                }
            }
            work.FID = gwf.FID;
        ...
        public string DoFlowOverFeiLiu(GenerWorkFlow gwf)
        {
            ...
            {
                case 0:
                    throw new Exception("@間違ってはいけません。");
                case 1:
                    ...
                    return "@現在の作業が完了し、フロー内のすべての作業が完了しました。";
                default:
        ...
        public string DoFlowSubOver()
        {
            ...
            if (DBAccess.RunSQLReturnValInt(sql) == 0)
            {
                ...
                wf.DoFlowOver(ActionType.FlowOver, "サブフローの終了", null, null);
                return "@現在のサブフローが完了し、メインフローが完了しました。";
            }
            else
            {
                return "@現在のサブフローが完了し、メインフローが完了しました(" + num + ")サブフローは完了していません。";
            }
        }
        /// <summary>
        /// 让父亲流程自动发送到下一步骤上去.
        ...
        public string LetParentFlowAutoSendNextSetp()
        {
            ...
            {
                BP.WF.Dev2Interface.Flow_DoFlowOver(this.HisGenerWorkFlow.PFlowNo, this.HisGenerWorkFlow.PWorkID, "サブフローが完了し、親フローが自動的に終了します。");
                return "親フローは自動的に終了します。";
            }
            ...
            try
            {
                ...
                if (DataType.IsNullOrEmpty(pEmp) == true)
                    throw new Exception("@サブフローを呼び出したスタッフは見つかりませんでした。");

                Emp emp = new Emp();
                emp.No = pEmp;
                if (emp.RetrieveFromDBSources() == 0)
                    throw new Exception("@サブフローの従業員番号を持ち上げます(" + pEmp + ")存在しないため、親フローを開始できません");
                ...
                if (BP.WF.Dev2Interface.Flow_IsCanDoCurrentWork( pGWF.WorkID, WebUser.No) == false)
                {
                    ...
                    BP.WF.Dev2Interface.Node_Shift( pGWF.WorkID, WebUser.No, "作業は自動的に引き渡され、次のステップに進みます。");
                }
               ...
               // GERpt rpt = new GERpt("ND" + int.Parse(this.HisGenerWorkFlow.PFlowNo) + "Rpt", this.HisGenerWorkFlow.PWorkID);
                ...
                return "@親フローを次のノードまで実行しました。" + objs.ToMsgOfHtml();
            }
            catch (Exception ex)
            {
                this.HisGenerWorkFlow.WFState = WFState.Complete;
                ...
                {
                    info += "@フロー設計エラー：自動的に実行される次のノードの受信者ルールが前のユーザーによって選択されたため、次のステップへの自動実行に失敗しました。";
                    return info;
                }

                return "@最後のサブフローが完了した後、親フローのノードが自動的に送信されるとエラーが発生します:" + ex.Message;
            }
        }
        /// <summary>
        /// 执行流程完成
...
//            }
        ...
        public string GenerFHStartWorkInfo()
        {
            ...
            switch (dt.Rows.Count)
            {
                case 0:
                    ...
                    throw new Exception("@開始ノードのデータが見つからず、フローが異常でした。 FID=" + this.FID + "、ノード：" + nd.Name + "ノードID：" + nd.NodeID);
                case 1:
                    msg = string.Format("@開始者： {0}  日付：{1} 開始されたフロータイトル：{2} 、正常に完了。",
                        dt.Rows[0]["Rec"].ToString(), dt.Rows[0]["RDT"].ToString(), dt.Rows[0]["Title"].ToString());
                    break;
                default:
                    msg = "@以下(" + dt.Rows.Count + ")人々が始めたフローは完了しました。";
                    foreach (DataRow dr in dt.Rows)
                    {
                        msg += "<br>スポンサー：" + dr["Rec"] + " 発売日：" + dr["RDT"] + " 題名：" + dr["Title"] + "<a href='./../../WF/WFRpt.htm?WorkID=" + dr["OID"] + "&FK_Flow=" + this.HisFlow.No + "' target=_blank>詳細…</a>";
                    }
                    break;
            }
            return msg;
        ...
        public string DoFix(string fixMsg)
        {
            if (this.HisGenerWorkFlow.WFState == WFState.Fix)
                throw new Exception("@現在凍結されており、再凍結はできません。");
            ...
            return this.WorkID+"-"+this.HisFlow.Name+"フリーズが正常に実行されました";
        }
        /// <summary>
        /// 执行解除冻结
        /// </summary>
        ...
        {
            if (this.HisGenerWorkFlow.WFState != WFState.Fix)
                throw new Exception("@現在の非凍結状態では解凍を実行できません。");
        ...
        public WorkFlow(string fk_flow, Int64 wkid)
        {
            ...
            if (wkid == 0)
                throw new Exception("@ジョブIDを指定せずにワークフローを作成することはできません。");
        ...
        public string DoHungUp(HungUpWay way, string relData, string hungNote)
        {
            if (this.HisGenerWorkFlow.WFState == WFState.HungUp)
                throw new Exception("@現在の状態はすでに一時停止されています。一時停止状態で実行することはできません。");
            ...
            if (way == HungUpWay.SpecDataRel)
                if (relData.Length < 10)
                    throw new Exception("@停止解除の日付が正しくありません(" + relData + ")");
            if (relData == null)
                relData = "";
            ...
            GenerWorkerLists wls = new GenerWorkerLists(this.WorkID, this.HisFlow.No);
            string url = Glo.ServerIP + "/" + this.VirPath + this.AppType + "/WorkOpt/OneWork/OneWork.htm?CurrTab=Track&FK_Flow=" + this.HisFlow.No + "&WorkID=" + this.WorkID + "&FID=" + this.HisGenerWorkFlow.FID + "&FK_Node=" + this.HisGenerWorkFlow.FK_Node;
            string mailDoc = "詳細情報：<A href = '" + url + "'>フロートラックを開く/A>";
            string title = "仕事:" + this.HisGenerWorkFlow.Title + " する" + WebUser.Name + "ハング" + hungNote;
            string emps = "";
            foreach (GenerWorkerList wl in wls)
            {
                if (wl.IsEnable == false)
            ...
            return "一時停止は正常に実行され、通知されました:" + emps;
        }
        /// <summary>
        /// 取消挂起
        /// </summary>
        ...
        {
            if (this.HisGenerWorkFlow.WFState != WFState.HungUp)
                throw new Exception("@一時停止されていないため、一時停止を解除できません。");
            ...
            if (hu.RetrieveFromDBSources() == 0)
                throw new Exception("@システムエラー、一時停止ポイントが見つかりませんでした");
            ...
            GenerWorkerLists wls = new GenerWorkerLists(this.WorkID, this.HisFlow.No);
            string url = Glo.ServerIP + "/" + this.VirPath + this.AppType + "/WorkOpt/OneWork/OneWork.htm?CurrTab=Track&FK_Flow=" + this.HisFlow.No + "&WorkID=" + this.WorkID + "&FID=" + this.HisGenerWorkFlow.FID + "&FK_Node=" + this.HisGenerWorkFlow.FK_Node;
            string mailDoc = "詳細情報：<A href = '" + url + "'>フロートラックを開</A>.";
            string title = "仕事:" + this.HisGenerWorkFlow.Title + " する" + WebUser.Name + "停止解除。";
            string emps = "";
            foreach (GenerWorkerList wl in wls)
            {
                if (wl.IsEnable == false)
            ...
            // 记录日志..
            WorkNode wn = new WorkNode(this.WorkID, this.HisGenerWorkFlow.FK_Node);
            wn.AddToTrack(ActionType.UnHungUp, WebUser.No, WebUser.Name, wn.HisNode.NodeID, wn.HisNode.Name, "中断されずに通知されました。" + emps);
            return null;
        }
        /// <summary>
        /// 撤消移交
        ...
        public string DoUnShift()
        {
            ...
            if (wls.Count == 0)
                return "現在のジョブなしでハンドオーバーに失敗しました。";
            ...
            wn.AddToTrack(ActionType.UnShift, WebUser.No, WebUser.Name, nd.NodeID, nd.Name, "ハンドオーバーをキャンセル");
            ...
            if (wls.Count == 1)
            {
                ...
                wl.IsPass = false;
                wl.Update();
                return "@ハンドオーバーのキャンセルは成功しました，<a href='" + Glo.CCFlowAppPath + "WF/MyFlow.htm?FK_Flow=" + this.HisFlow.No + "&FK_Node=" + wl.FK_Node + "&FID=" + wl.FID + "&WorkID=" + this.WorkID + "'><img src='" + Glo.CCFlowAppPath + "WF/Img/Btn/Do.gif' border=0/>仕事をする</A>";
            }
            ...
            if (mywl != null)
                return "@ハンドオーバーのキャンセルは成功しました，<a href='" + Glo.CCFlowAppPath + "WF/MyFlow.htm?FK_Flow=" + this.HisFlow.No + "&FK_Node=" + mywl.FK_Node + "&FID=" + mywl.FID + "&WorkID=" + this.WorkID + "'><img src='" + Glo.CCFlowAppPath + "WF/Img/Btn/Do.gif' border=0/>仕事をする</A>";
            ...
            return "@ハンドオーバーのキャンセルは成功しました，<a href='" + Glo.CCFlowAppPath + "WF/MyFlow.htm?FK_Flow=" + this.HisFlow.No + "&FK_Node=" + wk.FK_Node + "&FID=" + wk.FID + "&WorkID=" + this.WorkID + "'><img src='" + Glo.CCFlowAppPath + "WF/Img/Btn/Do.gif' border=0/>仕事をする</A>";
        }
        #endregion
    }
    /// <summary>
    ...
    public class WorkFlows : CollectionBase
    {
        ...
        {
            string infoMsg = "デッドノード情報をクリアする：";
            string errMsg = "デッドノードのエラーメッセージをクリアする：";
