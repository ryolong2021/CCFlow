    ...
    public class WorkFlowDeleteLog : EntityOID
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                Map map = new Map("WF_WorkFlowDeleteLog", "フロー削除ログ");
                 
                // 流程基础数据。
                map.AddTBIntPKOID(); 
                map.AddTBInt(GenerWorkFlowAttr.FID, 0, "FID", false, false);
                map.AddDDLEntities(GenerWorkFlowAttr.FK_Dept, null, "部門", new Port.Depts(), false);
                map.AddTBString(GenerWorkFlowAttr.Title, null, "題名", true, true, 0, 100, 100);
                map.AddTBString(GERptAttr.FlowStarter, null, "スポンサー", true, true, 0, 100, 100);
                map.AddTBDateTime(GERptAttr.FlowStartRDT, null, "開始時間", true, true);
                //map.AddDDLEntities(GenerWorkFlowAttr.FK_NY, null, "年月", new BP.Pub.NYs(), false);
                map.AddDDLEntities(GenerWorkFlowAttr.FK_Flow, null, "フロー", new Flows(), false);
                map.AddTBDateTime(GERptAttr.FlowEnderRDT, null, "最終処理時間", true, true);
                map.AddTBInt(GERptAttr.FlowEndNode, 0, "滞在ノード", true, true);
                map.AddTBFloat(GERptAttr.FlowDaySpan, 0, "スパン（日）", true, true);
                map.AddTBString(GERptAttr.FlowEmps, null, "参加者", false, false, 0, 100, 100);

                //删除信息.
                map.AddTBString(WorkFlowDeleteLogAttr.Oper, null, "担当者を削除", true, true, 0, 20, 10);
                map.AddTBString(WorkFlowDeleteLogAttr.OperDept, null, "人事部門の削除", true, true, 0, 20, 10);
                map.AddTBString(WorkFlowDeleteLogAttr.OperDeptName, null, "人物名を削除", true, true, 0, 200, 10);
                map.AddTBString(WorkFlowDeleteLogAttr.DeleteNote, "", "削除の理由", true, true, 0, 4000, 10);
                map.AddTBDateTime(WorkFlowDeleteLogAttr.DeleteDT, null, "日付を削除", true, true);
