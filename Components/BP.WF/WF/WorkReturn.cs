...
namespace BP.WF
{
    ...
    public class WorkReturn
    {
        ...
        /// 退回原因
        /// </summary>
        private string Msg = "返品理由が記入されていません。";
        /// <summary>
        /// 当前节点
        /// </summary>
        ...
        public WorkReturn(string fk_flow, Int64 workID, Int64 fid, int currNodeID, int returnToNodeID, string reutrnToEmp, bool isBackTrack, string returnInfo,string pageData=null)
        {
            ...
            if (returnToNodeID == 0)
            {
                ...
                if (dt.Rows.Count == 0)
                    throw new Exception("err@現在のノードを返すことは許可されていません。システムは、戻りルールに従って返すことができるノードを検出しませんでした");

                if (dt.Rows.Count != 1)
                    throw new Exception("err@現在のノードが返すことができるノードは[" + dt.Rows.Count + "]件、返されるノードを指定する必要があります");
        ...
        public string DoOrderReturn()
        {
            ...
            //执行退回的考核.
            Glo.InitCH(this.HisNode.HisFlow, this.HisNode, this.WorkID, this.FID, this.HisNode.Name + "評価のために戻る。");
            ...
            if (this.HisNode.FocusField != "")
            {
                ...
                {
                    Log.DefaultLogWriteLineError("戻るときのフォーカスフィールドエラーの更新" + ex.Message);
                }
            }
...
#warning 没有考虑两种流程数据存储模式。
            ...
            if (this.ReturnToNode.IsGuestNode)
            {
                ...
                return "作品はあなたから返却されました(" + this.ReturnToNode.Name + ")、戻る(" + gwf.GuestNo + "," + gwf.GuestName + ").\n\r" + text;
            }
            else
            {
                return "作品はあなたから返却されました(" + this.ReturnToNode.Name + ")、戻る(" + returnToEmp.No + "," + returnToEmp.Name + ").\n\r" + text;
            }
        }
        /// <summary>
        /// 要退回到父流程上去
        ...
        private string ReturnToParentFlow()
        {
            ...
            //返回退回信息.
            return "成功したリターン[" + gwfP.FlowName + " - " + this.ReturnToNode.Name + "]、戻る[" + toEmps + "].";
        }
        /// <summary>
        /// 执行退回.
        /// </summary>
            ...
            switch (this.HisNode.HisRunModel)
            {
                case RunModel.Ordinary: /* 1： 普通节点向下发送的*/
                    switch (ReturnToNode.HisRunModel)
                    {
                        ...
                        default:
                            throw new Exception("@フォールバックエラー：不正なデザインモードまたはフォールバックモードです。通常のセクションから子スレッドポイントまで");
                            break;
                    }
                    break;
                case RunModel.FL: /* 2: 分流节点向下发送的*/
                    switch (this.ReturnToNode.HisRunModel)
                    {
                        ...
                        default:
                            throw new Exception("@判定されないノードタイプ(" + ReturnToNode.Name + ")");
                            break;
                    }
                    break;
                ...
                case RunModel.FHL:  /* 4: 分流节点向下发送的 */
                    switch (this.ReturnToNode.HisRunModel)
                    {
                        ...
                        case RunModel.SubThread:/*4.5 子线程*/
                            return ExeReturn3_4();
                        default:
                            throw new Exception("@判定されないノードタイプ(" + this.ReturnToNode.Name + ")");
                    }
                    break;
                case RunModel.SubThread:  /* 5: 子线程节点向下发送的 */
                    switch (this.ReturnToNode.HisRunModel)
                    {
                        case RunModel.Ordinary: /*5.1 普通工作节点 */
                            throw new Exception("@不正な戻りモードです。管理者にフィードバックしてください。");
                        case RunModel.FL: /*5.2 分流点 */
                            /*子线程退回给分流点.*/
                            return ExeReturn5_2();
                        case RunModel.HL: /*5.3 合流点 */
                            throw new Exception("@不正な戻りモードです。管理者に報告してください。");
                    ...
                    break;
                default:
                    throw new Exception("@判断なしの戻り型" + this.HisNode.HisRunModel);
            }
            throw new Exception("@未審査の例外がシステムで発生しました。");
        }
        /// <summary>
        /// 分流点退回给子线程
        /// </summary>
        ...
        private string ExeReturn2_4()
        {
            ...
            return "情報を正常に返す" + this.ReturnToNode.Name + " 、戻る：(" + this.ReturnToEmp + ")";
        }
        /// <summary>
        /// 子线程退回给分流点
        /// </summary>
        ...
        private string ExeReturn5_2()
        {
            GenerWorkFlow gwf = new GenerWorkFlow(this.WorkID);
            gwf.FK_Node = this.ReturnToNode.NodeID;
            string info = "@作品は正常に返却されました（" + ReturnToNode.Name + "）に戻る";
            ...
            if (i != 1)
                throw new Exception("@現在のスタッフリストデータが欠落しており、フローエンジンが間違っています。");
        ...
        private string ExeReturn1_1()
        {
                ...
                || this.HisNode.TodolistModel == TodolistModel.Teamup)
            {
                ...
                if (BP.DA.DBAccess.RunSQL(sql) == 0 && 1 == 2)
                    throw new Exception("@エラーが返され、更新する対象データが見つかりませんでした。技術情報:" + sql);

                //杨玉慧 将流程的  任务池状态设置为  NONE
                sql = "UPDATE WF_GenerWorkFlow SET TaskSta=0 WHERE  WorkID=" + this.WorkID;
                if (BP.DA.DBAccess.RunSQL(sql) == 0 && 1 == 2)
                    throw new Exception("@エラーが返されました。更新するターゲットデータが見つかりませんでした。技術情報:" + sql);
            }
            ...
            {
                return "作品はあなたから返却されました(" + this.ReturnToNode.Name + ")、戻る(" + gwl.FK_Emp + "," + gwl.FK_EmpText + ").\n\r" + text;
            }
        }
        /// <summary>
        /// 增加日志
        ...
        {
            string filePath = BP.Sys.SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "ReturnLog" + Path.DirectorySeparatorChar + this.HisNode.FK_Flow + Path.DirectorySeparatorChar;
            if (System.IO.Directory.Exists(filePath) == false)
                System.IO.Directory.CreateDirectory(filePath);

            string file = filePath + Path.DirectorySeparatorChar + rw.MyPK;
            infoLog = "\r\n帰国者" + WebUser.No + "," + WebUser.Name + " \r\n戻りノード:" + fromND.Name + " \r\n戻る:" + toND.Name;
            infoLog += "\r\n返却時間:" + DataType.CurrentDataTime;
            infoLog += "\r\n理由:" + rw.BeiZhu;
            ...
            foreach (Node nd in fromND.HisToNodes)
            {
                ...
                if (nd.IsFL)
                {
                    ...
                    foreach (GenerWorkerList wl in wls)
                    {
                        ...
                        infoLog += "\r\n*****************************************************************************************";
                        infoLog += "\r\nノードID:" + subNd.NodeID + " 職名:" + subWK.EnDesc;
                        infoLog += "\r\nハンドラ:" + subWK.Rec + " , " + wk.RecOfEmp.Name;
                        infoLog += "\r\n受け取り時間:" + subWK.RDT + "処理時間:" + subWK.CDT;
                        infoLog += "\r\n ------------------------------------------------- ";
                ...
                {
                    infoLog += "\r\n*****************************************************************************************";
                    infoLog += "\r\nノードID:" + wk.NodeID + " 職名:" + wk.EnDesc;
                    infoLog += "\r\nハンドラ:" + wk.Rec + " , " + wk.RecOfEmp.Name;
                    infoLog += "\r\n受け取り時間:" + wk.RDT + " 処理時間:" + wk.CDT;
                    infoLog += "\r\n ------------------------------------------------- ";
        ...
        private WorkNode DoReturnSubFlow(int backtoNodeID, string msg, bool isHiden)
        {
            ...
            if (Glo.IsEnableSysMessage)
            {
                ...
                string title = string.Format("仕事帰り：フロー：{0}。仕事：{1}、帰国者：{2}、あなたはそれを処理する必要があります",
