    ...
    public class WorkUnSend
    {
        ...
        public WorkNode GetCurrentWorkNode()
        {
            ...
            if (gwf.RetrieveFromDBSources() == 0)
            {
                ...
                throw new Exception("@" + string.Format("ワークフロー{0}が完了しました", this.WorkID));
            }
        ...
        private string DoThreadUnSend()
        {
            ...
            if (nd.HisCancelRole == CancelRole.SpecNodes)
            {
                ...
                if (ncs.Count == 0)
                    throw new Exception("@フロー設計エラー、現在のノードを設定しました(" + wn.HisNode.Name + ")指定されたノードの担当者は引戻ことができますが、指定されたノードを設定していません。");
                ...
                if(DataType.IsNullOrEmpty(nds))
                    throw new Exception("@次の2つの理由により、送信取り消しを実行できません：1.ノードをキャンセルする機能がない; 2.フローの設計が間違っている、および元に戻すことができると指定したノードが、フロー中に渡したノードではない。");
            ...
            if (nd.HisCancelRole == CancelRole.OnlyNextStep)
            {
                ...
                if (num == 0)
                    throw new Exception("@現在のジョブが送信されていないか、次のジョブが処理されているため、送信をキャンセルできません");
                cancelToNodeID = wnPri.HisNode.NodeID;
                  
            }

            if (cancelToNodeID == 0)
                throw new Exception("@取り消されるノードが見つかりませんでした。");
            #endregion 求的撤销的节点.
                ...
                case NodeWorkType.WorkFL:
                   ...
                   nd.HisFlow.DoFlowEventEntity(EventListOfNode.UndoneBefore, nd, wn.HisWork, null);
                    ...
                    nd.HisFlow.DoFlowEventEntity(EventListOfNode.UndoneAfter, nd, wn.HisWork, null);
                      return "KillSubThared@子スレッドが正常に取り消されました。";
                default:
                    break;
          ...
          //  if (cancelToNode.HisNodeWorkType == NodeWorkType.StartWorkFL)
            ...
            // 记录日志..
            wn.AddToTrack(ActionType.UnSend, WebUser.No, WebUser.Name, cancelToNode.NodeID, cancelToNode.Name, "なし");
            ...
            if (wnOfCancelTo.HisNode.IsStartNode)
                return "@元に戻す操作が成功しました。" + msg;
            else
                return "@元に戻す操作が成功しました。" + msg;

            return "ジョブはあなたによってキャンセルされました:" + cancelToNode.Name;
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        private string DoUnSendIt()
        {
            ...
            if (gwf.WFState == WFState.Complete)
                return "err@フローは完了しており、キャンセルすることはできません";
            ...
            if (nd.HisCancelRole == CancelRole.None)
                throw new Exception("現在のノードは取り消せません。");


            if (nd.IsStartNode && nd.HisNodeWorkType != NodeWorkType.StartWorkFL)
                throw new Exception("現在のノードが開始ノードなので、元に戻すことはできません。");
            ...
            if (this.UnSendToNode == nd.NodeID)
            {
                    ...
                    || nd.HisNodeWorkType == NodeWorkType.WorkFHL)
                {
                    ...
                    if (dt == null || dt.Rows.Count == 0)
                        throw new Exception("err@フロー操作エラー：子スレッドがない場合、フローは保留状態でなければなりません");
            ...
            if (nd.CancelDisWhenRead == true)
            {
                ...
                if (i >= 1)
                    return "err@現在あります[" + i + "]スタッフが求人を開きました。キャンセルできません。";
                else
                {
                    //干流节点撤销到子线程
            ...
            if (DBAccess.RunSQLReturnValInt(sql, 0) != 0)
            {
                ...
                return "失効継続フローが正常に実行され、失効が["+gwf.NodeName+"]、引戻["+gwf.TodoEmps+"]";
            }
            #endregion 如果是越轨流程状态 .

            if (BP.WF.Dev2Interface.Flow_IsCanDoCurrentWork( this.WorkID, WebUser.No) == true)
                return "info@現在のジョブを処理する権限があり、キャンセルをすでに実行した可能性があります。リターンまたは送信機能を使用してください。";
            ...
            if (gwf.HuiQianTaskSta != HuiQianTaskSta.None)
            {
                ...
                if (DataType.IsNullOrEmpty(IsEnableUnSendWhenHuiQian) == false && IsEnableUnSendWhenHuiQian.Equals("0"))
                    return "info@現在のノードは副署名ステータスにあり、キャンセルできません。";
                ...
                //如果没有找到当前会签人.
                if (numOfmyGwl == 0)
                    return "err@現在のノード[" + gwf.NodeName + "]副署された状態であり、[" + gwf.TodoEmps + "]副署名の実行中はキャンセルを実行できません。";
            ...
            if (nd.HisCancelRole == CancelRole.SpecNodes)
            {
                ...
                if (dt.Rows.Count == 0)
                    throw new Exception("err@キャンセルフローが間違っています。送信をキャンセルする権限がありません。");
                ...
                if (cancelToNodeID == 0)
                    throw new Exception("@キャンセルフローが間違っています。キャンセルを実行する権限がなく、現在のノードはキャンセルを実行できません。");
            }
            ...
            if (nd.HisCancelRole == CancelRole.OnlyNextStep)
            {
                ...
                if (num == 0)
                    throw new Exception("err@現在のジョブが送信されていないか、次のジョブが処理されているため、送信をキャンセルできません");
                cancelToNodeID = wnPri.HisNode.NodeID;
            }

            if (cancelToNodeID == 0)
                throw new Exception("err@取り消されるノードが見つかりませんでした。");
            #endregion 求的撤销的节点.
                 ...
                 && nd.HisRunModel == RunModel.FL)
            {
                /* 检查一下是否还有没有完成的子线程，如果有就抛出不允许撤销的异常。 */
                  ...
                  if (BP.DA.DBAccess.RunSQLReturnValInt(sql) != 0)
                      return "err@終了していない子スレッドがあるため、取り消しは許可されません。";
                ...
                || cancelToNode.TodolistModel == TodolistModel.Teamup)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        /*如果是写作人员，就不允许他撤销 */
                        throw new Exception("@あなたはノードです[" + cancelToNode.Name + "]副署名者、キャンセルできません");
                    }
                }
            }
        ...
        private string DoUnSendFeiLiu(GenerWorkFlow gwf)
        {
            ...
            if (i == 0)
                throw new Exception("@現在のジョブは送信されていないため、送信の取り消しは実行できません");
            ...
            foreach (Node ndNext in nd.HisToNodes)
            {
                ...
                if (ndNext.HisRunModel == RunModel.SubThread)
                {
                    ...
                    foreach (GenerWorkFlow en in gwfs)
                        BP.WF.Dev2Interface.Flow_DeleteSubThread(gwf.FK_Flow, en.WorkID, "マージノードが送信をキャンセルする前に、子スレッドを削除します。");
                    continue;
                }
        ...
        private string DoUnSendInFeiLiuHeiliu(GenerWorkFlow gwf)
        {
            ...
            if (i == 0)
                throw new Exception("@現在の宛先変更作業が送信されていないため、送信取り消しを実行できません");
        ...
        public string DoUnSendHeiLiu_Main(GenerWorkFlow gwf)
        {
            ...
            if (i == 0)
                return "@作業を現在のノードに送信したわけではないため、元に戻すことはできません";

            WorkNode wn = this.GetCurrentWorkNode();
            WorkNode wnPri = new WorkNode(this.WorkID, priFLNode.NodeID);

            // 记录日志..
            wnPri.AddToTrack(ActionType.UnSend, WebUser.No, WebUser.Name, wnPri.HisNode.NodeID, wnPri.HisNode.Name, "なし");
        ...
        public string DoUnSendSubFlow(GenerWorkFlow gwf)
        {
            ...
            int num = wl.Retrieve(GenerWorkerListAttr.FK_Emp, BP.Web.WebUser.No,
                GenerWorkerListAttr.FK_Node, wnPri.HisNode.NodeID);
            if (num == 0)
                return "@現在のジョブは送信されていないため、送信の取り消しは実行できません";
            ...
            // 记录日志..
            wn.AddToTrack(ActionType.UnSend, WebUser.No, WebUser.Name, wn.HisNode.NodeID, wn.HisNode.Name, "なし");
