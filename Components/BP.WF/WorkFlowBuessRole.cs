        ...
        public static string GenerTitle(Flow fl, Work wk, Emp emp, string rdt)
        {
            ...
            if (DataType.IsNullOrEmpty(titleRole))
            {
                ...
                if (DataType.IsNullOrEmpty(titleRole) || titleRole.Contains("@") == false)
                    titleRole = "@WebUser.FK_DeptName-@WebUser.No,@WebUser.Nameは@RDTで開始.";
            }
            ...
            if (titleRole.Contains("@"))
            {
                ...
                throw new Exception("@フィールドのスペルに誤りがないか、タイトルの変数が置き換えられていないか確認してください。@" + titleRole);
            }
...
#warning 为什么，加这个代码？牺牲了很多效率，我注销了. by zhoupeng 2016.8.15
        ...
        public static Node RequestNextNode(Node currNode, Int64 workid, GenerWorkFlow currWorkFlow, GERpt enPara = null)
        {
            ...
            if (currNode.CondModel == CondModel.ByUserSelected)
            {
                if (currWorkFlow == null)
                    throw new Exception("@パラメータエラー：currWorkFlow");
                // 获取用户选择的节点.
                string nodes = currWorkFlow.Paras_ToNodes;
                if (DataType.IsNullOrEmpty(nodes))
                    throw new Exception("@ユーザーは送信先のノードを選択しませんでした。");
            ...
            if (nds.Count == 0)
                throw new Exception("@次のステップノードが見つかりません。");

            Conds dcsAll = new Conds();
            dcsAll.Retrieve(CondAttr.NodeID, currNode.NodeID, CondAttr.CondType, (int)CondType.Dir, CondAttr.PRI);
            if (dcsAll.Count == 0)
                throw new Exception("@ノード(" + currNode.NodeID + " , " + currNode.Name + ")方向条件を設定しません。分岐がある方向条件が必要です。");
            ...
            if (myNodes.Count == 0)
                throw new Exception("@現在の使用者(" + BP.Web.WebUser.Name + "),定義されたノードの方向条件が間違っています：{" + currNode.NodeID + currNode.Name + "}ノードから他のノードへ、定義されたすべての回転条件が無効です。");
            ...
            throw new Exception("@次のノードを見つける。");
        }
        #endregion
        ...
        public static DataTable RequetNextNodeWorkers(Flow fl, Node currNode, Node toNode, Entity enParas, Int64 workid)
        {
            ...
            if (toNode.HisDeliveryWay == DeliveryWay.ByDtlAsSubThreadEmps)
            {
                ...
                foreach (BP.Sys.MapDtl dtl in dtls)
                {
                    try
                    {
                        ...
                        if (dt.Rows.Count == 0 && toNode.HisWhenNoWorker == false)
                            throw new Exception("@フロー設計エラー、ノード（" + toNode.Name + "）に到達は指定されたノードにデータがなく、子スレッドのワーカーが見つかりません。");
                        return dt;
                    }
                    catch (Exception ex)
                    {
                ...
                throw new Exception("@シャントノードの詳細リストが子スレッドのデータソースとして見つかりません。フロー設計が間違っています。シャントノードフォームの詳細テーブルに、ユーザーフィールドで同意されたシステムフィールドがあるかどうかを確認してください。" + msg);
            }
            ...
            if (toNode.HisDeliveryWay == DeliveryWay.ByBindEmp)
            {
                ...
                dt = DBAccess.RunSQLReturnTable(ps);
                if (dt.Rows.Count == 0)
                    throw new Exception("@フロー設計エラー：次のノード(" + toNode.Name + ")スタッフをバインドしていません。");
                return dt;
            }
                    ...
                    if (i == 0)
                    {
                        if (toNode.HisDeliveryWay == DeliveryWay.BySelected)
                        {
                            if (currNode.CondModel != CondModel.SendButtonSileSelect)
                            {
                                ...
                                currNode.Update();
                                throw new Exception("@次のノードの受信者ルールは、前の手順の送信者セレクターに従って選択されていますが、現在のノードで受信者セレクターをアクティブにしておらず、システムによって既に自動的に設定されています。現在のウィンドウを閉じてからもう一度開いて、試行してください。");
                            }

                            throw new Exception("@次のステップ(" + toNode.Name + ")担当者を選択してください");
                        }
                        else
                        {
                            throw new Exception("@フロー設計エラー。FEEを書き直してから、ノード(" + toNode.Name + ")に対してアクセプターを設定します。詳細については、ccプロ​​セス設計マニュアルを参照してください。");
                        }
                    }
                ...
                || toNode.HisDeliveryWay == DeliveryWay.ByStarter)
            {
                ...
                foreach (string nd in nds)
                {
                    ...
                    if (DataType.IsNumStr(nd) == false)
                        throw new Exception("フロー設計エラー：設定したノード(" + toNode.Name + ")受信方法は指定されたノードポストに従って配信することですが、アクセスルール設定でノード番号を設定していません。");
                ...
                throw new Exception("@フロー設計エラー、ノード（" + toNode.Name + "）に到達は指定されたノード(" + strs + ")ファイルにデータがなく、スタッフが見つかりません。 @技術情報は次のとおりです。配信方法：BySpecNodeEmp sql=" + ps.SQLNoPara);
            }
            ...
            if (toNode.HisDeliveryWay == DeliveryWay.ByPreviousNodeFormEmpsField)
            {
                ...
                if (DataType.IsNullOrEmpty(emps))
                    throw new Exception("@フィールド[" + enParas.EnMap.Attrs.GetAttrByKey(specEmpFields).Desc + "]では受け入れ者を指定していますので、仕事は下に送れません。");
            ...
            if (toNode.HisDeliveryWay == DeliveryWay.ByDeptAndStation)
            {
                ...
                if (dt.Rows.Count > 0)
                    ...
                    throw new Exception("@ノードアクセスルール(" + toNode.HisDeliveryWay.ToString() + ")エラー：ノード(" + toNode.NodeID + "," + toNode.Name + "), 職位と部門の交差によって、受け入れ人の範囲が間違っていて、人員が見つからなかった：SQL=" + sql);
            }
            ...
            if (toNode.HisDeliveryWay == DeliveryWay.ByStationOnly)
            {
                ...
                if (dt.Rows.Count > 0)
                    ...
                    throw new Exception("@ノードアクセスルールエラー：ノード(" + toNode.NodeID + "," + toNode.Name + "), 職階のみで計算、人員は見つかりません：SQL=" + ps.SQLNoPara);
            }
            #endregion
            ...
            if (toNode.HisDeliveryWay == DeliveryWay.ByStationAndEmpDept)
            {
                ...
                if (dt.Rows.Count > 0)
                    ...
                    throw new Exception("@ノードアクセスルール(" + toNode.HisDeliveryWay.ToString() + ")エラー：ノード(" + toNode.NodeID + "," + toNode.Name + "), ジョブで計算されます（部署のコレクションを緯度と見なします）。技術情報、実行されたSQL=" + ps.SQLNoPara);
            }
            #endregion
            ...
            if (toNode.HisDeliveryWay == DeliveryWay.BySpecNodeEmpStation)
            {
                ...
                if (DataType.IsNumStr(para) == true)
                {
                    ...
                    dt = DBAccess.RunSQLReturnTable(ps);
                    if (dt.Rows.Count != 1)
                        throw new Exception("@フロー設計エラー、ノード（" + toNode.Name + "）に到達は指定されたノードにデータがなく、スタッフが見つかりません。");
                ...
                else
                {
                    if (enParas.Row.ContainsKey(para) == false)
                        throw new Exception("@受信者を検索するときに間違った@フィールド{" + para + "}RPTに含まれていません、フロー設計エラー。");
            ...
            if (currNode.GroupStaNDs != toNode.GroupStaNDs)
            {
                ...
                if (dt.Rows.Count == 0)
                {
                    ...
                    if (nextStations.Count == 0)
                        throw new Exception("@ノードには職階がありません:" + toNode.NodeID + "  " + toNode.Name);
                }
                else
                {
                    bool isInit = false;
...
#warning edit by peng, 用来确定不同岗位集合的传递包含同一个人的处理方式。
            ...
            while (true)
            {
                ...
                if (nowDeptID == "-1" || nowDeptID.ToString() == "0")
                {
                    ...
                    throw new Exception("@職階計算では受取人(" + toNode.Name + ")見つかりません。");
                }
        ...
        public static string DoCCAuto(Node node, GERpt rpt, Int64 workid, Int64 fid)
        {
            ...
            if (dt.Rows.Count == 0)
                return "@CCルールが設定され、CC担当者は見つかりませんでした。";

            string ccMsg = "@メッセージは自動的にコピーされます";
        ...
        public static string DoCCByEmps(Node nd, GERpt rptGE, Int64 workid, Int64 fid)
        {
            ...
            string mailTemp = BP.DA.DataType.ReadTextFile2Html(BP.Sys.SystemConfig.PathOfDataUser + Path.DirectorySeparatorChar + "EmailTemplete" + Path.DirectorySeparatorChar + "CC_" + WebUser.SysLang + ".txt");
